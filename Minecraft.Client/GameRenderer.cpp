...
CThread*		GameRenderer::m_updateThread;
CThread::EventArray* GameRenderer::m_updateEvents;
...
GameRenderer::GameRenderer(Minecraft *mc)
{
	...
	// Init these so they are setup before the tick
	for( int i = 0; i < 4; i++ )
	{
		fov[i] = oFov[i] = 1.0f;
	}
	...
	// set up the local players iteminhand renderers here - needs to be done with lighting enabled so that the render geometry gets compiled correctly
	...
	// changes brought forward from 1.8.2
	BufferedImage *img = new BufferedImage(16, 16, BufferedImage::TYPE_INT_RGB);
	for( int i = 0; i < NUM_LIGHT_TEXTURES; i++ )
	{
		lightTexture[i] = mc->textures->getTexture(img);		// changed to one light texture per level to support split screen
	}
	delete img;
#ifdef _LINUX
...
#ifdef MULTITHREAD_ENABLE
	m_updateEvents  = new CThread::EventArray(eUpdateEventCount, CThread::EventArray::e_modeAutoClear);
	m_updateEvents->Set(eUpdateEventIsFinished);

	InitializeCriticalSection(&m_csDeleteStack);
	m_updateThread = new CThread(runUpdate, NULL, "Chunk update");
#ifdef _LINUX
...
void GameRenderer::tick(bool first)		// add bFirst
{ 
	tickFov();
	tickLightTexture();		// change brought forward from 1.8.2
	fogBrO = fogBr;
...
// camera movement regardless of FPS
	...
	if( mc->player != mc->localplayers[ProfileManager.GetPrimaryPad()] ) return; //  for split screen - only do rest of processing for once per frame
...
void GameRenderer::pick(float a)
{
	...
	// added - stop blocks right at the edge of the world from being pickable so we shouldn't be able to directly destroy or create anything there
	if( mc->hitResult )
	{
		...
		// Only block the sides that are facing an outside block
...
void GameRenderer::tickFov()
{
	...
	int playerIdx = player ? player->GetPadIndex() : 0;
...
void GameRenderer::moveCameraToPlayer(float a)
{
	...
	else if (localplayer->ThirdPersonView())
	{
		...
		{
			// corrected bug where this used to just take player->xRot & yRot directly and so wasn't taking into account interpolation, allowing camera to go through walls
			...
			for (int i = 0; i < 8; i++)
			{
				...
				// corrected bug here where zo was also added to x component
			...
			// removed extra rotations here that aren't needed because our xRot/yRot don't ever
...
//  as we have more complex adjustments to make for fov & aspect on account of viewports
void GameRenderer::getFovAndAspect(float& fov, float& aspect, float a, bool applyEffects)
{
	// split out aspect ratio and fov here so we can adjust for viewports - we might need to revisit these as
	// they are maybe be too generous for performance. 
	aspect = mc->width / (float) mc->height;
	fov = getFov(a, applyEffects);

	if( ( mc->player->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_TOP ) ||
		( mc->player->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_BOTTOM ) )
	{
		aspect *= 2.0f;
		fov *= 0.7f;		// Reduce FOV to make things less fish-eye, at the expense of reducing vertical FOV from single player mode
	}
	else if( ( mc->player->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_LEFT ) ||
		( mc->player->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_RIGHT) )
	{
...
void GameRenderer::setupCamera(float a, int eye)
{
	...
	// have split out fov & aspect calculation so we can take into account viewports
	float aspect, fov;
	...
	if(app.GetGameSettings(mc->player->GetPadIndex(),eGameSetting_ViewBob) && !mc->player->abilities.flying && !bNoLegAnim && !bNoBobbingAnim) bobView(a);
...
void GameRenderer::renderItemInHand(float a, int eye)
{
	...
	// to turn off the hand for screenshots, but not when the item held is a map
	if ( localplayer!=NULL)
	{
		shared_ptr<ItemInstance> item = localplayer->inventory->getSelected();
		if(!(item && item->getItem()->id==Item::map_Id)  && app.GetGameSettings(localplayer->GetPadIndex(),eGameSetting_DisplayHand)==0 ) return;
	}
	...
	if(app.GetGameSettings(localplayer->GetPadIndex(),eGameSetting_ViewBob) && !localplayer->abilities.flying && !bNoLegAnim) bobView(a);
...
#endif
	RenderManager.TextureBindVertex(getLightTexture(mc->player->GetPadIndex(), mc->level));
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
...
{
	// Now doing light textures on PER PLAYER basis.
	// we *had* added separate light textures for all dimensions, and this loop to update them all here
	for(int j = 0; j < XUSER_MAX_COUNT; j++ )
	{
		...
		Level *level = player->level;		// was mc->level when it was just to update the one light texture
		...
		for (int i = 0; i < 256; i++)
		{
			...
			float brightness = 0.0f;	// TODO - was mc->options->gamma;
			if (_r > 1) _r = 1;
			if (_g > 1) _g = 1;
			if (_b > 1) _b = 1;
...
#if ( defined _LINUX || defined __LP64__ || _LINUX )
...
// so we can have a light texture for each player to support split screen
int GameRenderer::getLightTexture(int iPad, Level *level)
{
	...
	return lightTexture[iPad]; // Changing to Per Player lighting textures.
}
...
void GameRenderer::render(float a, bool bFirst)
{
	...
	glViewport(0, 0, mc->width, mc->height);	// added
...
int GameRenderer::runUpdate(LPVOID lpParam)
{
	...
	while(ShutdownManager::ShouldRun(ShutdownManager::eRenderChunkUpdateThread))
	{
		...
		// We Need to have this happen atomically to avoid deadlocks
...
// #endif
	...
	app.DebugPrintf("------------------EnableUpdateThread--------------------/n");
	updateRunning = true;
	m_updateEvents->Set(eUpdateCanRun);
	m_updateEvents->Set(eUpdateEventIsFinished);
#endif
...
#ifdef MULTITHREAD_ENABLE
	if( !updateRunning) return;
	app.DebugPrintf("------------------DisableUpdateThread--------------------/n");
	updateRunning = false;
...
{
//	if (updateLightTexture) updateLightTexture();	// TODO - Java 1.0.1 has this line enabled, should check why - don't want to put it in now in case it breaks split-screen
...
//	if (mc->cameraTargetPlayer == NULL)	// removed condition as we want to update this is mc->player changes for different local players
	{
...
//		Culler *frustum = new FrustumCuller();
		...
		if ( (i == 0) && updateChunks ) // added updateChunks condition
		{
			int PIXPass = 0;
		...
		glEnable(GL_FOG);
		MemSect(31);
		mc->textures->bindTexture(TN_TERRAIN);	//  L"/terrain.png"
		MemSect(0);
		Lighting::turnOff();
		...
		if (cameraFlip == 0 )
		{
			...
			// for entities, don't include the "a" factor that interpolates from the old to new position, as the AABBs for the entities are already fully at the new position
			...
			// When rendering entities, in the end if the dragon is hurt or we have a lot of entities we can end up wrapping
			...
			turnOnLightLayer(a);		// brought forward from 1.8.2
			particleEngine->renderLit(cameraEntity, a);
			Lighting::turnOff();
			setupFog(0, a);
			particleEngine->render(cameraEntity, a);
			PIXEndNamedEvent();
			turnOffLightLayer(a);		// brought forward from 1.8.2
		...
		mc->textures->bindTexture(TN_TERRAIN);		//  L"/terrain.png"
		MemSect(0);
		// have changed this fancy rendering option to work with our command buffers. The original used to use frame buffer flags to disable
		...
		if (true)	// (mc->options->fancyGraphics)
		{
			...
			if (visibleWaterChunks > 0)
			{
				...
				levelRenderer->render(cameraEntity, 1, a, updateChunks);	// chanaged, used to be renderSameAsLast but we don't support that anymore
				PIXEndNamedEvent();
			}
		...
		/* moved rain rendering to after clouds so that it alpha blends onto them properly
		...
		*/
		...
		// rain rendering moved here so that it renders after clouds & can blend properly onto them
		setupFog(0, a);
		glEnable(GL_FOG);
		PIXBeginNamedEvent(0,"Rendering snow and rain");
		renderSnowAndRain(a);
...
// this whole function updated from 1.8.2
void GameRenderer::renderSnowAndRain(float a)
{
	float rainLevel = mc->level->getRainLevel(a);
	if (rainLevel <= 0) return;

	// rain is relatively low poly, but high fill-rate - better to clip it
	...
	MemSect(31);
	mc->textures->bindTexture(TN_ENVIRONMENT_SNOW);	//  L"/environment/snow.png"
	MemSect(0);
	...
	int r = 5;
	// was if(mc.options.fancyGraphics) r = 10;
	...
	// some changes made here to access biome through new interface that caches results in levelchunk flags, as an optimisation
	...
	for (int x = x0 - r; x <= x0 + r; x++)
		for (int z = z0 - r; z <= z0 + r; z++)
		{
			...
			// changes here brought forward from 1.8.2
			...
			if (yy0 != yy1)
			{
				...
				if (level->getBiomeSource()->scaleTemp(temp, floor) >= 0.15f)
				{
					...
					t->color(br, br, br, 0.0f);														// added to soften the top visible edge of the rain
...
void GameRenderer::setupClearColor(float a)
{
    ...
    double yy = (player->yOld + (player->y - player->yOld) * a) * level->dimension->getClearColorScale();		// getClearColorScale brought forward from 1.2.3
...
void GameRenderer::setupFog(int i, float alpha)
{
	...
	// check for creative mode brought forward from 1.2.3
    bool creative = false;
...
//        }
	...
	else
	{
		...
		{
			// test for doing bedrockfog brought forward from 1.2.3
