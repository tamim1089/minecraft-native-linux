...
#endif
#include "Linux/PS3Extras/ShutdownManager.h"
...
MinecraftServer::MinecraftServer()
{
	...
	m_serverPausedEvent = new CThread::Event;
...
bool MinecraftServer::initServer(__int64 seed, NetworkGameInitData *initData, DWORD initSettings, bool findSeed)
{
    ...
    t.start();
		...
		app.DebugPrintf("/n*** SERVER SETTINGS ***/n");
		app.DebugPrintf("ServerSettings: host-friends-only is %s/n",(app.GetGameHostOption(eGameHostOption_FriendsOfFriends)>0)?"on":"off");
		app.DebugPrintf("ServerSettings: game-type is %s/n",(app.GetGameHostOption(eGameHostOption_GameType)==0)?"Survival Mode":"Creative Mode");
		app.DebugPrintf("ServerSettings: pvp is %s/n",(app.GetGameHostOption(eGameHostOption_PvP)>0)?"on":"off");
		app.DebugPrintf("ServerSettings: fire spreads is %s/n",(app.GetGameHostOption(eGameHostOption_FireSpreads)>0)?"on":"off");
		app.DebugPrintf("ServerSettings: tnt explodes is %s/n",(app.GetGameHostOption(eGameHostOption_TNT)>0)?"on":"off");
		app.DebugPrintf("/n");

		// TODO Init a load of settings based on data passed as params
		...
		// We should never have hacked clients flying when they shouldn't be like the PC version, so enable flying always
		...
		// Enabling flight to stop it kicking us when we use it
		...
		Socket::Initialise(connection);	// added
#else
		// removed
		...
		// Need to wait for levelGenerationOptions to load.
...
//        logger.info("Done (" + (System.nanoTime() - levelNanoTime) + "ns)! For help, type /"help/" or /"?/"");
		...
		//  passed in save data now - this is only required for the tutorial which is loaded by passing data directly in rather than using the storage manager
...
// added - extra thread to post processing on separate thread during level creation
...
//#ifndef _LINUX
	...
	while(server->m_postProcessRequests.size() && ShutdownManager::ShouldRun(ShutdownManager::ePostProcessThread) )
	{
		...
		if((server->m_postProcessRequests.size() % 10) == 0)
			printf("processing request %00d/n", server->m_postProcessRequests.size());
#endif
		Sleep(1);
#endif
		EnterCriticalSection(&server->m_postProcessCS);
...
//    }
	...
	// free levels here if there are already some?
	...
	app.DebugPrintf("Default game type: %d/n" , gameTypeId);
...
//        levels[i]->addListener(new ServerLevelListener(this, levels[i]));		// have moved this to the ServerLevel ctor so that it is set up in time for the first chunk to load, which might actually happen there

		// We set the levels difficulty based on the minecraft options
...
//		m_lastSentDifficulty = pMinecraft->options->difficulty;
		...
		app.DebugPrintf("MinecraftServer::loadLevel - Difficulty = %d/n",levels[i]->difficulty);
	...
	// Make a new thread to do post processing
	InitializeCriticalSection(&m_postProcessCS);

	// fix for 108310 - TCR #001 BAS Game Stability: TU12: Code: Compliance: Crash after creating world on "journey" seed.

	m_postUpdateThread = new CThread(runPostUpdate, this, "Post processing", 256*1024);
	...
	// Added this to temporarily make starting games on vita faster
#ifdef _LINUX
	int r = 48;
#else
	int r = 196;	
	...
	// This loop is changed in 1.0.1 to only process the first level (ie the overworld), but I think we still want to do them all
	int i = 0;
...
// 				storage->getSaveFile()->convertLevelChunks(level)
			...
			for( int x = -halfsidelen; x < halfsidelen; x++ )
			{
				for( int z = -halfsidelen; z < halfsidelen; z++ )
				{
					...
					level->cache->create(x,z, true);	// added parameter to disable postprocessing here
...
//                        lastTime = now;
                    ...
                    level->cache->create((spawnPos->x + x) >> 4, (spawnPos->z + z) >> 4, true);	// added parameter to disable postprocessing here
					PIXEndNamedEvent();
...
//                        ;
			...
			// removed this as now doing the recheckGaps call when each chunk is post-processed, so can happen on things outside of the spawn area too
#if 0
			// added this code to propagate lighting properly in the spawn area before we go sharing it with the local client or across the network
            for (int x = -r; x <= r && running; x += 16)
			{
                for (int z = -r; z <= r && running; z += 16)
				{
...
//	printf("Main thread complete at %dms/n",System::currentTimeMillis() - startTime);
	...
	if(levels[0]->dimension->id==0)
	{
		...
		{
			app.DebugPrintf("=== Leveldata has stronghold position/n");
		}
		app.DebugPrintf("===================================/n");
	}

//	printf("Post processing complete at %dms/n",System::currentTimeMillis() - startTime);

//	printf("Lighting complete at %dms/n",System::currentTimeMillis() - startTime);
	...
	// added - immediately save newly created level, like single player game
	// We also want to immediately save the tutorial
	if ( levels[0]->isNew )
		saveGameRules();
...
//    logger.info("Saving chunks");
    for (unsigned int i = 0; i < levels.length; i++)
	{
...
#endif
		// Save the levels in reverse order so we don't overwrite the level.dat
		...
		if( level )	// added check as level can be NULL if we end up in stopServer really early on due to network failure
		{
...
void MinecraftServer::Suspend()
{
	...
	// Flush stats and call PlayerSessionExit.
	...
	m_suspending = false;
	app.DebugPrintf("Suspend server: Elapsed time %f/n", fElapsedTime);
	PIXEndNamedEvent();
}
...
void MinecraftServer::stopServer()
{
	...
	// need to halt the rendering of the data, since we're about to remove it
#ifdef _LINUX
	...
	app.DebugPrintf("Stopping server/n");
//    logger.info("Stopping server");
	// If the primary player has signed out, then don't attempt to save anything
...
//    try {		// removed try/catch/finally
    if (initServer(seed, initData, initSettings,findSeed))
	{
		ServerLevel *levelNormalDimension = levels[0];
		// Set the Stronghold position in the leveldata if there isn't one in there
		Minecraft *pMinecraft = Minecraft::GetInstance();
        ...
        while (running && !s_bServerHalted)
		{
			...
			// When we pause the server, we don't want to count that as time passed
			// TU-1 hotifx - Remove this line. We want to make sure that we tick connections at the proper rate when paused
...
//						PIXReportCounter(L"Server time",(float)(after-before));
						...
						//  that the slow queue owner keeps cycling if it's not been used in a while
						int time = GetTickCount();
						if( ( s_slowQueuePacketSent ) || (  (time - s_slowQueueLastTime) > ( 2 * MINECRAFT_SERVER_SLOW_QUEUE_DELAY ) ) )
						{
//							app.DebugPrintf("Considering cycling: (%d) %d - %d -> %d > %d/n",s_slowQueuePacketSent, time, s_slowQueueLastTime, (time - s_slowQueueLastTime), (2*MINECRAFT_SERVER_SLOW_QUEUE_DELAY));
...
//						else
//						{
//							app.DebugPrintf("Not considering cycling: %d - %d -> %d > %d/n",time, s_slowQueueLastTime, (time - s_slowQueueLastTime), (2*MINECRAFT_SERVER_SLOW_QUEUE_DELAY));
//						}
					}
...
//					PIXReportCounter(L"Server ticks",(float)tickcount);
			...
			if(MinecraftServer::setTimeAtEndOfTick)
			{
				MinecraftServer::setTimeAtEndOfTick = false;
				 for (unsigned int i = 0; i < levels.length; i++)
				{
//					if (i == 0 || settings->getBoolean(L"allow-nether", true))		// we always have nether
					{
						ServerLevel *level = levels[i];
			...
			for(int i=0;i<XUSER_MAX_COUNT;i++)
			{
				...
				case eXuiServerAction_AutoSaveGame:
#if defined(_LINUX) || defined(_LINUX)
					{
						...
						qwDeltaTime.QuadPart = qwNewTime.QuadPart - qwTime.QuadPart;
						fElapsedTime = fSecsPerTick * ((FLOAT)(qwDeltaTime.QuadPart));
						app.DebugPrintf("Autosave: Elapsed time %f/n", fElapsedTime);
					}
					break;
#endif
				...
				case eXuiServerAction_SpawnMob:
					...
					mob->setDespawnProtected();		// default to being protected against despawning (has to be done after initial position is set)
				...
				case eXuiServerAction_ToggleThunder:
					...
					break;
				case eXuiServerAction_ServerSettingChanged_Usernames:
					players->broadcastAll( shared_ptr<ServerSettingsChangedPacket>( new ServerSettingsChangedPacket( ServerSettingsChangedPacket::HOST_OPTIONS, app.GetGameHostOption(eGameHostOption_Usernames)) ) );
					break;
				...
				case eXuiServerAction_ExportSchematic:
					...
					if( !s_bServerHalted )
					{
						ConsoleSchematicFile::StructureInitParam *initData = (ConsoleSchematicFile::StructureInitParam *)param;
#ifdef _LINUX
				...
				case eXuiServerAction_SetCameraLocation:
#ifndef _CONTENT_PACKAGE
					{
						...
						app.DebugPrintf(	"DEBUG: Player=%i/n", pos->player );
						app.DebugPrintf(	"DEBUG: Teleporting to pos=(%f.2, %f.2, %f.2), looking at=(%f.2,%f.2)/n",
   ...
   //     while (running)
	...
	// Stop the server when the loops complete, as the finally would do
	stopServer();
	stopped = true;
}
...
void MinecraftServer::tick()
{
	...
	//  need to update client difficulty levels based on the servers
	Minecraft *pMinecraft = Minecraft::GetInstance();
	// sending this on the host changing the difficulty in the menus
...
// #ifndef _LINUX
			...
			// used to be in a while loop, but we don't want the server locking up for a big chunk of time (could end up trying to process 1,000,000 lights...)
			// Instead call this once, which will try and process up to 2000 lights per tick
//			printf("lights: %d/n",level->getLightsToUpdate());
            while(level->updateLights() )
				;
			__int64 st2 = System::currentTimeMillis();
			PIXEndNamedEvent();
			PIXBeginNamedEvent(0,"Entity tick %d",i);
			//  to stop ticking entities in levels when players are not in those levels.
...
//			printf(">>>>>>>>>>>>>>>>>>>>>> Tick %d %d %d : %d/n", st1 - st0, st2 - st1, st3 - st2, st0 - stc );
			stc = st0;
// #endif// _LINUX
        }
    }
...
//        commands->handleCommand(input);		// removed - TODO - do we want equivalent of console commands?
    }
}
...
bool MinecraftServer::canSendOnSlowQueue(INetworkPlayer *player)
{
	...
	{
//		app.DebugPrintf("Slow queue OK for player #%d/n", player->GetSessionIndex());
		return true;
	}
...
//	app.DebugPrintf("Cycled slow queue index to %d/n", s_slowQueuePlayerIndex);
}

// sets up a vector of flags to indicate which entities (with small Ids) have been removed from the level, but are still haven't constructed a network packet
