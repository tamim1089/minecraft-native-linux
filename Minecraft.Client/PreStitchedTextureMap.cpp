...
void PreStitchedTextureMap::stitch()
{
	...
	int clamp = Texture::WM_WRAP; // Don't clamp as it causes issues with how we signal non-mipmmapped textures to the pixel shader //Texture::WM_CLAMP;
...
{
#ifndef _CONTENT_PACKAGE
	app.DebugPrintf("Not implemented!/n");
	__debugbreak();
#endif
	return NULL;
#if 0
...
// register is a reserved keyword in C++
Icon *PreStitchedTextureMap::registerIcon(const wstring &name)
{
	Icon *result = NULL;
	if (name.empty())
	{
		app.DebugPrintf("Don't register NULL/n");
#ifndef _CONTENT_PACKAGE
		__debugbreak();
#endif
	...
	{
#ifndef _CONTENT_PACKAGE
		wprintf(L"Could not find uv data for icon %ls/n", name.c_str() );
		__debugbreak();
#endif
		result = missingPosition;
	}
...
void PreStitchedTextureMap::loadUVs()
{
	...
	{
		// We only need to populate this once at the moment as we have hardcoded positions for each texture
	...
	else
	{
		...
		texturesByName[L"grass_top"]->setFlags(Icon::IS_GRASS_TOP);			//  for faster determination of texture type in tesselation
		...
		texturesByName[L"grass_side"]->setFlags(Icon::IS_GRASS_SIDE);		//  for faster determination of texture type in tesselation
