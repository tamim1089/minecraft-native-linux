...
// added - helper function to set up vertex arrays
...
Cube::Cube(ModelPart *modelPart, int xTexOffs, int yTexOffs, float x0, float y0, float z0, int w, int h, int d, float g, int faceMask /* = 63 */, bool bFlipPoly3UVs) :		// added faceMask, added bFlipPoly3UVs to reverse the uvs back so player skins display right
	x0(x0),
	y0(y0),
	z0(z0),
	x1(x0 + w),
...
// 	this->z1 = z0 + d;
	...
	// added ability to mask individual faces
	int faceCount = 0;
