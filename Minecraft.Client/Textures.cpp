...
CRender::eTextureFormat Textures::TEXTURE_FORMAT = CRender::TEXTURE_FORMAT_RxGyBzAw;
...
{
//    pixels = MemoryTracker::createIntBuffer(2048 * 2048);	// now just creating this buffer when we need it
	...
	*/
	...
	// Changed these to our PreStitchedTextureMap from TextureMap
	...
	// added - preload a set of commonly used textures that can then be referenced directly be an enumerated type rather by string
	loadIndexedTextures();
}
...
intArray Textures::loadTexturePixels(TEXTURE_NAME texId, const wstring& resourceName)
{
...

    {
		...
		// if resourceName isn't in the map, it should add an element and as that will use the default constructor, its
...
// textures default to standard 32-bit RGBA format, but where we can, use an 8-bit format. There's 3 different varieties of these currently
...
{
	// These texture formats are not currently in the render header
#ifdef _LINUX
	if(resourceName == L"/environment/clouds.png")
	{
		TEXTURE_FORMAT = CRender::TEXTURE_FORMAT_R1G1B1Ax;
	}
	else if(resourceName == L"%blur%/misc/pumpkinblur.png")
	{
		TEXTURE_FORMAT = CRender::TEXTURE_FORMAT_R0G0B0Ax;
	}
	...
	{
		TEXTURE_FORMAT = CRender::TEXTURE_FORMAT_RxGxBxAx;
	}
...
#endif
	{
		TEXTURE_FORMAT = CRender::TEXTURE_FORMAT_RxGyBzAw;
	}
}
...
// 	printf("Textures::loadTexture name - %s/n",buf);
	...
	// added special cases to avoid mipmapping on clouds & shadows
	if( (resourceName == L"environment/clouds.png") ||
...
//    try {
	...
	{
		// Get resource above just returns the name for texture packs
...
int Textures::getTexture(BufferedImage *img, CRender::eTextureFormat format, bool mipmap)
{
    int id = MemoryTracker::genTextures();
...
{
//	printf("Textures::loadTexture BufferedImage %d/n",id);
...
{
//	printf("Textures::loadTexture BufferedImage with blur and clamp %d/n",id);
	int iMipLevels=1;
	MemSect(33);
    glBindTexture(GL_TEXTURE_2D, id);
	...
	// now creating a buffer of the size we require dynamically
	...
	if (MIPMAP) 
	{
		// In the new XDK, the CreateTexture will fail if the number of mipmaps is higher than the width & height passed in will allow!
		int iWidthMips=1;
		int iHeightMips=1;
		...
		//RenderManager.TextureSetTextureLevels(5);	// 
	...
	if (MIPMAP)
	{
        for (int level = 1; level < iMipLevels; level++)
		{
			...
			// added tempData so we aren't overwriting source data
			unsigned int *tempData = new unsigned int[ww * hh];
			// added - have we loaded mipmap data for this level? Use that rather than generating if possible
...
#ifndef _LINUX
						// convert our RGBA texels to ARGB that crispBlend is expecting
    ...
    /*
        ...
        */
	delete pixels;	// now creating this dynamically
	MemSect(0);
}
...
void Textures::replaceTexture(intArray rawPixels, int w, int h, int id)
{
    ...
    ByteBuffer *pixels =  MemoryTracker::createByteBuffer(w * h * 4);	// now creating dynamically
    pixels->put(newPixels);
    pixels->position(0)->limit(newPixels.length);
	delete [] newPixels.data;
...
// added. This is a more minimal version of replaceTexture that assumes the texture bytes are already in order, and so doesn't do any of the extra copying round
...
// adding for texture in memory (from global title storage)
int Textures::loadMemTexture(const wstring& url, const wstring& backup)
{
	...
	if(texture != NULL)
	{
		if (texture->loadedImage != NULL && !texture->isLoaded)
		{
			// Disable mipmapping in general for skins & capes. Have seen problems with edge-on polys for some eg mumbo jumbo
			...
			if (texture->id < 0)
			{
				texture->id = getTexture(texture->loadedImage, CRender::TEXTURE_FORMAT_RxGyBzAw, MIPMAP);
			}
			else
			{
				loadTexture(texture->loadedImage, texture->id);
...
MemTexture *Textures::addMemTexture(const wstring& name,MemTextureProcessor *processor)
{
	...
	if(texture == NULL)
	{
		...
		{
			// Make an entry for this anyway and we can populate it later
...
void Textures::tick(bool updateTextures, bool tickDynamics)	//  updateTextures parameter & tickDynamics
{
	MemSect(22);
	if(tickDynamics)
	{
		// added - if we aren't updating the final renderer textures, just tick each of the dynamic textures instead. This is used so that in frames were we have multiple
		...
		// added - tell renderer that we're about to do a block of dynamic texture updates, so we can unlock the resources after they are done rather than a series of locks/unlocks
		//RenderManager.TextureDynamicUpdateStart();		
		terrain->cycleAnimationFrames();
        items->cycleAnimationFrames();
		//RenderManager.TextureDynamicUpdateEnd();	// see comment above
	}

	// go over all the memory textures once per frame, and free any that haven't been used for a while. Ones that are being used will
...
BufferedImage *Textures::readImage(TEXTURE_NAME texId, const wstring& name)	//  InputStream *in
{
