...
public:
	...
	// Added this so that we can manage UGC privileges
...
public:
    ...
    void teleport(double x, double y, double z, float yRot, float xRot, bool sendPacket = true); //  sendPacket param
...
public:
	// Handlers only valid in debug mode
	...
	//  as we need to set this from outside sometimes
	void setPlayer(shared_ptr<ServerPlayer> player) { this->player = player; }
	shared_ptr<ServerPlayer> getPlayer() { return player; }

	//  to signal a disconnect from another thread
	void closeOnTick() { m_bCloseOnTick = true; }

	//  so that we can send on textures that get received after this connection requested them
