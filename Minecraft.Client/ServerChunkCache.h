...
private:
	...
	// added for multithreaded support
	CRITICAL_SECTION m_csLoadCreate;
	// size of cache is defined by size of one side - must be even
	int XZSIZE;
	int XZOFFSET;
...
public:
	...
	//  Added; Remove chunk from the toDrop queue.
#ifdef _LARGE_WORLDS	
...
private:
	...
	void updatePostProcessFlag(short flag, int x, int z, int xo, int zo, LevelChunk *lc);	// 
...
#else
	// Was 24, but lowering it drastically so that we can trickle save chunks
...
private:
	typedef struct _SaveThreadData
	{
		...
		CThread::Event *notificationEvent;
		CThread::Event *wakeEvent; // This is a handle to the one fired by the producer thread
	} SaveThreadData;
