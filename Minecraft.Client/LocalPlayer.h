...
protected:
	...
	// Last time we checked for achievement uunlocks.
...
public:
    ...
    void move(double xa, double ya, double za, bool noEntityCubes=false);	// added noEntityCubes parameter
	...
	// moved these in from the minecraft structure, since they are per player things for splitscreen
	//int ticks;
	int missTime;
	int lastClickTick[2];
	bool isRaining ;
...
public:
    ...
    virtual bool openContainer(shared_ptr<Container> container);		//  bool return
    virtual bool startCrafting(int x, int y, int z);					//  bool return
	virtual bool startEnchanting(int x, int y, int z);					//  bool return
	virtual bool startRepairing(int x, int y, int z);
    virtual bool openFurnace(shared_ptr<FurnaceTileEntity> furnace);	//  bool return
	virtual bool openBrewingStand(shared_ptr<BrewingStandTileEntity> brewingStand); //  bool return
    virtual bool openTrap(shared_ptr<DispenserTileEntity> trap);		//  bool return
	...
	// have changed 3rd person view to be 0 if not enabled, 1 for mode like original, 2 reversed mode
	...
	void SetPadIndex(int iPad)	{m_iPad=iPad;}
	int GetPadIndex()			{return m_iPad;}
	void SetPlayerRespawned(bool bVal)			{m_bPlayerRespawned=bVal;}
	bool GetPlayerRespawned()					{return m_bPlayerRespawned;}

	// Moved these in here from the minecraft structure since they are local player related
	void handleMouseDown(int button, bool down);
	bool handleMouseClick(int button);

	// added for improved autorepeat
