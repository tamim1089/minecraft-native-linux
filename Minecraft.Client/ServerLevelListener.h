...
// renamed class to ServerLevelListener to avoid clash with LevelListener
...
public:
	...
	// virtual void addParticle(const wstring& name, double x, double y, double z, double xa, double ya, double za);
	virtual void addParticle(ePARTICLE_TYPE name, double x, double y, double z, double xa, double ya, double za); // 
    virtual void allChanged();
    virtual void entityAdded(shared_ptr<Entity> entity);
    virtual void entityRemoved(shared_ptr<Entity> entity);
	virtual void playerRemoved(shared_ptr<Entity> entity);		// for when a player is removed from the level's player array, not just the entity storage
    ...
    virtual void setTilesDirty(int x0, int y0, int z0, int x1, int y1, int z1, Level *level);	// added level param
