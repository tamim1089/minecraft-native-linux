...
{
	// Assume that we only need one input class, even though the java has subclasses for keyboard/controller
	// This function is based on the ControllerInput class in the Java, and will probably need changed
	//OutputDebugString("INPUT: Beginning input tick/n");

	Minecraft *pMinecraft=Minecraft::GetInstance();
	int iPad=player->GetPadIndex();

	//  minecraft movement seems to be the wrong way round, so invert x!
	...
	// in flying mode, don't actually toggle sneaking
	...
	// WESTY : Invert look Y if required.
	...
	//OutputDebugString("INPUT: End input tick/n");
}