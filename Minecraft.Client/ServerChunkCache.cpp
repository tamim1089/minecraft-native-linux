...
bool ServerChunkCache::hasChunk(int x, int z)
{
	...
	// Request for chunks outside the range always return an emptyChunk, so just return true here to say we have it
...
{
	// we're not dropping things anymore now that we have a fixed sized cache
#ifdef _LARGE_WORLDS
...
// this is the original (and virtual) interface to create
LevelChunk *ServerChunkCache::create(int x, int z)
{
	return create(x, z, false);
}

LevelChunk *ServerChunkCache::create(int x, int z, bool asyncPostProcess)	// added extra parameter
{
	int ix = x + XZOFFSET;
	int iz = z + XZOFFSET;
	// Check we're in range of the stored level
	...
	if( ( chunk == NULL ) || ( chunk->x != x ) || ( chunk->z != z ) )
	{
...
#endif // _LINUX
		{
			...
			EnterCriticalSection(&m_csLoadCreate);
			// added - this will run a recalcHeightmap if source is a randomlevelsource, which has been split out from source::getChunk so that
			...
			// If post-processing is to be async, then let the server know about requests rather than processing directly here. Note that
			...
			if( asyncPostProcess )
			{
				// TODO This should also be calling the same code as chunk->checkPostProcess, but then we cannot guarantee we are in the server add the post-process request
			...
			// Now try and fix up any chests that were saved pre-1.8.2. We don't want to do this to this particular chunk as we don't know if all its neighbours are loaded yet, and we
...
// Split out this function so that we get a chunk without loading entities
...
#ifdef _LARGE_WORLDS
// this special variation on getChunk also checks the unloaded chunk cache. It is called on a host machine from the client-side level when:
...
// normally we try and set these flags when a chunk is post-processed. However, when setting in a north or easterly direction the
...
// add a flag to a chunk to say that one of its neighbours has completed post-processing. If this completes the set of
...
bool ServerChunkCache::save(bool force, ProgressListener *progressListener)
{
	...
	// added this to support progressListner
	int count = 0;
	...
	if(!force)
	{
		//app.DebugPrintf("Unsaved chunks = %d/n", level->getUnsavedChunkCount() );
		...
		for (unsigned int i = 0; i < m_loadedChunkList.size(); i++)
		{
			...
			if (chunk->shouldSave(force))
			{
				...
				// added this to support progressListener
				if (progressListener != NULL)
				{
					if (++cc % 10 == 0)
					{
	...
	{
#if 1 //_LARGE_WORLDS
		// We have multiple for threads for all saving as part of the storage, so use that rather than new threads here
		...
		CThread::Event *wakeEvent[3]; // This sets off the threads that are waiting to continue
		CThread::Event *notificationEvent[3]; // These are signalled by the threads to let us know they are complete
		CThread *saveThreads[3];
		DWORD threadId[3];
		...
		for(unsigned int i = 0; i < 3; ++i)
		{
			...
			wakeEvent[i] = new CThread::Event(); //CreateEvent(NULL,FALSE,FALSE,NULL);
			threadData[i].wakeEvent = wakeEvent[i];
			
			notificationEvent[i] = new CThread::Event(); //CreateEvent(NULL,FALSE,FALSE,NULL);
		...
		{
			workingThreads = 0;
			PIXBeginNamedEvent(0,"Setting tasks for save threads/n");
			for(unsigned int j = 0; j < 3; ++j)
			{
				...
				while(!chunkSet && i < sortedChunkList.size())
				{
					...
					if (chunk->shouldSave(force) || threadData[j].saveEntities)
					{
						...
						//app.DebugPrintf("Chunk to save set for thread %d/n", j);

						if(saveThreads[j] == NULL)
						{
							char threadName[256];
							sprintf(threadName,"Save thread %d/n",j);
							...
							saveThreads[j] = new CThread(runSaveThreadProc,(void *)&threadData[j],threadName);
				...
				if( !chunkSet )
				{
					...
					//app.DebugPrintf("No chunk to save set for thread %d/n",j);
				}
			}
			PIXEndNamedEvent();
			PIXBeginNamedEvent(0,"Waking save threads/n");
			// Start the worker threads going
			for(unsigned int k = 0; k < 3; ++k)
			{
				//app.DebugPrintf("Waking save thread %d/n",k);
				threadData[k].wakeEvent->Set(); //SetEvent(threadData[k].wakeEvent);
			}
			PIXEndNamedEvent();
			PIXBeginNamedEvent(0,"Waiting for completion of save threads/n");
			//app.DebugPrintf("Waiting for %d save thread(s) to complete/n", workingThreads);
		...
		//app.DebugPrintf("Clearing up worker threads/n");
		// Stop all the worker threads by giving them nothing to process then telling them to start
		unsigned char validThreads = 0;
		for(unsigned int i = 0; i < 3; ++i)
		{
			//app.DebugPrintf("Settings chunk to NULL for save thread %d/n", i);
			threadData[i].chunkToSave = NULL;

			//app.DebugPrintf("Setting wake event for save thread %d/n",i);
		...
		for(unsigned int i = 0; i < 3; ++i)
		{
			//app.DebugPrintf("Closing handles for save thread %d/n", i);
...
int ServerChunkCache::runSaveThreadProc(LPVOID lpParam)
{
	...
	//app.DebugPrintf("Save thread has started/n");

	while(params->chunkToSave != NULL)
	{
		PIXBeginNamedEvent(0,"Saving entities");
		//app.DebugPrintf("Save thread has started processing a chunk/n");
		...
		//app.DebugPrintf("Save thread has alerted producer that it is complete/n");
	...
	//app.DebugPrintf("Thread is exiting as it has no chunk to process/n");
