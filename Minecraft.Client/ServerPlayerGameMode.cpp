...
bool ServerPlayerGameMode::destroyBlock(int x, int y, int z)
{
	...
	// In creative mode, the point where we need to tell the renderer that we are about to destroy a tile via destroyingTileAt is quite complicated.
	...
	if (isCreative())
	{
		...
		// a bit of a hack here, but if we want to tell the client that it needs to inform the renderer of a block being destroyed, then send a block 255 instead of a 0. This is handled in ClientConnection::handleTileUpdate
		if( tup->block == 0 )
		{
