...
{
	// changing this to a game host option ot hide usernames
	...
	{
		//app.Debugprintf("Cannot move to position (%f, %f, %f), falling back to (%f, %f, %f)/n", x, y, z, tempX, y, tempZ);
...
//  override to capture event for tutorial messages
...
void MultiplayerLocalPlayer::ride(shared_ptr<Entity> e)
{
	...
	if( isRiding )
	{
		...
		TelemetryManager->RecordEnemyKilledOrOvercome(GetPadIndex(), 0, y, 0, 0, 0, 0, eventType);
	}
...
void MultiplayerLocalPlayer::setAndBroadcastCustomSkin(DWORD skinId)
{
	...
	wprintf(L"Skin for local player %ls has changed to %ls (%d)/n", name.c_str(), customTextureUrl.c_str(), getPlayerDefaultSkin() );
#endif
	if(getCustomSkin() != oldSkinIndex) connection->send( shared_ptr<TextureAndGeometryChangePacket>( new TextureAndGeometryChangePacket( shared_from_this(), app.GetPlayerSkinName(GetPadIndex()) ) ) );
}
...
#ifndef _CONTENT_PACKAGE
	wprintf(L"Cape for local player %ls has changed to %ls/n", name.c_str(), customTextureUrl2.c_str());
#endif
	if(getCustomCape() != oldCapeIndex) connection->send( shared_ptr<TextureChangePacket>( new TextureChangePacket( shared_from_this(), TextureChangePacket::e_TextureChange_Cape, app.GetPlayerCapeName(GetPadIndex()) ) ) );
}
