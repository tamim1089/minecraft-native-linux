...
void PistonPieceRenderer::render(shared_ptr<TileEntity> _entity, double x, double y, double z, float a, bool setColor, float alpha, bool useCompiled)
{
    ...
    if (tile != NULL && entity->getProgress(a) <= 1)	// changed condition from < to <= as our chunk update is async to main thread and so we can have to render these with progress of 1
	{
        Tesselator *t = Tesselator::getInstance();
        bindTexture(TN_TERRAIN);

        Lighting::turnOff();
		glColor4f(1, 1, 1, 1);	// this wouldn't be needed in real opengl as the block render has vertex colours and so this isn't use, but our pretend gl always modulates with this
