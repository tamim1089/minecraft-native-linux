...
HRESULT CScene_DebugOverlay::OnNotifyPressEx(HXUIOBJ hObjPressed, XUINotifyPress* pNotifyPressData, BOOL& rfHandled)
{
	...
	/*else if( hObjPressed == m_saveToDisc ) // Doesn't look like we use this debug option anymore.
	{
...
RegionFile *CScene_DebugOverlay::getRegionFile(unordered_map<File, RegionFile *, FileKeyHash, FileKeyEq> &newFileCache, ConsoleSaveFile *saveFile, const wstring &prefix, int chunkX, int chunkZ)		// TODO was synchronized
{
    ...
    newFileCache[file] = reg;	   // this was originally a softReferenc
    return reg;
}
