...
#include "XUI_Transfer.h"


//----------------------------------------------------------------------------------
// Performs initialization tasks - retrieves controls.
//----------------------------------------------------------------------------------
HRESULT CScene_Transfer::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	m_iX=-1;
	...
	if(StorageManager.TMSPP_InFileList(CStorage::eGlobalStorage_TitleUser,m_iPad,L"Linux/SlotList"))
	{
		// there is a slot list file with details of the saves
		CStorage::ETMSStatus status=StorageManager.TMSPP_ReadFile(m_iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,"Linux/SlotList",&CScene_Transfer::TMSPPSlotListReturned,this);
...
//----------------------------------------------------------------------------------
int CScene_Transfer::TMSPPWriteReturned(LPVOID pParam,int iPad,int iUserData)
{
	CScene_Transfer* pClass = (CScene_Transfer *) pParam;
...
// TMSPPSlotListReturned callback
//----------------------------------------------------------------------------------
int CScene_Transfer::TMSPPDeleteReturned(LPVOID pParam,int iPad,int iUserData)
{
	CScene_Transfer* pClass = (CScene_Transfer *) pParam;
...
// TMSPPSlotListReturned callback
//----------------------------------------------------------------------------------
int CScene_Transfer::TMSPPSlotListReturned(LPVOID pParam,int iPad,int iUserData,CStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
	CScene_Transfer* pClass = (CScene_Transfer *) pParam;
...
//----------------------------------------------------------------------------------
HRESULT CScene_Transfer::OnDestroy()
{
	return S_OK;
}
...
//----------------------------------------------------------------------------------
HRESULT CScene_Transfer::OnNotifyPressEx(HXUIOBJ hObjPressed, XUINotifyPress* pNotifyPressData, BOOL& rfHandled)
{
...
HRESULT CScene_Transfer::BuildSlotFile(int iIndexBeingUpdated,PBYTE pbImageData,DWORD dwImageBytes )
{
	...
	*((unsigned int *)pbNewSlotFileData)=m_MaxSlotC;
	...
	loadingParams->func = &CScene_Transfer::UploadSaveThreadProc;
...
int CScene_Transfer::UploadSaveThreadProc( LPVOID lpParameter )
{
	HRESULT hr = S_OK;
	char szFilename[32];
	CScene_Transfer* pClass = (CScene_Transfer *) lpParameter;
	...
	CStorage::ETMSStatus eStatus=StorageManager.TMSPP_WriteFile(pClass->m_iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,CStorage::TMS_UGCTYPE_NONE,"Linux/SlotList",(PCHAR) pClass->m_pbSlotListFile, pClass->m_uiSlotListFileBytes,
		&CScene_Transfer::TMSPPWriteReturned,lpParameter, 0);

	if(eStatus!=CStorage::ETMSStatus_WriteInProgress)
	{
		// failed
		pClass->m_bWaitingForWrite=false;
	}
	else
	{
		...
		while(pClass->m_bWaitingForWrite && (hr == S_OK))
		{
			...
			{
				// This causes the fullscreenprogress scene to ignore the action it was given
				hr = ERROR_CANCELLED;
			}			
		}
		...
		CStorage::ELoadGameStatus eLoadStatus=StorageManager.LoadSaveData(&pClass->m_XContentData,CScene_Transfer::LoadSaveDataReturned,lpParameter);
		...
		{
			sprintf( szFilename, "Linux/Slot%.2d%.2d", pClass->m_uiSlotID,i );
			PCHAR pchData=((PCHAR)pClass->m_pvSaveMem)+i*uiChunkSize;
		
			pClass->m_bWaitingForWrite=true;
			if(uiBytesLeft>=uiChunkSize)
			{
				eStatus=StorageManager.TMSPP_WriteFile(pClass->m_iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,CStorage::TMS_UGCTYPE_NONE,szFilename,pchData, uiChunkSize,
					&CScene_Transfer::TMSPPWriteReturned,lpParameter, 0);
			...
			else
			{
				...
				eStatus=StorageManager.TMSPP_WriteFile(pClass->m_iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,CStorage::TMS_UGCTYPE_NONE,szFilename,pchData, uiBytesLeft,
					&CScene_Transfer::TMSPPWriteReturned,lpParameter, 0);
			}
...
int CScene_Transfer::LoadSaveDataReturned(void *pParam,bool bContinue)
{
	CScene_Transfer* pClass = (CScene_Transfer*)pParam;
...
HRESULT CScene_Transfer::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	if(m_bIgnoreInput) return S_OK;
	...
	case VK_PAD_X:
		...
		if(m_pbSlotListFile!=NULL)
		{
			...
			CStorage::ETMSStatus eStatus=StorageManager.TMSPP_DeleteFile(m_iPad,"Linux/SlotList",CStorage::TMS_FILETYPE_BINARY,&CScene_Transfer::TMSPPDeleteReturned,this, 0);
...
HRESULT CScene_Transfer::OnNotifySelChanged(HXUIOBJ hObjSource, XUINotifySelChanged *pNotifySelChangedData, BOOL& bHandled)
{
...
HRESULT CScene_Transfer::OnTransitionStart( XUIMessageTransition *pTransition, BOOL& bHandled )
{
...
HRESULT CScene_Transfer::OnFontRendererChange()
{
	// update the tooltips
	CXuiSceneBase::SetTooltips( DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK, m_iX, -1,-1,-1,-1,-1,-1,true);
...
HRESULT CScene_Transfer::OnNotifySetFocus(HXUIOBJ hObjSource, XUINotifyFocus *pNotifyFocusData, BOOL& bHandled)
{
	// update the tooltips
	CXuiSceneBase::SetTooltips( DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK, m_iX, -1,-1,-1,-1,-1,-1,true);
...
HRESULT CScene_Transfer::OnNotifyKillFocus(HXUIOBJ hObjSource, XUINotifyFocus *pNotifyFocusData, BOOL& bHandled)
{
	return S_OK;
}
