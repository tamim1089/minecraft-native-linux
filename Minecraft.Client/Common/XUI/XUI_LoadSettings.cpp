		...
		if(m_bGameModeSurvival != true || m_bHasBeenInCreative)
		{
			...
			else // it's creative mode
			{
				...
				if(m_bHasBeenInCreative)
				{
					...
					{
						CStorage::ELoadGameStatus eLoadStatus=StorageManager.LoadSaveData(&m_XContentData,CScene_LoadGameSettings::LoadSaveDataReturned,this);
...
int CScene_LoadGameSettings::CheckResetNetherReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
{
	// stop people double pressing this
	...
	if(hObjPressed==m_ButtonLoad)
	{
		...
		if(m_MoreOptionsParams.dwTexturePack!=0)
		{
			...
			if(pTexturePack==NULL)
			{
				...
				// if the full offer id is 0, then the texture pack dlc load failed
			...
			{		
				...
				if(m_pDLCPack && !m_pDLCPack->hasPurchasedFile( DLCManager::e_DLCType_Texture, L"" ))
				{
					...
					if(!ProfileManager.IsSignedInLive(pNotifyPressData->UserIndex))
					{
						...
						StorageManager.RequestMessageBox(IDS_PRO_NOTONLINE_TITLE, IDS_PRO_LINUXLIVE_NOTIFICATION, uiIDA, 1);
						return S_OK;
					}
					else
					{
...
HRESULT CScene_LoadGameSettings::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	case VK_PAD_RSHOULDER:
		{
			app.NavigateToScene(m_iPad,eUIScene_Transfer,m_params);
		}
		*/
		break;
	}
...
int CScene_LoadGameSettings::ConfirmLoadReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
	...
	if(result==CStorage::EMessage_ResultAccept) 
	{
		...
		{
			CStorage::ELoadGameStatus eLoadStatus=StorageManager.LoadSaveData(&pClass->m_XContentData,CScene_LoadGameSettings::LoadSaveDataReturned,pClass);
...
int CScene_LoadGameSettings::DeleteSaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
...
int CScene_LoadGameSettings::DeviceRemovedDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
HRESULT CScene_LoadGameSettings::OnTransitionStart( XUIMessageTransition *pTransition, BOOL& bHandled )
{
	...
	{	
		...
		// Need to check for installed DLC, which might have happened while you were on the More Options scene
...
int CScene_LoadGameSettings::UnlockTexturePackReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
int CScene_LoadGameSettings::TexturePackDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_LoadGameSettings *pClass = (CScene_LoadGameSettings *)pParam;
#ifdef _LINUX
