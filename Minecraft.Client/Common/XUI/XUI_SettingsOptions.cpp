...
HRESULT CScene_SettingsOptions::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	XuiControlSetText(m_InGameUsernames,app.GetString(IDS_IN_GAME_USERNAMES));
	...
	m_InGameUsernames.SetCheck( (app.GetGameSettings(m_iPad,eGameSetting_UsernamesVisible)!=0)?TRUE:FALSE);
	...
	// If you are in-game, only the game host can change in-game usernames, and you can't change difficulty
	...
	bool bRemoveInGameUsernames=false;
	...
	if(bRemoveInGameUsernames)
	{
		m_InGameUsernames.SetShow(FALSE);
		m_InGameUsernames.SetEnable(FALSE);
		m_InGameUsernames.GetBounds(&fWidth, &fHeight);

		// move the mash-up worlds option up 
		if(m_bMashUpWorldsUnhideOption)
		{
			D3DXVECTOR3 vec;
			XuiElementGetPosition(m_InGameUsernames,&vec);
...
HRESULT CScene_SettingsOptions::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	case VK_ESCAPE:
		...
		app.SetGameSettings(m_iPad,eGameSetting_UsernamesVisible,m_InGameUsernames.IsChecked()?1:0);
		...
		// don't action changes here or we might write to the profile on backing out here and then get a change in the settings all, and write again on backing out there
...
HRESULT CScene_SettingsOptions::OnTransitionStart( XUIMessageTransition *pTransition, BOOL& bHandled )
{
	...
	{		
		...
		hr=XuiControlGetVisual(m_InGameUsernames.m_hObj,&hVisual);
		hr=XuiElementGetChildById(hVisual,L"text_Button",&hCheckboxText);
		hr=XuiElementGetPosition(hCheckboxText,&vecCheckboxText);
		hr=XuiElementGetPosition(m_InGameUsernames.m_hObj,&vecCheckbox);		
		...
		for(int i=0;i<OPTIONS_SLIDER_SETTINGS_MAX;i++)
		{
			...
			// the text measuring doesn't seem to be long enough - add a fudge
			xuiRect.right+=25.0f;
		...
		hr=XuiTextPresenterMeasureText(hCheckboxText, m_InGameUsernames.GetText(), &xuiRect);
		m_InGameUsernames.GetBounds(&fWidth,&fHeight);
		...
		{		
			...
			m_InGameUsernames.SetBounds(fMaxTextLen,fHeight);
