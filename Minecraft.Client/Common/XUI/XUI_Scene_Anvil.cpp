...
HRESULT CXuiSceneAnvil::OnNotifyValueChanged (HXUIOBJ hObjSource, XUINotifyValueChanged* pValueChangedData, BOOL& rfHandled)
{
	if(hObjSource == m_editName)
	{
		...
		// This was stopping the player deleting the last character in the name using the chatpad
...
// Added to support auto-save. Need to re-associate on a navigate back
...
{
	// The edit control already handles A/Start presses, so ignore anything else
