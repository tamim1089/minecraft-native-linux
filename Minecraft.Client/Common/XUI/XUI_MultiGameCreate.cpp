...
int CScene_MultiGameCreate::UnlockTexturePackReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
int CScene_MultiGameCreate::WarningTrialTexturePackReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_MultiGameCreate* pScene = (CScene_MultiGameCreate*)pParam;
	pScene->m_bIgnoreInput = false;
	pScene->SetShow( TRUE );
...
int CScene_MultiGameCreate::ConfirmCreateReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
void CScene_MultiGameCreate::CreateGame(CScene_MultiGameCreate* pClass, DWORD dwLocalUsersMask)
{
	...
	__int64 seedValue = 0; //BiomeSource::findSeed(isFlat?LevelType::lvl_flat:LevelType::lvl_normal);	// was (new Random())->nextLong() - now trying to actually find a seed to suit our requirements
	...
	{
		seedValue = BiomeSource::findSeed(isFlat?LevelType::lvl_flat:LevelType::lvl_normal);	// was (new Random())->nextLong() - now trying to actually find a seed to suit our requirements
	}
...
HRESULT CScene_MultiGameCreate::OnNotifySelChanged( HXUIOBJ hObjSource, XUINotifySelChanged* pNotifySelChangedData, BOOL& bHandled )
{
	if(hObjSource == m_pTexturePacksList->m_hObj)
	{
		...
		// Removed expand description check, taken care of elsewhere.
	}
...
int CScene_MultiGameCreate::TexturePackDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_MultiGameCreate *pClass = (CScene_MultiGameCreate *)pParam;
