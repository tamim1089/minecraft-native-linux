...
HRESULT CXuiSceneBase::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	for(unsigned int idx = 0; idx < XUSER_MAX_COUNT; ++idx)
	{
		...
		case 0:
			...
			XuiElementGetChildById(hTemp,L"XuiUsername",&m_hUsernameA[0]);
			break;
		case 1:
			XuiElementGetChildById(m_hObj,L"BasePlayer1",&hTemp);
			XuiElementGetChildById(hTemp,L"XuiUsername",&m_hUsernameA[1]);
			break;
		case 2:
			XuiElementGetChildById(m_hObj,L"BasePlayer2",&hTemp);
			XuiElementGetChildById(hTemp,L"XuiUsername",&m_hUsernameA[2]);
			break;
		case 3:
			XuiElementGetChildById(m_hObj,L"BasePlayer3",&hTemp);
			XuiElementGetChildById(hTemp,L"XuiUsername",&m_hUsernameA[3]);
			break;
		}
	}
...
void CXuiSceneBase::_TickAllBaseScenes()
{
	...
	// added to upsell the texture pack if you join from invite and don't have it
	...
	else
	{
		...
		for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
		{
			if(pMinecraft->localplayers[i] != NULL && pMinecraft->localplayers[i]->dimension == 1 && !ui.GetMenuDisplayed(i) && app.GetGameSettings(i,eGameSetting_DisplayHUD))
			{
				...
				if(pMinecraft->localplayers[i]->m_iScreenSection == CRender::VIEWPORT_TYPE_FULLSCREEN)
				{
				...
				else
				{
					...
					if((pMinecraft->localplayers[i]->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_TOP) ||
						(pMinecraft->localplayers[i]->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_BOTTOM))
					{
						switch(iGuiScale)
						{
						case 0:
...
HRESULT CXuiSceneBase::_ShowTooltip( unsigned int iPad, unsigned int tooltip, bool show )
{
	...
	if ( (ProfileManager.GetLockedProfile()!=-1) && app.GetGameSettings(iPad,eGameSetting_DisplayHUD)==0 )
	{
		...
		XuiElementSetShow(m_hUsernameA[iPad],FALSE);
	...
	//else
	{
		...
		//also set the usernames and the 4th quadrant logo
		if(ProfileManager.GetLockedProfile()!=-1)
		{
			if(app.GetGameSettings(iPad,eGameSetting_DisplaySplitscreenUsernames)==1)
			{
				unsigned char ucAlpha=app.GetGameSettings(ProfileManager.GetPrimaryPad(),eGameSetting_InterfaceOpacity);
				float fVal;
				...
				XuiElementSetOpacity(m_hUsernameA[iPad],fVal);
			}
...
HRESULT CXuiSceneBase::_ShowSavingMessage( unsigned int iPad, CStorage::ESavingMessage eVal )
{
	switch(eVal)
	{
...
HRESULT CXuiSceneBase::_SetPlayerBaseScenePosition( unsigned int iPad, EBaseScenePosition position )
{
	...
	// Shift the tooltips
	D3DXVECTOR3 tooltipsPos,crouchIconPos,saveIconPos,vBackPos,vUsernamePos,vBossHealthPos;
	tooltipsPos.z=crouchIconPos.z=saveIconPos.z=vBackPos.z=vBossHealthPos.z=0.0f;
	vBackPos.x=0.0f;
	vBackPos.y=0.0f;

	m_playerBaseScenePosition[iPad] = position;
	float fTempWidth, fTooltipHeight, fTooltipHeightSmall,fUsernameWidth,fUsernameHeight,fBossHealthWidth, fBossHealthHeight;
	...
	XuiElementGetBounds(m_hUsernameA[iPad],&fUsernameWidth, &fUsernameHeight);
	...
	case e_BaseScene_Top_Left:
		...
		vUsernamePos.x=XUI_BASE_SCENE_WIDTH_HALF-fUsernameWidth - 10.0f;
		vUsernamePos.y=SAFEZONE_HALF_HEIGHT;
	...
	case e_BaseScene_Top: // Top & Bottom - indent by a quarter screen
		...
		vBackPos.y=0.0f;
		vUsernamePos.x=XUI_BASE_SCENE_WIDTH - XUI_BASE_SCENE_WIDTH_QUARTER - fUsernameWidth - SAFEZONE_HALF_WIDTH;
		vUsernamePos.y=SAFEZONE_HALF_HEIGHT;
		vBossHealthPos.x = XUI_BASE_SCENE_WIDTH_QUARTER-(fBossHealthWidth/2);
		vBossHealthPos.y = SAFEZONE_HALF_HEIGHT;
		break;
	...
	case e_BaseScene_Bottom_Right:
		...
		fBackWidth=XUI_BASE_SCENE_WIDTH_HALF;
		fBackHeight=XUI_BASE_SCENE_HEIGHT_HALF;
		vUsernamePos.x=XUI_BASE_SCENE_WIDTH_HALF-fUsernameWidth-SAFEZONE_HALF_WIDTH;
		vUsernamePos.y=0.0f;
		vBossHealthPos.x = XUI_BASE_SCENE_WIDTH_QUARTER-(fBossHealthWidth/2);
		vBossHealthPos.y = 0.0f;
		break;
	case e_BaseScene_Left:
		...
		vUsernamePos.x=XUI_BASE_SCENE_WIDTH_HALF-fUsernameWidth- 10.0f;
		vUsernamePos.y=SAFEZONE_HALF_HEIGHT - XUI_BASE_SCENE_HEIGHT_QUARTER;
		vBossHealthPos.x = XUI_BASE_SCENE_WIDTH_QUARTER-(fBossHealthWidth/2);
	...
	vUsernamePos.z=0.0f;
	XuiElementSetPosition( m_hUsernameA[iPad], &vUsernamePos );
	XuiElementSetPosition( m_BossHealthGroup[iPad], &vBossHealthPos );


	// If we already have some scenes open, then call this to update their positions
	...
	{
		// Can only do this once we know what the player's UI settings are, so we need to have the player game settings read
...
void CXuiSceneBase::_UpdateSelectedItemPos(unsigned int iPad)
{
		...
		(RenderManager.IsHiDef() || RenderManager.IsWidescreen()) )
	{
		...
		// selected the gui scale based on the slider settings, and on whether we're in Creative or Survival		
		float fYOffset=0.0f;
	...
	else
	{
		...
		// If it's in split screen vertical, adjust the position
		// Adjust selectedItemPos based on what gui is displayed
		...
		{	
			...
			// selected the gui scale based on the slider settings
...
HRESULT CXuiSceneBase::_DisplayUsername( unsigned int iPad, BOOL bDisplay )
{

	if(app.DebugSettingsOn() && (app.GetGameSettingsDebugMask()&(1L<<eDebugSetting_DebugLeaderboards)))
	{
		XuiControlSetText(m_hUsernameA[iPad],L"WWWWWWWWWWWWWWWW");
	}
	else
	{
		...
		{		
			if(Minecraft::GetInstance() != NULL && Minecraft::GetInstance()->localplayers[iPad]!=NULL)
			{
				wstring wsUsername = convStringToWstring( ProfileManager.GetUsername(iPad));
				XuiControlSetText(m_hUsernameA[iPad],wsUsername.c_str());

			}
			else
			{
				XuiControlSetText(m_hUsernameA[iPad],L"");
			}
		}
	}
	// The host decides whether these are on or off
	if(app.GetGameSettings(ProfileManager.GetPrimaryPad(),eGameSetting_DisplaySplitscreenUsernames)!=0)
	{		
		XuiElementSetShow(m_hUsernameA[iPad],bDisplay);
		...
		if((bDisplay==TRUE) &&(ProfileManager.GetLockedProfile()!=-1))
		{
			...
			XuiElementSetOpacity(m_hUsernameA[iPad],0.01f*fVal);
		}
	}
	else
	{
	...
	for( unsigned int i = 0; i < BUTTONS_TOOLTIP_MAX; ++i )
	{
		if( m_visible[iPad][ i ] == TRUE )
		{
			if( i>0 && lastVisible!=-1 )
			{
				...
				// This is for horizontal layout, will need changed if we do vertical layout
...
HRESULT CXuiSceneBase::ShowSavingMessage( unsigned int iPad, CStorage::ESavingMessage eVal )
{
	if( CXuiSceneBase::Instance != NULL )
	{
...
HRESULT CXuiSceneBase::DisplayUsername( unsigned int iPad, BOOL bDisplay )
{

	CXuiSceneBase::Instance->_DisplayUsername(iPad,bDisplay);
	return S_OK;
}
