...
HRESULT CXuiCtrlMinecraftSkinPreview::OnRender(XUIMessageRender *pRenderData, BOOL &bHandled )
{
	...
	// Turning on hideGui while we do this stops the name rendering in split-screen
...
// Modified version of MobRenderer::render that does not require an actual entity
void CXuiCtrlMinecraftSkinPreview::render(EntityRenderer *renderer, double x, double y, double z, float rot, float a)
{
   ...
   //getAttackAnim(mob, a);
	...
	// Remember to reset these values once the rendering is done if you add another one
	model->attackTime = 0;
