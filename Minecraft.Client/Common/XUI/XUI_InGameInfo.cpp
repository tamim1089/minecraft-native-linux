...
HRESULT CScene_InGameInfo::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	// ignore repeats to stop the scene displaying and quitting right away if you hold the back button down
	...
	case VK_PAD_Y:
		if(playersList.TreeHasFocus() && (playersList.GetItemCount() > 0) && (playersList.GetCurSel() < m_playersCount) )
		{
			...
			if( player != NULL )
			{
				PlayerUID uid = ((NetworkPlayerLinux *)player)->GetUID();
				if( uid != INVALID_UID )
					hr = XShowGamerCardUI(pInputData->UserIndex, uid);
			}
		}
		break;
	case VK_PAD_X:
...
int CScene_InGameInfo::KickPlayerReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
