...
#ifdef _LINUX
	static int TMSReadFileListReturned(void *pParam,int iPad,CStorage::PTMSPP_FILE_LIST pTmsFileList);
	static int TMSFileWriteReturned(void *pParam,int iPad,int iResult);
	static int TMSFileReadReturned(void *pParam,int iPad,CStorage::PTMSPP_FILEDATA pData);
#endif
};
