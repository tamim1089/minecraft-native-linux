...
// 	}
	...
	// Load up any texture pack data we have locally in the XZP
...
HRESULT CScene_MultiGameJoinLoad::GetSaveInfo(  )
{
	...
	else
	{
		...
		m_iSaveInfoC=0;
#ifdef _LINUX
		CStorage::ESGIStatus eSGIStatus=StorageManager.GetSavesInfo(ProfileManager.GetPrimaryPad(),&CScene_MultiGameJoinLoad::GetSavesInfoCallback,this,"savegame.dat");
		...
		//CStorage::ESaveGameState eStatus=StorageManager.GetSavesInfo(ProfileManager.GetPrimaryPad(),&CScene_MultiGameJoinLoad::GetSavesInfoCallback,this,"savegame.dat");
...
int CScene_MultiGameJoinLoad::DeviceRemovedDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
HRESULT CScene_MultiGameJoinLoad::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	case VK_PAD_Y:
		if(m_pGamesList->TreeHasFocus() && m_pGamesList->GetItemCount() > 0)
		{
			...
			PlayerUID uid = pSelectedSession->data.hostPlayerUID;
			if( uid != INVALID_UID )
				hr = XShowGamerCardUI(ProfileManager.GetLockedProfile(), uid);
			CXuiSceneBase::PlayUISFX(eSFX_Press);
		}
		else if(DoesSavesListHaveFocus())
		{
			...
			if(ProfileManager.IsSignedInLive( m_iPad ))
			{	
				// required for a delete of the save if it's found to be a corrupted save
...
HRESULT CScene_MultiGameJoinLoad::OnNavReturn(HXUIOBJ hSceneFrom,BOOL& rfHandled)
{
	...
	{	
		if(ProfileManager.IsSignedInLive( m_iPad ))
		{
			iY=IDS_TOOLTIPS_UPLOAD_SAVE_FOR_LINUXONE;
		}
...
HRESULT CScene_MultiGameJoinLoad::OnTransitionStart( XUIMessageTransition *pTransition, BOOL& bHandled )
{
	...
	{
		// We may have had to unload our font renderer in this scene if one of the save files
	...
	else if(pTransition->dwTransType == XUI_TRANSITION_TO || pTransition->dwTransType == XUI_TRANSITION_BACKTO)
	{
		...
		// Need to check for installed DLC, which might have happened while you were on the info scene
...
{
	// TODO - Don't think we can do this - if a 2nd player signs in here with an offline profile, the signed in LIVE player gets re-logged in, and bMultiplayerAllowed is false briefly 
	switch(pTimer->nId)
	{
...
int CScene_MultiGameJoinLoad::DeleteSaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_MultiGameJoinLoad* pClass = (CScene_MultiGameJoinLoad*)pParam;
	// results switched for this dialog
	if(result==CStorage::EMessage_ResultDecline) 
...
int CScene_MultiGameJoinLoad::SaveTransferDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_MultiGameJoinLoad* pClass = (CScene_MultiGameJoinLoad*)pParam;
	// results switched for this dialog
	if(result==CStorage::EMessage_ResultAccept) 
	{
		...
		// ensure we've switched to the right title group id for uploading to
		app.TMSPP_SetTitleGroupID(SAVETRANSFER_GROUP_ID);
		StorageManager.GetSaveCacheFileInfo(iIndex,ContentData);
		CStorage::ELoadGameStatus eLoadStatus=StorageManager.LoadSaveData(&ContentData,CScene_MultiGameJoinLoad::LoadSaveDataReturned,pClass);
...
int CScene_MultiGameJoinLoad::UploadSaveThreadProc( LPVOID lpParameter )
{
...
void CScene_MultiGameJoinLoad::DeleteFile(CScene_MultiGameJoinLoad *pClass, char *filename)
{
	...
	CStorage::ETMSStatus result = StorageManager.TMSPP_DeleteFile(
		ProfileManager.GetPrimaryPad(),
		filename,
		CStorage::TMS_FILETYPE_BINARY,
		&CScene_MultiGameJoinLoad::DeleteComplete,
		pClass,
		NULL);

	if(result != CStorage::ETMSStatus_DeleteInProgress)
	{
...
void CScene_MultiGameJoinLoad::UploadFile(CScene_MultiGameJoinLoad *pClass, char *filename, LPVOID data, DWORD size)
{
	...
	CStorage::ETMSStatus result = StorageManager.TMSPP_WriteFileWithProgress(
			ProfileManager.GetPrimaryPad(),
			CStorage::eGlobalStorage_TitleUser,
			CStorage::TMS_FILETYPE_BINARY,
			CStorage::TMS_UGCTYPE_NONE,
			filename,
			(CHAR *)data, 
			size,
	...
	if(result != CStorage::ETMSStatus_WriteInProgress)
	{
...
int CScene_MultiGameJoinLoad::SaveOptionsDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_MultiGameJoinLoad* pClass = (CScene_MultiGameJoinLoad*)pParam;
	...
	{
		if(result==CStorage::EMessage_ResultDecline) // rename
		{
...
int CScene_MultiGameJoinLoad::LoadSaveDataReturned(void *pParam,bool bContinue)
{
	...
	if(bContinue==true)
	{
		...
		LoadingInputParams *loadingParams = new LoadingInputParams();
		loadingParams->func = &CScene_MultiGameJoinLoad::UploadSaveThreadProc;
...
int CScene_MultiGameJoinLoad::KeyboardReturned(void *pParam,bool bSet)
{
	...
	if((pClass->m_wchNewName[0]!=0) && bSet)
	{
		...
		CStorage::ELoadGameStatus eLoadStatus=StorageManager.LoadSaveData(&XContentData,CScene_MultiGameJoinLoad::LoadSaveDataForRenameReturned,pClass);
...
int CScene_MultiGameJoinLoad::TexturePackDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CScene_MultiGameJoinLoad *pClass = (CScene_MultiGameJoinLoad *)pParam;

	// Exit with or without saving
	// Decline means install full version of the texture pack in this dialog
	if(result==CStorage::EMessage_ResultDecline || result==CStorage::EMessage_ResultAccept) 
	{
...
#ifdef _LINUX
bool CScene_MultiGameJoinLoad::GetSavesInfoCallback(LPVOID pParam,int iTotalSaveInfoC, CStorage::CACHEINFOSTRUCT *InfoA, int iPad, HRESULT hResult)
{
	CScene_MultiGameJoinLoad *pClass=(CScene_MultiGameJoinLoad *)pParam;
	CXuiCtrl4JList::LIST_ITEM_INFO ListInfo;
	...
	{
		pbImageData+=sizeof(CStorage::CACHEINFOSTRUCT)*iTotalSaveInfoC;
	}
