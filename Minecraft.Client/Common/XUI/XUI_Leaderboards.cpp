...
void CScene_Leaderboards::UpdateTooltips()
{
	...
	// no room on the screen for the LT/RT prompt
	/*
	...
	case VK_PAD_Y:
		{
			...
			if( m_leaderboard.m_currentEntryCount > 0 )
			{
				...
				if( selection >= m_leaderboard.m_entryStartIndex-1 &&
					selection < (m_leaderboard.m_entryStartIndex+m_leaderboard.m_currentEntryCount-1) )
				{
					PlayerUID uid = m_leaderboard.m_entries[selection - (m_leaderboard.m_entryStartIndex-1)].m_uid;
					if( uid != INVALID_UID )
					{
						XShowGamerCardUI(ProfileManager.GetLockedProfile(), uid);
						CXuiSceneBase::PlayUISFX(eSFX_Press);
					}
				}
			}
				...
				if( m_leaderboard.m_currentEntryCount > 0 )
				{
						...
						selection < (m_leaderboard.m_entryStartIndex+m_leaderboard.m_currentEntryCount-1) )
					{
							...
							!m_leaderboard.m_entries[selection - (m_leaderboard.m_entryStartIndex-1) ].m_bRequestedFriend )
						{
							...
							if( uid != INVALID_UID )
							{
								XShowFriendRequestUI(ProfileManager.GetLockedProfile(), uid);
								CXuiSceneBase::PlayUISFX(eSFX_Press);
							}
						}
					}
...
void CScene_Leaderboards::GetFriends()
{
	...
	for( unsigned int friendIndex=0 ; friendIndex<numFriends ; ++friendIndex )
	{
		...
		{
			m_filteredFriends[m_numFilteredFriends++] = m_friends[friendIndex].uid;
		}
	}
	m_filteredFriends[m_numFilteredFriends++] = LeaderboardManager::Instance()->GetMyUID();
}
#endif
...
HRESULT CScene_Leaderboards::OnGetSourceDataText(XUIMessageGetSourceText *pGetSourceTextData,BOOL& bHandled)
{
	if( pGetSourceTextData->bItemData )
	{
		...
		else
		{
			...
			{
				pGetSourceTextData->szText = m_leaderboard.m_entries[index].m_username;
				bHandled = TRUE;
			}
...
HRESULT CScene_Leaderboards::OnGetSourceDataImage(XUIMessageGetSourceImage* pGetImage, BOOL& bHandled)
{
	if( (pGetImage->iData == 2) && (pGetImage->bItemData) )
	{
		...
		// we're not allowed to show flags any more
// 		else
// 		{
// 			unsigned int index = item - (m_leaderboard.m_entryStartIndex-1);
...
void CScene_Leaderboards::PopulateLeaderboard(bool noResults)
{
	...
	{
		hr=XuiElementGetChildById(visual,L"XuiLabel_Username",&hTemp);
		if(hTemp)
		{
			XuiControlSetText(hTemp,app.GetString( IDS_LEADERBOARD_USERNAME ));
		...
		// Fix for leaderboards taking forever to update. Was looping over m_totalEntryCount which is ~2mil in
		...
		{	
			...
			HXUIOBJ button;
			D3DXVECTOR3 vPos;
			// fix for #13768 - Leaderboards: Player scores appear misaligned when viewed under the "My Score" leaderboard filter
...
{
	leaderboardEntry->m_uid = statsRow->uid;
	...
	//strcpy(statsRow->szUsername,"WWWWWWWWWWWWWWWW");

	MultiByteToWideChar(
		CP_UTF8,
		0,
		statsRow->szUsername,
		strlen(statsRow->szUsername),
		leaderboardEntry->m_username,
		MAX_USERNAME_SIZE );
	leaderboardEntry->m_username[strlen(statsRow->szUsername)] = L'\0';
...
// 			leaderboardEntry->m_columns[i-1]=uiVal;
	...
	//Is the player
	if( statsRow->uid == ((LeaderboardManager*)LeaderboardManager::Instance())->GetMyUID() )
	{
	...
	else
	{
		...
		{
			if( m_friends[friendIndex].uid == statsRow->uid )
			{
				if( ( m_friends[friendIndex].dwFriendState & ( XONLINE_FRIENDSTATE_FLAG_SENTREQUEST | XONLINE_FRIENDSTATE_FLAG_RECEIVEDREQUEST ) ) == 0 )
				{
