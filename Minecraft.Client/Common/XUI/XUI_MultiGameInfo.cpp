...
HRESULT CScene_MultiGameInfo::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	XuiControlSetText(m_labelUsernamesOn,app.GetString(IDS_LABEL_USERNAMES));
	...
	// don't delete this - it's part of the joinload structure
	//delete initData;
	...
	if(app.GetGameHostOption(uiGameHostSettings,eGameHostOption_Usernames))	m_usernamesOn.SetText( app.GetString(IDS_ON) );
	else m_usernamesOn.SetText( app.GetString(IDS_OFF) );
...
HRESULT CScene_MultiGameInfo::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	case VK_PAD_Y:
		...
		{
			PlayerUID uid = m_selectedSession->data.players[playersList.GetCurSel()];
			if( uid != INVALID_UID )
