...
HRESULT CScene_SettingsUI::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	m_SplitScreenUsernames.SetCheck( (app.GetGameSettings(m_iPad,eGameSetting_DisplaySplitscreenUsernames)!=0)?TRUE:FALSE);
	m_SplitScreenUsernames.SetText(app.GetString( IDS_CHECKBOX_DISPLAY_SPLITSCREEN_USERNAMES ));
	...
	{	
		// If the game has started, then you need to be the host to change the in-game usernames
		if(!bPrimaryPlayer)
		{	
			// hide things we don't want the splitscreen player changing
			m_SplitScreen.SetShow(FALSE);
			m_SplitScreen.SetEnable(FALSE);
			m_SplitScreenUsernames.SetShow(FALSE);
			m_SplitScreenUsernames.SetEnable(FALSE);
			...
			m_SplitScreenUsernames.GetBounds(&fWidth, &fHeight);
			fRemoveHeight+=fHeight+4.0f; // add padding
...
HRESULT CScene_SettingsUI::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	case VK_ESCAPE:
		...
		app.SetGameSettings(m_iPad,eGameSetting_DisplaySplitscreenUsernames,m_SplitScreenUsernames.IsChecked()?1:0);
...
HRESULT CScene_SettingsUI::OnTransitionStart( XUIMessageTransition *pTransition, BOOL& bHandled )
{
	...
	{		
		...
		// now the m_SplitScreenUsernames checkbox - let's just use the visual we already have...

		hr=XuiTextPresenterMeasureText(hCheckboxText, m_SplitScreenUsernames.GetText(), &xuiRect);
		m_SplitScreenUsernames.GetBounds(&fWidth,&fHeight);
		// need to add the size of the checkbox graphic
		if((xuiRect.right+vecCheckbox.x+vecCheckboxText.x)>fMaxTextLen) fMaxTextLen=xuiRect.right+vecCheckbox.x+vecCheckboxText.x;
		if(fWidth>fMaxLen) fMaxLen=fWidth;
		...
		{		
			...
			m_SplitScreenUsernames.SetBounds(fMaxTextLen,fHeight);
