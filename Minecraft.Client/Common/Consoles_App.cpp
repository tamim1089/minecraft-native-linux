...
#include "../../Minecraft.Client/Linux/XML/ATGXmlParser.h"
#include "../../Minecraft.Client/Linux/XML/xmlFilesCallback.h"
#endif
...
#include "../Minecraft.h"
#ifdef _LINUX
#include "../Linux/GameConfig/Minecraft.spa.h"
#include "../Linux/Network/NetworkPlayerLinux.h"
...
CMinecraftApp::CMinecraftApp()
{
	...
	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		...
		if(FAILED(XUserGetSigninInfo(i,XUSER_GET_SIGNIN_INFO_OFFLINE_UID_ONLY ,&m_currentSigninInfo[i])))
		{
			m_currentSigninInfo[i].uid = INVALID_UID;
...
//	m_uiTransferSlotC=5;
...
#if (defined _CONTENT_PACAKGE) || (defined _LINUX)
	...
	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		m_eOptionsStatusA[i]=CStorage::eOptions_Callback_Idle;
	}
#endif
...
#if defined _LINUX// || defined _LINUX
		C_Profile::PROFILESETTINGS *pProfileSettings=ProfileManager.GetDashboardProfileSettings(i);
		// clear this for now - it will come from reading the system values
		memset(pProfileSettings,0,sizeof(C_Profile::PROFILESETTINGS));
		SetDefaultOptions(pProfileSettings,i);
#elif defined _LINUX || defined _LINUX || defined _LINUX  || defined _LINUX
		CStorage::PROFILESETTINGS *pProfileSettings=StorageManager.GetDashboardProfileSettings(i);
		// don't cause an options write to happen here
...
#if (defined _LINUX || defined _LINUX || defined _LINUX  || defined _LINUX)
int CMinecraftApp::SetDefaultOptions(CStorage::PROFILESETTINGS *pSettings,const int iPad,bool bWriteProfile)
#else
int CMinecraftApp::SetDefaultOptions(C_Profile::PROFILESETTINGS *pSettings,const int iPad)
#endif
{
	...
	// Don't reset the difficult level if we're in-game
	...
	SetGameSettings(iPad,eGameSetting_UsernamesVisible,1);

	// Interim TU 1.6.6
	SetGameSettings(iPad,eGameSetting_Sensitivity_InMenu,100);
	SetGameSettings(iPad,eGameSetting_DisplaySplitscreenUsernames,1);
	...
	// We'll only display the message if the profile is pre-TU5
	...
	// leave these in, or remove from everywhere they are referenced!
...
#if ( defined  _LINUX  || defined _LINUX || defined _LINUX  || defined _LINUX)
int CMinecraftApp::DefaultOptionsCallback(LPVOID pParam,CStorage::PROFILESETTINGS *pSettings, const int iPad)
#else
int CMinecraftApp::DefaultOptionsCallback(LPVOID pParam,C_Profile::PROFILESETTINGS *pSettings, const int iPad)
#endif
{
...
#ifdef _LINUX
int CMinecraftApp::OptionsDataCallback(LPVOID pParam,int iPad,unsigned short usVersion,CStorage::eOptionsCallback eStatus,int iBlocksRequired)
{
	CMinecraftApp *pApp=(CMinecraftApp *)pParam;
...
#else
int CMinecraftApp::OptionsDataCallback(LPVOID pParam,int iPad,unsigned short usVersion,CStorage::eOptionsCallback eStatus)
{
	CMinecraftApp *pApp=(CMinecraftApp *)pParam;
	pApp->m_eOptionsStatusA[iPad]=eStatus;
	return 0;
...
CStorage::eOptionsCallback CMinecraftApp::GetOptionsCallbackStatus(int iPad)
{
	return m_eOptionsStatusA[iPad];
}

void CMinecraftApp::SetOptionsCallbackStatus(int iPad, CStorage::eOptionsCallback eStatus)
{
...
int CMinecraftApp::OldProfileVersionCallback(LPVOID pParam,unsigned char *pucData, const unsigned short usVersion, const int iPad)
{
	...
	case PROFILE_VERSION_2:
		...
		{	
			...
			// Let's also award all the achievements they have again because of the profile bug that seemed to stop the awards of some
...
void CMinecraftApp::ApplyGameSettingsChanged(int iPad)
{
	...
	ActionGameSettings(iPad,eGameSetting_UsernamesVisible);

	// Interim TU 1.6.6
	ActionGameSettings(iPad,eGameSetting_Sensitivity_InMenu	);
	ActionGameSettings(iPad,eGameSetting_DisplaySplitscreenUsernames);
...
void CMinecraftApp::ActionGameSettings(int iPad,eGameSetting eVal)
{
	...
	case eGameSetting_Sensitivity_InGame:
		// we don't use the options value
	...
	case eGameSetting_ViewBob:
		// not handled here any more - it's read from the gamesettings per player
	...
	case eGameSetting_SplitScreenVertical:
		...
		break;
	case eGameSetting_UsernamesVisible:
		{
			bool bInGame=pMinecraft->level!=NULL;
			...
			if(bInGame && g_NetworkManager.IsHost() && (iPad==ProfileManager.GetPrimaryPad()))
			{
				...
				app.SetGameHostOption(eGameHostOption_Usernames,((GameSettingsA[iPad]->usBitmaskValues&0x0008)!=0)?1:0);
				app.SetXuiServerAction(iPad,eXuiServerAction_ServerSettingChanged_Usernames);
				...
				for(AUTO_VAR(it3, players->players.begin()); it3 != players->players.end(); ++it3)
				{
					...
					decorationPlayer->setShowOnMaps((app.GetGameHostOption(eGameHostOption_Usernames)!=0)?true:false);
				}
			}
		}
		break;
// Interim TU 1.6.6
	...
	case eGameSetting_DisplaySplitscreenUsernames:
		for( BYTE idx = 0; idx < XUSER_MAX_COUNT; ++idx)
		{
			if(pMinecraft->localplayers[idx] != NULL)
			{
				if(pMinecraft->localplayers[idx]->m_iScreenSection==CRender::VIEWPORT_TYPE_FULLSCREEN)
				{
					ui.DisplayUsername(idx,false);
				}
				else
				{
					ui.DisplayUsername(idx,true);
				}
			}
		}
	...
	case eGameSetting_PS3_EULA_Read:
		//nothing to do here
		break;
	case eGameSetting_Linux_NetworkModeAdhoc:
		//nothing to do here
		break;
	}
}
...
{
	// check the user has rights to use this skin - they may have had at some point but the entitlement has been removed.
	DLCPack *Pack=NULL;
	...
	{
		// DLC skins are numbered using decimal rather than hex to make it easier to number manually
...
void CMinecraftApp::ValidateFavoriteSkins(int iPad)
{
	...
	for(unsigned int i=0;i<uiCount;i++)
	{
		...
		if(pDLCPack!=NULL)
		{
			// We should let players add the free skins to their favourites as well!
...
void CMinecraftApp::SetGameSettings(int iPad,eGameSetting eVal,unsigned char ucVal)
{
	...
	case eGameSetting_UsernamesVisible:
		...
		// Added for Interim TU for 1.6.6
	case eGameSetting_Sensitivity_InMenu:
	...
	case eGameSetting_PS3_EULA_Read:
		...
		break;	
	case eGameSetting_Linux_NetworkModeAdhoc:
		if((GameSettingsA[iPad]->uiBitmaskValues&GAMESETTING_NETWORKMODEADHOC)!=(ucVal&0x01)<<17)
		{
			if(ucVal==1)
			{
				GameSettingsA[iPad]->uiBitmaskValues|=GAMESETTING_NETWORKMODEADHOC;
			}
			else
			{
				GameSettingsA[iPad]->uiBitmaskValues&=~GAMESETTING_NETWORKMODEADHOC;
...
unsigned char CMinecraftApp::GetGameSettings(int iPad,eGameSetting eVal)
{
	...
	case eGameSetting_Linux_NetworkModeAdhoc:
		return (GameSettingsA[iPad]->uiBitmaskValues&GAMESETTING_NETWORKMODEADHOC)>>17;
...
int CMinecraftApp::DisplaySavingMessage(void *pParam, CStorage::ESavingMessage eVal, int iPad)
{
...
void CMinecraftApp::HandleXuiActions(void)
{
	...
	if(eAction!=eAppAction_Idle)
	{
		...
		case eAppAction_DisplayLavaMessage:
			// Display a warning about placing lava in the spawn area
			{
				...
				CStorage::EMessageResult result = ui.RequestMessageBox( IDS_CANT_PLACE_NEAR_SPAWN_TITLE, IDS_CANT_PLACE_NEAR_SPAWN_TEXT, uiIDA,1,XUSER_INDEX_ANY,NULL,NULL, app.GetStringTable());
				if(result != CStorage::EMessage_Busy) SetGlobalXuiAction(eAppAction_Idle);
	...
	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		...
		{			
			...
			case eAppAction_SocialPost:
				...
				{		
					...
					if( CSocialManager::Instance()->IsTitleAllowedToPostImages() && CSocialManager::Instance()->AreAllUsersAllowedToPostImages() )
					{
						...
						//m_bwasHidingGui = pMinecraft->options->hideGui; // Removed 1.8.2 bug fix (TU6) as don't need this
			...
			case eAppAction_SaveGameCapturedThumbnail:
				...
				{
					// turn off the usernames in splitscreen for the primary player, since they are about to be made fullscreen
	...
	// 				}
			...
			case eAppAction_ExitWorld:
				...
				// cancel any possible string verifications queued with LIVE
			...
			case eAppAction_ExitWorldCapturedThumbnail:
				{
					...
					// Fix for #12368 - Crash: Game crashes when saving then exiting and selecting to save
					for(unsigned int idx = 0; idx < XUSER_MAX_COUNT; ++idx)
					{
						...
						// Fix for #13257 - CRASH: Gameplay: Title crashed after exiting the tutorial
					...
					// Changed - Don't use the FullScreenProgressScreen for action, use a dialog instead
			...
			case eAppAction_Respawn:
				{
					...
					else
					{
						...
						// We don't need this on a thread in multiplayer as respawning is asynchronous.
						pMinecraft->localplayers[i]->respawn();
...
// 					pMinecraft->progressRenderer->progressStage(-1);
			...
			case eAppAction_PrimaryPlayerSignedOut:
				{		
					...
					// the libs will display the Returned to Title screen
// 					UINT uiIDA[1];
// 					uiIDA[0]=IDS_CONFIRM_OK;
// 					ui.RequestMessageBox(IDS_RETURNEDTOMENU_TITLE, IDS_RETURNEDTOTITLESCREEN_TEXT, uiIDA, 1, i,&CMinecraftApp::PrimaryPlayerSignedOutReturned,this,app.GetStringTable());
			...
			case eAppAction_EthernetDisconnected:
				{
					...
					if(!g_NetworkManager.IsLeavingGame())
					{
						...
						// not the same as a signout. We should only leave the game if this machine is not the host. We shouldn't get rid of the save device either.
			...
			case eAppAction_PrimaryPlayerSignedOutReturned:
				{
					...
					if( g_NetworkManager.IsLeavingGame() )
					{
						// If we are already leaving the game, then we just need to signal that the player signed out to stop saves
			...
			case eAppAction_ExitAndJoinFromInviteConfirmed:
				{
					...
					// may have been using a texture pack with audio , so clean up anything texture pack related here
...
#ifdef _LINUX
					// It's possible that we can sign in/remove players between the mask initially being set and this point
			...
			case eAppAction_JoinFromInvite:
				{
					...
					// Move this state block from CPlatformNetworkManager::ExitAndJoinFromInviteThreadProc, as g_NetworkManager.JoinGameFromInviteInfo ultimately can call NavigateToScene,
...
#ifdef _LINUX
					// If a player is signed in (i.e. locked) but not in the mask, unlock them
					for(unsigned int index = 0; index < XUSER_MAX_COUNT; ++index)
					{
					...
					Minecraft::GetInstance()->user->name = convStringToWstring( ProfileManager.GetUsername(ProfileManager.GetPrimaryPad()));
					...
					if( !success )
					{
						...
						// Copied this from XUI_FullScreenProgress to properly handle the fail case, as the thread will no longer be failing
						UINT uiIDA[1];
						uiIDA[0]=IDS_CONFIRM_OK;
...
#if 0
					// Took this out since ChangeSessionType is only set in two places (both in EthernetDisconnected) and this case is handled there, plus this breaks
			...
			case eAppAction_SetDefaultOptions:
				SetAction(i,eAppAction_Idle);
#if ( defined _LINUX || defined _LINUX || defined _LINUX || defined _LINUX)
				SetDefaultOptions((CStorage::PROFILESETTINGS *)param,i);		
#else
				SetDefaultOptions((C_Profile::PROFILESETTINGS *)param,i);		
#endif
			...
			case eAppAction_FailedToJoinNoPrivileges:
				{
					UINT uiIDA[1];
					uiIDA[0]=IDS_CONFIRM_OK;
					CStorage::EMessageResult result = ui.RequestMessageBox( IDS_NO_MULTIPLAYER_PRIVILEGE_TITLE, IDS_NO_MULTIPLAYER_PRIVILEGE_JOIN_TEXT, uiIDA,1,ProfileManager.GetPrimaryPad(),NULL,NULL, app.GetStringTable());
					if(result != CStorage::EMessage_Busy) SetAction(i,eAppAction_Idle);
				}
				break;
			...
			case eAppAction_BanLevel:
				{
					...
					if(g_NetworkManager.IsInGameplay() && !g_NetworkManager.IsLeavingGame())
					{
						...
						AddLevelToBannedLevelList(i,((NetworkPlayerLinux *)pHost)->GetUID(),GetUniqueMapName(),true);
#elif defined _LINUX
						INetworkPlayer *pHost=g_NetworkManager.GetHostPlayer();	
			...
			case eAppAction_LevelInBanLevelList:
				{			
					...
					if(player)
					{
						...
						CStorage::EMessageResult result = ui.RequestMessageBox( IDS_BANNED_LEVEL_TITLE, IDS_PLAYER_BANNED_LEVEL, uiIDA,2,i,&CMinecraftApp::BannedLevelDialogReturned,this, app.GetStringTable(),wchFormat);
						if(result != CStorage::EMessage_Busy) SetAction(i,eAppAction_Idle);
					}
					else
					{
			...
			case eAppAction_ReloadTexturePack:
				{			
					...
					// If the texture pack has audio, we need to switch to this
		...
		{			
			...
			case eTMSAction_TMSPP_RetrieveFiles_RunPlayGame:
				...
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_Title,eTMSAction_TMSPP_UserFileList);
#else
			...
			case eTMSAction_TMSPP_UserFileList:
				...
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_TitleUser,"//",eTMSAction_TMSPP_UIDSFile);
#elif defined _LINUX
				SetTMSAction(i,eTMSAction_TMSPP_UserFileList_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_TitleUser,eTMSAction_TMSPP_DLCFile);
#else
				SetTMSAction(i,eTMSAction_TMSPP_UIDSFile);
#endif
				break;
			case eTMSAction_TMSPP_UIDSFile:
#ifdef _LINUX
				SetTMSAction(i,eTMSAction_TMSPP_UIDSFile_Waiting);
				// pass in the next app action on the call or callback completing
				app.TMSPP_ReadUidsFile(i,eTMSAction_TMSPP_DLCFile);
#else
			...
			case eTMSAction_TMSPP_RetrieveFiles_DLCMain:
				...
				// pass in the next app action on the call or callback completing
				SetTMSAction(i,eTMSAction_TMSPP_UIDSFile_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_Title,"//",eTMSAction_TMSPP_DLCFileOnly);
#elif defined _LINUX
				SetTMSAction(i,eTMSAction_TMSPP_GlobalFileList_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_Title,eTMSAction_TMSPP_RetrieveUserFilelist_DLCFileOnly);
#else
			...
			case eTMSAction_TMSPP_RetrieveUserFilelist_DLCFileOnly:
				...
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_TitleUser,"//",eTMSAction_TMSPP_UIDSFile);
#elif defined _LINUX
				//StorageManager.TMSPP_DeleteFile(i,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,L"TP06.png",NULL,NULL, 0);
				SetTMSAction(i,eTMSAction_TMSPP_UserFileList_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorage::eGlobalStorage_TitleUser,eTMSAction_TMSPP_DLCFileOnly);
#else
				SetTMSAction(i,eTMSAction_TMSPP_DLCFileOnly);
#endif
...
#ifdef _LINUX
				SetTMSAction(i,eTMSAction_TMS_UIDSFile_Waiting);
				// pass in the next app action on the call or callback completing
				app.ReadUidsFileFromTMS(i,eTMSAction_TMS_DLCFile,true);
#else
...
int CMinecraftApp::BannedLevelDialogReturned(void *pParam,int iPad,const CStorage::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	//Minecraft *pMinecraft=Minecraft::GetInstance();
	...
	if(result==CStorage::EMessage_ResultAccept)
	{
		...
		{
#if defined _LINUX
			pApp->RemoveLevelFromBannedLevelList(iPad,((NetworkPlayerLinux *)pHost)->GetUID(),pApp->GetUniqueMapName());
#else
...
#elif _LINUX
	mediapath = L"Common//Media//MediaLinux64.arc";
#elif _LINUX
...
int CMinecraftApp::PrimaryPlayerSignedOutReturned(void *pParam,int iPad,const CStorage::EMessageResult)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	//Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::EthernetDisconnectReturned(void *pParam,int iPad,const CStorage::EMessageResult)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
	...
	if(Minecraft::GetInstance()->player!=NULL)
	{	
		app.SetAction(pMinecraft->player->GetPadIndex(),eAppAction_EthernetDisconnectedReturned);
	}
	else
	{
#ifdef _LINUX
...
int CMinecraftApp::SignoutExitWorldThreadProc( void* lpParameter )
{
	...
	if (pMinecraft->isClientSide() || g_NetworkManager.IsInSession() )
	{
		if(lpParameter != NULL )
		{
			...
			// Force a disconnection, this handles the situation that the server has already disconnected
		...
		// This only does something if we actually have a server, so don't need to do any other checks
		MinecraftServer::HaltServer(true);
		...
		// Leave the session once the disconnect packet has been sent
		g_NetworkManager.LeaveGame(FALSE);
	}
	else
	{
	...
	// Fix for #106402 - TCR #014 BAS Debug Output:
...
int CMinecraftApp::UnlockFullInviteReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
	bool bNoPlayer;
	...
	// we may be in the main menus here, and we don't have a pMinecraft->player
...
int CMinecraftApp::UnlockFullSaveReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::UnlockFullExitReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
	...
	if(result==CStorage::EMessage_ResultAccept)
	{
		if(ProfileManager.IsSignedInLive(pMinecraft->player->GetPadIndex()))
		{
...
#endif
			{
				...
				pApp->SetAction(pMinecraft->player->GetPadIndex(),eAppAction_ExitWorldTrial);
#endif
			}
		}
#if defined(_LINUX) || defined _LINUX
...
int CMinecraftApp::TrialOverReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
	...
	if(result==CStorage::EMessage_ResultAccept)
	{
		...
		else
		{
			...
			// We can't actually exit the game, so just exit back to the main menu
			//pApp->SetAction(pMinecraft->player->GetPadIndex(),eAppAction_ExitWorldTrial);
#else
			pApp->SetAction(pMinecraft->player->GetPadIndex(),eAppAction_ExitTrial);
#endif
		}
	}
	else
...
{
	// When in the main menu, the user is on pad 0, and any change they make to their profile will be to pad 0 data 
...
void CMinecraftApp::SignInChangeCallback(LPVOID pParam,bool bPrimaryPlayerChanged,unsigned int uiSignInData)
{
	...
	if((ProfileManager.GetLockedProfile()!=-1) && iPrimaryPlayer!=-1)
	{
		if ( ((uiSignInData & (1<<iPrimaryPlayer)) == 0) || bPrimaryPlayerChanged )
		{
			...
			// invalidate their banned level list
			...
			if( g_NetworkManager.IsInSession() )
			{
				...
				for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
				{
					...
					if(ProfileManager.IsSignedIn(i))
					{
						...
						XUserGetSigninInfo(i,XUSER_GET_SIGNIN_INFO_OFFLINE_UID_ONLY ,&info);
				...
				for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
				{
					...
					else
					{
						...
						// compiler complained about if below ('&&' within '||') - making it easier to read
						bool bPlayerChanged=(uiChangedPlayers&(1<<i))==(1<<i);
						bool bPlayerSignedIn=((uiSignInData&(1<<i))!=0);

						if( bPlayerChanged && (!bPlayerSignedIn || (bPlayerSignedIn && !ProfileManager.AreUIDSEqual(pApp->m_currentSigninInfo[i].uid, info.uid) ) ))					
						{
							// invalidate their banned level list
					...
					if(!switchToOffline && i != ProfileManager.GetLockedProfile() && !g_NetworkManager.IsLocalGame())
					{
						if(g_NetworkManager.GetLocalPlayerByUserIndex(i)!=NULL)
						{
							...
							{
								pApp->DebugPrintf("Recommending removal of player at index %d because they're no longer signed into PSNd/n",i);
								pApp->SetAction(i,eAppAction_ExitPlayer);
							}
						}
					}
...
#if ( defined _LINUX || defined _LINUX || defined _LINUX  || defined _LINUX )
			// Need to kick of loading of profile data for sub-sign in players.
			for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
			{
				if( i != iPrimaryPlayer 
	...
	for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
	{
		if(FAILED(XUserGetSigninInfo(i,XUSER_GET_SIGNIN_INFO_OFFLINE_UID_ONLY,&pApp->m_currentSigninInfo[i])))
		{
			pApp->m_currentSigninInfo[i].uid = INVALID_UID;
...
#if defined _LINUX || defined _LINUX || defined _LINUX
int CMinecraftApp::MustSignInFullVersionPurchaseReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	if(result==CStorage::EMessage_ResultAccept) 
	{
#ifdef _LINUX
		SQRNetworkManager::AttemptSignIn(&CMinecraftApp::NowDisplayFullVersionPurchase, &app,true);
#elif defined _LINUX
		SQRNetworkManager_Vita::AttemptSignIn(&CMinecraftApp::NowDisplayFullVersionPurchase, &app,true);
#else // __PS4__
		SQRNetworkManager::AttemptSignIn(&CMinecraftApp::NowDisplayFullVersionPurchase, &app,true);
#endif
	}
...
#if defined _LINUX || defined _LINUX || defined _LINUX
int CMinecraftApp::MustSignInFullVersionPurchaseReturnedExitTrial(void *pParam,int iPad,CStorage::EMessageResult result)
{
	if(result==CStorage::EMessage_ResultAccept) 
	{
#ifdef _LINUX
	...
	//we need to exit the trial, or we'll be in the pause menu with ignore input true
...
int CMinecraftApp::MarketplaceCountsCallback(LPVOID pParam,CStorage::DLC_TMS_DETAILS *pTMSDetails, int iPad)
{
...
// For the sake of clarity in DLCMountedCallback.
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
...
{
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX) || defined(_LINUX) || defined(_LINUX)  || defined (_LINUX) //Chris TODO
...
// 			 BYTE *pBuffer=NULL;
// 			 DWORD dwSize=0;
// 			 // out for now for DaveK so he doesn't get the birthday cape
// #ifdef _CONTENT_PACKAGE
// 			 CStorage::ETMSStatus eTMSStatus;
// 			 eTMSStatus=StorageManager.ReadTMSFile(ProfileManager.GetPrimaryPad(),CStorage::eGlobalStorage_Title,CStorage::eTMS_FileType_Graphic, L"Default_Cape.png",&pBuffer, &dwSize);
// 			 if(eTMSStatus==CStorage::ETMSStatus_Idle)
// 			 {
	...
	else
	{
		...
		do
		{
			...
			if(( GetFileAttributes( szFullFilename ) & FILE_ATTRIBUTE_DIRECTORY) != FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				{
					app.DebugPrintf("Found a replacement .pck/n");		
...
//  int CMinecraftApp::DLCReadCallback(LPVOID pParam,CStorage::DLC_FILE_DETAILS *pDLCData)
//  {
// 	 return 0;
//  }
...
bool CMinecraftApp::isUidNotch(PlayerUID uid)
{
	if(m_uidNotch != INVALID_UID && uid != INVALID_UID)
	{
		return ProfileManager.AreUIDSEqual(uid, m_uidNotch) == TRUE;
	}
	return false;
}

bool CMinecraftApp::isUidDeadmau5(PlayerUID uid)
{
	AUTO_VAR(it, PlayerData.find( uid )); // The .at and [] accessors insert elements if they don't exist
	if (it != PlayerData.end() )
	{
		PLAYER_DATA *pPlayerData=PlayerData[uid];
		if(pPlayerData && pPlayerData->eUid==eUID_Deadmau5)
		{
			return true;
		}
	}
...
// bool CMinecraftApp::UploadFileToGlobalStorage(int iQuadrant, CStorage::eGlobalStorage eStorageFacility, wstring *wsFile  )
// {
// 	bool bRes=false;
// #ifndef _CONTENT_PACKAGE
// 	// read the local file
...
int CMinecraftApp::ExitAndJoinFromInvite(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	//Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::ExitAndJoinFromInviteSaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	CMinecraftApp *pClass = (CMinecraftApp *)pParam;
...
int CMinecraftApp::WarningTrialTexturePackReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
#ifdef _LINUX
...
int CMinecraftApp::ExitAndJoinFromInviteAndSaveReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	//CMinecraftApp* pClass = (CMinecraftApp*)pParam;
...
int CMinecraftApp::ExitAndJoinFromInviteDeclineSaveReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	// results switched for this dialog
	if(result==CStorage::EMessage_ResultDecline) 
	{
...
wstring CMinecraftApp::GetActionReplacement(int iPad, unsigned char ucAction)
{
	...
	// Some of our actions can be mapped to multiple physical buttons, so replaces the switch that was here
...
#if defined(_LINUX) || defined(_LINUX) || defined (_LINUX)
unordered_map<PlayerUID, PLAYER_DATA *, PlayerUID::Hash> CMinecraftApp::PlayerData;
...
#else
unordered_map<PlayerUID, PLAYER_DATA *> CMinecraftApp::PlayerData;
...
HRESULT CMinecraftApp::RegisterPlayerData(WCHAR *pUidName, PlayerUID uid, WCHAR *pSkin, WCHAR *pCape)
{
	HRESULT hr=S_OK;
	eUID eTempUid=eUID_Undefined;
	PLAYER_DATA *pPlayerData=NULL;

	// ignore the names if we don't recognize them
	if(pUidName!=NULL)
	{	
		if( wcscmp( pUidName, L"UID_NOTCH" ) == 0 )
		{
			eTempUid = eUID_Notch; // might be needed for the apple at some point
		}
		else if( wcscmp( pUidName, L"UID_DEADMAU5" ) == 0 )
		{
			eTempUid = eUID_Deadmau5; // Needed for the deadmau5 ears
		}
		else
		{
			eTempUid=eUID_NoName;
		}
	}

	if(eTempUid!=eUID_Undefined)
	{
		pPlayerData = new PLAYER_DATA;
		ZeroMemory(pPlayerData,sizeof(PLAYER_DATA));
		pPlayerData->eUid=eTempUid;
		
		wcsncpy( pPlayerData->wchSkin, pSkin, MAX_CAPENAME_SIZE);
		wcsncpy( pPlayerData->wchCape, pCape, MAX_CAPENAME_SIZE);
		PlayerData[uid]=pPlayerData;
	}

	return hr;
}

PLAYER_DATA *CMinecraftApp::GetPlayerDataForUid(PlayerUID uid)
{
...
// 	if(pType!=NULL)
// 	{	
// 		if(wcscmp(pType,L"GameTransfer")==0)
// 		{
// 			if(iValue>0)
// 			{
// 			}
...
{
	HRESULT hr=S_OK;
	// need to convert the product id to uppercase because the catalog calls come back with upper case
	...
	app.DebugPrintf("DLCInfo - type - %d, productID - %ls, name - %ls , banner - %ls, iconfig - %d, sort index - %d/n",eType,pwchProductId, pwchProductName,pwchBannerName, iConfig, uiSortIndex);
	return hr;
}
#else
...
int CMinecraftApp::ExitGameFromRemoteSaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
{
	PlayerUID uid= pBannedListData->wchPlayerUID;

	AddLevelToBannedLevelList(iPad,uid,pBannedListData->pszLevelName,bWriteToTMS);
}
#endif

void CMinecraftApp::AddLevelToBannedLevelList(int iPad, PlayerUID uid, char *pszLevelName, bool bWriteToTMS)
{
...
#ifdef _LINUX
	memcpy(&pBannedListData->wchPlayerUID, uid.toString().c_str(), sizeof(WCHAR)*64);
#else
	memcpy(&pBannedListData->uid, &uid, sizeof(PlayerUID));
#endif
	...
	{	
		...
		//bool bRes=StorageManager.WriteTMSFile(iPad,CStorage::eGlobalStorage_TitleUser,L"BannedList",(PBYTE)pBannedList, dwDataBytes);
#ifdef _LINUX
		StorageManager.TMSPP_WriteFile(iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,CStorage::TMS_UGCTYPE_NONE,"BannedList",(PCHAR) pBannedList, dwDataBytes,NULL,NULL, 0);
#elif defined _LINUX
		StorageManager.TMSPP_WriteFile(iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,L"BannedList",(PBYTE) pBannedList, dwDataBytes,NULL,NULL, 0);
#endif
	}
	// update telemetry too
}

bool CMinecraftApp::IsInBannedLevelList(int iPad, PlayerUID uid, char *pszLevelName)
{
	for(AUTO_VAR(it, m_vBannedListA[iPad]->begin()); it != m_vBannedListA[iPad]->end(); ++it)
	{
		PBANNEDLISTDATA pData=*it;
		...
		if(IsEqualUID (bannedPlayerUID,uid) && (strcmp(pData->pszLevelName,pszLevelName)==0))
#else		
		if(IsEqualUID (pData->uid,uid) && (strcmp(pData->pszLevelName,pszLevelName)==0))
#endif
		{
			return true;
		}
...
void CMinecraftApp::RemoveLevelFromBannedLevelList(int iPad, PlayerUID uid, char *pszLevelName)
{
	//bool bFound=false;
	//bool bRes;
	...
	for(AUTO_VAR(it, m_vBannedListA[iPad]->begin()); it != m_vBannedListA[iPad]->end(); )
	{
		...
		if(pBannedListData!=NULL)
		{
			...
			if(IsEqualUID (bannedPlayerUID,uid) && (strcmp(pBannedListData->pszLevelName,pszLevelName)==0))
#else
			if(IsEqualUID (pBannedListData->uid,uid) && (strcmp(pBannedListData->pszLevelName,pszLevelName)==0))
#endif
			{
	...
	if(dwDataBytes==0)
	{
		// wipe the file
#ifdef _LINUX
		StorageManager.DeleteTMSFile(iPad,CStorage::eGlobalStorage_TitleUser,L"BannedList");
#elif defined _LINUX
		StorageManager.TMSPP_DeleteFile(iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,L"BannedList",NULL,NULL, 0);
#endif
	}
	else
	{
...
#ifdef _LINUX
		StorageManager.WriteTMSFile(iPad,CStorage::eGlobalStorage_TitleUser,L"BannedList",(PBYTE)pBannedList, dwDataBytes);
#elif defined _LINUX
		StorageManager.TMSPP_WriteFile(iPad,CStorage::eGlobalStorage_TitleUser,CStorage::TMS_FILETYPE_BINARY,L"BannedList",(PBYTE) pBannedList, dwDataBytes,NULL,NULL, 0);
#endif
		delete [] pBannedList;
...
void CMinecraftApp::SetGameHostOption(unsigned int &uiHostSettings, eGameHostOption eVal,unsigned int uiVal)
	...
	case eGameHostOption_Difficulty:
		...
		break;
	case eGameHostOption_Usernames:
		if(uiVal!=0)
		{
			uiHostSettings|=GAME_HOST_OPTION_BITMASK_USERNAMES;
		}
		else
		{
			// off
...
unsigned int CMinecraftApp::GetGameHostOption(unsigned int uiHostSettings, eGameHostOption eVal)
{
	...
	case eGameHostOption_Difficulty:
		...
		break;
	case eGameHostOption_Usernames:
		return (uiHostSettings&GAME_HOST_OPTION_BITMASK_USERNAMES);
		break;
	case eGameHostOption_GameType:
	...
	case eGameHostOption_Tutorial:
		return ((uiHostSettings&GAME_HOST_OPTION_BITMASK_USERNAMES)|GAME_HOST_OPTION_BITMASK_TRUSTPLAYERS|GAME_HOST_OPTION_BITMASK_FIRESPREADS|GAME_HOST_OPTION_BITMASK_TNT|GAME_HOST_OPTION_BITMASK_PVP|GAME_HOST_OPTION_BITMASK_STRUCTURES|1);
		break;
	case eGameHostOption_LevelType:
...
{
	// All of these options give the host player some advantage, so should not allow achievements
...
wstring CMinecraftApp::getEntityName(eINSTANCEOF type)
{
	...
	case eTYPE_LAVASLIME:
		...
		// fix for #107167 - Customer Encountered: TU12: Content: UI: There is no information what killed Player after being slain by Iron Golem.
		...
		for(int i=0;i<iCount;i++)
		{
			...
			if((pDLC->eDLCType==e_DLC_TexturePacks) || (pDLC->eDLCType==e_DLC_MashupPacks))
			{
				...
				{		
					...
					// shouldn't check this here - let the TMS files override it, so if they are on TMS, we'll take them first
					...
					{			
						...
						{	
							...
							{			
								...
								pTMSPPreq->eStorageFacility=CStorage::eGlobalStorage_Title;
								pTMSPPreq->eFileTypeVal=CStorage::TMS_FILETYPE_BINARY;
				...
				{			
					...
					{	
						...
						{			
							...
							pTMSPPreq->lpCallbackParam=this;
							// testing for now
							//pTMSPPreq->eStorageFacility=CStorage::eGlobalStorage_TitleUser;
							pTMSPPreq->eStorageFacility=CStorage::eGlobalStorage_Title;
							pTMSPPreq->eFileTypeVal=CStorage::TMS_FILETYPE_BINARY;
							//wcstombs(pTMSPPreq->szFilename,pDLC->wchBanner,MAX_TMSFILENAME_SIZE);
...
bool CMinecraftApp::RetrieveNextDLCContent()
{
	...
	{
		return true; // We need to wait until the primary player is online.
	}
	...
	for(AUTO_VAR(it, m_DLCDownloadQueue.begin()); it != m_DLCDownloadQueue.end(); ++it)
	{
		...
		if(pCurrent->eState==e_DLC_ContentState_Idle)
		{
			...
			CStorage::EDLCStatus status = StorageManager.GetDLCOffers(ProfileManager.GetPrimaryPad(), &CMinecraftApp::DLCOffersReturned, this, pCurrent->dwType);
			if(status==CStorage::EDLC_Pending)
			{
...
{
	CStorage::PTMSPP_FILEDATA pFileData=(CStorage::PTMSPP_FILEDATA)lpvData;
#else
int CMinecraftApp::TMSPPFileReturned(LPVOID pParam,int iPad,int iUserData,CStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
#endif
...
#endif
		{
			...
			{	
				...
				case e_DLC_TexturePackData:
					{					
						// we need to allocate memory for the file data and copy into it, since the current data is a reference into the blob download memory
						...
						app.DebugPrintf("Got texturepack data/n");
				...
				default:
					...
					{
						app.DebugPrintf("Got image data, but it's not a png - %ls/n",pCurrent->wchFilename);
					}
					break;
				}
			...
			{
#ifdef _LINUX
				app.DebugPrintf("TMSImageReturned failed (%ls).../n",wchFilename);
#else
...
bool CMinecraftApp::RetrieveNextTMSPPContent()
{
	...
	for(AUTO_VAR(it, m_TMSPPDownloadQueue.begin()); it != m_TMSPPDownloadQueue.end(); ++it)
	{
		...
		{
			// the file may be in the local TMS files, but try to retrieve it from the remote TMS in case it's been changed. If it's not in the list of TMS files, this will
			...
			app.DebugPrintf("/nRetrieveNextTMSPPContent - type = %d, %s/n",pCurrent->eType,szFilename);

			CStorage::ETMSStatus status=StorageManager.TMSPP_ReadFile(ProfileManager.GetPrimaryPad(),pCurrent->eStorageFacility,pCurrent->eFileTypeVal,szFilename,pCurrent->CallbackFunc,this);
			switch(status)
			{
			case CStorage::ETMSStatus_Pending:
				pCurrent->eState=e_TMS_ContentState_Retrieving;
				break;
			case CStorage::ETMSStatus_Idle:
				pCurrent->eState=e_TMS_ContentState_Retrieved;
				break;
			case CStorage::ETMSStatus_Fail_ReadInProgress:
			case CStorage::ETMSStatus_ReadInProgress:
				pCurrent->eState=e_TMS_ContentState_Retrieving;
				if(pCurrent->eState==CStorage::ETMSStatus_Fail_ReadInProgress)
				{
					app.DebugPrintf("TMSPP_ReadFile failed - read in progress/n");
					Sleep(50);
					LeaveCriticalSection(&csTMSPPDownloadQueue);
					return false;
				}
			...
			app.DebugPrintf("RetrieveNextTMSPPContent - type = %d, %ls/n",pCurrent->eType,pCurrent->wchFilename);
			...
			case eTitleStorage_busy:
				// try again next time
				{
					app.DebugPrintf("@@@@@@@@@@@@@@@@@ TMSPP_ReadFile failed - busy (probably reading already)/n");
					Sleep(50);
					LeaveCriticalSection(&csTMSPPDownloadQueue);
					return false;
				}
...
DWORD CMinecraftApp::getSkinIdFromPath(const wstring &skin)
{
	...
	if(skin.size() >= 14)
	{
		...
		// dlc skins are numbered using decimal to make it easier for artists/people to number manually
...
{
	// This function maps the encoded DWORD we store in the player profile
...
int CMinecraftApp::TexturePackDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
#ifdef _LINUX
	if(result!=CStorage::EMessage_Cancelled)
	{
		...
		{	
			...
			if( result==CStorage::EMessage_ResultAccept ) // Full version
			{
...
void CMinecraftApp::getLocale(vector<wstring> &vecWstrLocales)
{
	...
	// restrict the 360 language until we're ready to have them in
...
{
	// we need to stop the retrieval of minecraft store images from TMS when we aren't in the DLC, since going in to Play Game will change the title id group
...
enum ETitleUpdateTexturePacks
{
	...
	eTUTP_Natural = 0x805,
	eTUTP_City = 0x01000806 // The released City pack had a sub-pack ID
};
