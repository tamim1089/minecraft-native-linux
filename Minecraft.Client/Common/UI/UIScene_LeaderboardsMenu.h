...
{
private:
	// Because the kills leaderboard doesn't a peaceful entry there are some special
	...
	struct LeaderboardEntry {
		...
		DWORD		m_rank;
		WCHAR		m_wcRank[12];
		WCHAR		m_username[MAX_USERNAME_SIZE+1];
		//int			m_locale;
		...
		int			m_idsErrorMessage;		// Non-zero if this entry has an error message instead of results.
	};
	...
	static int ExitLeaderboards(void *pParam,int iPad,CStorage::EMessageResult result);
