...
UIScene_SettingsOptionsMenu::UIScene_SettingsOptionsMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	m_checkboxInGameUsernames.init(app.GetString(IDS_IN_GAME_USERNAMES),eControl_InGameUsernames,(app.GetGameSettings(m_iPad,eGameSetting_UsernamesVisible)!=0));
	...
	{
		removeControl(&m_checkboxInGameUsernames, true);
	}
...
void UIScene_SettingsOptionsMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_CANCEL:
		if(pressed)
		{
			...
			app.SetGameSettings(m_iPad,eGameSetting_UsernamesVisible,m_checkboxInGameUsernames.IsChecked()?1:0);
