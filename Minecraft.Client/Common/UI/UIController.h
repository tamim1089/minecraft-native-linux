...
#endif
	// These should be in the order that they reference each other (i.e. they can only reference one with a lower value in the enum)
	enum ELibraries
	{
		eLibrary_Platform,
...
private:
	...
	CRender::eViewportType m_currentRenderViewport;
...
public:
	...
	void getRenderDimensions(CRender::eViewportType viewport, S32 &width, S32 &height);
	void setupRenderPosition(CRender::eViewportType viewport);
...
public:
	...
	virtual void DisplayUsername(unsigned int iPad, bool show);
	...
	virtual void ShowSavingMessage(unsigned int iPad, CStorage::ESavingMessage eVal);
	...
	// Only because of the different StringTable type, should really fix the libraries
#ifndef _LINUX
	virtual CStorage::EMessageResult RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad=XUSER_INDEX_ANY,
						int( *Func)(LPVOID,int,const CStorage::EMessageResult)=NULL,LPVOID lpParam=NULL, CStringTable *pStringTable=NULL, WCHAR *pwchFormatString=NULL,DWORD dwFocusButton=0, bool bIsError = true);
#else
	virtual CStorage::EMessageResult RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad=XUSER_INDEX_ANY,
						int( *Func)(LPVOID,int,const CStorage::EMessageResult)=NULL,LPVOID lpParam=NULL, StringTable *pStringTable=NULL, WCHAR *pwchFormatString=NULL,DWORD dwFocusButton=0, bool bIsError = true);
#endif

	CStorage::EMessageResult RequestUGCMessageBox(UINT title = -1, UINT message = -1, int iPad = -1, int( *Func)(LPVOID,int,const CStorage::EMessageResult) = NULL, LPVOID lpParam = NULL);
	CStorage::EMessageResult RequestContentRestrictedMessageBox(UINT title = -1, UINT message = -1, int iPad = -1, int( *Func)(LPVOID,int,const CStorage::EMessageResult) = NULL, LPVOID lpParam = NULL);
