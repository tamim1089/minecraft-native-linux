...
UIScene_SettingsUIMenu::UIScene_SettingsUIMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	m_checkboxShowSplitscreenUsernames.init(app.GetString(IDS_CHECKBOX_DISPLAY_SPLITSCREEN_USERNAMES),eControl_ShowSplitscreenUsernames,(app.GetGameSettings(m_iPad,eGameSetting_DisplaySplitscreenUsernames)!=0));
	...
	if(bInGame)
	{
		...
		{	
			...
			removeControl(&m_checkboxShowSplitscreenUsernames, true);
		}
	}
...
void UIScene_SettingsUIMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_CANCEL:
		if(pressed)
		{
			...
			app.SetGameSettings(m_iPad,eGameSetting_DisplaySplitscreenUsernames,m_checkboxShowSplitscreenUsernames.IsChecked()?1:0);
