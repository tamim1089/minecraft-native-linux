...
#ifdef _LINUX
#include "Linux/Network/SQRNetworkManager_AdHoc_Vita.h"
#endif
	...
	case SCE_COMMON_DIALOG_STATUS_FINISHED:
		{
			...
			else
			{
				...
				ui.RequestMessageBox(IDS_PLAY_OFFLINE,IDS_NO_ONLINE_SUBSCRIPTION, uiIDA, 1, ProfileManager.GetPrimaryPad(),&UIScene_CreateWorldMenu::ContinueOffline,dynamic_cast<UIScene_CreateWorldMenu*>(this),app.GetStringTable(), 0, 0, false);
			}
		}
		break;
	default:
...
#ifdef _LINUX
int UIScene_CreateWorldMenu::ContinueOffline(void *pParam,int iPad,CStorage::EMessageResult result)
{
	...
	case eControl_EditSeed:
		{
			...
			case XC_LANGUAGE_TCHINESE:
				...
				break;
			default:
				// Use a different keyboard for non-asian languages so we don't have prediction on
...
// 			ui.RequestMessageBox( IDS_FAILED_TO_CREATE_GAME_TITLE, IDS_NO_ONLINE_SUBSCRIPTION, uiIDA,2,ProfileManager.GetPrimaryPad(),&UIScene_CreateWorldMenu::PSPlusReturned,this, app.GetStringTable(),NULL,0,false);
			return;
		}
	}
#endif
...
#ifdef _LINUX
			else if(isOnlineGame && isSignedInLive && (bOnlineSubscription==false))
			{
				m_bIgnoreInput=false;
				setVisible( true );
...
void UIScene_CreateWorldMenu::CreateGame(UIScene_CreateWorldMenu* pClass, DWORD dwLocalUsersMask)
{
	...
	{
		param->findSeed = true;	// java code sets the seed to was (new Random())->nextLong() here - we used to at this point find a suitable seed, but now just set a flag so this is performed in Minecraft::Server::initServer.
	}
	...
	app.SetGameHostOption(eGameHostOption_Usernames,app.GetGameSettings(pClass->m_iPad,eGameSetting_UsernamesVisible)?1:0);
...
int UIScene_CreateWorldMenu::ConfirmCreateReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_CreateWorldMenu* pClass = (UIScene_CreateWorldMenu*)pParam;
...
#ifdef _LINUX
int UIScene_CreateWorldMenu::MustSignInReturnedPSN(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_CreateWorldMenu* pClass = (UIScene_CreateWorldMenu *)pParam;
	pClass->m_bIgnoreInput = false;
    ...
    if(result==CStorage::EMessage_ResultAccept) 
    {
        SQRNetworkManager::AttemptSignIn(&UIScene_CreateWorldMenu::StartGame_SignInReturned, pClass, false, iPad);
    }

    return 0;
}

// int UIScene_CreateWorldMenu::PSPlusReturned(void *pParam,int iPad,CStorage::EMessageResult result)
// {
// 	int32_t iResult;
// 	UIScene_CreateWorldMenu *pClass = (UIScene_CreateWorldMenu *)pParam;
