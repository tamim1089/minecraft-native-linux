...
UIScene_LoadMenu::UIScene_LoadMenu(int iPad, void *initData, UILayer *parentLayer) : IUIScene_StartGame(iPad, parentLayer)
{
	...
	// read the settings for the online flag. We'll only save this setting if the user changed it.
	...
	if(getSceneResolution() == eSceneResolution_1080)
	{
		...
		// to stop an offline game being able to select the online flag
	...
	else
	{
		...
		{
			app.DebugPrintf("Requesting the save thumbnail/n");
			...
			CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(&pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex],&LoadSaveDataThumbnailReturned,this,true);
#else
			CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(&pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex],&LoadSaveDataThumbnailReturned,this);
#endif
			m_bShowTimer = true;
		}
#if defined(_LINUX) 
	...
	else
	{
		...
		// Any texture packs available that we don't have installed?
#if defined(_LINUX) || defined(_LINUX)
	...
	case SCE_COMMON_DIALOG_STATUS_FINISHED:
		{
			...
			else
			{
				...
				ui.RequestMessageBox(IDS_PLAY_OFFLINE,IDS_NO_ONLINE_SUBSCRIPTION, uiIDA, 1, ProfileManager.GetPrimaryPad(),&UIScene_LoadMenu::ContinueOffline,this,app.GetStringTable(), 0, 0, false);
			}
		}
		break;
	default:
...
#ifdef _LINUX
int UIScene_LoadMenu::ContinueOffline(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
void UIScene_LoadMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	// Inform user why their game must be offline.
#if defined _LINUX
	...
	if(m_MoreOptionsParams.dwTexturePack!=0)
	{
		...
		if(m_pDLCPack && !m_pDLCPack->hasPurchasedFile( DLCManager::e_DLCType_Texture, L"" ))
		{
			...
			if(!ProfileManager.IsSignedInLive(m_iPad))
			{
				...
				ui.RequestMessageBox(IDS_PRO_NOTONLINE_TITLE, IDS_PRO_LINUXLIVE_NOTIFICATION, uiIDA, 1);
				m_bIgnoreInput = false;
				return;
			}
			else */
...
void UIScene_LoadMenu::LaunchGame(void)
{
	...
	{			
		...
		{
			// Need different text for Survival mode with a level that has been saved in Creative
			if(m_bGameModeSurvival)
			{
			...
			else // it's creative mode
			{
				...
				{
					// We don't really need to tell the user this will have achievements disabled, since they already saved it in creative
					...
					else
					{
						...
						PSAVE_DETAILS pSaveDetails=StorageManager.ReturnSavesInfo();
#ifndef _LINUX
						app.DebugPrintf("Loading save s [%s]/n",pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex].UTF8SaveTitle,pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex].UTF8SaveFilename);
#endif
						CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex],&LoadSaveDataReturned,this);

#if TO_BE_IMPLEMENTED
						if(eLoadStatus==CStorage::ELoadGame_DeviceRemoved)
						{
							// disable saving 
							StorageManager.SetSaveDisabled(true);
...
int UIScene_LoadMenu::CheckResetNetherReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
	...
	else if(result==CStorage::EMessage_ResultAccept)
	{
...
int UIScene_LoadMenu::ConfirmLoadReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
	...
	if(result==CStorage::EMessage_ResultAccept) 
	{
		...
		else
		{
			...
			PSAVE_DETAILS pSaveDetails=StorageManager.ReturnSavesInfo();
#ifndef _LINUX
			app.DebugPrintf("Loading save %s [%s]/n",pSaveDetails->SaveInfoA[(int)pClass->m_iSaveGameInfoIndex].UTF8SaveTitle,pSaveDetails->SaveInfoA[(int)pClass->m_iSaveGameInfoIndex].UTF8SaveFilename);
#endif
			CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[(int)pClass->m_iSaveGameInfoIndex],&LoadSaveDataReturned,pClass);
...
#ifdef _LINUX
			else if(isOnlineGame && (bOnlineSubscription==false))
			{
				pClass->setVisible( true );
				pClass->m_bIgnoreInput=false;
...
// 				ui.RequestMessageBox( IDS_FAILED_TO_CREATE_GAME_TITLE, IDS_NO_ONLINE_SUBSCRIPTION, uiIDA,2,ProfileManager.GetPrimaryPad(),&UIScene_LoadMenu::PSPlusReturned,pClass, app.GetStringTable(),NULL,0,false);
			}
		...
		{
			//  not sure why we aren't checking the content restriction for the main player here when multiple controllers are connected - adding now
			if(isOnlineGame && noUGC )
			{
				pClass->setVisible( true );
				ui.RequestUGCMessageBox();
...
int UIScene_LoadMenu::TrophyDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
	return LoadDataComplete(pClass);
}

int UIScene_LoadMenu::DeleteSaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
...
// Shared functionality that is the same whether we needed a quadrant sign-in or not
void UIScene_LoadMenu::StartGameFromSave(UIScene_LoadMenu* pClass, DWORD dwLocalUsersMask)
{
	...
	app.SetGameHostOption(eGameHostOption_Usernames,app.GetGameSettings(pClass->m_iPad,eGameSetting_UsernamesVisible));
...
void UIScene_LoadMenu::LoadLevelGen(LevelGenerationOptions *levelGen)
{
	...
	// If we only have one controller connected, then don't show the sign-in UI again
	...
	app.SetGameHostOption(eGameHostOption_Usernames,app.GetGameSettings(m_iPad,eGameSetting_UsernamesVisible));
...
#ifdef _LINUX
int UIScene_LoadMenu::MustSignInReturnedPSN(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_LoadMenu* pClass = (UIScene_LoadMenu *)pParam;
	pClass->m_bIgnoreInput = false;
    ...
    if(result==CStorage::EMessage_ResultAccept) 
    {
        SQRNetworkManager::AttemptSignIn(&UIScene_LoadMenu::StartGame_SignInReturned, pClass, false, iPad);
    }

    return 0;
}

// int UIScene_LoadMenu::PSPlusReturned(void *pParam,int iPad,CStorage::EMessageResult result)
// {
// 	int32_t iResult;
// 	UIScene_LoadMenu *pClass = (UIScene_LoadMenu *)pParam;
