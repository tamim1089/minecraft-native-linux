...
// Enable this to override the Iggy Allocator
...
#include "Linux64/Iggy/include/iggyexpruntime.h"
#endif
...
#ifdef _LINUX
#include "Linux/Iggy/include/iggyperfmon.h"
#include "Linux/Iggy/include/iggyperfmon_orbis.h"
#elif defined _LINUX
#include "Linux/Iggy/include/iggyperfmon.h"
#elif defined _LINUX
#include "Linux/Iggy/include/iggyperfmon.h"
#include "Linux/Iggy/include/iggyperfmon_ps3.h"
#elif defined _LINUX
#include "Linux/Iggy/include/iggyperfmon.h"
#include "Linux/Iggy/include/iggyperfmon_psp2.h"
#elif defined __LINUX
#include "Linux64/Iggy/include/iggyperfmon.h"
#endif
...
static void * RADLINK AllocateFunction ( void * alloc_callback_user_data , size_t size_requested , size_t * size_returned )
{
	...
	*size_returned = size_requested;
	...
	app.DebugPrintf(app.USER_SR, "Allocating %d, new total: %d/n", size_requested, UIController::iggyAllocCount);
...
static void RADLINK DeallocateFunction ( void * alloc_callback_user_data , void * ptr )
{
	...
	app.DebugPrintf(app.USER_SR, "Freeing %d, new total %d/n", size, UIController::iggyAllocCount);
#ifdef EXCLUDE_IGGY_ALLOCATIONS_FROM_HEAP_INSPECTOR
	__real_free(ptr);
#else
	free(ptr);
...
UIController::UIController()
{
	...
	// This is a bit of a hack until we change the Minecraft initialisation to store the proper screen size for other platforms
	...
	m_currentRenderViewport = CRender::VIEWPORT_TYPE_FULLSCREEN;
...
{
	if(bVal) app.DebugPrintf("System UI showing/n");
	else app.DebugPrintf("System UI stopped showing/n");
...
void UIController::SetupFont()
{
	...
	case XC_LANGUAGE_KOREAN:
		...
		break;
		// Cyrillic characters have been added to this font now, (4/July/14)
	...
	default:
		bBitmapFont=true;
		// m_mcTTFFont = new UITTFFont("Common/Media/font/GameFont.ttf", 0x2022); // Shouldn't be using this.
		break;
	}
	...
	if(bBitmapFont)
	{
		...
		if(m_moj7==NULL)
		{
			m_moj7 = new UIBitmapFont(SFontData::GameFont_7);
			m_moj7->registerFont();
		}
		if(m_moj11==NULL)
		{
			m_moj11 = new UIBitmapFont(SFontData::GameFont_11);
			m_moj11->registerFont();
		}
	}
	else
	{
		app.DebugPrintf("IggyFontSetIndirectUTF8/n");
		IggyFontSetIndirectUTF8( "GameFont7", -1, IGGY_FONTFLAG_all, "GameFont_TTF",-1 ,IGGY_FONTFLAG_none );
		IggyFontSetIndirectUTF8( "GameFont11", -1, IGGY_FONTFLAG_all, "GameFont_TTF",-1 ,IGGY_FONTFLAG_none );
	}
}
...
void UIController::loadSkins()
{
...
#ifdef _LINUX
	platformSkinPath = L"skinLinux.swf";
#elif defined _LINUX
	...
	if(m_fScreenHeight==1080.0f)
	{
		platformSkinPath = L"skinLinuxHD.swf";	
	}
	else
	{
		platformSkinPath = L"skinLinux.swf";	
...
#if defined(_LINUX)
	// Load the 720/480 skins so that we have something to fallback on during development
#ifndef _FINAL_BUILD
...
IggyLibrary UIController::loadSkin(const wstring &skinPath, const wstring &skinName)
{
	...
	// We need to load the platformskin before the normal skin, as the normal skin requires some elements from the platform skin
	if(!skinPath.empty() && app.hasArchiveFile(skinPath))
	{
			...
			&memoryInfo ))
		{
			...
			app.DebugPrintf(app.USER_SR, "%ls - %.*s, static: %dB, dynamic: %dB/n", skinPath.c_str(), memoryInfo.subcategory_stringlen, memoryInfo.subcategory, memoryInfo.static_allocation_bytes, memoryInfo.dynamic_allocation_bytes);
			++iteration;
		}

		app.DebugPrintf(app.USER_SR, "%ls - Total static: %dB (%dKB)/n", skinPath.c_str(), totalStatic, totalStatic/1024);
#endif
	}
	return lib;
}
...
void UIController::ReloadSkin()
{
	...
	m_reloadSkinThread = new CThread(reloadSkinThreadProc, (void*)this, "Reload skin thread");
...
void UIController::handleInput()
{
	...
	{
#ifdef _LINUX
		// Added exception for primary play who migh've uttered speech commands.
...
void UIController::handleKeyPress(unsigned int iPad, unsigned int key)
{
	...
	if(pressed) app.DebugPrintf("Pressed %d/n",key);
	if(released) app.DebugPrintf("Released %d/n",key);
	// Repeat handling
	if(pressed)
	{
		// Start repeat timer
	...
	//	if (whichFont)
	//	{
	//		IggyFontSetIndirectUTF8( "GameFont_7", -1, IGGY_FONTFLAG_all, "GameFont_TTF",-1 ,IGGY_FONTFLAG_none );
	//		IggyFontSetIndirectUTF8( "GameFont_11", -1, IGGY_FONTFLAG_all, "GameFont_TTF",-1 ,IGGY_FONTFLAG_none );
	//		whichFont = false;
	//	}
	//	else
	//	{
	//		IggyFontSetIndirectUTF8( "GameFont_7", -1, IGGY_FONTFLAG_all, "GameFont_7",-1 ,IGGY_FONTFLAG_none );
	//		IggyFontSetIndirectUTF8( "GameFont_11", -1, IGGY_FONTFLAG_all, "GameFont_11",-1 ,IGGY_FONTFLAG_none );
	//		whichFont = true;
	//	}
	//}
	//else
		...
		key == ACTION_MENU_STICK_PRESS)
	{
		...
		app.DebugPrintf(app.USER_SR, "********************************/n");
		app.DebugPrintf(app.USER_SR, "BEGIN TOTAL SWF MEMORY USAGE/n/n");
		for(unsigned int i = 0; i < eUIGroup_COUNT; ++i)
		{
		...
		for(unsigned int i = 0; i < eLibrary_Count; ++i)
		{
			...
			app.DebugPrintf(app.USER_SR, "Library static: %dB , Library dynamic: %d, ID: %d/n", libraryStatic, libraryDynamic, i);
		}
		app.DebugPrintf(app.USER_SR, "Total static: %d , Total dynamic: %d/n", totalStatic, totalDynamic);
		app.DebugPrintf(app.USER_SR, "/n/nEND TOTAL SWF MEMORY USAGE/n");
		app.DebugPrintf(app.USER_SR, "********************************/n/n");
	} 
	else 
#endif
#endif
...
void UIController::getRenderDimensions(CRender::eViewportType viewport, S32 &width, S32 &height)
{
	switch( viewport )
	{
	case CRender::VIEWPORT_TYPE_FULLSCREEN:
		width = (S32)(getScreenWidth());
		height = (S32)(getScreenHeight());
		break;
	case CRender::VIEWPORT_TYPE_SPLIT_TOP:
	case CRender::VIEWPORT_TYPE_SPLIT_BOTTOM:
		width = (S32)(getScreenWidth() / 2);
		height = (S32)(getScreenHeight() / 2);
		break;
	case CRender::VIEWPORT_TYPE_SPLIT_LEFT:
	case CRender::VIEWPORT_TYPE_SPLIT_RIGHT:
		width = (S32)(getScreenWidth() / 2);
		height = (S32)(getScreenHeight() / 2);
		break;
	case CRender::VIEWPORT_TYPE_QUADRANT_TOP_LEFT:
	case CRender::VIEWPORT_TYPE_QUADRANT_TOP_RIGHT:
	case CRender::VIEWPORT_TYPE_QUADRANT_BOTTOM_LEFT:
	case CRender::VIEWPORT_TYPE_QUADRANT_BOTTOM_RIGHT:
		width = (S32)(getScreenWidth() / 2);
		height = (S32)(getScreenHeight() / 2);
		break;
	}
...
void UIController::setupRenderPosition(CRender::eViewportType viewport)
{
...
void UIController::setupCustomDrawGameState()
{
	...
	// We don't need to clear this here as iggy hasn't written anything to the depth buffer.
...
GDrawTexture * RADLINK UIController::TextureSubstitutionCreateCallback ( void * user_callback_data , IggyUTF16 * texture_name , S32 * width , S32 * height , void * * destroy_callback_data )
{
	...
	{
		app.DebugPrintf("Found substitution texture %ls, with %d bytes/n", (wchar_t *)texture_name,it->second.length);
		...
		if( image.getData() != NULL )
		{
			...
			int id = t->getTexture(&image,CRender::TEXTURE_FORMAT_RxGyBzAw,false);

			// All our flash controls that allow replacing textures use a special 64x64 symbol
			...
			*destroy_callback_data = (void *)id;
			...
			app.DebugPrintf("Found substitution texture %ls (%d) - %dx%d/n", (wchar_t *)texture_name, id, image.getWidth(), image.getHeight());
	...
	{
		app.DebugPrintf("Could not find substitution texture %ls/n", (wchar_t *)texture_name);
		return NULL;
	}
}
...
void RADLINK UIController::TextureSubstitutionDestroyCallback ( void * user_callback_data , void * destroy_callback_data , GDrawTexture * handle )
{
	...
	int id=(int)llVal;
	app.DebugPrintf("Destroying iggy texture %d/n", id);
...
bool UIController::NavigateToScene(int iPad, EUIScene scene, void *initData, EUILayer layer, EUIGroup group)
{
	...
	if(GetMenuDisplayed(iPad))
	{
		...
		case eUIScene_TradingMenu:
			app.DebugPrintf("IGNORING NAVIGATE - we're trying to navigate to a user selected scene when there's already a scene up: pad:%d, scene:%d/n", iPad, scene);
			return false;
			break;
		}
	}
	...
	case eUIScene_FullscreenProgress:
		{
			// The fullscreen progress scene should not interfere with any other scene stack, so should be placed in it's own group/layer
...
bool UIController::NavigateBack(int iPad, bool forceUsePad, EUIScene eScene, EUILayer eLayer)
{
	...
	if( app.GetGameStarted() )
	{
		...
		// autosave in fullscreen doesn't clear the menuDisplayed flag
		else
		{
...
void UIController::NavigateToHomeMenu()
{
	...
	// just about to switched to the default texture pack , so clean up anything texture pack related here
...
{
	app.DebugPrintf(app.USER_SR, "UIController::SetIgnoreAutosaveMenuDisplayed is not implemented/n");
}
	...
	if(bVal)
	{
		if(iPad==XUSER_INDEX_ANY)
		{
			for(int i=0;i<XUSER_MAX_COUNT;i++)
			{
				...
				// Fix for #11018 - Functional: When the controller is unplugged during active gameplay and plugged back in at the resulting pause menu, it will demonstrate dual-functionality.
				m_bMenuToBeClosed[i]=false;
			}
		}
		else
...
#ifdef _LINUX
			// When in-game, allow player to toggle the 'Pause' and 'IngameInfo' menus via Kinnect.
...
void UIController::SetTooltips( unsigned int iPad, int iA, int iB, int iX, int iY, int iLT, int iRT, int iLB, int iRB, int iLS, bool forceUpdate)
{
	...
	// strip out any that are not applicable on the platform
#ifndef _LINUX
...
{
	app.DebugPrintf(app.USER_SR, "UIController::RefreshTooltips is not implemented/n");
}
...
void UIController::DisplayUsername(unsigned int iPad, bool show)
{
	// The host decides whether these are on or off
	if( app.GetGameSettings(ProfileManager.GetPrimaryPad(),eGameSetting_DisplaySplitscreenUsernames) == 0)
	{
		show = false;
	}
	EUIGroup group = (EUIGroup)(iPad+1);
...
{
	app.DebugPrintf(app.USER_SR, "UIController::UpdateSelectedItemPos not implemented/n");
}
...
void UIController::HandleDLCMountingComplete()
{
	for(unsigned int i = 0; i < eUIGroup_COUNT; ++i)
	{
		app.DebugPrintf("UIController::HandleDLCMountingComplete - m_groups[%d]/n",i);
...
{
	//app.DebugPrintf(app.USER_SR, "UIController::HandleDLCInstalled not implemented/n");
	for(unsigned int i = 0; i < eUIGroup_COUNT; ++i)
	{
...
void UIController::HandleDLCLicenseChange()
{
	for(unsigned int i = 0; i < eUIGroup_COUNT; ++i)
	{
		app.DebugPrintf("UIController::HandleDLCLicenseChange - m_groups[%d]/n",i);
...
{
	app.DebugPrintf(app.USER_SR, "UIController::HandleTMSDLCFileRetrieved not implemented/n");
}

void UIController::HandleTMSBanFileRetrieved(int iPad)
{
	app.DebugPrintf(app.USER_SR, "UIController::HandleTMSBanFileRetrieved not implemented/n");
}

void UIController::HandleInventoryUpdated(int iPad)
{
	app.DebugPrintf(app.USER_SR, "UIController::HandleInventoryUpdated not implemented/n");
}
...
void UIController::UpdatePlayerBasePositions()
{
	...
	for( BYTE idx = 0; idx < XUSER_MAX_COUNT; ++idx)
	{
		if(pMinecraft->localplayers[idx] != NULL)
		{
			...
			{
				DisplayUsername(idx,false);
			}
			else
			{
				DisplayUsername(idx,true);
			}
			m_groups[idx+1]->SetViewportType((CRender::eViewportType)pMinecraft->localplayers[idx]->m_iScreenSection);
		}
		else
		{
			// This is a legacy thing from our XUI implementation that we don't need
...
#ifndef _LINUX
			m_groups[idx+1]->SetViewportType(CRender::VIEWPORT_TYPE_FULLSCREEN);
#endif
			DisplayUsername(idx,false);
		}
	}
...
{
	// We shouldn't need to implement this
}

void UIController::HideAllGameUIElements()
{
	// We might not need to implement this
	app.DebugPrintf(app.USER_SR, "UIController::HideAllGameUIElements not implemented/n");
}
...
void UIController::ShowSavingMessage(unsigned int iPad, CStorage::ESavingMessage eVal)
{
	bool show = false;
	switch(eVal)
	{
	case CStorage::ESavingMessage_None:
		show = false;
		break;
	case CStorage::ESavingMessage_Short:
	case CStorage::ESavingMessage_Long:
		show = true;
		break;
	}
...
// For the different StringTable classes. Should really fix the libraries.
#ifndef _LINUX
CStorage::EMessageResult UIController::RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad,
														   int( *Func)(LPVOID,int,const CStorage::EMessageResult),LPVOID lpParam, CStringTable *pStringTable, WCHAR *pwchFormatString,DWORD dwFocusButton, bool bIsError)
#else
CStorage::EMessageResult UIController::RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad,
														   int( *Func)(LPVOID,int,const CStorage::EMessageResult),LPVOID lpParam, StringTable *pStringTable, WCHAR *pwchFormatString,DWORD dwFocusButton, bool bIsError)
#endif
{
	...
	if( completed )
	{
		...
		return CStorage::EMessage_Pending;
	}
	else
	{
		return CStorage::EMessage_Busy;
	}
}

CStorage::EMessageResult UIController::RequestUGCMessageBox(UINT title/* = -1 */, UINT message/* = -1 */, int iPad/* = -1*/, int( *Func)(LPVOID,int,const CStorage::EMessageResult)/* = NULL*/, LPVOID lpParam/* = NULL*/)
{
	...
	return CStorage::EMessage_ResultAccept; 
#elif defined(_LINUX)
...
CStorage::EMessageResult UIController::RequestContentRestrictedMessageBox(UINT title/* = -1 */, UINT message/* = -1 */, int iPad/* = -1*/, int( *Func)(LPVOID,int,const CStorage::EMessageResult)/* = NULL*/, LPVOID lpParam/* = NULL*/)
{
	// Default title / messages
	if (title == -1)
	{
...
{
	/* As described in an email from Sean.
	...
	incorrectly.
	*/
#if defined _LINUX || defined _LINUX || defined __LP64__
...
void UIController::TouchBoxAdd(UIControl *pControl,EUIGroup eUIGroup,EUILayer eUILayer,EUIScene eUIscene, UIControl *pMainPanelControl)
{
	...
	//  add main panel offset if controls do not live in the root scene
	...
	//  override control width / height where needed
	...
	{
		app.DebugPrintf("NOT adding touchbox %d,%d,%d,%d/n",puiElement->x1,puiElement->y1,puiElement->x2,puiElement->y2);
	}
	else
	{
		app.DebugPrintf("Adding touchbox %d,%d,%d,%d/n",puiElement->x1,puiElement->y1,puiElement->x2,puiElement->y2);
...
void UIController::TouchBoxRebuild(UIScene *pUIScene)
{
	...
	for (AUTO_VAR(it, pUIScene->GetControls()->begin()); it != itEnd; it++)
	{
		...
		{
			//  update the control (it might have been moved by flash / AS)
...
bool UIController::TouchBoxHit(UIScene *pUIScene,S32 x, S32 y)
{
	...
	//  let's do the transformation from touch resolution to screen resolution here, so our touchbox values always are in screen resolution!
		...
		for (AUTO_VAR(it, m_TouchBoxes[eUIGroup][eUILayer][eUIscene].begin()); it != itEnd; it++)
		{
			...
			if(element->pControl->getHidden() == false && element->pControl->getVisible()) // ignore removed controls
			{
				if((x>=element->x1) &&(x<=element->x2) && (y>=element->y1) && (y<=element->y2))
				{
					...
					{
						app.DebugPrintf("SET m_ActiveUIElement (Layer: %i) at x = %i y = %i/n", (int)eUILayer, (int)x, (int)y);
	...
	//app.DebugPrintf("MISS at x = %i y = %i/n", (int)x, (int)y);
	m_HighlightedUIElement = NULL;
	return false;
}
...
void UIController::HandleTouchInput(unsigned int iPad, unsigned int key, bool bPressed, bool bRepeat, bool bReleased)
{
	...
	{
		app.DebugPrintf("touch input pressed/n");
		...
		case UIControl::eTouchControl:
			...
			break;
		default:
			app.DebugPrintf("PRESSED - UNHANDLED UI ELEMENT/n");
			break;
		}
	}
	else if(bRepeat)	// REPEAT HANDLING
	{
		...
		case UIControl::eTouchControl:
			...
			break;
		default:
			app.DebugPrintf("REPEAT - UNHANDLED UI ELEMENT/n");
			break;
		}
	}
	if(bReleased)	// RELEASED HANDLING
	{
		app.DebugPrintf("touch input released/n");
		switch(m_ActiveUIElement->pControl->getControlType())
		{
		case UIControl::eButton:
		...
		case UIControl::eTouchControl:
			...
			break;
		default:
			app.DebugPrintf("RELEASED - UNHANDLED UI ELEMENT/n");
			break;
		}
	}
