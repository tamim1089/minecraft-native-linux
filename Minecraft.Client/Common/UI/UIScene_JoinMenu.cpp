...
void UIScene_JoinMenu::tick()
{
	...
	if( m_friendInfoUpdatedOK )
	{
		...
		m_labelLabels[eLabel_UsernamesOn].init(app.GetString(IDS_LABEL_USERNAMES));
		...
		if(app.GetGameHostOption(uiGameHostSettings,eGameHostOption_Usernames))	m_labelValues[eLabel_UsernamesOn].init( app.GetString(IDS_ON) );
		else m_labelValues[eLabel_UsernamesOn].init( app.GetString(IDS_OFF) );
	...
	if( m_friendInfoUpdatedERROR )
	{
		...
		m_labelValues[eLabel_Difficulty].init(app.GetString(IDS_DIFFICULTY_TITLE_PEACEFUL));
		m_labelValues[eLabel_GameType].init( app.GetString(IDS_CREATIVE) );
		m_labelValues[eLabel_UsernamesOn].init( app.GetString(IDS_OFF) );
...
int UIScene_JoinMenu::ErrorDialogReturned(void *pParam, int iPad, const CStorage::EMessageResult)
{
	UIScene_JoinMenu *scene = (UIScene_JoinMenu *)pParam;
	ui.NavigateBack(scene->m_iPad);
...
void UIScene_JoinMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_Y:
		...
		{
			PlayerUID uid = m_selectedSession->searchResult.m_playerUids[m_buttonListPlayers.getCurrentSelection()];
	...
	case UPDATE_PLAYERS_TIMER_ID:
		{
			...
			PlayerUID selectedPlayerUID = m_selectedSession->data.players[playersList.GetCurSel()];
			...
			if( success )
			{
				...
				for(unsigned int i = 0; i < MINECRAFT_NET_MAX_PLAYERS; ++i)
				{
					if( m_selectedSession->data.players[i] != NULL )
					{
						if(m_selectedSession->data.players[i] == selectedPlayerUID) selectedIndex = i;
