...
UIScene_MainMenu::UIScene_MainMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
...
#else
	m_buttons[(int)eControl_GameHelp].init(app.GetString(IDS_LINUX_HELP_APP), eControl_GameHelp);
#endif
	...
	// Clear out any loaded game rules
	app.setLevelGenerationOptions(NULL);

	// Reset the leaving game flag so that we correctly handle signouts while in the menus
...
void UIScene_MainMenu::handleGainFocus(bool navBack)
{
	...
	// This needs to come before SetLockedProfile(-1) as it wipes the XbLive contexts.
	if (!navBack)
	{
	...
	// remove the "hobo humping" message legal say we can't have, and the 1080p one for Vita
#ifdef _LINUX
...
#ifdef _LINUX
	case eControl_GameHelp:
		ui.PlayUISFX(eSFX_Press);

		m_eAction=eAction_RunGameHelp;
		signInReturnedFunc = &UIScene_MainMenu::GameHelp_SignInReturned;
		break;
#endif
...
void UIScene_MainMenu::customDrawSplash(IggyCustomDrawCallbackRegion *region)
{
	...
	// Move this to the ctor when the main menu is not the first scene we navigate to
...
int UIScene_MainMenu::MustSignInReturned(void *pParam, int iPad, CStorage::EMessageResult result)
{
...
#ifdef _LINUX						 					  
		case eAction_RunGameHelp:		ProfileManager.RequestSignInUI(false, false,  true, false, true, &UIScene_MainMenu::GameHelp_SignInReturned,		pClass,	iPad ); break;
#endif
		}
	}
	else
...
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
int UIScene_MainMenu::MustSignInReturnedPSN(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_MainMenu* pClass = (UIScene_MainMenu*)pParam;
	...
	if(result==CStorage::EMessage_ResultAccept) 
	{
		...
		case eAction_RunLeaderboardsPSN:
			SQRNetworkManager::AttemptSignIn(&UIScene_MainMenu::Leaderboards_SignInReturned, pClass);
			break;
		case eAction_RunGamePSN:
			SQRNetworkManager::AttemptSignIn(&UIScene_MainMenu::CreateLoad_SignInReturned, pClass);
			break;
		case eAction_RunUnlockOrDLCPSN:
			SQRNetworkManager::AttemptSignIn(&UIScene_MainMenu::UnlockFullGame_SignInReturned, pClass);
			break;
		}
#elif defined _LINUX
		switch(pClass->m_eAction)
		...
		case eAction_RunLeaderboardsPSN:
			...
			//Now Sign-in
			SQRNetworkManager_Vita::AttemptSignIn(&UIScene_MainMenu::Leaderboards_SignInReturned, pClass);
			break;
		case eAction_RunGamePSN:
			SQRNetworkManager_Vita::AttemptSignIn(&UIScene_MainMenu::CreateLoad_SignInReturned, pClass);
			break;
		case eAction_RunUnlockOrDLCPSN:
			//Save settings change
			app.SetGameSettings(0, eGameSetting_Linux_NetworkModeAdhoc, 0);
			...
			CGameNetworkManager::setAdhocMode(false);
			//Now Sign-in
			SQRNetworkManager_Vita::AttemptSignIn(&UIScene_MainMenu::UnlockFullGame_SignInReturned, pClass);
			break;
		}
#else
		switch(pClass->m_eAction)
		{
		case eAction_RunLeaderboardsPSN:
			SQRNetworkManager::AttemptSignIn(&UIScene_MainMenu::Leaderboards_SignInReturned, pClass, true, iPad);
			break;
		case eAction_RunGamePSN:
			SQRNetworkManager::AttemptSignIn(&UIScene_MainMenu::CreateLoad_SignInReturned, pClass, true, iPad);
			break;
		case eAction_RunUnlockOrDLCPSN:
			SQRNetworkManager::AttemptSignIn(&UIScene_MainMenu::UnlockFullGame_SignInReturned, pClass, true, iPad);
			break;
		}
...
int UIScene_MainMenu::HelpAndOptions_SignInReturned(void *pParam,bool bContinue,int iPad)
{
	...
	if(bContinue)
	{
		// Don't we only need to update rich-presence if the sign-in status changes.
		ProfileManager.SetCurrentGameActivity(iPad, CONTEXT_PRESENCE_MENUS, false);
...
int UIScene_MainMenu::CreateLoad_SignInReturned(void *pParam, bool bContinue, int iPad)
{
	...
	if(bContinue)
	{
		// We only need to update rich-presence if the sign-in status changes.
		ProfileManager.SetCurrentGameActivity(iPad, CONTEXT_PRESENCE_MENUS, false);
		...
		else
		{
			...
			if(ProfileManager.IsFullVersion())
			{
				...
				if(bSignedInLive)
				{
					// Need to check for installed DLC
...
//						if(app.GetTMSDLCInfoRead() && app.GetBanListRead(iPad))
						...
						{
							Minecraft *pMinecraft=Minecraft::GetInstance();
							pMinecraft->user->name = convStringToWstring( ProfileManager.GetUsername(ProfileManager.GetPrimaryPad()));
						...
						if(app.GetTMSDLCInfoRead() && app.GetTMSUIDsFileRead() && app.GetBanListRead(iPad))
						{
...
#else
					app.DebugPrintf("Offline Profile returned not implemented/n");		
#ifdef _LINUX
					ui.ShowPlayerDisplayname(true);
#endif
					ui.NavigateToScene(ProfileManager.GetPrimaryPad(), eUIScene_LoadOrJoinMenu);
			...
			{
				// if this is the trial game, we can't have any networking
...
#ifdef _LINUX
int UIScene_MainMenu::GameHelp_SignInReturned(void *pParam, bool bContinue, int iPad)
{
	UIScene_MainMenu* pClass = (UIScene_MainMenu*)pParam;
...
int UIScene_MainMenu::ExitGameReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	//UIScene_MainMenu* pClass = (UIScene_MainMenu*)pParam;
...
void UIScene_MainMenu::RefreshChatAndContentRestrictionsReturned_PlayGame(void *pParam)
{
	...
	// Check if there is a patch for the game
...
int UIScene_MainMenu::PlayOfflineReturned(void *pParam, int iPad, CStorage::EMessageResult result)
{
	UIScene_MainMenu* pClass = (UIScene_MainMenu*)pParam;
...
void UIScene_MainMenu::RunPlayGame(int iPad)
{
	...
	else
	{
		...
		if(ProfileManager.IsFullVersion())
		{
			...
			bool bSignedInLive = ProfileManager.IsSignedInLive(iPad);
#ifdef _LINUX
			if(app.GetGameSettings(ProfileManager.GetPrimaryPad(),eGameSetting_Linux_NetworkModeAdhoc) == true)
			{
				CGameNetworkManager::setAdhocMode(true);
				bSignedInLive = SQRNetworkManager_AdHoc_Vita::GetAdhocStatus();
				app.DebugPrintf("Adhoc mode signed in : %s/n", bSignedInLive ? "true" : "false");
			}
			else
			{
				CGameNetworkManager::setAdhocMode(false);
				app.DebugPrintf("NET mode signed in : %s/n", bSignedInLive ? "true" : "false");
			}
...
void UIScene_MainMenu::tick()
{
	...
	if(m_bLaunchFullVersionPurchase)
	{
		...
		// if there's a commerce error - store down, player can't access store - let the DisplayFullVersionPurchase show the error
	...
	// check for a trial version changing to a full version
	if(m_bTrialVersion)
	{
		if(ProfileManager.IsFullVersion())
		{
	...
	// SQRNetworkManager::tickErrorDialog also runs the error dialog, so wrap this so this doesn't terminate a signin dialog
...
#if TO_BE_IMPLEMENTED
		// You can be offline and still can go into help and options
...
#ifdef _LINUX
int UIScene_MainMenu::SelectNetworkModeReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_MainMenu* pClass = (UIScene_MainMenu*)pParam;

	if(result==CStorage::EMessage_ResultAccept) 
	{
		app.DebugPrintf("Setting network mode to NET/n");
		app.SetGameSettings(0, eGameSetting_Linux_NetworkModeAdhoc, 0);
	}
	else if(result==CStorage::EMessage_ResultDecline) 
	{
		app.DebugPrintf("Setting network mode to Adhoc/n");
		app.SetGameSettings(0, eGameSetting_Linux_NetworkModeAdhoc, 1);
	}
	pClass->updateTooltips();
	return 0;
}
