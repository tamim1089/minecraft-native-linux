...
void IUIScene_AbstractContainerMenu::Initialize(int iPad, AbstractContainerMenu* menu, bool autoDeleteMenu, int startIndex,ESceneSection firstSection,ESceneSection maxSection, bool bNavigateBack)
{
	...
	// New tool tips to support pointer prototype.
	//UpdateTooltips();
	// Default tooltips.
	...
	// don't set the eToolTipPickupPlace_OLD here - let the timer do it.
	...
	// To indicate if pointer has left menu window area.
	m_bPointerOutsideMenu = false;

	// Store the enum range for the current scene
...
void IUIScene_AbstractContainerMenu::onMouseTick()
{
	...
	else
	{
		...
		if ( ( fabs( fInputX ) >= 0.01f ) || ( fabs( fInputY ) >= 0.01f ) )
		{
...
#endif
			// The cursor moves too fast in SD mode
			...
			//printf( "IN %.2f  VEL %.2f  ACC %.2f/n", fInputY, m_fPointerVelY, m_fPointerAccelY );
	...
	else
	{
		if ( !bStickInput )
		{
			...
			// TomK - only if this particular component allows so!
	...
	if ( bPointerIsOverSlot )
	{
		...
		if ( bSlotHasItem )
		{
			...
			// show tooltips for quick use of armour
			...
			// show tooltips for quick use of fuel or ingredient
			...
			// show tooltips for quick use of ingredients in brewing
	...
	// do not allow sub pixel positions or we get broken lines in box edges
...
bool IUIScene_AbstractContainerMenu::handleKeyDown(int iPad, int iAction, bool bRepeat)
{
	...
	//if(pMinecraft->player->GetPadIndex()!=pInputData->UserIndex) return S_OK;
	...
	case ACTION_MENU_Y:
		if(!bRepeat)
		{
			...
			// TU8: Remove this fix, and fix the tooltip display instead as customers liked the feature
		...
		break;
		// Also enable start to exit the scene. This key is also not constrained by the tutorials.
	...
	case ACTION_MENU_B:
		{
			...
			// Fix for #11302 - TCR 001: Network Connectivity: Host crashed after being killed by the client while accessing a chest during burst packet loss.
	...
	case ACTION_MENU_PAGEUP:
		{
			// Do nothing except stop this being passed anywhere else
			bHandled = true;
		}
		break;
#ifdef _LINUX
	...
	if( validKeyPress == TRUE )
	{
		...
		else
		{
			...
			else
			{
				...
				// For pointer system we can legally drop items outside of the window panel here, or may press button while
