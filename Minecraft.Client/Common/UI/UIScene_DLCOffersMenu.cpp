...
void UIScene_DLCOffersMenu::handleTimerComplete(int id)
{
	...
	case PLAYER_ONLINE_TIMER_ID:
		...
		if(ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad())==false)
		{
			...
			uiIDA[0]=IDS_OK;
			CStorage::EMessageResult result = ui.RequestMessageBox( IDS_CONNECTION_LOST, g_NetworkManager.CorrectErrorIDS(IDS_CONNECTION_LOST_LIVE_NO_EXIT), uiIDA,1,ProfileManager.GetPrimaryPad(),UIScene_DLCOffersMenu::ExitDLCOffersMenu,this, app.GetStringTable());
		}
#endif
		break;
	}
...
int UIScene_DLCOffersMenu::ExitDLCOffersMenu(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
void UIScene_DLCOffersMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_UP:
		if(pressed)
		{
			// TomK don't proceed if there is no DLC to navigate through
			if(m_iTotalDLC > 0)
			{
				if(m_iCurrentDLC > 0)
					m_iCurrentDLC--;
...
{	
	...
	//  can't call settexturename from a callback
...
void UIScene_DLCOffersMenu::tick()
{
		...
		if((m_bProductInfoShown==false) && app.GetCommerceProductListRetrieved() && app.GetCommerceProductListInfoRetrieved())
		{
			...
			for(int i=0;i<m_iTotalDLC;i++)
			{
				...
				if(bDLCIsAvailable && (bFirstItemSet==false))
				{
					...
					// info.longDescription isn't null terminated
	...
	else
	{
		...
		// just update the details based on what the current selection is / TomK-don't proceed if total DLC is 0 (bug 4757)
...
void UIScene_DLCOffersMenu::GetDLCInfo( int iOfferC, bool bUpdateOnly )
{
	...
	else
	{
		...
		for(int i = 0; i < iOfferC; i++)
		{
			...
			{
				app.DebugPrintf("Unknown offer - %ls/n",xOffer.wszOfferName);
			}
		}
		...
		for(int i = 0; i < uiDLCCount; i++)
		{
			...
			{		
				...
				// Rog requested we remove the Minecraft at the start of the name. It's required for the Bing search, but gets in the way here
				app.DebugPrintf("Adding %ls at %d/n",wstrTemp.c_str(), i);

				if(wcsncmp(L"Minecraft ",wstrTemp.c_str(),10)==0)
				{
					app.DebugPrintf("Removing Minecraft from name/n");
					WCHAR *pwchNewName=(WCHAR *)wstrTemp.c_str();
					wstrTemp=&pwchNewName[10];
				}

#ifdef _LINUX
				// the hasPurchased comes from the local installed package info
