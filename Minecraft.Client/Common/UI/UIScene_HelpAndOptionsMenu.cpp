...
UIScene_HelpAndOptionsMenu::UIScene_HelpAndOptionsMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	/* we should never remove a control before the other buttons controls are initialised!
...
#ifdef _LINUX
	// in order to buy the skin packs, we need the signed offer ids for them, which we get in the availability info
	...
	// do not need a storage device to see this menu - just need one when you choose to re-install them
	...
	//StorageManager.TMSPP_GetUserQuotaInfo(CStorage::eGlobalStorage_TitleUser,iPad);
