...
UIScene_FullscreenProgress::UIScene_FullscreenProgress(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	thread = new CThread(params->func, params->lpParam, "FullscreenProgress");
	thread->SetProcessor(CPU_CORE_UI_SCENE); // TODO Make sure this is a good thread/core to use
...
void UIScene_FullscreenProgress::tick()
{
	...
	if(currentProgress != m_lastProgress)
	{
		...
		//app.DebugPrintf("Updated progress value/n");
	}
		...
		else
		{
			...
			else
			{
				...
				case e_ProgressCompletion_AutosaveNavigateBack:
					...
					{	
						// Fix for #65437 - Customer Encountered: Code: Settings: Autosave option doesn't work when the Host goes into idle state during gameplay.
				...
				case e_ProgressCompletion_NavigateBackToScene:
					...
					ui.UpdatePlayerBasePositions();
					// If used correctly this scene will not have interfered with any other scene at all, so just navigate back
					navigateBack();
					break;
