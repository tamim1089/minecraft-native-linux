...
UIScene_SaveMessage::UIScene_SaveMessage(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	// If we have a signed in user connected, let's get the DLC now
	...
	case PROFILE_LOADED_TIMER_ID:
		{
			...
			case CStorage::eOptions_Callback_Read_Fail:
#ifdef _LINUX
			case CStorage::eOptions_Callback_Write_Fail:
			case CStorage::eOptions_Callback_Write:
#endif
				// set defaults - which has already been done				
				killTimer(PROFILE_LOADED_TIMER_ID);
				ui.NavigateToHomeMenu();
				SQRNetworkManager::SafeToRespondToGameBootInvite();
				app.SetOptionsCallbackStatus(0,CStorage::eOptions_Callback_Idle);
				break;
			case CStorage::eOptions_Callback_Read_CorruptDeleted:
				killTimer(PROFILE_LOADED_TIMER_ID);
				ui.NavigateToHomeMenu();
				SQRNetworkManager::SafeToRespondToGameBootInvite();
				app.SetOptionsCallbackStatus(0,CStorage::eOptions_Callback_Idle);
				break;
			case CStorage::eOptions_Callback_Read_Corrupt:
				// get the user to delete the options file
				app.DebugPrintf("Corrupt options file/n");
				app.SetOptionsCallbackStatus(0,CStorage::eOptions_Callback_Read_CorruptDeletePending);
				m_bIgnoreInput=false;
...
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
int UIScene_SaveMessage::DeleteOptionsDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
