...
void UIScene_LeaderboardsMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	/*app.DebugPrintf(
			"     m_newSel = %i       [bottomId] = %i       [topId] = %i, [size] = %i/n",
			m_newSel,
	...
	case ACTION_MENU_Y:
		{
			...
			if(m_leaderboard.m_totalEntryCount > 0)
			{
				...
				if( selection >= GetEntryStartIndex() &&
					selection < (GetEntryStartIndex() + m_leaderboard.m_entries.size()) )
				{
					PlayerUID uid = m_leaderboard.m_entries[selection - GetEntryStartIndex()].m_uid;
					if( uid != INVALID_UID )
					{
				...
				if( m_leaderboard.m_entries.size() > 0 )
				{
						...
						selection < (GetEntryStartIndex() + m_leaderboard.m_entries.size()) )
					{
							...
							)
						{
							...
							if( uid != INVALID_UID )
							{
								ProfileManager.ShowAddFriend(m_iPad,uid);
								ui.PlayUISFX(eSFX_Press);
							}
						}
					}
...
void UIScene_LeaderboardsMenu::ReadStats(int startIndex)
{
	...
	//app.DebugPrintf("Requesting stats read %d - %d - %d/n", m_currentLeaderboard, startIndex == -1 ? m_currentFilter : LeaderboardManager::eFM_TopRank, m_currentDifficulty);
	...
	{
		// Friends filter shouldn't switch to toprank.
	...
	case LeaderboardManager::eFM_MyScore:
		{
			PlayerUID uid;
			ProfileManager.GetUID(ProfileManager.GetPrimaryPad(),&uid, true);
			LeaderboardManager::Instance()->ReadStats_MyScore(	this,
				m_currentDifficulty, (LeaderboardManager::EStatsType) m_currentLeaderboard,
				uid /*ignored on Linux*/,
				m_newReadSize
				);
		}
		break;
...
bool UIScene_LeaderboardsMenu::OnStatsReadComplete(LeaderboardManager::eStatsReturn retIn, int numResults, LeaderboardManager::ViewOut results)
{
	...
	//bool noResults = LeaderboardManager::Instance()->GetStatsState() != LeaderboardManager::eStatsState_Ready;
	bool ret;

	//app.DebugPrintf("Leaderboards read %d stats/n", numResults);
...
bool UIScene_LeaderboardsMenu::RetrieveStats()
{
	if(app.DebugSettingsOn() && (app.GetGameSettingsDebugMask()&(1L<<eDebugSetting_DebugLeaderboards)))
	{
		...
		for(unsigned int entryIndex=0; entryIndex < NUM_ENTRIES; entryIndex++)
		{
			...
			m_leaderboard.m_entries[entryIndex].m_uid = INVALID_UID;
			...
			swprintf(m_leaderboard.m_entries[entryIndex].m_username, 17, L"WWWWWWWWWWWWWWWW");
...
void UIScene_LeaderboardsMenu::CopyLeaderboardEntry(LeaderboardManager::ReadScore *statsRow, int leaderboardEntryIndex, bool isDistanceLeaderboard)
{
	...
	leaderboardEntry->m_uid = statsRow->m_uid;
	...
	swprintf(leaderboardEntry->m_username, MAX_USERNAME_SIZE, L"%ls",wstr.c_str());
#else
	memcpy(leaderboardEntry->m_username, statsRow->m_name.data(), statsRow->m_name.size() * sizeof(wchar_t));
#endif
...
// 			leaderboardEntry->m_columns[i-1]=uiVal;
	...
	//Is the player
	PlayerUID myUid;
	ProfileManager.GetUID(ProfileManager.GetPrimaryPad(),&myUid,true);
	if( statsRow->m_uid == myUid )
	{
	...
	else
	{
		...
		{
			if( m_friends[friendIndex].uid == statsRow->m_uid )
			{
...
void UIScene_LeaderboardsMenu::PopulateLeaderboard(LeaderboardManager::eStatsReturn ret)
{
	...
	{
		m_listEntries.setupTitles( app.GetString( IDS_LEADERBOARD_RANK ), app.GetString( IDS_LEADERBOARD_USERNAME ) );
		...
		for(DWORD i = startIndex; i < (startIndex + entryCount); ++i)
		{
			...
			if (idsErrorMessage > 0)
			{
				m_listEntries.addDataSet(
					...
					m_leaderboard.m_entries[i].m_username,
				
					true, // Has error message to display.
			...
			else
			{
				m_listEntries.addDataSet(
					...
					//  | The bDisplayMessage Flag defines if Leaderboard Data should be 
...
#if !(defined(_LINUX) || defined(_LINUX)) // Temp to get the win build running, but so we check this for other platforms
...
void UIScene_LeaderboardsMenu::customDraw(IggyCustomDrawCallbackRegion *region)
{
	...
	if (slotId == -1)
	{
		//app.DebugPrintf("This is not the control we are looking for/n");
	}
	else
	{
...
void UIScene_LeaderboardsMenu::handleTimerComplete(int id)
{
	...
	case PLAYER_ONLINE_TIMER_ID:
		...
		if(ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad())==false)
		{
			...
			uiIDA[0]=IDS_OK;
			CStorage::EMessageResult result = ui.RequestMessageBox( IDS_CONNECTION_LOST, g_NetworkManager.CorrectErrorIDS(IDS_CONNECTION_LOST_LIVE_NO_EXIT), uiIDA,1,ProfileManager.GetPrimaryPad(),UIScene_LeaderboardsMenu::ExitLeaderboards,this, app.GetStringTable());
		}
#endif
		break;
	}
...
int UIScene_LeaderboardsMenu::ExitLeaderboards(void *pParam,int iPad,CStorage::EMessageResult result)
{
