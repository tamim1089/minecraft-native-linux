...
{
	app.DebugPrintf("UITTFFont opening %s/n",path.c_str());
	...
	if( file == INVALID_HANDLE_VALUE )
	{
		...
		app.DebugPrintf("Failed to open TTF file with error code %d (%x)/n", error, error);
		assert(false);
	}
	...
	if(dwFileSize!=0)
	{
		...
		IggyFontInstallTruetypeUTF8 ( (void *)pbData, IGGY_TTC_INDEX_none, "GameFont_TTF", -1, IGGY_FONTFLAG_none );

		IggyFontInstallTruetypeFallbackCodepointUTF8( "GameFont_TTF", -1, IGGY_FONTFLAG_none, fallbackCharacter );

		// These are so we can use the default flash controls
