...
CStorage::SAVETRANSFER_FILE_DETAILS UIScene_LoadOrJoinMenu::m_debugTransferDetails;
#endif
#endif
...
int UIScene_LoadOrJoinMenu::LoadSaveDataThumbnailReturned(LPVOID lpParam,PBYTE pbThumbnail,DWORD dwThumbnailBytes)
{
    ...
    app.DebugPrintf("Received data for save thumbnail/n");
    ...
    else
    {
        ...
        app.DebugPrintf("Save thumbnail data is NULL, or has size 0/n");
    }
...
int UIScene_LoadOrJoinMenu::LoadSaveCallback(LPVOID lpParam,bool bRes)
{
    ...
    if(bRes)
    {
        app.DebugPrintf("Loaded save OK/n");
    }
    return 0;
}
...
#ifdef _LINUX
	// in order to buy the skin packs & texture packs, we need the signed offer ids for them, which we get in the availability info
...
#ifdef _LINUX
    // there may be texture packs we don't have, so use the info from TMS for this
...
#endif
#endif // _LINUX || __ORBIS
    }
    else
    {
        // we need to check that there is enough space left to create a copy of the save (for a rename)
...
{
#ifdef _LINUX
	app.DebugPrintf("missing InputManager.DestroyKeyboard on Vita !!!!!!/n");
#endif
...
#if (defined  _LINUX  || defined _LINUX || defined _LINUX || defined _LINUX || defined _LINUX)
    ...
    if(hasFocus(m_iPad))
    {
        ...
        if(!m_bExitScene && m_bSavesDisplayed && !m_bRetrievingSaveThumbnails && !m_bAllLoaded)
        {
            if( m_iRequestingThumbnailId < (m_buttonListSaves.getItemCount() - m_iDefaultButtonsC ))
            {
                ...
                app.DebugPrintf("Requesting the first thumbnail/n");
                // set the save to load
                PSAVE_DETAILS pSaveDetails=StorageManager.ReturnSavesInfo();
                CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(&pSaveDetails->SaveInfoA[(int)m_iRequestingThumbnailId],&LoadSaveDataThumbnailReturned,this);

                if(eLoadStatus!=CStorage::ESaveGame_GetSaveThumbnail)
                {
        ...
        else if (m_bSavesDisplayed && m_bSaveThumbnailReady)
        {
            ...
            if(!m_bExitScene)
            {
                ...
                if( m_iRequestingThumbnailId < (m_buttonListSaves.getItemCount() - m_iDefaultButtonsC ))
                {
                    app.DebugPrintf("Requesting another thumbnail/n");
                    // set the save to load
                    PSAVE_DETAILS pSaveDetails=StorageManager.ReturnSavesInfo();
                    CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(&pSaveDetails->SaveInfoA[(int)m_iRequestingThumbnailId],&LoadSaveDataThumbnailReturned,this);
                    if(eLoadStatus!=CStorage::ESaveGame_GetSaveThumbnail)
...
void UIScene_LoadOrJoinMenu::GetSaveInfo()
{
    ...
    if(app.DebugSettingsOn() && app.GetLoadSavesFromFolderEnabled())
    {
        ...
        if(m_pSaveDetails==NULL)
        {
            CStorage::ESaveGameState eSGIStatus= StorageManager.GetSavesInfo(m_iPad,NULL,this,"save"); 
        }
#endif
...
#if TO_BE_IMPLEMENTED
        if(eSGIStatus==CStorage::ESGIStatus_NoSaves)
        {
            uiSaveC=0;
            m_controlSavesTimer.setVisible( false );
...
void UIScene_LoadOrJoinMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
    ...
    case ACTION_MENU_Y:
        ...
        if(getControlFocus() == eControl_GamesList && m_buttonListGames.getItemCount() > 0)
        {
            ...
            PlayerUID uid = pSelectedSession->searchResult.m_playerUids[0];
            if( uid != INVALID_UID ) ProfileManager.ShowProfileCard(ProfileManager.GetLockedProfile(),uid);
            ui.PlayUISFX(eSFX_Press);
        }
#endif // _LINUX || _LINUX
        break;
...
{
    // No reason to set value if keyboard was cancelled
    UIScene_LoadOrJoinMenu *pClass=(UIScene_LoadOrJoinMenu *)lpParam;
...
{
    app.DebugPrintf(app.USER_SR, "UIScene_LoadOrJoinMenu::handleInitFocus - %d , %d/n", (int)controlId, (int)childId);
}

void UIScene_LoadOrJoinMenu::handleFocusChange(F64 controlId, F64 childId) 
{
    app.DebugPrintf(app.USER_SR, "UIScene_LoadOrJoinMenu::handleFocusChange - %d , %d/n", (int)controlId, (int)childId);
...
{
    app.DebugPrintf("remoteStorageGetCallback err : 0x%08x/n", error_code);
    assert(error_code == 0);
...
void UIScene_LoadOrJoinMenu::CheckAndJoinGame(int gameIndex)
{
	...
	{
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
		// is the player allowed to join games?
		bool noUGC=false;
		...
		bool bOnlineSubscription=true;
		int iPadWithNoSubscription=0;
		...
		for(unsigned int i = 0; i < XUSER_MAX_COUNT; ++i)
		{
			if( InputManager.IsPadConnected(i) || ProfileManager.IsSignedIn(i) )
			{
				...
				if(ProfileManager.HasOnlineSubscription(i)==false)
				{
					bOnlineSubscription=false;
					break;
				}
			}
		}
		...
		else if (!isSignedInLive)
		{
			...
			{
				m_bIgnoreInput = false;
				// This is a bit messy and is due to the library incorrectly returning false for IsSignedInLive if the npAvailability isn't SCE_OK
		...
		else if(bOnlineSubscription==false)
		{
			// PS Plus upsell
			// upsell psplus
			...
			param.userId = ProfileManager.getUserID(iPadWithNoSubscription);

			iResult=sceNpCommerceDialogOpen(&param);

			//                     UINT uiIDA[2];
			//                     uiIDA[0]=IDS_CONFIRM_OK;
			//                     ui.RequestMessageBox( IDS_FAILED_TO_CREATE_GAME_TITLE, IDS_NO_ONLINE_SUBSCRIPTION, uiIDA,2,ProfileManager.GetPrimaryPad(),&UIScene_LoadOrJoinMenu::PSPlusReturned,this, app.GetStringTable(),NULL,0,false);
...
int UIScene_LoadOrJoinMenu::DeleteSaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
    ...
    if(result==CStorage::EMessage_ResultDecline && validSelection) 
    {
        if(app.DebugSettingsOn() && app.GetLoadSavesFromFolderEnabled())
        {
            pClass->m_bIgnoreInput=false;
...
void UIScene_LoadOrJoinMenu::LoadRemoteFileFromDisk(char* remoteFilename)
{
    ...
    LoadSaveFromDisk(&remoteFile, SAVE_FILE_PLATFORM_LINUX);
}
#endif


int UIScene_LoadOrJoinMenu::SaveOptionsDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;
    ...
    switch(result)
    {
    case CStorage::EMessage_ResultDecline:  // rename
        {
    ...
    case CStorage::EMessage_ResultThirdOption:  // delete -
        {
            // delete the save game
            // Have to ask the player if they are sure they want to delete this game
            UINT uiIDA[2];
...
#ifdef SONY_REMOTE_STORAGE_UPLOAD
    case CStorage::EMessage_ResultFourthOption: // upload to cloud
        {
			UINT uiIDA[2];
			uiIDA[0]=IDS_CONFIRM_OK;
			uiIDA[1]=IDS_CONFIRM_CANCEL;
...
#if defined _LINUX  || defined _LINUX
    case CStorage::EMessage_ResultFourthOption: // copy save
        {
			UINT uiIDA[2];
			uiIDA[0]=IDS_CONFIRM_OK;
			uiIDA[1]=IDS_CONFIRM_CANCEL;
    ...
    case CStorage::EMessage_Cancelled:
    default:
        {
            // reset the tooltips
            pClass->updateTooltips();
...
int UIScene_LoadOrJoinMenu::TexturePackDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_LoadOrJoinMenu *pClass = (UIScene_LoadOrJoinMenu *)pParam;
    ...
    if(result==CStorage::EMessage_ResultAccept) 
    {
		...
		{	
			...
			{	
				// Fix for XB1: #165863 - XR-074: Compliance: With no active network connection user is unable to convert from Trial to Full texture pack and is not messaged why.
				UINT uiIDA[1] = { IDS_CONFIRM_OK };
				ui.RequestMessageBox(IDS_PRO_NOTONLINE_TITLE, IDS_PRO_LINUXLIVE_NOTIFICATION, uiIDA, 1, iPad, NULL, NULL, app.GetStringTable()); 
			}
		}
#endif	
...
#if defined _LINUX || defined _LINUX || defined _LINUX
int UIScene_LoadOrJoinMenu::MustSignInReturnedPSN(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;
	...
	{
#if defined(_LINUX)
		SQRNetworkManager::AttemptSignIn(&UIScene_LoadOrJoinMenu::PSN_SignInReturned, pClass);
#elif defined _LINUX
		SQRNetworkManager_Vita::AttemptSignIn(&UIScene_LoadOrJoinMenu::PSN_SignInReturned, pClass);
#else
		SQRNetworkManager::AttemptSignIn(&UIScene_LoadOrJoinMenu::PSN_SignInReturned, pClass, false, iPad);
#endif
	}
	else
	{
...
int UIScene_LoadOrJoinMenu::CreateDummySaveDataCallback(LPVOID lpParam,bool bRes)
{
	...
	else
	{
		...
		app.DebugPrintf("CreateDummySaveDataCallback failed/n");
...
int UIScene_LoadOrJoinMenu::CrossSaveGetSavesInfoCallback(LPVOID lpParam, SAVE_DETAILS *pSaveDetails, bool bRes)
{
	...
	{
		pClass->m_eSaveTransferState = eSaveTransfer_Error;
		app.DebugPrintf("CrossSaveGetSavesInfoCallback failed/n");
	}
	return 0;
}
...
int UIScene_LoadOrJoinMenu::LoadCrossSaveDataCallback( void *pParam,bool bIsCorrupt, bool bIsOwner )
{
	...
	{
		pClass->m_eSaveTransferState = eSaveTransfer_Error;
		app.DebugPrintf("LoadCrossSaveDataCallback failed /n");

	}
	return 0;
}

int UIScene_LoadOrJoinMenu::CrossSaveFinishedCallback(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) pParam;
...
int UIScene_LoadOrJoinMenu::RemoteSaveNotFoundCallback(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) pParam;
	pClass->m_eSaveTransferState = eSaveTransfer_Idle;
	return 0;
...
int UIScene_LoadOrJoinMenu::DownloadSonyCrossSaveThreadProc( LPVOID lpParameter )
{
    ...
    while(pClass->m_eSaveTransferState!=eSaveTransfer_Idle)
    {
		...
		case eSaveTransfer_GetRemoteSaveInfo:
			app.DebugPrintf("UIScene_LoadOrJoinMenu getSaveInfo/n");
		...
		case eSaveTransfer_CreateDummyFile:
			{
				...
				CStorage::ESaveGameState saveState = StorageManager.SaveSaveData( &UIScene_LoadOrJoinMenu::CreateDummySaveDataCallback, lpParameter );
				if(saveState == CStorage::ESaveGame_Save)
				{
					pClass->m_eSaveTransferState = eSaveTransfer_CreatingDummyFile;
				}
				else
				{
					app.DebugPrintf("Failed to create dummy save file/n");
					pClass->m_eSaveTransferState = eSaveTransfer_Error;
				}
			}
			break;
		...
		case eSaveTransfer_GetSavesInfo:
			{
				...
				app.getRemoteStorage()->waitForStorageManagerIdle();
				app.DebugPrintf("CALL GetSavesInfo B/n");
				CStorage::ESaveGameState eSGIStatus= StorageManager.GetSavesInfo(pClass->m_iPad,&UIScene_LoadOrJoinMenu::CrossSaveGetSavesInfoCallback,pClass,"save");
		...
		case eSaveTransfer_GetFileData:
		{
			...
			{
				pClass->m_eSaveTransferState = eSaveTransfer_Error;
				app.DebugPrintf("app.getRemoteStorage()->getSaveData failed/n");
		...
		case eSaveTransfer_LoadSaveFromDisc:
		{
			...
			{
				pClass->m_eSaveTransferState = eSaveTransfer_Error;
				app.DebugPrintf("CrossSaveGetSavesInfoCallback failed - couldn't find save/n");
			}
			else
			{
#ifdef _LINUX
				CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[saveInfoIndex],&LoadCrossSaveDataCallback,pClass, true);
#else
				CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[saveInfoIndex],&LoadCrossSaveDataCallback,pClass);
#endif
				if(eLoadStatus == CStorage::ESaveGame_Load)
				{
...
#if defined(_LINUX) || defined(_LINUX)
				while(StorageManager.GetSaveState() != CStorage::ESaveGame_Idle )
				{
					Sleep(10);
					StorageManager.Tick();
				}
        ...
        case eSaveTransfer_Error:
			{
				if(bSaveFileCreated)
				{
					...
					if(saveInfoIndex == -1)
					{
						app.DebugPrintf("eSaveTransfer_Error failed - couldn't find save/n");
						assert(0);
						pClass->m_eSaveTransferState = eSaveTransfer_ErrorMesssage;
					}
					else
					...
					app.getRemoteStorage()->waitForStorageManagerIdle();
						CStorage::ESaveGameState eDeleteStatus = StorageManager.DeleteSaveData(&pSaveDetails->SaveInfoA[saveInfoIndex],UIScene_LoadOrJoinMenu::CrossSaveDeleteOnErrorReturned,pClass);
					if(eDeleteStatus == CStorage::ESaveGame_Delete)
					{
						pClass->m_eSaveTransferState = eSaveTransfer_ErrorDeletingSave;
					}
					else
					{
						app.DebugPrintf("StorageManager.DeleteSaveData failed!!/n");
						pClass->m_eSaveTransferState = eSaveTransfer_ErrorMesssage;
					}
				}
				}
...
void UIScene_LoadOrJoinMenu::SaveTransferReturned(LPVOID lpParam, SonyRemoteStorage::Status s, int error_code)
{
	...
	{
        pClass->m_eSaveTransferState = eSaveTransfer_Error;
		app.DebugPrintf("SaveTransferReturned failed with error code : 0x%08x/n", error_code);
	}
...
// Waiting for Sony to fix canceling a save upload
...
int UIScene_LoadOrJoinMenu::CrossSaveUploadFinishedCallback(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) pParam;
...
void UIScene_LoadOrJoinMenu::CancelSaveUploadCallback(LPVOID lpParam)
{
	...
	app.DebugPrintf("m_saveTransferUploadCancelled = true/n");
	ui.SetTooltips( DEFAULT_XUI_MENU_USER, -1, -1, -1, -1,-1,-1,-1,-1);		// MGH -  added - remove the "cancel" tooltip, so the player knows it's underway (really needs a "cancelling" message)

	pClass->m_bIgnoreInput = true;
}

int UIScene_LoadOrJoinMenu::SaveTransferDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;
	// results switched for this dialog
	if(result==CStorage::EMessage_ResultAccept) 
...
void UIScene_LoadOrJoinMenu::LaunchSaveTransfer()
{
	...
	stateContainer->m_eSaveTransferState = CStorage::eSaveTransfer_Idle;
	stateContainer->m_pClass = this;

    LoadingInputParams *loadingParams = new LoadingInputParams();
    loadingParams->func = &UIScene_LoadOrJoinMenu::DownloadSaveThreadProc;
...
int UIScene_LoadOrJoinMenu::DownloadSaveThreadProc( LPVOID lpParameter )
{
    Compression::UseDefaultThreadStorage();
	...
	while(StorageManager.SaveTransferClearState()!=CStorage::eSaveTransfer_Idle)
	{
		Sleep(5);
	}
    ...
    while((pStateContainer->m_eSaveTransferState!=CStorage::eSaveTransfer_Idle) && pStateContainer->m_bSaveTransferInProgress && !pStateContainer->m_bSaveTransferCancelled)
    {
        switch(pStateContainer->m_eSaveTransferState)
        {
        case CStorage::eSaveTransfer_Idle:
            break;
        case CStorage::eSaveTransfer_FileSizeRetrieved:
            ...
            case eSaveTransferFile_Marker:
				if(UIScene_LoadOrJoinMenu::s_ulFileSize == 0)
				{
					...
					pStateContainer->m_eSaveTransferState=CStorage::eSaveTransfer_Idle;
				}
				else
				{
            ...
            break;
        case CStorage::eSaveTransfer_GettingFileData:

            break;
        case CStorage::eSaveTransfer_FileDataRetrieved:
            switch(UIScene_LoadOrJoinMenu::s_eSaveTransferFile)
            {
            case eSaveTransferFile_Marker:
            ...
            case eSaveTransferFile_Metadata:
                {
					...
					// Don't set this any more. We added it so that we could share the ban list data for this save
            ...
            case eSaveTransferFile_SaveData:
                {
                    ...
                    pStateContainer->m_eSaveTransferState=CStorage::eSaveTransfer_Saving;

#else
                    pSave = new ConsoleSaveFileOriginal( wSaveName, m_transferData.data, m_transferData.length, false, SAVE_FILE_PLATFORM_X360 );
                    pStateContainer->m_eSaveTransferState=CStorage::eSaveTransfer_Converting;
#endif
            ...
            break;
        case CStorage::eSaveTransfer_Converting:
#if 0
...
#endif
            break;
        case CStorage::eSaveTransfer_Saving:
            // the directory level storage & therefore the ConsoleSaveSplit instance, which needs to be around until all the sub files have completed saving.
#if defined(_LINUX) || defined(_LINUX)
			pMinecraft->progressRenderer->progressStage(IDS_PROGRESS_SAVING_TO_DISC);            
			...
			while(StorageManager.GetSaveState() != CStorage::ESaveGame_Idle )
            {
				...
				// DO NOT tick this here. The main thread should be the only place ticking the StorageManager. You WILL get crashes.
                //StorageManager.Tick();
            }
#endif
            ...
            //	pClass->m_eSaveTransferState=CStorage::eSaveTransfer_Idle;
            //	pMinecraft->progressRenderer->progressStage( IDS_SAVE_TRANSFER_DOWNLOAD_AND_CONVERT_COMPLETE );
...
{	
    ...
    {
        ZeroMemory(&m_debugTransferDetails, sizeof(CStorage::SAVETRANSFER_FILE_DETAILS) );
...
#endif
    {
        do
		{
			...
			pMinecraft->progressRenderer->progressStage( IDS_SAVETRANSFER_STAGE_GET_DETAILS );
			Sleep(1);
			pClass->m_eSaveTransferState=StorageManager.SaveTransferGetDetails(pClass->m_iPad,CStorage::eGlobalStorage_TitleUser,filename,&UIScene_LoadOrJoinMenu::SaveTransferReturned,pClass);
		}
		while(pClass->m_eSaveTransferState == CStorage::eSaveTransfer_Busy && !pClass->m_bSaveTransferCancelled );
    }
}
...
void UIScene_LoadOrJoinMenu::RequestFileData( SaveTransferStateContainer *pClass, wchar_t *filename )
{
...
#endif
    {
        do
		{
			...
			Sleep(1);
			pClass->m_eSaveTransferState=StorageManager.SaveTransferGetData(pClass->m_iPad,CStorage::eGlobalStorage_TitleUser,filename,&UIScene_LoadOrJoinMenu::SaveTransferReturned,&UIScene_LoadOrJoinMenu::SaveTransferUpdateProgress,pClass,pClass);
		}
		while(pClass->m_eSaveTransferState == CStorage::eSaveTransfer_Busy && !pClass->m_bSaveTransferCancelled );
    }
}

int UIScene_LoadOrJoinMenu::SaveTransferReturned(LPVOID lpParam,CStorage::SAVETRANSFER_FILE_DETAILS *pSaveTransferDetails)
{
    SaveTransferStateContainer* pClass = (SaveTransferStateContainer *) lpParam;
    app.DebugPrintf("Save Transfer - size is %d/n",pSaveTransferDetails->ulFileLen);

    // if the file data is null, then assume this is the file size retrieval
    if(pSaveTransferDetails->pbData==NULL)
    {
        pClass->m_eSaveTransferState=CStorage::eSaveTransfer_FileSizeRetrieved;
    ...
    else
    {
        ...
        pClass->m_eSaveTransferState=CStorage::eSaveTransfer_FileDataRetrieved;
    }
...
int UIScene_LoadOrJoinMenu::NeedSyncMessageReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_LoadOrJoinMenu *pClass = (UIScene_LoadOrJoinMenu *)pParam;
...
#if defined _LINUX || defined _LINUX
int UIScene_LoadOrJoinMenu::CopySaveDialogReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;
...
int UIScene_LoadOrJoinMenu::CopySaveDataReturned(LPVOID lpParam, bool success, CStorage::ESaveGameState stat)
{
	ui.EnterCallbackIdCriticalSection();
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)ui.GetSceneFromCallbackId((size_t)lpParam);
	...
	if(pClass)
	{
		...
		else
		{
			...
			if( stat == CStorage::ESaveGame_CopyCompleteFailLocalStorage )
			{
				ui.LeaveCallbackIdCriticalSection();
			...
			else if( stat == CStorage::ESaveGame_CopyCompleteFailQuota )
			{
				ui.LeaveCallbackIdCriticalSection();
...
int UIScene_LoadOrJoinMenu::CopySaveErrorDialogFinishedCallback(void *pParam,int iPad,CStorage::EMessageResult result)
{
	ui.EnterCallbackIdCriticalSection();
