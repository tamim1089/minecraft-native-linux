...
class CMemoryPoolFixed : public CMemoryPool
{ 
	// Basic type define
...
public:
	...
	CMemoryPoolFixed(uint sizeOfEachBlock,	uint numOfBlocks)
	{
		CreatePool(sizeOfEachBlock, numOfBlocks);
	}

	~CMemoryPoolFixed() { DestroyPool(); }
...
class CMemoryPoolGrow : public CMemoryPool
{
...
public:
	CMemoryPoolGrow(uint32_t size = 64*1024)
	{
		size = Align(size, 4);
		m_totalSize = size;
