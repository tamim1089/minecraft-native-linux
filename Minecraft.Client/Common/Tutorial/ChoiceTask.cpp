...
bool ChoiceTask::isCompleted()
{
	...
	else
	{
		...
		if(!m_bConfirmMappingComplete && InputManager.GetValue(pMinecraft->player->GetPadIndex(), m_iConfirmMapping) > 0 )
		{
			m_bConfirmMappingComplete = true;
		}
		if(!m_bCancelMappingComplete && InputManager.GetValue(pMinecraft->player->GetPadIndex(), m_iCancelMapping) > 0 )
		{
...
void ChoiceTask::sendTelemetry()
{
	...
	if( m_eTelemetryEvent != eTelemetryChallenges_Unknown )
	{
		...
		TelemetryManager->RecordEnemyKilledOrOvercome(pMinecraft->player->GetPadIndex(), 0, 0, 0, 0, 0, 0, m_eTelemetryEvent);
	}
}
