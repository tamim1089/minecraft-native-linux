...
GameRuleDefinition *ConsoleGenerateStructure::addChild(ConsoleGameRules::EGameRuleType ruleType)
{
	...
	{
		rule = new StructureActionGenerateBox();
		m_actions.push_back((StructureActionGenerateBox *)rule);
	}
	else if(ruleType == ConsoleGameRules::eGameRuleType_PlaceBlock)
	{
		rule = new StructureActionPlaceBlock();
		m_actions.push_back((StructureActionPlaceBlock *)rule);
	}
	else if(ruleType == ConsoleGameRules::eGameRuleType_PlaceContainer)
	{
		rule = new StructureActionPlaceContainer();
		m_actions.push_back((StructureActionPlaceContainer *)rule);
	}
	else if(ruleType == ConsoleGameRules::eGameRuleType_PlaceSpawner)
	{
		rule = new StructureActionPlaceSpawner();
		m_actions.push_back((StructureActionPlaceSpawner *)rule);
	}
	else
	{
...
{	
	...
	for(AUTO_VAR(it, m_actions.begin()); it != m_actions.end(); ++it)
	{
		...
		case ConsoleGameRules::eGameRuleType_GenerateBox:
			{
				StructureActionGenerateBox *genBox = (StructureActionGenerateBox *)action;
				genBox->generateBoxInLevel(this,level,chunkBB);
			}
			break;
		case ConsoleGameRules::eGameRuleType_PlaceBlock:
			{
				StructureActionPlaceBlock *pPlaceBlock = (StructureActionPlaceBlock *)action;
				pPlaceBlock->placeBlockInLevel(this,level,chunkBB);
			}
			break;
		case ConsoleGameRules::eGameRuleType_PlaceContainer:
			{
				StructureActionPlaceContainer *pPlaceContainer = (StructureActionPlaceContainer *)action;
				pPlaceContainer->placeContainerInLevel(this,level,chunkBB);
			}
			break;
		case ConsoleGameRules::eGameRuleType_PlaceSpawner:
			{
				StructureActionPlaceSpawner *pPlaceSpawner = (StructureActionPlaceSpawner *)action;
