...
StructureActionPlaceBlock::StructureActionPlaceBlock()
{
	m_x = m_y = m_z = m_tile = m_data = 0;
}

void StructureActionPlaceBlock::writeAttributes(DataOutputStream *dos, UINT numAttrs)
{
...
void StructureActionPlaceBlock::addAttribute(const wstring &attributeName, const wstring &attributeValue)
{
	if(attributeName.compare(L"x") == 0)
	{
		int value = _fromString<int>(attributeValue);
		m_x = value;
		app.DebugPrintf("StructureActionPlaceBlock: Adding parameter x=%d/n",m_x);
	}
	else if(attributeName.compare(L"y") == 0)
	{
		int value = _fromString<int>(attributeValue);
		m_y = value;
		app.DebugPrintf("StructureActionPlaceBlock: Adding parameter y=%d/n",m_y);
	}
	else if(attributeName.compare(L"z") == 0)
	{
		int value = _fromString<int>(attributeValue);
		m_z = value;
		app.DebugPrintf("StructureActionPlaceBlock: Adding parameter z=%d/n",m_z);
	}
	else if(attributeName.compare(L"block") == 0)
	{
		int value = _fromString<int>(attributeValue);
		m_tile = value;
		app.DebugPrintf("StructureActionPlaceBlock: Adding parameter block=%d/n",m_tile);
	}
	else if(attributeName.compare(L"data") == 0)
	{
		int value = _fromString<int>(attributeValue);
		m_data = value;
		app.DebugPrintf("StructureActionPlaceBlock: Adding parameter data=%d/n",m_data);
	}
	else
	{
...
bool StructureActionPlaceBlock::placeBlockInLevel(StructurePiece *structure, Level *level, BoundingBox *chunkBB)
{
	app.DebugPrintf("StructureActionPlaceBlock - placing a block/n");
