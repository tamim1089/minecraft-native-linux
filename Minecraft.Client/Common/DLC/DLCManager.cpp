...
DWORD DLCManager::checkForCorruptDLCAndAlert(bool showMessage /*= true*/)
{
	...
	if(corruptDLCCount > 0 && showMessage)
	{
		...
		if(corruptDLCCount == 1 && firstCorruptPack != NULL)
		{
			...
			CStorage::EMessageResult result = ui.RequestMessageBox( IDS_CORRUPT_DLC_TITLE, IDS_CORRUPT_DLC, uiIDA,1,ProfileManager.GetPrimaryPad(),NULL,NULL, app.GetStringTable(),wchFormat);

		}
		else
		{
			CStorage::EMessageResult result = ui.RequestMessageBox( IDS_CORRUPT_DLC_TITLE, IDS_CORRUPT_DLC_MULTIPLE, uiIDA,1,ProfileManager.GetPrimaryPad(),NULL,NULL, app.GetStringTable());
		}
	}
...
bool DLCManager::readDLCDataFile(DWORD &dwFilesProcessed, const string &path, DLCPack *pack, bool fromArchive)
{
	...
	HANDLE file = CreateFileA(finalPath.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
#elif defined(_LINUX)
...
bool DLCManager::processDLCDataFile(DWORD &dwFilesProcessed, PBYTE pbData, DWORD dwLength, DLCPack *pack)
{
	...
	uiCurrentByte+=sizeof(int);
	CStorage::DLC_FILE_PARAM *pParams = (CStorage::DLC_FILE_PARAM *)&pbData[uiCurrentByte];
	//DWORD dwwchCount=0;
	for(unsigned int i=0;i<uiParameterCount;i++)
	{
		...
		uiCurrentByte+= sizeof(CStorage::DLC_FILE_PARAM)+(pParams->dwWchCount*sizeof(WCHAR));
		pParams = (CStorage::DLC_FILE_PARAM *)&pbData[uiCurrentByte];
	}
	//ulCurrentByte+=ulParameterCount * sizeof(CStorage::DLC_FILE_PARAM);

	unsigned int uiFileCount=*(unsigned int *)&pbData[uiCurrentByte];
	uiCurrentByte+=sizeof(int);
	CStorage::DLC_FILE_DETAILS *pFile = (CStorage::DLC_FILE_DETAILS *)&pbData[uiCurrentByte];

	DWORD dwTemp=uiCurrentByte;
	for(unsigned int i=0;i<uiFileCount;i++)
	{
		dwTemp+=sizeof(CStorage::DLC_FILE_DETAILS)+pFile->dwWchCount*sizeof(WCHAR);
		pFile = (CStorage::DLC_FILE_DETAILS *)&pbData[dwTemp];
	}
	PBYTE pbTemp=((PBYTE )pFile);//+ sizeof(CStorage::DLC_FILE_DETAILS)*ulFileCount;
	pFile = (CStorage::DLC_FILE_DETAILS *)&pbData[uiCurrentByte];
	...
	for(unsigned int i=0;i<uiFileCount;i++)
	{
		...
		pbTemp+=sizeof(int);
		pParams = (CStorage::DLC_FILE_PARAM *)pbTemp;
		for(unsigned int j=0;j<uiParameterCount;j++)
		{
			...
			pbTemp+=sizeof(CStorage::DLC_FILE_PARAM)+(sizeof(WCHAR)*pParams->dwWchCount);
			pParams = (CStorage::DLC_FILE_PARAM *)pbTemp;
		}
		//pbTemp+=ulParameterCount * sizeof(CStorage::DLC_FILE_PARAM);
		...
		else if(dlcFile != NULL)
		{
			...
			// TODO -  Remove the need for this vSkinNames vector, or manage it differently
			switch(pFile->dwType)
			{
		...
		uiCurrentByte+=sizeof(CStorage::DLC_FILE_DETAILS)+pFile->dwWchCount*sizeof(WCHAR);
			...
			if(it != parameterMapping.end() )
			{
				if(type==e_DLCType_PackConfig)
				{
					...
					{				
						...
						// numbered using decimal to make it easier for artists/people to number manually
