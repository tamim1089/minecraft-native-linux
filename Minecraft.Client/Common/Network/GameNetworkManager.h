...
#elif defined _LINUX
#include "../../Linux/Network/PlatformNetworkManagerDurango.h"
#else
#include "PlatformNetworkManagerStub.h"
#endif
...
{
#ifdef _LINUX
	friend class CPlatformNetworkManagerLinux;
#elif defined _LINUX || defined _LINUX || defined _LINUX
...
public:
	...
	INetworkPlayer	*GetPlayerByUid(PlayerUID uid);
	INetworkPlayer	*GetPlayerBySmallId(unsigned char smallId);
	wstring			GetDisplayNameByUsername(wstring username);
	...
	static int		MustSignInReturned_0(void *pParam,int iPad,CStorage::EMessageResult result);
	static int		PSNSignInReturned_0(void* pParam, bool bContinue, int iPad);

	static int		MustSignInReturned_1(void *pParam,int iPad,CStorage::EMessageResult result);
...
private:
	...
	CThread::Event*	m_hServerStoppedEvent;
	CThread::Event*	m_hServerReadyEvent;
	bool m_bInitialised;
