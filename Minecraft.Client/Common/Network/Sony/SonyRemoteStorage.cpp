...
#elif defined _LINUX
static const char sc_localSaveFilename[] = "CloudSave_Linux.bin";
...
bool SonyRemoteStorage::setData( PSAVE_INFO info, CallbackFunc cb, LPVOID lpParam )
{
	...
	CStorage::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(info,&LoadSaveDataThumbnailReturned,this);
	return true;
}
...
int SonyRemoteStorage::LoadSaveDataThumbnailReturned(LPVOID lpParam,PBYTE pbThumbnail,DWORD dwThumbnailBytes)
{
	...
	pClass->m_SetDataThread = new CThread(setDataThread, pClass, "setDataThread");
...
bool SonyRemoteStorage::saveIsAvailable()
{
	if(m_getInfoStatus != e_infoFound)
		return false;
#ifdef _LINUX
	return (getSavePlatform() == SAVE_FILE_PLATFORM_LINUX); 
#elif defined _LINUX
	return (getSavePlatform() == SAVE_FILE_PLATFORM_LINUX); 
#else // _LINUX
	return true;
...
{
	CStorage::ESaveGameState storageState = StorageManager.GetSaveState();
	while(storageState != CStorage::ESaveGame_Idle)
	{
		Sleep(10);
