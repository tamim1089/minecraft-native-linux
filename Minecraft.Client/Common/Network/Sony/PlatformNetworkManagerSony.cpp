...
void CPlatformNetworkManagerSony::HandleStateChange(SQRNetworkManager::eSQRNetworkManagerState oldState, SQRNetworkManager::eSQRNetworkManagerState newState, bool idleReasonIsSessionFull)
{
	...
	{
		// We may be accepting an invite from the DLC menu, so hide the icon
#ifdef _LINUX
	...
	// If the client joins just as the host is exiting, then they can skip to leaving without passing through ending
	else if( newState == SQRNetworkManager::SNM_STATE_ENDING )
	{
		...
		// Only the host can leave here - the clients will hang if m_bLeavingGame is set to true here
		if( m_pSQRNet->IsHost() )
		{
			m_bLeavingGame = true;
		}
...
void CPlatformNetworkManagerSony::HandleDataReceived(SQRNetworkPlayer *playerFrom, SQRNetworkPlayer *playerTo, unsigned char *data, unsigned int dataSize)
{
	...
	if( playerTo->IsHost() )
	{
		...
		//app.DebugPrintf( "Pushing data into host read queue for user /"%ls/"/n", pPlayerFrom->GetUsername());
	...
	else
	{
		...
		//app.DebugPrintf( "Pushing data into read queue for user /"%ls/"/n", apPlayersTo[dwPlayer]->GetUsername());
		if(socket != NULL)
...
void CPlatformNetworkManagerSony::HandlePlayerJoined(SQRNetworkPlayer *              pSQRPlayer)
{
	...
	// We create a fake socket for every where that we need an INBOUND queue of game data. Outbound
    ...
    if( pSQRPlayer->IsLocal() )
    {
        ...
        if( pSQRPlayer->IsHost() )
        {
			...
			// No socket for the localhost as it uses a special loopback queue
...
void CPlatformNetworkManagerSony::HandlePlayerLeaving(SQRNetworkPlayer *pSQRPlayer)
{
	...
	if( networkPlayer )
	{
		...
		if( socket != NULL )
		{
			...
			// TODO We may still be using this at the point that the player leaves the session.
...
bool CPlatformNetworkManagerSony::Initialise(CGameNetworkManager *pGameNetworkManager, int flagIndexSize)
{
...
#ifdef _LINUX
	m_pSQRNet = new SQRNetworkManager(this);
	...
	// seems we can't initialise both adhoc and psn comms - from Rohan - "having adhoc matching and matching2 library initialised together results in undesired behaviour", but probably having other parts initialised also is 'undesirable'

 	m_pSQRNet_Vita->Initialise();

	if(ProfileManager.IsSignedInPSN(ProfileManager.GetPrimaryPad()))
	{
		m_pSQRNet_Vita->AttemptSignIn(NULL, NULL);
	}
...
bool CPlatformNetworkManagerSony::RemoveLocalPlayerByUserIndex( int userIndex )
{
	...
	if(pNetworkPlayer != NULL)
	{
		...
		if( socket != NULL )
		{
			...
			CThread* thread = new CThread(&CPlatformNetworkManagerSony::RemovePlayerOnSocketClosedThreadProc, pNetworkPlayer, "RemovePlayerOnSocketClosed");
	...
	if(pNetworkPlayer != NULL)
	{
		...
		if( socket != NULL )
		{
			...
			if( IsInSession() )
			{
				...
				// Clear our reference to this socket
...
{
	// Fix for #10490 - TCR 001 BAS Game Stability: When a party of four players leave a world to join another world without saving the title will crash.
...
{
// #ifdef _LINUX
	// We probably did this earlier as well, but just to be sure!
...
{
	// Don't assert, just make it a multiple of 8! This count is calculated from a load of separate values,
...
{
#if 0
	return L"Queue messages: " + _toString(((NetworkPlayerLinux *)GetHostPlayer())->GetQNetPlayer()->GetSendQueueSize( NULL, QNET_GETSENDQUEUESIZE_MESSAGES ) )
		+ L" Queue bytes: " + _toString( ((NetworkPlayerLinux *)GetHostPlayer())->GetQNetPlayer()->GetSendQueueSize( NULL, QNET_GETSENDQUEUESIZE_BYTES  ) );
#else
	return L"";
#endif
}
...
wstring CPlatformNetworkManagerSony::GatherRTTStats()
{
	...
	{
		SQRNetworkPlayer *pSQRPlayer = ((NetworkPlayerLinux *)GetPlayerByIndex( i ))->GetQNetPlayer();
...
void CPlatformNetworkManagerSony::TickSearch()
{
	...
	else
	{
		if( !m_pSQRNet->FriendRoomManagerIsBusy() )
		{
			...
			// Every second seems a bit much as it makes the friend list flash every time it updates. Changed this to 5 seconds.
			if( usingAdhocMode())
				searchDelay = 5000;
#endif
...
INetworkPlayer * CPlatformNetworkManagerSony::GetPlayerByUid(PlayerUID uid)
{
	return getNetworkPlayer(m_pSQRNet->GetPlayerByUid(uid));
}
