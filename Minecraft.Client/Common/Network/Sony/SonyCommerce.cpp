...
CThread*				SonyCommerce::m_tickThread = NULL;
...
void SonyCommerce::CheckForTrialUpgradeKey_Callback(LPVOID param, bool bFullVersion)
{
	...
	if(ProfileManager.IsFullVersion())
	{
		...
		// licence has been checked, so we're ok to install the trophies now
 		ProfileManager.InitialiseTrophies(	SQRNetworkManager::GetSceNpCommsId(),
 										SQRNetworkManager::GetSceNpCommsSig());
...
// 	strncpy(tempInfo.ratingImageUrl, ratingInfo.imageUrl, SCE_NP_COMMERCE2_URL_LEN);
	...
	{
		// more than one sku id! We have to be able to use the sku id returned for a product, so there is not supposed to be more than 1
...
void SonyCommerce::UpgradeTrialCallback2(LPVOID lpParam,int err)
{
	...
	if(err != CELL_OK)
	{
		...
		CStorage::EMessageResult result = ui.RequestMessageBox( IDS_PRO_UNLOCKGAME_TITLE, IDS_NO_DLCOFFERS, uiIDA,1,ProfileManager.GetPrimaryPad());
	}
...
// 		}
	...
	case e_message_commerceEnd:	
		...
		// we don't seem to handle the error code here
...
// 		s_memContainer = SYS_MEMORY_CONTAINER_ID_INVALID;
	...
	case e_event_commerceCheckoutFinished:
		...
		// if there's been an error - like dlc already purchased, the runcallback has already happened, and will crash this time
...
void SonyCommerce::CreateSession( CallbackFunc cb, LPVOID lpParam )
{
	...
	if(m_tickThread == NULL)
		m_tickThread = new CThread(TickLoop, NULL, "SonyCommerce tick");
...
// fill out the long description and the price for the product
