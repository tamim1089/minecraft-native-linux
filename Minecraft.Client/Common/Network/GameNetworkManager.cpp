...
#ifdef _LINUX
	s_pPlatformNetworkManager = new CPlatformNetworkManagerLinux();
...
void CGameNetworkManager::DoWork()
{
	...
	if (m_pUpsell != NULL && m_pUpsell->hasResponse())
	{
		...
		if (ProfileManager.HasOnlineSubscription(iPad_checking))
		{
...
bool CGameNetworkManager::_RunNetworkGame(LPVOID lpParameter)
{
	...
	// if this is the trial game, start the trial timer
...
bool	CGameNetworkManager::StartNetworkGame(Minecraft *minecraft, LPVOID lpParameter)
{
	...
	if( g_NetworkManager.IsHost() )
	{
		...
		CThread* thread = new CThread(&CGameNetworkManager::ServerThreadProc, lpParameter, "Server", 256 * 1024);
...
//		printf("Server ready to go!/n");
	...
	// Wait a while to make sure that DLC is loaded. This is the last point before the network communication starts
	...
	{
		// Host needs to generate a unique multiplayer id for sentient telemetry reporting
	...
	do
	{
		...
		// We were ticking this way too fast which could cause the connection to time out
	...
	// Fix for #11279 - CRASH: TCR 001: BAS Game Stability: Signing out of game will cause title to crash
	...
	if( connection->isStarted() && !connection->isClosed() )
	{
		...
		for(int idx = 0; idx < XUSER_MAX_COUNT; ++idx)
		{
			...
			// If they are in the session, then we should add them to the game. Otherwise we won't be able to add them later
			INetworkPlayer *pNetworkPlayer = g_NetworkManager.GetLocalPlayerByUserIndex(idx);
			if( pNetworkPlayer == NULL )
				continue;
			...
			//minecraft->createExtraLocalPlayer(idx, (convStringToWstring( ProfileManager.GetUsername(idx) )).c_str(), idx, connection);

			// Open the socket on the server end to accept incoming data
			Socket::addIncomingSocket(socket);

			connection->send( shared_ptr<PreLoginPacket>( new PreLoginPacket(convStringToWstring( ProfileManager.GetUsername(idx) )) ) );
...
INetworkPlayer	*CGameNetworkManager::GetPlayerByUid(PlayerUID uid)
{
	return s_pPlatformNetworkManager->GetPlayerByUid( uid );
}
...
#ifdef _LINUX
wstring CGameNetworkManager::GetDisplayNameByUsername(wstring username)
{
	return s_pPlatformNetworkManager->GetDisplayNameByUsername(username);
}
#endif
...
#ifdef _LINUX
	// Wait for the loading/saving to finish.
	while (StorageManager.GetSaveState() != CStorage::ESaveGame_Idle)	Sleep(10);
#endif
...
int	CGameNetworkManager::MustSignInReturned_0(void *pParam,int iPad,CStorage::EMessageResult result)
{
	if(result==CStorage::EMessage_ResultAccept) 
	{
#ifdef _LINUX
		SQRNetworkManager::AttemptSignIn(&CGameNetworkManager::PSNSignInReturned_0, pParam,true);
#elif defined _LINUX
		SQRNetworkManager_Vita::AttemptSignIn(&CGameNetworkManager::PSNSignInReturned_0, pParam,true);
#elif defined _LINUX
		SQRNetworkManager::AttemptSignIn(&CGameNetworkManager::PSNSignInReturned_0, pParam,true);
#endif
	}
...
int CGameNetworkManager::PSNSignInReturned_0(void* pParam, bool bContinue, int iPad)
{
	...
	if( bContinue )
	{
		...
		{
#if defined _LINUX || defined __VITA__
			if(!SQRNetworkManager::UpdateInviteData((SQRNetworkManager::PresenceSyncInfo *)inviteData->pInviteInfo))
			{
				bContinue = false;
			}
#elif defined _LINUX
...
int	CGameNetworkManager::MustSignInReturned_1(void *pParam,int iPad,CStorage::EMessageResult result)
{
	if(result==CStorage::EMessage_ResultAccept) 
	{
#ifdef _LINUX
		SQRNetworkManager::AttemptSignIn(&CGameNetworkManager::PSNSignInReturned_1, pParam,true);
#elif defined _LINUX
		SQRNetworkManager_Vita::AttemptSignIn(&CGameNetworkManager::PSNSignInReturned_1, pParam,true);
#elif defined _LINUX
		SQRNetworkManager::AttemptSignIn(&CGameNetworkManager::PSNSignInReturned_1, pParam,true);
#endif
	}
...
int CGameNetworkManager::PSNSignInReturned_1(void* pParam, bool bContinue, int iPad)
{
	...
	if( bContinue )
	{
		...
		{
#if defined _LINUX || defined __VITA__
			if(!SQRNetworkManager::UpdateInviteData((SQRNetworkManager::PresenceSyncInfo *)inviteInfo))
			{
				bContinue = false;
			}
#elif defined _LINUX
...
int CGameNetworkManager::ChangeSessionTypeThreadProc( void* lpParam )
{
	...
	if( g_NetworkManager.m_bLastDisconnectWasLostRoomOnly )
	{
		...
		{
			CStorage::EMessageResult result = ui.RequestMessageBox( IDS_PROGRESS_CONVERTING_TO_OFFLINE_GAME, IDS_ERROR_PSN_SIGN_OUT, uiIDA,1,ProfileManager.GetPrimaryPad());
		}
		else
		{
			CStorage::EMessageResult result = ui.RequestMessageBox( IDS_ERROR_NETWORK_TITLE, IDS_PROGRESS_CONVERTING_TO_OFFLINE_GAME, uiIDA,1,ProfileManager.GetPrimaryPad());
		}
	}
	else
	{
		CStorage::EMessageResult result = ui.RequestMessageBox( IDS_CONNECTION_LOST, g_NetworkManager.CorrectErrorIDS(IDS_CONNECTION_LOST_LIVE_NO_EXIT), uiIDA,1,ProfileManager.GetPrimaryPad());
	}
	...
	if( g_NetworkManager.m_bFullSessionMessageOnNextSessionChange )
	{
		UINT uiIDA[1];
		uiIDA[0]=IDS_CONFIRM_OK;
		CStorage::EMessageResult result = ui.RequestMessageBox( IDS_PROGRESS_CONVERTING_TO_OFFLINE_GAME, IDS_IN_PARTY_SESSION_FULL, uiIDA,1,ProfileManager.GetPrimaryPad());
		pMinecraft->progressRenderer->progressStartNoAbort( IDS_PROGRESS_CONVERTING_TO_OFFLINE_GAME );
	...
	if( pServer != NULL )
	{
		for(unsigned int index = 0; index < XUSER_MAX_COUNT; ++index)
		{
			if(ProfileManager.IsSignedIn(index) && pMinecraft->localplayers[index] != NULL )
			{
				PlayerUID localPlayerUid = pMinecraft->localplayers[index]->getUid();

				PlayerList *players = pServer->getPlayers();
				for(AUTO_VAR(it, players->players.begin()); it < players->players.end(); ++it)
				{
					shared_ptr<ServerPlayer> servPlayer = *it;
					if( servPlayer->getUid() == localPlayerUid )
					{
...
void CGameNetworkManager::StateChange_AnyToEnding(bool bStateWasPlaying)
{
	...
	{
		// If the host is saving then it might take a while to quite the session, so do it ourself
		//m_bLeavingGame = true;
...
void CGameNetworkManager::StateChange_AnyToIdle()
{
	...
	// If we are changing session type there is a race between that thread setting the game to local, and this setting it to not local
...
void CGameNetworkManager::CreateSocket( INetworkPlayer *pNetworkPlayer, bool localPlayer )
{
	...
	else
	{
		...
		// May be other states we want to accept aswell
...
{
	if (g_NetworkManager.IsInGameplay()) // Wait to do this at StartNetworkGame if not in-game yet.
	{
		// Update RichPresence when a player joins the game.
...
void CGameNetworkManager::GameInviteReceived( int userIndex, const INVITE_INFO *pInviteInfo)
{
	...
	// Check that all players are authorised for PsPlus, present upsell to players that aren't and try again.
	for (unsigned int index = 0; index < XUSER_MAX_COUNT; index++)
	{
		if (	 ProfileManager.IsSignedIn(index)
			&&	!ProfileManager.HasOnlineSubscription(userIndex) )
		{
...
// 		else
	...
	for(unsigned int index = 0; index < XUSER_MAX_COUNT; ++index)
	{
		if(ProfileManager.IsSignedIn(index) )
		{
			//  we shouldn't bring any inactive players into the game, except for the invited player (who may be an inactive player)
			// If we are not in a game, then bring in all players signed in
	...
	if(joiningUsers > 1 && !RenderManager.IsHiDef() && userIndex != ProfileManager.GetPrimaryPad())
	{
		...
		// it's possible there is no primary pad here, when accepting an invite from the dashboard
...
void CGameNetworkManager::HandleInviteWhenInMenus( int userIndex, const INVITE_INFO *pInviteInfo)
{
	...
	else
	{
		...
		// If we accept an invite from the main menu before going to play game we need to load the DLC
		...
		// Fix for #10936 - MP Lab: TCR 001: Matchmaking: Player is stuck in a soft-locked state after selecting the guest account when prompted
...
{
	return ProfileManager.GetUsername(playerIdx);
}

void CGameNetworkManager::ServerReadyCreate(bool create)
{
	m_hServerReadyEvent = ( create ? ( new CThread::Event ) : NULL );
}
...
{
	m_hServerStoppedEvent = ( create ? ( new CThread::Event ) : NULL );
}
