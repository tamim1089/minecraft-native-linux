...
#include "../../Linux/Network/NetworkPlayerLinux.h"		// TODO - stub version of this?
...
void CPlatformNetworkManagerStub::NotifyPlayerJoined(IQNetPlayer *pQNetPlayer	)
{
	...
	NetworkPlayerLinux *networkPlayer = (NetworkPlayerLinux *)addNetworkPlayer(pQNetPlayer);
...
// 			{
// 				m_hostGameSessionData.players[i] = ((NetworkPlayerLinux *)pNetworkPlayer)->GetUID();
// 				char *temp;
// 				temp = (char *)wstringtofilename( pNetworkPlayer->GetOnlineName() );
// 				memcpy(m_hostGameSessionData.szPlayers[i],temp,MAX_USERNAME_SIZE);
// 			}
// 			else
// 			{
// 				m_hostGameSessionData.players[i] = NULL;
// 				memset(m_hostGameSessionData.szPlayers[i],0,MAX_USERNAME_SIZE);
// 			}
// 		}
// 		else
// 		{
...
// 		}
// 	}
// 	m_hostGameSessionData.hostPlayerUID = ((NetworkPlayerLinux *)GetHostPlayer())->GetQNetPlayer()->GetUid();
...
wstring CPlatformNetworkManagerStub::GatherRTTStats()
{
	...
	{
		IQNetPlayer *pQNetPlayer = ((NetworkPlayerLinux *)GetPlayerByIndex( i ))->GetQNetPlayer();
...
{
	NetworkPlayerLinux *pNetworkPlayer = new NetworkPlayerLinux(pQNetPlayer);
...
INetworkPlayer * CPlatformNetworkManagerStub::GetPlayerByUid(PlayerUID uid)
{
	return getNetworkPlayer( m_pIQNet->GetPlayerByUid(uid)) ;
}
...
{
#ifdef _LINUX
	// Headless server port: we are the local host; there is no remote session to drop, so the
	// server's "session still alive?" abort-guards in loadLevel/save should pass. TODO(port):
	// drive the real QNet state machine if multiplayer is brought up.
	return true;
#endif
