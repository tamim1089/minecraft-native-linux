...
typedef struct  
{
	...
	// Adding new values for interim TU for 1.6.6
	// bit 9 -				Display usernames in splitscreen		
	...
	union
	{
		struct
		{
			...
			// 0x00000010 - eGameSetting_Linux_NetworkModeAdhoc - on
			...
			// See comment for GAME_SETTINGS_PROFILE_DATA_BYTES below
			// was 192
			...
			// don't need to define the padded space, the union with ucReservedSpace will make the sizeof GAME_SETTINGS correct
		};
...
{ 
	...
	CStorage::eGlobalStorage eStorageFacility;
	CStorage::eTMS_FILETYPEVAL eFileTypeVal;
	//char szFilename[MAX_TMSFILENAME_SIZE];
#ifdef _LINUX
	int( *CallbackFunc)(LPVOID,int,int,LPVOID, WCHAR *);
#else
	int( *CallbackFunc)(LPVOID,int,int,CStorage::PTMSPP_FILEDATA, LPCSTR szFilename);
#endif
