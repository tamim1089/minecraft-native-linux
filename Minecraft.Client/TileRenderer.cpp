...
void TileRenderer::tesselateInWorldFixedTexture( Tile* tile, int x, int y, int z, Icon *fixedTexture )	//  to differentiate from tesselateInWorld
{
...
void TileRenderer::tesselateInWorldNoCulling( Tile* tile, int x, int y, int z, int forceData,
											  shared_ptr< TileEntity > forceEntity )	//  forceData, forceEntity param
{
	noCulling = true;
									 ...
									 shared_ptr< TileEntity > forceEntity )	//  forceData, forceEntity param
{
	...
	case Tile::SHAPE_BLOCK:
		{
			// added these faceFlags so we can detect whether this block is going to have no visible faces and early out
...
bool TileRenderer::tesselateCauldronInWorld(CauldronTile *tt, int x, int y, int z)
{
    ...
    const float cWidth = ( 2.0f / 16.0f ) - ( 1.0f / 128.0f );		// Moved by 1/128th (smallest movement possible with our vertex storage) to remove gap at edge of cauldron
...
void TileRenderer::tesselateDiodeInWorld( DiodeTile* tt, int x, int y, int z, int dir )
{
	...
	// This block gets moved in a later version, but we don't need that yet
	// BEGIN TORCH SECTION
	{
...
void TileRenderer::tesselatePistonBaseForceExtended( Tile* tile, int x, int y, int z, int forceData )	//  forceData param
{
	noCulling = true;
...
bool TileRenderer::tesselatePistonBaseInWorld( Tile* tt, int x, int y, int z, bool forceExtended, int forceData ) //  forceData param
{
...
void TileRenderer::tesselatePistonArmNoCulling( Tile* tile, int x, int y, int z, bool fullArm, int forceData )		//  forceData param
{
	noCulling = true;
...
bool TileRenderer::tesselatePistonExtensionInWorld( Tile* tt, int x, int y, int z, bool fullArm, int forceData )	//  forceData param
{
	int				data = ( forceData == -1 ) ? level->getData( x, y, z ) : forceData;
	...
	switch ( facing )
	{
		case Facing::DOWN:
			...
			t->tex2( getLightColor(tt,  level, x, y , z ) );		// renderPistonArmDown doesn't set its own tex2 so just inherited from previous tesselateBlockInWorld
		...
		case Facing::WEST:
			...
			tesselateBlockInWorld( tt, x, y, z );					// renderPistonArmDown doesn't set its own tex2 so just inherited from previous tesselateBlockInWorld
...
bool TileRenderer::tesselateWaterInWorld( Tile* tt, int x, int y, int z )
{
    ...
    float offs = 0.001f;
	// added. Farm tiles often found beside water, but they consider themselves non-solid as they only extend up to 15.0f / 16.0f.
...
bool TileRenderer::tesselateBlockInWorld( Tile* tt, int x, int y, int z )
{
	...
	if ( Tile::lightEmission[tt->id] == 0 )//TODO/remove (Minecraft::useAmbientOcclusion())
	{
...
// brought changes forward from 1.8.2
...
{
	// the texture is (originally) obtained for each face in the block, if those faces are visible. For a lot of blocks,
	...
	// these changes brought forward from 1.2.3
	...
	if ( faceFlags & 0x01 )
	{
		if ( blsmooth > 0 )
		{
			if ( tileShapeY0 <= 0 ) pY--;		// condition brought forward from 1.2.3
	...
	if ( faceFlags & 0x02 )
	{
		if ( blsmooth > 0 )
		{
			if ( tileShapeY1 >= 1 ) pY++;		// condition brought forward from 1.2.3
	...
	if ( faceFlags & 0x04 )
	{
		if ( blsmooth > 0 )
		{
			if ( tileShapeZ0 <= 0 ) pZ--;		// condition brought forward from 1.2.3
			...
			if (smoothShapeLighting && false) //minecraft->options->ambientOcclusion >= Options::AO_MAX)		// disabling AO_MAX until we work out exactly what its peformance implications are/what it fixes
			{
		...
		if ( fancy && (tex->getFlags() == Icon::IS_GRASS_SIDE) && !hasFixedTexture() )
		{
			...
			c3b *= pBaseBlue;
			c4b *= pBaseBlue;
			bool prev = t->setMipmapEnable( false );	// this is rendering the little bit of grass at the top of the side of dirt, don't mipmap it
	...
	if ( faceFlags & 0x08 )
	{
		if ( blsmooth > 0 )
		{
			if ( tileShapeZ1 >= 1 ) pZ++;		// condition brought forward from 1.2.3
	...
	if ( faceFlags & 0x10 )
	{
		if ( blsmooth > 0 )
		{
			if ( tileShapeX0 <= 0 ) pX--;		// condition brought forward from 1.2.3
	...
	if ( faceFlags & 0x20 )
	{
		if ( blsmooth > 0 )
		{
			if ( tileShapeX1 >= 1 ) pX++;		// condition brought forward from 1.2.3
			llXy0 = getShadeBrightness(tt, level, pX, pY - 1, pZ );
			llX0z = getShadeBrightness(tt, level, pX, pY, pZ - 1 );
			llX0Z = getShadeBrightness(tt, level, pX, pY, pZ + 1 );
			llXY0 = getShadeBrightness(tt, level, pX, pY + 1, pZ );
