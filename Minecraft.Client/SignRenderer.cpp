...
void SignRenderer::render(shared_ptr<TileEntity> _sign, double x, double y, double z, float a, bool setColor, float alpha, bool useCompiled)
{
    ...
    bindTexture(TN_ITEM_SIGN);	//  L"/item/sign.png"
    ...
    for (int i = 0; i < MAX_SIGN_LINES; i++)		// was sign.messages.length
	{
		if(sign->IsVerified())
		{
			if(sign->IsCensored())
		...
		{
            msg = L"> " + msg + L" <";
            font->draw(msg, -font->width(msg) / 2, i * 10 - (MAX_SIGN_LINES) * 5, col);		// (MAX_SIGN_LINES) was sign.messages.length
        }
		else
		{
            font->draw(msg, -font->width(msg) / 2, i * 10 - (MAX_SIGN_LINES) * 5, col);		// (MAX_SIGN_LINES) was sign.messages.length
