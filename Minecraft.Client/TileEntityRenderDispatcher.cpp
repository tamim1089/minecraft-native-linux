...
TileEntityRenderer *TileEntityRenderDispatcher::getRenderer(eINSTANCEOF e)
{
	...
	AUTO_VAR(it, renderers.find( e )); // The .at and [] accessors insert elements if they don't exist
...
/* not doing this hierarchical search anymore. We need to explicitly add renderers for any eINSTANCEOF type that we want to be able to render
    if (it == renderers.end() && e != TileEntity::_class)
	{
        r = getRenderer(dynamic_cast<TileEntity::Class *>( e->getSuperclass() ));
		// added condition here to only add if a valid renderer found
