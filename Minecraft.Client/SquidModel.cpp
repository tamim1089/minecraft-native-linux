...
SquidModel::SquidModel() : Model()
{
    ...
    for (int i = 0; i < TENTACLES_LENGTH; i++)			// 8 was tentacles.length
	{
        tentacles[i] = new ModelPart(this, 48, 0);

        double angle = i * PI * 2.0 / (double) TENTACLES_LENGTH;	// 8 was tentacles.length
        ...
        angle = i * PI * -2.0 / (double) TENTACLES_LENGTH + PI * .5;  // 8 was tentacles.length
...
void SquidModel::render(shared_ptr<Entity> entity, float time, float r, float bob, float yRot, float xRot, float scale, bool usecompiled)
{
    ...
    for (int i = 0; i < TENTACLES_LENGTH; i++) // 8 was tentacles.length // Was 9 but I made it 8 as the array is [0,8)
	{
