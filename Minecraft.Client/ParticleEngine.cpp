...
{
//    if (level != NULL)	// removed - we want level to be initialised to *something*
	{
        this->level = level;
    }
...
{
	// change brought forward from 1.2.3
    float xa = Camera::xa;
    float za = Camera::za;
    ...
    for (int tt = 0; tt < TEXTURE_COUNT; tt++)
	{
		...
		MemSect(31);
        if (tt == MISC_TEXTURE || tt == DRAGON_BREATH_TEXTURE) textures->bindTexture(TN_PARTICLES);	//  L"/particles.png"
        if (tt == TERRAIN_TEXTURE)  textures->bindTexture(TN_TERRAIN);	//  L"/terrain.png"
        if (tt == ITEM_TEXTURE) textures->bindTexture(TN_GUI_ITEMS);	//  L"/gui/items.png"
		MemSect(0);
...
{
	// added. We call this before ParticleEngine::render in the general render per player, so if we
...
{
    this->level = level;
	// we've now got a set of particle vectors for each dimension, and only clearing them when its game over & the level is set to NULL
	if( level == NULL )
	{
		for( int l = 0; l < 3; l++ )
		{
