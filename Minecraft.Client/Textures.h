...
public:
	static bool MIPMAP;
	static CRender::eTextureFormat TEXTURE_FORMAT;
...
private:
    ...
    //IntBuffer *pixels;	// removed so we don't have a permanent buffer kicking round using up 1MB

	unordered_map<wstring, HttpTexture *> httpTextures;
	// Added for GTS textures
...
private:
	...
	void setTextureFormat(const wstring& resourceName);	// 

public:
	void bindTexture(const wstring &resourceName);
	void bindTexture(int resourceId); // 

	//  public for use in XUI controls
	void bind(int id);
...
public:
	...
	int getTexture(BufferedImage *img, CRender::eTextureFormat format = CRender::TEXTURE_FORMAT_RxGyBzAw, bool mipmap = true);
...
public:
	...
	void replaceTextureDirect(intArray rawPixels, int w, int h, int id);		//  as optimisation
	void replaceTextureDirect(shortArray rawPixels, int w, int h, int id);		//  as optimisation
    ...
    void tick(bool updateTextures, bool tickDynamics = true);		//  updateTextures parameter & tickDynamics
public:
	void reloadAll();
	void stitch();
