...
#if defined _LINUX
	static const int MAX_COMMANDBUFFER_ALLOCATIONS = 512 * 1024 * 1024;		// added
#elif defined _LINUX
	static const int MAX_COMMANDBUFFER_ALLOCATIONS = 448 * 1024 * 1024;		// added - hard limit is 512 so giving a lot of headroom here for fragmentation (have seen 16MB lost to fragmentation in multiplayer crash dump before)
#elif defined _LINUX
	static const int MAX_COMMANDBUFFER_ALLOCATIONS = 110 * 1024 * 1024;		// added
#else
	static const int MAX_COMMANDBUFFER_ALLOCATIONS = 55 * 1024 * 1024;		// added
#endif
public:
...
public:
	...
	void render(AABB *b);
	void setDirty(int x0, int y0, int z0, int x1, int y1, int z1, Level *level);		// added level param
	void tileChanged(int x, int y, int z);
	void tileLightChanged(int x, int y, int z);
	void setTilesDirty(int x0, int y0, int z0, int x1, int y1, int z1, Level *level);	// added level param

#ifdef _LINUX
	void cull_SPU(int playerIndex, Culler *culler, float a);
	void waitForCull_SPU();
	CSpursJobQueue::Port* m_jobPort_CullSPU;
	CSpursJobQueue::Port* m_jobPort_FindNearestChunk;
	...
	void addParticle(ePARTICLE_TYPE eParticleType, double x, double y, double z, double xa, double ya, double za); // 
	shared_ptr<Particle> addParticleInternal(ePARTICLE_TYPE eParticleType, double x, double y, double z, double xa, double ya, double za); // 
	void entityAdded(shared_ptr<Entity> entity);
	void entityRemoved(shared_ptr<Entity> entity);
	void playerRemoved(shared_ptr<Entity> entity) {}		// for when a player is removed from the level's player array, not just the entity storage
...
private:
	...
	// this block of declarations was scattered round the code but have gathered everything into one place
	rteMap renderableTileEntities;			// changed - was vector<shared_ptr<TileEntity>, now hashed by chunk so we can find them
	CRITICAL_SECTION					m_csRenderableTileEntities;
	MultiPlayerLevel *level[4];					// now one per player
	Textures *textures;
	//    vector<Chunk *> *sortedChunks[4];	// removed - not sorting our chunks anymore
	ClipChunkArray chunks[4];			// now one per player
	int lastPlayerCount[4];				// added
	int xChunks, yChunks, zChunks;
	int chunkLists;
	Minecraft *mc;
	TileRenderer *tileRenderer[4];		// now one per player
	int ticks;
	...
	int frame;
	int repeatList;
	double xOld[4];						// now one per player
	double yOld[4];						// now one per player
	double zOld[4];						// now one per player

	int totalChunks, offscreenChunks, occludedChunks, renderedChunks, emptyChunks;
	static const int RENDERLISTS_LENGTH = 4;		// added
...
public:
	...
	// Destroyed Tile Management - these things added so we can track tiles which have been recently destroyed, and
	...
	// added for new render list handling
	...
	static CThread *rebuildThreads[MAX_CHUNK_REBUILD_THREADS];
	static CThread::EventArray *s_rebuildCompleteEvents;
	static CThread::Event *s_activationEventA[MAX_CHUNK_REBUILD_THREADS];
