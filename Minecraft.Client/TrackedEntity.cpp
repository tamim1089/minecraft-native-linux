	...
	else
	{
		if(e->riding == NULL)
		{
			...
			if (tickCount++ % updateInterval == 0 || e->hasImpulse)
			{
				...
				// this pos flag used to be set based on abs(xn) etc. but that just seems wrong
				bool pos = abs(xa) >= TOLERANCE_LEVEL || abs(ya) >= TOLERANCE_LEVEL || abs(za) >= TOLERANCE_LEVEL;
				// changed rotation to be generally sent as a delta as well as position
				...
				if (xa < -128 || xa >= 128 || ya < -128 || ya >= 128 || za < -128 || za >= 128
					// I fixed the initialisation of teleportDelay in the ctor, but we managed this far without out
						// and would prefer not to have all the extra traffix so ignore it
							// Fix for #9579 - GAMEPLAY: Boats with a player in them slowly sink under the water over time, and with no player in them they float into the sky.
								...
								)
				{
					...
					//			printf("%d: New teleport rot %d/n",e->entityId,yRotn);
					yRotp = yRotn;
					xRotp = xRotn;
				}
				else
				{
					if (pos && rot)
					{
						//  the movement is small enough, and there's no xrot, then use the new smaller packets
						if( ( xa >= -16 ) && ( xa <= 15 ) &&
							( za >= -16 ) && ( za <= 15 ) &&
							( ya >= -32 ) && ( ya <= 31 ) &&
							( xRota == 0 ))
						...
						else
						{
							...
							//					printf("%d: New posrot %d + %d = %d/n",e->entityId,yRotp,yRota,yRotn);
							c0b++;
						}
					}
					else if (pos)
					{
						//  the movement is small enough, then use the new smaller packets
						if( ( xa >= -8 ) && ( xa <= 7 ) &&
							( za >= -8 ) && ( za <= 7 ) &&
							( ya >= -16 ) && ( ya <= 15 ) )
						{
					...
					{
						//  there's no x rotation, then use the new smaller packet type
						if( xRota == 0 )
						{
							// Clamp rotations that are too big
							if( yRota < -16 )
						...
						{
							//					printf("%d: New rot %d + %d = %d/n",e->entityId,yRotp,yRota,yRotn);
				...
				//		{
				//			printf("%d: %d + %d = %d (%f)/n",e->entityId,xRotp,xRota,xRotn,e->xRot);
				//		}
			}
			
		}
		else // Added: Mobs in minecarts weren't synching their invisibility.
		{
			shared_ptr<SynchedEntityData> entityData = e->getEntityData();
			if (entityData->isDirty())
				broadcastAndSend( shared_ptr<SetEntityDataPacket>( new SetEntityDataPacket(e->entityId, entityData, false) ) );
...
void TrackedEntity::broadcast(shared_ptr<Packet> packet)
{
	...
	{
		// due to the knockback on a player being hit, we need to send to all players, but limit the network traffic here to players that have not already had it sent to their system
		vector< shared_ptr<ServerPlayer> > sentTo;

		// don't send to a player we've already sent this data to that shares the same machine. 
		...
		// 					if(emp!=NULL)
		// 					{
		// 						app.DebugPrintf("Not sending this SetEntityMotionPacket to player - it's already been sent to a player on their console/n");
		// 					}
		// #endif
						}
					}
...
{
	// We call update players when the entity has moved more than a certain amount at the start of it's tick
	...
	// added. Try and find other players who are in the same dimension as this one and on the same machine, and extend our visibility
	...
	// We need to ensure that we send the mount before the rider, so check that the player has been added to the seenBy list
	if(forRider)
	{
		canBeSeenBy = canBeSeenBy && (seenBy.find(sp) != seenBy.end());
	}

	// ADDED! An entities mount has to be visible before the entity visible,
...
void TrackedEntity::updatePlayer(EntityTracker *tracker, shared_ptr<ServerPlayer> sp)
{
		...
		&&	seenBy.find(sp) == seenBy.end()				)
	{
		...
		if (plr != NULL)
		{
			app.DebugPrintf( "TrackedEntity:: Player '%ls' is now visible to player '%ls', %s./n",
				plr->name.c_str(), sp->name.c_str(),
				(e->riding==NULL?"not riding minecart":"in minecart")
				);
		}

		//  brought forward to fix when Item Frames
...
{
	// for some reason this isn't currently working, and is causing players to not appear until we are really close to them. Not sure
...
shared_ptr<Packet> TrackedEntity::getAddEntityPacket()
{
	if (e->removed)
	{
		app.DebugPrintf("Fetching addPacket for removed entity - %ls/n", e->getAName().c_str());
	}

	// replacing with a switch, rather than tons of ifs
	...
	case eTYPE_SERVERPLAYER:
		{
			...
			{
				uid = player->getUid();
				OnlineUid = player->getOnlineUid();
			}
			//  yHeadRotp param to fix #102563 - TU12: Content: Gameplay: When one of the Players is idle for a few minutes his head turns 180 degrees.
			return shared_ptr<AddPlayerPacket>( new AddPlayerPacket(dynamic_pointer_cast<Player>(e), uid, OnlineUid, xp, yp, zp, yRotp, xRotp, yHeadRotp ) );
		}
		break;
	case eTYPE_MINECART:
		{
	...
	case eTYPE_ITEM_FRAME:
		{
			shared_ptr<ItemFrame> frame = dynamic_pointer_cast<ItemFrame>(e);
			{
				...
				app.DebugPrintf("eTYPE_ITEM_FRAME xyz %d,%d,%d/n",ix,iy,iz);
			}
...
/*
    ...
    if (e->GetType() == eTYPE_SERVERPLAYER )
	{
		shared_ptr<ServerPlayer> player = dynamic_pointer_cast<ServerPlayer>(e);
		UID uid = INVALID_UID;
		UID OnlineUid = INVALID_UID;
		if( player != NULL )
		{
			uid = player->getUid();
			OnlineUid = player->getOnlineUid();
		}
		return shared_ptr<AddPlayerPacket>( new AddPlayerPacket(dynamic_pointer_cast<Player>(e), uid, OnlineUid, xp, yp, zp, yRotp, xRotp ) );
    }
