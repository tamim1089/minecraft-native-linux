...
private:
	...
	CThread::EventQueue* levelTickEventQueue;
...
public:
	int width, height;
	int width_phys, height_phys; // added
...
private:
	...
	bool reloadTextures;
public:
	Level *oldLevel; //  added to keep a handle on an old level so we can delete it
	//HANDLE m_hPlayerRespawned; //  so we can wait in menus until it is done (for async in multiplayer)
public:
	...
	// so we can have debugoptions in the server
	unsigned int	uiDebugOptionsA[XUSER_MAX_COUNT];

	// Added these so that we can show a Xui scene while connecting
...
private:
	int rightClickDelay;
public:
	// this should really be in localplayer
...
public:
	...
	//bool wasDown ;
private:
	//	void checkScreenshot();		// removed
	//    String grabHugeScreenshot(File workDir2, int width, int height, int ssWidth, int ssHeight);	// removed
...
public:
	...
	// Added the doForceStatsSave param
	...
	// added to force in the 'other' level when the main player creates the level at game load time
	void forceaddLevel(MultiPlayerLevel *level);
	void prepareLevel(int title);	// changed to public
	void fileDownloaded(const wstring& name, File *file);
	//  OpenGLCapabilities getOpenGLCapabilities();	// removed
	...
	ClientConnection *getConnection(int iPad); //  added iPad param
	static void main();
...
public:
	...
	// When a client requests a texture, it should add it to here while we are waiting for it
