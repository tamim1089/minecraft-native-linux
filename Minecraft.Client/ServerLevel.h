...
private:
	...
	CRITICAL_SECTION				m_tickNextTickCS;	// 
	set<TickNextTickData, TickNextTickDataKeyCompare> tickNextTickList; //  TreeSet
	unordered_set<TickNextTickData, TickNextTickDataKeyHash, TickNextTickDataKeyEq> tickNextTickSet; //  HashSet

	vector<Pos *> m_queuedSendTileUpdates; // 
...
private:
	...
	static WeighedTreasureArray RANDOM_BONUS_ITEMS;	// brought forward from 1.3.2
...
public:
	...
	bool AllPlayersAreSleeping()			{ return allPlayersSleeping;} //  for a message to other players
...
protected:
	...
	void generateBonusItemsNearSpawn();	// brought forward from 1.3.2
...
public:
	...
	void saveToDisc(ProgressListener *progressListener, bool autosave); // 

private:
	void saveLevelData();

	typedef unordered_map<int, shared_ptr<Entity> , IntKeyHash2, IntKeyEq> intEntityMap;
	intEntityMap entitiesById;	// was IntHashMap, using same hashing function as this uses
protected:
...
public:
	...
	static CThread::EventArray*	m_updateTrigger;
	static CRITICAL_SECTION		m_updateCS[3];

	static CThread*			m_updateThread;
