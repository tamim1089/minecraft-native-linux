...
{		
	...
	// reorganised a bit so we can free allocations
...
void EnderDragonRenderer::renderModel(shared_ptr<Entity> _mob, float wp, float ws, float bob, float headRotMinusBodyRot, float headRotx, float scale)
{
	...
	if (mob->dragonDeathTime > 0)
	{
		...
		bindTexture(mob->customTextureUrl, TN_MOB_ENDERDRAGON_SHUFFLE); //  "/mob/enderdragon/shuffle.png"
...
void EnderDragonRenderer::render(shared_ptr<Entity> _mob, double x, double y, double z, float rot, float a)
{
	...
	if (mob->nearestCrystal != NULL)
	{
		...
		// Rotating the healing beam too
		static float fRot=0.0f;
		glRotatef(fRot, 0, 0, 1);
		fRot+=0.5f;		// rate of rotation changed from 5.0 to 0.5 for photosensitivity reasons
		if(fRot>=360.0f)
		{
			fRot=0.0f;
		}
		...
		bindTexture(TN_MOB_ENDERDRAGON_BEAM); //  "/mob/enderdragon/beam.png"

		glShadeModel(GL_SMOOTH);

		float v0 = 0 - (mob->tickCount + a) * 0.005f;		// rate of movement changed from 0.01 to 0.005 for photosensitivity reasons
...
int EnderDragonRenderer::prepareArmor(shared_ptr<Mob> _mob, int layer, float a)
{
	...
	bindTexture(TN_MOB_ENDERDRAGON_ENDEREYES); //  "/mob/enderdragon/ender_eyes.png"
	float br = 1;
	glEnable(GL_BLEND);
	// We probably don't need to do this on 360 either (as we force it back on the renderer)
