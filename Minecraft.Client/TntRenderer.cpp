...
void TntRenderer::render(shared_ptr<Entity> _tnt, double x, double y, double z, float rot, float a)
{
    ...
    if (tnt->life / 5 % 2 == 0)
	{
        ...
        glColor4f(1, 1, 1, br);
		tileRenderer->setColor = false;					//  so that renderTile doesn't set its own colour here
        tileRenderer->renderTile(Tile::tnt, 0, 1);
		tileRenderer->setColor = true;					//  so that renderTile doesn't set its own colour here
        glColor4f(1, 1, 1, 1);
        glDisable(GL_BLEND);
