...
public:
	...
	public:
		...
		//  sendPacket param so we can aggregate the initial send into one much smaller packet
        ...
        bool broadcastChanges(bool allowRegionUpdate);		// added parm
...
private:
	unordered_map<__int64,PlayerChunk *,LongKeyHash,LongKeyEq> chunks;	// was LongHashMap
    vector<PlayerChunk *> changedChunks;
	vector<PlayerChunkAddRequest> addRequests; // 
	void tickAddRequests(shared_ptr<ServerPlayer> player);	// 
...
private:
	...
	void getChunkAndAddPlayer(int x, int z, shared_ptr<ServerPlayer> player);	// 
	void getChunkAndRemovePlayer(int x, int z, shared_ptr<ServerPlayer> player);	// 
public:
