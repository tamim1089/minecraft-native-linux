...
// the Option sub-class used to be an java enumerated type, trying to emulate that functionality here
...
{
	// was a switch statement which we can't do with our Option:: pointer types
...
{
	// should these wstrings append rather than add?
...
//    try {
		...
		// was new BufferedReader(new FileReader(optionsFile));
        BufferedReader *br = new BufferedReader(new InputStreamReader( new FileInputStream( optionsFile ) ) );
			
        wstring line = L"";
        while ((line = br->readLine()) != L"")	// was check against NULL - do we need to distinguish between empty lines and a fail here?
		{
			// removed try/catch
//            try {
                wstring cmds[2];
...
//            }
		...
		//KeyMapping.resetMapping(); //  implemented
        br->close();
//    } catch (Exception e) {
...
//    try {
	...
	// original used a PrintWriter & FileWriter, but seems a bit much implementing these just to do this
		FileOutputStream fos = FileOutputStream(optionsFile);
		DataOutputStream dos = DataOutputStream(&fos);
//        PrintWriter pw = new PrintWriter(new FileWriter(optionsFile));

		dos.writeChars(L"music:" + _toString<float>(music) + L"/n");
        dos.writeChars(L"sound:" + _toString<float>(sound) + L"/n");
        dos.writeChars(L"invertYMouse:" + wstring(invertYMouse ? L"true" : L"false") + L"/n");
