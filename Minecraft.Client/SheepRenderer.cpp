...
int SheepRenderer::prepareArmor(shared_ptr<Mob> _sheep, int layer, float a)
{
    ...
    if (layer == 0 && !sheep->isSheared() &&
		!sheep->isInvisibleTo(Minecraft::GetInstance()->player)) // Todo, merge with java fix (for invisible sheep armour) in '1.7.5'.
	{
		MemSect(31);
		bindTexture(TN_MOB_SHEEP_FUR);	//  L"/mob/sheep_fur.png"
		MemSect(0);
