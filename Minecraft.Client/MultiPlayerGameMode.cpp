...
bool MultiPlayerGameMode::useItemOn(shared_ptr<Player> player, Level *level, shared_ptr<ItemInstance> item, int x, int y, int z, int face, Vec3 *hit, bool bTestUseOnly, bool *pbUsedItem)
{
	...
	// Adding a test only version to allow tooltips to be displayed
	if(!bTestUseOnly)
	{
		ensureHasSentCarriedItem();
	}
	...
	if (t > 0 && player->isAllowedToUse(Tile::tiles[t]))
	{
		if(bTestUseOnly)
		{
			...
			case Tile::bed_Id: // special case for a bed
				...
				else if (t==Tile::bed_Id) // You can still use items on record players (ie. set fire to them).
				{
	...
	// In Java we send the use packet before the above check for item being NULL
	...
	{
		// Bit of a hack, however seems preferable to any larger changes which would have more chance of causing unwanted side effects. 
	...
	// Do the action before we send the packet, so that our predicted count is sent in the packet and the server 
...
bool MultiPlayerGameMode::useItem(shared_ptr<Player> player, Level *level, shared_ptr<ItemInstance> item, bool bTestUseOnly)
{
	...
	//  added for tooltips to test use only
	if(bTestUseOnly)
	{
