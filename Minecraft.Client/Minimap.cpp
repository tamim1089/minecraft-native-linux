...
Minimap::Minimap(Font *font, Options *options, Textures *textures, bool optimised)
{
...
#ifdef _LINUX
	mapTexture = textures->getTexture(img, CRender::TEXTURE_FORMAT_RxGyBzAw5551, false );	// make sure we aren't mipmapping as we never set the data for mipmaps
#else
    mapTexture = textures->getTexture(img, CRender::TEXTURE_FORMAT_RxGyBzAw, false );	// make sure we aren't mipmapping as we never set the data for mipmaps
#endif
	delete img;
    for (int i = 0; i < w * h; i++)
	{
	...
	// generate the colour mapping that we'll be needing as a LUT to minimise processing we actually need to do during normal rendering
	if( genLUT )
	{
		reloadColours();
	}
...
void Minimap::reloadColours()
{
	...
	//  that this code has been extracted pretty much as it was in Minimap::render, although with some byte order changes
	for( int i = 0; i < (14 * 4); i++ )	// 14 material colours currently, 4 brightnesses of each
	{
		if (i / 4 == 0)
		{
			// changed byte order to save having to reorder later
#ifdef _LINUX
			LUT[i] = 0;
#else
			LUT[i] = (((i + i / w) & 1) * 8 + 16);
...
{
	// only update every 8 renders, as an optimisation
	...
	if( !m_optimised || ( renderCount & 7 ) == 0 )
	{
		for (int i = 0; i < w * h; i++)
		{
			...
			// moved the code that used to run here into a LUT that is generated once in the ctor above
			pixels[i] = LUT[val];
		}
	}
	renderCount++;

	// changed - have changed texture generation here to put the bytes in the right order already, so we don't have to do any copying round etc. in the texture replacement itself
    ...
    t->begin();
	// moved to -0.02 to stop z fighting ( was -0.01)
	...
	// stack the map icons
	float fIconZ=-0.04f;// moved to -0.04 (was -0.02) to stop z fighting
	for( vector<MapItemSavedData::MapDecoration *>::iterator it = data->decorations.begin(); it != itEnd; it++ )
	{
		...
		// For item frame renders, the player is NULL. We do not want to show player icons on the frames.
	...
	fIconZ=-0.04f;// moved to -0.04 (was -0.02) to stop z fighting
...
//        glRotatef(0, 1, 0, 0);
    ...
    glScalef(1, 1, 1);
// Don't render the text name, except in debug
//#if 1
//#ifdef _DEBUG
