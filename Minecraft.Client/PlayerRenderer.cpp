...
const unsigned int PlayerRenderer::s_nametagColors[MINECRAFT_NET_MAX_PLAYERS] = 
{
	...
	0xffcc3333, // RED
	0xff3333cc, // BLUE
#ifndef _LINUX		// only 4 player on Vita
	0xffcc33cc, // PINK
	0xffcc6633, // ORANGE
	0xffcccc33, // YELLOW
...
int PlayerRenderer::prepareArmor(shared_ptr<Mob> _player, int layer, float a)
{
	...
	// need to disable rendering armour for some special skins (Daleks)
...
void PlayerRenderer::render(shared_ptr<Entity> _mob, double x, double y, double z, float rot, float a)
{
	...
	// any additional parts to turn on for this player (skin dependent)
...
void PlayerRenderer::renderName(shared_ptr<Mob> _mob, double x, double y, double z)
{
    ...
    if (Minecraft::renderNames() && mob != entityRenderDispatcher->cameraEntity
		&& !mob->isInvisibleTo(Minecraft::GetInstance()->player) ) // Todo, move to LivingEntityRenderer.
	{
        float size = 1.60f;
        ...
        if (dist < maxDist * maxDist)
		{
            ...
            if (mob->isSneaking())
			{
				...
				if(app.GetGameHostOption(eGameHostOption_Usernames)==0)
				{
					// turn off usernames if the host has set them off
					return;
				}
...
void PlayerRenderer::additionalRendering(shared_ptr<Mob> _mob, float a)
{
	...
	if (mob != NULL && app.isUidDeadmau5( mob->getUid() ) && bindTexture(mob->customTextureUrl, L"" ))
	{
        for (int i = 0; i < 2; i++)
		{
...
//	if (bindTexture(mob->cloakTexture, L"" ))
	if (bindTexture(mob->customTextureUrl2, L"" ) && !mob->isInvisible())
	{
		...
		// Fix for sprint-flying causing the cape to rotate up by 180 degrees or more
...
void PlayerRenderer::renderHand()
{
	...
	// does this skin have its arm0 disabled? (Dalek, etc)
...
//  override to stop rendering shadow if player is invisible
