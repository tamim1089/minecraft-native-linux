...
DragonModel::DragonModel(float g) : Model()
{
	...
	// compile now to avoid random performance hit first time cubes are rendered
	// Not just performance, but alpha+depth tests don't work right unless we compile here
...
void DragonModel::render(shared_ptr<Entity> entity, float time, float r, float bob, float yRot, float xRot, float scale, bool usecompiled)
{
	...
	for (int i = 0; i < 5; i++) 
	{
		...
		neck->yRot = rotWrap(dragon->getHeadPartYRotDiff(i, start, p)) * PI / 180.0f * rotScale; //  "p[0] - start[0] with call to getHeadPartYRotDiff
		neck->xRot = rr + (float) (dragon->getHeadPartYOffset(i, start, p)) * PI / 180.0f * rotScale * 5.0f; //  "p[1] - start[1]" with call to getHeadPartYOffset
	...
	head->yRot = rotWrap(dragon->getHeadPartYRotDiff(6, start, p)) * PI / 180.0f * 1; //  "p[0] - start[0] with call to getHeadPartYRotDiff
	head->xRot = (float) (dragon->getHeadPartYOffset(6, start, p)) * PI / 180.0f * rotScale * 5.0f; // 
