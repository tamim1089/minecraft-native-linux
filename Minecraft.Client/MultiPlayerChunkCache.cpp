...
// added - find out if we actually really do have a chunk in our cache
...
{
	// We do want to drop any entities in the chunks, especially for the case when a player is dead as they will
    ...
    if (!chunk->isEmpty())
	{
		...
		// We just want to clear out the entities in the chunk, but everything else should be valid
		chunk->loaded = true;
    }
}
...
LevelChunk *MultiPlayerChunkCache::create(int x, int z)
{
	...
	if( chunk == NULL )
	{
		...
		{
			// We are about to use shared data, abort if the server is stopped and the data is deleted.
		...
		else
		{
			...
			// changed to use new methods for lighting
