...
private:
	...
	int		 lightTexture[NUM_LIGHT_TEXTURES];		// changed so that we have one lightTexture per level, to support split screen
...
public:
    ...
    void unZoomRegion();
private:
	void getFovAndAspect(float& fov, float& aspect, float a, bool applyEffects); // 
public:
...
public:
	void render(float a, bool bFirst);		//  bFirst
...
protected:
    ...
    volatile int yMod;
public:
	void setupGuiScreen(int forceScale=-1);	// added forceScale parameter
...
public:
	...
	static CThread::EventArray* m_updateEvents;
	enum EUpdateEvents
	{
		eUpdateCanRun,
