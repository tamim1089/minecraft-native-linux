...
#include "../Minecraft.World/nbt.h"
#include "../Minecraft.World/Random.h"
#include "../Minecraft.World/Mth.h"
#include "AchievementPopup.h"
#include "CritParticle.h"

// WESTY : Added for new achievements.
#include "../Minecraft.World/Item.h"
#include "../Minecraft.World/MapItem.h"
#include "../Minecraft.World/Tile.h"

// Added for tutorial callbacks
#include "Minecraft.h"
...
LocalPlayer::LocalPlayer(Minecraft *minecraft, Level *level, User *user, int dimension) : Player(level)
{
	...
	if( user != NULL )
	{
		this->name = user->name;
		m_UUID = name;
		//wprintf(L"Created LocalPlayer with name %ls/n", name.c_str() );
		PLAYER_DATA *pPlayerData=app.GetPlayerDataForUid(getOnlineUid());
		if(pPlayerData)
		{
			customTextureUrl=pPlayerData->wchSkin;
		}

	}
	input = NULL;
	m_iPad = -1;
	m_iScreenSection=CRender::VIEWPORT_TYPE_FULLSCREEN; // assume singleplayer default
	m_bPlayerRespawned=false;
	ullButtonsPressed=0LL;
	ullDpad_last = ullDpad_this = ullDpad_filtered = 0;

	// moved in from the minecraft structure
	//ticks=0;
	missTime=0;
	lastClickTick[0] = 0;
	lastClickTick[1] = 0;
	...
	// added for auto repeat in creative mode
...
void LocalPlayer::aiStep()
{
    ...
    // this.heightOffset = input.sneaking?1.30f:1.62f;	// this was already commented out
    if (input->sneaking)	// removed - TODO replace
	{
	...
	// If we can fly, then we should be able to sprint without requiring food. This is particularly a problem for people who save a survival
	// world with low food, then reload it in creative.
	if(abilities.mayfly || isAllowedToFly() ) enoughFoodToSprint = true;

	// altered this slightly to make sure that the joypad returns to below returnTreshold in between registering two movements up to runThreshold
	...
	// try not stopping sprint on collision
	...
	// Fix for #52705 - Customer Encountered: Player can fly in bed while being in Creative mode.
	if (!isSleeping() && (abilities.mayfly || isAllowedToFly() ))
	{
		//  to require jump button to released after being tapped twice to trigger move between flying / not flying
		...
		else if( ( !input->jumping ) && ( jumpTriggerTime > 0 ) && twoJumpsRegistered )
		{
			...
			printf("flying was %s/n", abilities.flying ? "on" : "off");
#endif
			abilities.flying = !abilities.flying;
#ifndef _CONTENT_PACKAGE
			printf("flying is %s/n", abilities.flying ? "on" : "off");
#endif
			jumpTriggerTime = 0;
			twoJumpsRegistered = false;
			if( abilities.flying ) input->sneaking = false;	// would we ever intentially want to go into flying mode whilst sneaking?
		}
	}
	...
	{
		//            yd = 0;
		// note that the 0.42 added for going down is to make it match with what happens when you jump - jumping itself adds 0.42 to yd in Mob::jumpFromGround
		if (ullButtonsPressed & (1LL<<MINECRAFT_ACTION_SNEAK_TOGGLE) ) yd -= ( 0.15 + 0.42 );		// for flying mode, MINECRAFT_ACTION_SNEAK_TOGGLE isn't a toggle but just indicates that this button is down
		if (input->jumping)
		{
			noJumpDelay = 0;
			yd += 0.15;
	...
	// If we're in Creative Mode, allow flying on ground
	...
	if( abilities.flying )//minecraft->options->isFlying )
	{
		...
		// To let the player build easily while flying, we need to change this
...
#ifndef _CONTENT_PACKAGE
		app.DebugPrintf("LocalPlayer::changeDimension from 1 to 1 but WinScreen has not been implemented./n");
		__debugbreak();
#endif
		}
		else
...
{
	bool success = app.LoadSignEntryMenu(GetPadIndex(), sign );
...
{
	bool success = app.LoadContainerMenu(GetPadIndex(), inventory, container );
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadCrafting3x3Menu(GetPadIndex(), dynamic_pointer_cast<LocalPlayer>( shared_from_this() ), x, y, z );
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadEnchantingMenu(GetPadIndex(), inventory, x, y, z, level );
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadRepairingMenu(GetPadIndex(), inventory, level, x, y, z );
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadFurnaceMenu(GetPadIndex(),inventory, furnace);
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadBrewingStandMenu(GetPadIndex(),inventory, brewingStand);
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadTrapMenu(GetPadIndex(),inventory, trap);
	if( success ) ui.PlayUISFX(eSFX_Press);
...
{
	bool success = app.LoadTradingMenu(GetPadIndex(),inventory, traderTarget, level);
	if( success ) ui.PlayUISFX(eSFX_Press);
...
void LocalPlayer::hurtTo(int newHealth, ETelemetryChallenges damageSource)
{
	...
	if( this->health <= 0)
	{
		...
		TelemetryManager->RecordPlayerDiedOrFailed(GetPadIndex(), 0, y, 0, 0, carriedId, 0, damageSource);

		// if there are any xuiscenes up for this player, close them
		if(ui.GetMenuDisplayed(GetPadIndex()))
		{
			ui.CloseUIScenes(GetPadIndex());
		}
	}
...
void LocalPlayer::respawn()
{
	...
	minecraft->respawnPlayer(GetPadIndex(), 0, 0);
}
...
{
	minecraft->gui->displayClientMessage(messageId, GetPadIndex());
}

void LocalPlayer::awardStat(Stat *stat, byteArray param)
{
#ifdef _LINUX
	// Maybe we want to fine tune this later? #TODO
	if (	   !ProfileManager.IsGuest(GetPadIndex()) 
    ...
    if (stat->isAchievement())
	{
		...
		// changed to attempt to award everytime - the award may need a storage device, so needs a primary player, and the player may not have been a primary player when they first 'got' the award
		// so let the award manager figure it out
        //if (!minecraft->stats[m_iPad]->hasTaken(ach))
		{
            // Don't display the java popup
			//minecraft->achievementPopup->popup(ach);

			// Added this function in the libraries as some achievements don't get awarded to all players
			...
			{
				// We don't (currently) care about the gamerscore, so setting to a default of 0 points
				TelemetryManager->RecordAchievementUnlocked(m_iPad,ach->getAchievementID(),0);

				// Some awards cause a menu to popup. This can be bad, especially if you are surrounded by mobs!
		...
		// AWARD : The Haggler, Acquire 30 emeralds.
		{
			...
			if ( stat == emeraldMined || stat == emeraldBought )
			{
				...
				app.DebugPrintf(
					...
					"emerald_mined=%i, emerald_bought=%i, sum=%i./n",
		...
		// AWARD : It's a Sign, craft and place a sign.
		{
			...
			if ( stat == craftSign || stat == placeWallsign || stat == placeSignpost )
			{
				...
				app.DebugPrintf(
					...
					"crafted=%i, placedWallSigns=%i, placedSignposts=%i./n",
...
{
	// We should not accept any input while asleep, except the above to wake up
    ...
    if (down && minecraft->hitResult != NULL && minecraft->hitResult->type == HitResult::TILE && button == 0)
	{
		...
		// addition to stop layer mining out of the top or bottom of the world
		// Allow this for The End
...
bool LocalPlayer::handleMouseClick(int button)
{
	...
	if (button == 0) 
	{
		//app.DebugPrintf("handleMouseClick - Player %d is swinging/n",GetPadIndex());
		swing();
	}
	...
	{
		if (button == 0 && minecraft->localgameModes[GetPadIndex()]->hasMissTime()) missTime = 10;
	}
	else if (minecraft->hitResult->type == HitResult::ENTITY)
	{
		if (button == 0)
		{
			minecraft->gameMode->attack(minecraft->localplayers[GetPadIndex()], minecraft->hitResult->entity);
		}
		if (button == 1)
		{
			// if we milk a cow here, and end up with a bucket of milk, the if (mayUse && button == 1) further down will 
			...
			if( minecraft->gameMode->interact(minecraft->localplayers[GetPadIndex()], minecraft->hitResult->entity) )
			{
				mayUse = false;
			}
		}
	...
	else if (minecraft->hitResult->type == HitResult::TILE)
	{
		...
		else
		{
			...
			bool usedItem = false;
			if (minecraft->gameMode->useItemOn(minecraft->localplayers[GetPadIndex()], level, item, x, y, z, face, minecraft->hitResult->pos, false, &usedItem))
			{
				...
				mayUse = false;
				//app.DebugPrintf("Player %d is swinging/n",GetPadIndex());
				swing();
			}
			if (item == NULL)
			{
			...
			else if (item->count != oldCount || minecraft->localgameModes[GetPadIndex()]->hasInfiniteItems())
			{
	...
	if (mayUse && button == 1)
	{
		...
		if (item != NULL)
		{
			if (minecraft->gameMode->useItem(minecraft->localplayers[GetPadIndex()], level, item))
			{
