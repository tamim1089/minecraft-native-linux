...
public:
	...
	// for 'Adventure Time' achievement.
	Biome *currentBiome;
...
public:
	...
	~ServerPlayer();
	void flagEntitiesToBeRemoved(unsigned int *flags, bool *removedFound);			// 
...
public:
	...
	virtual bool startRepairing(int x, int y, int z);					//  bool return
	...
	virtual bool openTrading(shared_ptr<Merchant> traderTarget); //  bool return
...
public:
	...
	static int getFlagIndexForChunk(const ChunkPos& pos, int dimension);		// added
	int getPlayerViewDistanceModifier(); // returns a number which is subtracted from the default view distance
...
protected:
	//  to record telemetry of player deaths, this should store the last source of damage
