...
#ifdef _LINUX
#include "Linux/SPU_Tasks/Texture_blit/Texture_blit.h"
#include "CSpursJob.h"
...
CSpursJobQueue::Port* g_texBlitJobQueuePort;
#endif //_LINUX
...
void Texture::_init(const wstring &name, int mode, int width, int height, int depth, int wrapMode, int format, int minFilter, int magFilter, bool mipMap)
{
	...
	if(g_texBlitJobQueuePort == NULL)
		g_texBlitJobQueuePort = new CSpursJobQueue::Port("CSpursJob_Texture_blit");
#endif
	this->name = name;
	this->mode = mode;
	this->width = width;
...
void Texture::blit(int x, int y, Texture *source, bool rotated)
{
	...
	for(unsigned int level = 0; level < m_iMipLevels; ++level)
	{
		...
		{
			app.DebugPrintf("ran out of tex blit slots, stalling for completion/n");
		...
		CSpursJob_Texture_blit blitJob(&dataIn);
...
void Texture::transferFromImage(BufferedImage *image)
{
	...
	if (imgWidth > width || imgHeight > height)
	{
		...
		//	", " + height + "). Ignoring.");
		app.DebugPrintf("transferFromImage called with a BufferedImage with dimensions (%d, %d) larger than the Texture dimensions (%d, %d). Ignoring./n", imgWidth, imgHeight, width, height);
		return;
	}
...
//  from older versions for where we create mip-maps for levels that do not have pre-made graphics
...
void Texture::updateOnGPU()
{
	...
	//else if(height != 1)
	//{
		//  check so we can differentiate between which RenderManager function to call
		if(!m_bInitialised)
		{			
			...
			unsigned char *newData = RenderManager.TextureData(width,height,data[0]->getBuffer(),0,CRender::TEXTURE_FORMAT_RxGyBzAw);
			ByteBuffer *oldBuffer = data[0];
			data[0] = new ByteBuffer(data[0]->getSize(), (byte*) newData);
			delete oldBuffer;
			newData += width * height * 4;
#else
			RenderManager.TextureData(width,height,data[0]->getBuffer(),0,CRender::TEXTURE_FORMAT_RxGyBzAw);
#endif
...
#else
					RenderManager.TextureData(levelWidth,levelHeight,data[level]->getBuffer(),level,CRender::TEXTURE_FORMAT_RxGyBzAw);
#endif
				}
			}
