...
{
	// original version used generics and thus had an input parameter of type Arrow rather than shared_ptr<Entity>  we have here - 
	// do some casting around instead
	shared_ptr<Arrow> arrow = dynamic_pointer_cast<Arrow>(_arrow);
    bindTexture(TN_ITEM_ARROWS); // was L"/item/arrows.png"
...
//    glNormal3f(ss, 0, 0);		// changed to use tesselator
    t->begin();
	t->normal(1,0,0);
...
//    glNormal3f(-ss, 0, 0);	// changed to use tesselator
    ...
    for (int i = 0; i < 4; i++)
	{
        ...
        glRotatef(90, 1, 0, 0);
//        glNormal3f(0, 0, ss);		// changed to use tesselator
        t->begin();
		t->normal(0,0,1);
