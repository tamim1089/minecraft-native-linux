...
// this class is fairly substantially altered as there didn't seem any point in porting code for banning, whitelisting, ops etc.
...
void PlayerList::placeNewPlayer(Connection *connection, shared_ptr<ServerPlayer> player, shared_ptr<LoginPacket> packet)
{
	bool newPlayer = load(player);
		...
		if( !g_NetworkManager.IsLocalGame() )
		{
			...
			{
				((NetworkPlayerSony *)networkPlayer)->SetUID( packet->m_onlineUid );
			}
		}
#endif
...
//        logger.info(getName() + " logged in with entity id " + playerEntity.entityId + " at (" + playerEntity.x + ", " + playerEntity.y + ", " + playerEntity.z + ")");
		...
		// Moved this here so we can send player-model texture and geometry data.
		...
		if(newPlayer)
		{
			...
			int centreZC = 0;
#endif
			// Give every player a map the first time they join a server
			player->inventory->setItem( 9, shared_ptr<ItemInstance>( new ItemInstance(Item::map_Id, 1, level->getAuxValueForMap(player->getUid(),0,centreXC, centreZC, mapScale ) ) ) );
		...
		if(!player->customTextureUrl.empty() && player->customTextureUrl.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(player->customTextureUrl))
		{
			if(	server->getConnection()->addPendingTextureRequest(player->customTextureUrl))
			{
				...
				wprintf(L"Sending texture packet to get custom skin %ls from player %ls/n",player->customTextureUrl.c_str(), player->name.c_str());
#endif
		...
		if(!player->customTextureUrl2.empty() && player->customTextureUrl2.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(player->customTextureUrl2))
		{
			...
			{
#ifndef _CONTENT_PACKAGE
				wprintf(L"Sending texture packet to get custom skin %ls from player %ls/n",player->customTextureUrl2.c_str(), player->name.c_str());
#endif
		...
		//  to store UGC settings
		playerConnection->m_friendsOnlyUGC = packet->m_friendsOnlyUGC;
		playerConnection->m_offlineUID = packet->m_offlineUid;
		playerConnection->m_onlineUID = packet->m_onlineUid;
        ...
        // removed, since it needs to be localised in the language the client is in
		...
		player->doTick(true, true, false);	// added - force sending of the nearest chunk before the player is teleported, so we have somewhere to arrive on...
...
int PlayerList::getMaxRange()
{
 ...
 //  return val to bool to check if new player or loaded player
...
{
	// Some adjustments to make sure the current players position is correct
	// Make sure that the player is on the ground, and in the centre x/z of the current column
	app.DebugPrintf("Original pos is %f, %f, %f in dimension %d/n", player->x, player->y, player->z, player->dimension);
	...
	app.DebugPrintf("New pos is %f, %f, %f in dimension %d/n", player->x, player->y, player->z, player->dimension);
	...
	app.DebugPrintf("Final pos is %f, %f, %f in dimension %d/n", player->x, player->y, player->z, player->dimension);

	// If we are in the nether and the above while loop has put us above the nether then we have a problem
	...
	// We also use this mechanism to force a spawn point in the overworld for players who were in the save when the reset nether option was applied
	if(level->dimension->id == -1 && player->y > 125)
	{
		app.DebugPrintf("Player in the nether tried to spawn at y = %f, moving to overworld/n", player->y);
		...
		app.DebugPrintf("Updated pos is %f, %f, %f in dimension %d/n", player->x, player->y, player->z, player->dimension);
	}
}
...
void PlayerList::add(shared_ptr<ServerPlayer> player)
{
	...
	// Swapped these lines about so that we get the chunk visiblity packet way ahead of all the add tracked entity packets
...
{
    save(player);
	//We don't want to save the map data for guests, so when we are sure that the player is gone delete the map
	...
	// Save all the players currently in the game, which will also free up unused map id slots if required, and remove old players
	saveAll(NULL,false);
}

shared_ptr<ServerPlayer> PlayerList::getPlayerForLogin(PendingConnection *pendingConnection, const wstring& userName, PlayerUID uid, PlayerUID onlineUid)
{
	...
	player->gameMode->player = player; //  as had to remove this assignment from ServerPlayer ctor
	player->setUid( uid ); // 
	player->setOnlineUid( onlineUid ); // 
...
shared_ptr<ServerPlayer> PlayerList::respawn(shared_ptr<ServerPlayer> serverPlayer, int targetDimension, bool keepAllPlayerData)
{
	...
	if( isPrimary )
	{
		if( isEmptying )
		{
			app.DebugPrintf("Emptying this dimension/n");
			serverPlayer->getLevel()->getTracker()->clear(serverPlayer);
		}
		else
		{
			app.DebugPrintf("Transferring... storing flags/n");
			serverPlayer->getLevel()->getTracker()->removeEntity(serverPlayer);
		}
	}
	else
	{
		app.DebugPrintf("Not primary player/n");
		serverPlayer->getLevel()->getTracker()->removeEntity(serverPlayer);
	}
	...
	PlayerUID playerUid = serverPlayer->getUid();
	PlayerUID playerOnlineUid = serverPlayer->getOnlineUid();
	...
	player->setOnlineUid( playerOnlineUid ); // 

	// Don't reuse the id. If we do, then the player can be re-added after being removed, but the add packet gets sent before the remove packet
	...
	// Added this as we need to know earlier if the player is the player for this connection so that
	...
	// Dying before this point in the tutorial is pretty annoying,
...
void PlayerList::toggleDimension(shared_ptr<ServerPlayer> player, int targetDimension)
{
	...
	if( isPrimary )
	{
		if( isEmptying )
		{
			app.DebugPrintf("Toggle... Emptying this dimension/n");
			player->getLevel()->getTracker()->clear(player);
		}
		else
		{
			app.DebugPrintf("Toggle...  transferring/n");
			player->getLevel()->getTracker()->removeEntity(player);
		}
	}
	else
	{
		app.DebugPrintf("Toggle...  Not primary player/n");
		player->getLevel()->getTracker()->removeEntity(player);
	}

    ServerLevel *oldLevel = server->getLevel(player->dimension);

	// Do this much earlier so we don't end up unloading chunks in the wrong dimension
	...
	// Fix for #46423 - TU5: Art: Code: No burning animation visible after entering The Nether while burning
	player->clearFire(); // Stop burning if travelling through a portal

	//  Added so that we remove entities from the correct level, after the respawn packet we will be in the wrong level
    ...
    double xt = player->x;
    double zt = player->z;
    double scale = newLevel->getLevelData()->getHellScale(); //  was 8 but this is all we can fit in
	...
	// Fix for #64683 - Customer Encountered: TU7: Content: Gameplay: Potion effects are removed after using the Nether Portal
	vector<MobEffectInstance *> *activeEffects = player->getActiveEffects();
	for(AUTO_VAR(it, activeEffects->begin()); it != activeEffects->end(); ++it)
	{
		MobEffectInstance *effect = *it;
...
{
	// brought changes to how often this is sent forward from 1.2.3
	...
	while(!m_smallIdsToClose.empty())
	{
		...
		for(unsigned int i = 0; i < players.size(); i++)
		{
			...
			// May be being a bit overprotective with all the NULL checks, but adding late in TU7 so want to be safe
	...
	while(!m_smallIdsToKick.empty())
	{
		...
		if( selectedPlayer != NULL )
		{
			...
			{
//#ifdef _LINUX
				PlayerUID uid = selectedPlayer->GetUID();
				...
				{
					shared_ptr<ServerPlayer> p = players.at(i);
					PlayerUID playersUid = p->getOnlineUid();
					if (p != NULL && ProfileManager.AreUIDSEqual(playersUid, uid ) )
					{
						player = p;
						break;
					}
				...
				if (player != NULL)
				{
					m_bannedUids.push_back( player->getOnlineUid() );
					// If we have kicked a player, make sure that they have no privileges if they later try to join the world when trust players is off
		...
		for(unsigned int i = 0; i < receiveAllPlayers[dim].size(); ++i)
		{
			...
			if(currentPlayer->removed)
			{
				...
				if(newPlayer != NULL)
				{
					...
					app.DebugPrintf("Replacing primary player %ls with %ls in dimension %d/n", currentPlayer->name.c_str(), newPlayer->name.c_str(), dim);
				}
			}
		}
	}
...
// make sure that any tile updates for the chunk at this location get prioritised for sending
...
shared_ptr<ServerPlayer> PlayerList::getPlayer(const wstring& name)
{
    for (unsigned int i = 0; i < players.size(); i++) 
	{
        ...
        if (p->name == name)	// used to be case insensitive (using equalsIgnoreCase) - imagine we'll be shifting to UIDs anyway
		{
            return p;
        }
    }
...
shared_ptr<ServerPlayer> PlayerList::getPlayer(PlayerUID uid)
{
	...
	{
        shared_ptr<ServerPlayer> p = players[i];
        if (p->getUid() == uid || p->getOnlineUid() == uid)	// used to be case insensitive (using equalsIgnoreCase) - imagine we'll be shifting to UIDs anyway
		{
            return p;
        }
    }
...
{
	// altered so that we don't send to the same machine more than once. Add the source player to the machines we have "sent" to as it doesn't need to go to that
	// machine either
    ...
    for (unsigned int i = 0; i < players.size(); i++)
	{
		...
		// don't send to the same machine more than once
		bool dontSend = false;
		if( sentTo.size() )
		{
...
void PlayerList::saveAll(ProgressListener *progressListener, bool bDeleteGuestMaps /*= false*/)
{
	...
	// playerIo can be NULL if we have have to exit a game really early on due to network failure
	if(playerIo)
	{
...
void PlayerList::sendLevelInfo(shared_ptr<ServerPlayer> player, ServerLevel *level)
{
	...
	{
		// Fix for #44836 - Customer Encountered: Out of Sync Weather [A-10]
...
void PlayerList::removePlayerFromReceiving(shared_ptr<ServerPlayer> player, bool usePlayerDimension /*= true*/, int dimension /*= 0*/)
{
...
#ifndef _CONTENT_PACKAGE
	app.DebugPrintf("Requesting remove player %ls as primary in dimension %d/n", player->name.c_str(), dimIndex);
#endif
	...
	{
#ifndef _CONTENT_PACKAGE
		app.DebugPrintf("Remove: Removing player %ls as primary in dimension %d/n", player->name.c_str(), dimIndex);
#endif
	...
	if( thisPlayer != NULL && playerRemoved )
	{
		for(AUTO_VAR(itP, players.begin()); itP != players.end(); ++itP)
		{
			...
			{
#ifndef _CONTENT_PACKAGE
				app.DebugPrintf("Remove: Adding player %ls as primary in dimension %d/n", newPlayer->name.c_str(), dimIndex);
#endif
	...
	{
#ifndef _CONTENT_PACKAGE
		app.DebugPrintf("Remove: Qnet player for %ls was NULL so re-checking all players/n", player->name.c_str() );
#endif
		// Something went wrong, or possibly the QNet player left before we got here.
		...
		for(AUTO_VAR(itP, players.begin()); itP != players.end(); ++itP)
		{
			...
			if( checkingPlayer != NULL )
			{
				...
				{
#ifndef _CONTENT_PACKAGE
					app.DebugPrintf("Remove: Adding player %ls as primary in dimension %d/n", newPlayer->name.c_str(), newPlayerDim);
#endif
...
void PlayerList::addPlayerToReceiving(shared_ptr<ServerPlayer> player)
{
...
#ifndef _CONTENT_PACKAGE
	app.DebugPrintf("Requesting add player %ls as primary in dimension %d/n", player->name.c_str(), playerDim);
#endif
	...
	{
#ifndef _CONTENT_PACKAGE
		app.DebugPrintf("Add: Qnet player for player %ls is NULL so not adding them/n", player->name.c_str() );
#endif
	...
	{
#ifndef _CONTENT_PACKAGE
		app.DebugPrintf("Add: Adding player %ls as primary in dimension %d/n", player->name.c_str(), playerDim);
#endif
...
bool PlayerList::isUidBanned(PlayerUID uid)
{
	if( uid == INVALID_UID ) return false;

	bool banned = false;

	for( AUTO_VAR(it, m_bannedUids.begin()); it != m_bannedUids.end(); ++it )
	{
		if( ProfileManager.AreUIDSEqual( uid, *it ) )
		{
			banned = true;
			break;
		}
