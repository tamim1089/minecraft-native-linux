...
//	Collections.sort(levelList);	// TODO - get sort functor etc.
	selectedWorld = -1;
}
...
// Not used, so commenting to stop the build failing
#if 0
...
void SelectWorldScreen::worldSelected(int id)
{
    ...
    if (worldFolderName == L"")	// was NULL comparison
	{
...
void SelectWorldScreen::render(int xm, int ym, float a)
{
	...
	if( count++ >= 100 )
	{
		...
		{
			// For some obscures reason the "delete" button is called "renameButton" and vice versa.
...
{
	parent->renderBackground();	// was SelectWorldScreen.this.renderBackground();
}
...
void SelectWorldScreen::WorldSelectionList::renderItem(int i, int x, int y, int h, Tesselator *t)
{
	...
	wchar_t buffer[20];
	// Currently shows years as 4 digits, where java only showed 2
	swprintf(buffer,20,L"%d/%d/%d %d:%02d",time.wDay, time.wMonth, time.wYear, time.wHour, time.wMinute); // TODO Localise this
    id = id + L" (" + buffer;
