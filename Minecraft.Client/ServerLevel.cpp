...
CThread* ServerLevel::m_updateThread = NULL;
CThread::EventArray* ServerLevel::m_updateTrigger;
...
ServerLevel::ServerLevel(MinecraftServer *server, shared_ptr<LevelStorage>levelStorage, const wstring& levelName, int dimension, LevelSettings *levelSettings) : Level(levelStorage, levelName, levelSettings, Dimension::getNew(dimension), false)
{
	...
	// this this used to be called in parent ctor via a virtual fn
	chunkSource = createChunkSource();
	// optimisation - keep direct reference of underlying cache here
	chunkSourceCache = chunkSource->getCache();
	chunkSourceXZSize = chunkSource->m_XZSize;

	// The listener used to be added in MinecraftServer::loadLevel but we need it to be set up before we do the next couple of things, or else chunks get loaded before we have the entity tracker set up to listen to them
	this->server = server;
	...
	else if ( (dimension==0) && levelData->getSpawnBonusChest() ) // still would like bonus chests to respawn.
	{
		// added isFindingSpawn as we want any chunks we are looking in here for suitable locations for the bonus chest to actually create those chunks rather than just get emptychunks if they aren't loaded
		isFindingSpawn = true;
	...
	// added initialisers
	// Allowing spawn edit for our game, and consider a better solution for the possible griefing
...
void ServerLevel::tick()
{
	...
	if (allPlayersAreSleeping())
	{
		...
		if (!somebodyWokeUp)
		{
			...
			// WESTY : Changed so that time update goes through stats tracking update code.
	...
	// for Minecraft 1.8, spawn friendlies really rarely	- altered from once every 400 ticks to 40 ticks as we depend on this a more than the original since we don't have chunk post-process spawning
	...
	{
		skyDarken = newDark;
		if (!SharedConstants::TEXTURE_LIGHTING)	// change brought forward from 1.8.2
		{
	...
	//temporarily disabling saves as they are causing gameplay to generally stutter quite a lot

	__int64 time = levelData->getTime() + 1;
// Putting this back in, but I have reduced the number of chunks that save when not forced
#ifdef _LARGE_WORLDS
...
#endif
	{
		//app.DebugPrintf("Incremental save/n");
...
// Changes made here to move a section of code (which randomly determines which tiles in the current chunks to tick, and is very cache unfriendly by nature)
...
void ServerLevel::tickTiles()
{
	...
	//printf("Total ticks - Grass: %d, Lava: %d, Other: %d, Total: %d/n", grassTicks, lavaTicks, otherTicks, grassTicks + lavaTicks + otherTicks);
	...
	for (AUTO_VAR(it, chunksToPoll.begin()); it != itEndCtp; it++)
	{
		...
		// don't let this actually load/create any chunks, we'll let the normal updateDirtyChunks etc. processes do that, so it can happen on another thread
		if( !this->hasChunk(cp.x, cp.z) ) continue;

		// When adding a 5th player to the game, the number of chunksToPoll is greater than the size of
		...
		// changes here brought forrward from 1.2.3
...
void ServerLevel::tick(shared_ptr<Entity> e, bool actual)
{
    ...
    if (e->rider.lock() == NULL || (dynamic_pointer_cast<Player>(e->rider.lock())==NULL) )		// was !(e->rider instanceof Player)
	{
        Level::tick(e, actual);
    }
}
...
{
	// This will look like a bug to players, and we really should have a message to explain why we're not allowing lava to be placed at or near a spawn point
	// We'll need to do this in a future update

	// Let's allow water near the spawn point, but not lava
	if(content!=Tile::lava_Id)
	{
		// allow this to be used
		return true;
	}
	else if(dimension->id == 0) // Only limit this in the overworld
	{
...
void ServerLevel::setInitialSpawn(LevelSettings *levelSettings)
{
	...
	{
		app.DebugPrintf("Level::setInitialSpawn - Unable to find spawn biome/n");
	}
	...
	{
		//  changed to stay within our level limits
...
// starting bonus chest
	// added - scan the spawn area first to see if there's already a chest near here
...
#ifdef _LARGE_WORLDS
		// Only do this if there are players in the level
		if(chunkMap->players.size() > 0)
		{
			// This will come in a later change anyway
			// clean cache
...
shared_ptr<Explosion> ServerLevel::explode(shared_ptr<Entity> source, double x, double y, double z, float r, bool fire, bool destroyBlocks)
{
	...
	for(AUTO_VAR(it, players.begin()); it != players.end(); ++it)
	{
        ...
        if (player->distanceToSqr(x, y, z) < 64 * 64)
		{
			...
			//app.DebugPrintf("Sending %s with knockback (%f,%f,%f)/n", knockbackOnly?"knockbackOnly":"allExplosion",knockbackVec->x,knockbackVec->y,knockbackVec->z);
...
void ServerLevel::setTimeAndAdjustTileTicks(__int64 newTime)
{
	...
	// can't directly adjust m_delay in a set as it has a const interator, since changing values in here might change the ordering of the elements in the set.
...
// Sometimes we want to update tiles on the server from the main thread (eg SignTileEntity when string verify returns)
...
// added special versions of addEntity and extra processing on entity removed and added so we can limit the number of itementities created
bool ServerLevel::addEntity(shared_ptr<Entity> e)
{
	...
	{
//		printf("Adding item entity count %d/n",m_itemEntities.size());
		EnterCriticalSection(&m_limiterCS);
		if( m_itemEntities.size() >= MAX_ITEM_ENTITIES )
		{
//			printf("Adding - doing remove/n");
	...
	else if( dynamic_pointer_cast<HangingEntity>(e) != NULL )
	{
		...
		if( m_hangingEntities.size() >= MAX_HANGING_ENTITIES )
		{
			...
			// refuse to add the entity, since we'll be removing one already there, and it may be an item frame with something in it.
			LeaveCriticalSection(&m_limiterCS);
			return FALSE;
	...
	{
//		printf("Adding arrow entity count %d/n",m_arrowEntities.size());
		EnterCriticalSection(&m_limiterCS);
...
//		printf("entity added: item entity count now %d/n",m_itemEntities.size());
		LeaveCriticalSection(&m_limiterCS);
	}
	else if( dynamic_pointer_cast<HangingEntity>(e) != NULL )
	{
...
//		printf("entity added: arrow entity count now %d/n",m_arrowEntities.size());
		LeaveCriticalSection(&m_limiterCS);
	}
	else if( dynamic_pointer_cast<ExperienceOrb>(e) != NULL )
	{
...
//		printf("entity added: experience orb entity count now %d/n",m_arrowEntities.size());
		LeaveCriticalSection(&m_limiterCS);
	}
...
void ServerLevel::entityRemovedExtra(shared_ptr<Entity> e)
{
	...
	{
		EnterCriticalSection(&m_limiterCS);
//		printf("entity removed: item entity count %d/n",m_itemEntities.size());
		AUTO_VAR(it, find(m_itemEntities.begin(),m_itemEntities.end(),e));
		if( it != m_itemEntities.end() )
		{
//			printf("Item to remove found/n");
			m_itemEntities.erase(it);
		}
//		printf("entity removed: item entity count now %d/n",m_itemEntities.size());
		LeaveCriticalSection(&m_limiterCS);
	} 
	else if( dynamic_pointer_cast<HangingEntity>(e) != NULL )
	{
	...
	{
		EnterCriticalSection(&m_limiterCS);
//		printf("entity removed: arrow entity count %d/n",m_arrowEntities.size());
...
//		printf("entity removed: arrow entity count now %d/n",m_arrowEntities.size());
		LeaveCriticalSection(&m_limiterCS);
	} 
	else if( dynamic_pointer_cast<ExperienceOrb>(e) != NULL )
	{
		EnterCriticalSection(&m_limiterCS);
//		printf("entity removed: experience orb entity count %d/n",m_arrowEntities.size());
...
//		printf("entity removed: experience orb entity count now %d/n",m_arrowEntities.size());
		LeaveCriticalSection(&m_limiterCS);
	} 
	else if( dynamic_pointer_cast<PrimedTnt>(e) != NULL )
	{
...
int ServerLevel::runUpdate(void* lpParam)
{
	...
	while(ShutdownManager::ShouldRun(ShutdownManager::eRunUpdateThread))
	{
		...
		// Grass and Lava ticks currently take up the majority of all tile updates, so I am limiting them
		int grassTicks = 0;
		int lavaTicks = 0;
		for( unsigned int iLev = 0; iLev < 3; ++iLev )
		{
			...
			{
				// some of these tile ticks will check things in neighbouring tiles, causing chunks to load/create that aren't yet in memory.
				...
				for (int j = 0; j < 80; j++)
				{
					...
					// Added shouldTileTick as some tiles won't even do anything if they are set to tick and use up one of our updates
