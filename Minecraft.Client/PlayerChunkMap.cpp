...
PlayerChunkMap::PlayerChunk::PlayerChunk(int x, int z, PlayerChunkMap *pcm) : pos(x,z)
{
	...
	// added make sure our lights are up to date as soon as we make it. This is of particular concern for local clients, who have their data
...
// construct an an array of flags that indicate which entities are still waiting to have network packets sent out to say that they have been removed
...
{
	//app.DebugPrintf("--- Adding player to chunk x=%d/tz=%d/n",x, z);
    if (find(players.begin(),players.end(),player) != players.end())
	{
		// At the start of the game, lots of chunks are added, and we can then move into an area that is outside the diameter of our starting area, 
		// but is inside the area loaded at the start.
		app.DebugPrintf("--- Adding player to chunk x=%d/t z=%d, but they are already in there!/n",pos.x, pos.z);
		return;

		//assert(false);
// was			throw new IllegalStateException("Failed to add player. " + player + " already is in chunk " + x + ", " + z);
	}

    player->seenChunks.insert(pos);

	//  the sendPacket check. See PlayerChunkMap::add for the usage
...
void PlayerChunkMap::PlayerChunk::remove(shared_ptr<ServerPlayer> player)
{
	...
	//app.DebugPrintf("--- PlayerChunkMap::PlayerChunk::remove x=%d/tz=%d/n",x,z);
	AUTO_VAR(it, find(players.begin(),players.end(),player));
	if ( it == players.end())
	{
		app.DebugPrintf("--- INFO - Removing player from chunk x=%d/t z=%d, but they are not in that chunk!/n",pos.x, pos.z);
	...
	// I don't think there's any point sending these anymore, as we don't need to unload chunks with fixed sized maps
	// We do need to send these to unload entities in chunks when players are dead. If we do not and the entity is removed
    ...
    if (player->connection != NULL && player->seenChunks.find(pos) != player->seenChunks.end())
	{
		...
		if( thisNetPlayer != NULL )
		{
			...
			{
				//wprintf(L"Sending ChunkVisiblity packet false for chunk (%d,%d) to player %ls/n", x, z, player->name.c_str() );
				player->connection->send( shared_ptr<ChunkVisibilityPacket>( new ChunkVisibilityPacket(pos.x, pos.z, false) ) );
			}
		}
		else
		{
			//app.DebugPrintf("PlayerChunkMap::PlayerChunk::remove - QNetPlayer is NULL/n");
		}
    }
...
void PlayerChunkMap::PlayerChunk::broadcast(shared_ptr<Packet> packet)
{
    ...
    for (unsigned int i = 0; i < players.size(); i++)
	{
		...
		// don't send to a player we've already sent this data to that shares the same machine. TileUpdatePacket,
		...
		//  to get the flag index for the player before we send a packet. This flag is updated when we queue
...
bool PlayerChunkMap::PlayerChunk::broadcastChanges(bool allowRegionUpdate)
{
	...
	{
		// to allow limiting of region update packets created
		if( !prioritised )
		{
	...
	{
		//  we only get here if changes is less than MAX_CHANGES_BEFORE_RESEND (10) we only need to send a byte value in the packet
...
{
	// some changes here so that we only send one region update per tick. The chunks themselves also
	...
	// Added 1.1 but not relevant to us as we never no 0 players anyway, and don't think we should be dropping stuff
	//if (players.isEmpty()) {
...
// added. If a chunk exists, add a player to it straight away. If it doesn't exist,
...
// added. If the chunk and player are in the queue to be added, remove from there. Otherwise
...
// added - actually create & add player to a playerchunk, if there is one queued for this player.
...
//            }
	...
	//  so we send an area packet rather than one visibility packet per chunk
...
void PlayerChunkMap::remove(shared_ptr<ServerPlayer> player)
{
	...
	// added - also remove any queued requests to be added to playerchunks here
...
// have changed this so that we queue requests to add the player to chunks if they
...
void PlayerChunkMap::move(shared_ptr<ServerPlayer> player)
{
	...
	for (int x = xc - radius; x <= xc + radius; x++)
		...
		{ 
			...
			{
				// changed from separate getChunk & add so we can wrap these operations up and queue
				getChunkAndAddPlayer(x, z, player);
			}

			if (!chunkInRange(x - xd, z - zd, xc, zc))
			{
				// changed from separate getChunk & remove so we can wrap these operations up and queue
