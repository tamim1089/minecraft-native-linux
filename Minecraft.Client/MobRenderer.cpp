...
{
	// added - this used to use generics so the input parameter could be a mob (or derived type), but we aren't
...
//	{
        ...
        for (int i = 0; i < MAX_ARMOR_LAYERS; i++)
		{
			...
			if (armorType > 0)
			{
				...
				// added condition here for rendering player as part of the gui. Avoiding rendering the glint here as it involves using its own blending, and for gui rendering
				...
				if( !entityRenderDispatcher->isGuiRender )
				{
					if ((armorType & 0xf) == 0xf)		//MGH - fix for missing enchantment glow
					{
						...
						bindTexture(TN__BLUR__MISC_GLINT); //  "%blur%/misc/glint.png"
						glEnable(GL_BLEND);
						float br = 0.5f;
        ...
        if (((overlayColor >> 24) & 0xff) > 0 || mob->hurtTime > 0 || mob->deathTime > 0)
		{
			...
			// changed these renders to not use the compiled version of their models, because otherwise the render states set
...
//  parameter for color here so that we can colour players names
void MobRenderer::renderNameTag(shared_ptr<Mob> mob, const wstring& OriginalName, double x, double y, double z, int maxDist, int color /*= 0xffffffff*/)
{
	...
	// If it's beyond readable distance, then just render a coloured box
	...
	if (player != NULL && app.isUidDeadmau5( player->getUid() ) ) offs = -10;
