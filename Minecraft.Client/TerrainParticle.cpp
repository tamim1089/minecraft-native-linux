...
{
    this->tile = tile;
    this->setTex(textures, tile->getTexture(0, data));		// change brought forward from 1.8.2 to fix purple particles on door damage
    this->gravity = tile->gravity;
    rCol = gCol = bCol = 0.6f;
    size /= 2;
}

shared_ptr<TerrainParticle> TerrainParticle::init(int x, int y, int z, int data)	// added data parameter
{
    if (tile == Tile::grass) return dynamic_pointer_cast<TerrainParticle>( shared_from_this() );
    int col = tile->getColor(level, x, y, z, data);	// added data parameter
...
void TerrainParticle::render(Tesselator *t, float a, float xa, float ya, float za, float xa2, float za2)
{
	...
	// don't render terrain particles that are less than a metre away, to try and avoid large particles that are causing us problems with 
	// photosensitivity testing
	float distSq = (x*x + y*y + z*z);
	if( distSq < 1.0f ) return;

	float br = SharedConstants::TEXTURE_LIGHTING ? 1.0f : getBrightness(a);		// change brought forward from 1.8.2
