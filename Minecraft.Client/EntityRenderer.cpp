...
void EntityRenderer::renderShadow(shared_ptr<Entity> e, double x, double y, double z, float pow, float a)
{
	...
	// local players seem to have a position at their head, and remote players have a foot position.
...
{
	if( !entityRenderDispatcher->isGuiRender )		// added, don't render shadow in gui as it uses its own blending, and we have globally enabled blending for interface opacity
	{
