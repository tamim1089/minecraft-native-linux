...
int SpiderRenderer::prepareArmor(shared_ptr<Mob> _spider, int layer, float a)
{
    ...
    if (layer!=0) return -1;
	MemSect(31);
    bindTexture(TN_MOB_SPIDER_EYES);	//  L"/mob/spider_eyes.png"
	MemSect(0);
	...
	{
		// was 0xf0f0 but that looks like it is a mistake - maybe meant to be 0xf000f0 to enable both sky & block lighting? choosing 0x00f0 here instead
	...
	// this doesn't seem right - surely there should be an else in here?
    glColor4f(1, 1, 1, br);
    return 1;
}
