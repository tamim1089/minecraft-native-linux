...
bool GameMode::destroyBlock(int x, int y, int z, int face)
{
	...
	//  Let the rendering side of thing know we are about to destroy the tile, so we can synchronise collision with async render data upates.
	...
	// before we remove the tile, recalc the heightmap - setTile depends on this being valid to be able to do
