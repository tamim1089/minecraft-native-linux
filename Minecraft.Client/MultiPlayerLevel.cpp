	...
	: Level(shared_ptr<MockedLevelStorage >(new MockedLevelStorage()), L"MpServer", Dimension::getNew(dimension), levelSettings, false)
{
	...
	// We should never be setting a specific spawn position for a multiplayer, this should only be set by receiving a packet from the server
	...
	// as there are some times when we don't want to add tile updates to the updatesToReset vector
...
void MultiPlayerLevel::tick()
{
	...
	// Copy the connections vector to prevent crash when moving to Nether
	...
	// As elements in the updatesToReset vector are inserted with a fixed initial lifetime, the elements at the front should always be the oldest
	...
	// added this section. Each tick we'll check a different block, and force it to share data if it has been
	...
	// also similar thing tosee if we can compress the lighting in any of these chunks. This is slightly different
	...
	if( ( updateTick % 60 ) == 0 )
	{
		...
		if( totalChunks )
		{
			...
			printf("%d: %d chunks, %d BL (%d + %d), %d SL (%d + %d ) (out of %d) - total %d %% (%dMB mem free)/n",
	...
	if( ( updateTick % 60 ) == 0 )
	{
		...
		if( totalChunks )
		{
			...
			printf("%d: %d chunks, %d data (out of %d) - total %d %% (%dMB mem free)/n",
	...
	if( ( updateTick % 60 ) == 0 )
	{
		...
		printf("*****************************************************************************************************************************************/n");
		printf("TODO: Report upper chunk data as well/n");
		for( int lcs_x = 0; lcs_x < ls; lcs_x++ )
			for( int lcs_z = 0; lcs_z < ls; lcs_z++ )
			{
				if( Level::reallyHasChunk(lcs_x - ( ls / 2), lcs_z - ( ls / 2 ) ) )
		...
		if( totalChunks )
		{
			printf("%d (0) %d (1) %d (2) %d (4) %d (8)/n",total0/totalChunks,total1/totalChunks,total2/totalChunks,total4/totalChunks,total8/totalChunks);
			MEMORYSTATUS memStat;
			GlobalMemoryStatus(&memStat);

			printf("%d: %d chunks, %d KB (out of %dKB) : %d %% (%dMB mem free)/n",
...
{
	chunksToPoll.clear();	// added or else we don't reset this set at all in a multiplayer level... think current java now resets in buildAndPrepareChunksToPoll rather than the calling functions
...
{
	// Add this remove from the reEntries collection to stop us continually removing and re-adding things,
...
//  to remove the entities from the forced list
...
bool MultiPlayerLevel::setDataNoUpdate(int x, int y, int z, int data)
{
    int t = getTile(x, y, z);
    int d = getData(x, y, z);
	// added - if this is the host, then stop sharing block data with the server at this point
	unshareChunkAt(x,z);
...
bool MultiPlayerLevel::doSetTileAndData(int x, int y, int z, int tile, int data)
{
    ...
    if( (Level::setTileAndData(x, y, z, tile, data) || g_NetworkManager.IsHost() ) )
	{
		...
		{
			// This got removed from the tileUpdated function in TU14. Adding it back here as we need it
...
// the game used to tick 1000 tiles in a random region +/- 16 units round the player. We've got a 3x3 region of 8x8x8 chunks round each
...
void MultiPlayerLevel::animateTickDoWork()
{
	...
	for( int i = 0; i < ticksPerChunk; i++ )
	{
		for( AUTO_VAR(it, chunksToAnimate.begin()); it != chunksToAnimate.end(); it++ )
		{
			...
			int t = getTile(x, y, z);
			if (random->nextInt(8) > y && t == 0 && dimension->hasBedrockFog())			// test for bedrock fog brought forward from 1.2.3
			{
...
void MultiPlayerLevel::removeAllPendingEntityRemovals()
{
	...
	// Is there a reason do this in a separate loop? Thats what the Java does...
...
// removes all tile entities in the given region from both level & levelchunks
        ...
        if (te->x >= x0 && te->y >= y0 && te->z >= z0 && te->x < x1 && te->y < y1 && te->z < z1)
		{
			...
			if (lc != NULL)
			{
				...
				if( Tile::tiles[tileId] == NULL || !Tile::tiles[tileId]->isEntityTile())
				{
					...
					// Chests can create new tile entities when being removed, so disable this
