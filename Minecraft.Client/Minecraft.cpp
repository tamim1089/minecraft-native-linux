...
//  this on will change the graph at the bottom of the debug overlay to show the number of packets of each type added per fram
...
{
	// added this block of initialisers
	gameMode = NULL;
	hasCrashed = false;
	...
	this->parent = parent;
	// Our actual physical frame buffer is always 1280x720 ie in a 16:9 ratio. If we want to do a 4:3 mode, we are telling the original minecraft code
	...
	levelTickEventQueue = new CThread::EventQueue(levelTickUpdateFunc, levelTickThreadInitFunc, "LevelTick_EventQueuePoll");
...
void Minecraft::init()
{
	...
	*/
	...
	//    openGLCapabilities = new OpenGLCapabilities();	// removed
	...
	if (connectToIp != L"")	// was NULL comparison
	{
		//        setScreen(new ConnectScreen(this, connectToIp, connectToPort));		// put back in
	}
	else
	{
...
void Minecraft::setScreen(Screen *screen)
{
	...
	//Do not force a stats save here
	...
	else if (player != NULL && !ui.GetMenuDisplayed(player->GetPadIndex()) && player->getHealth() <= 0)
	{
		//screen = new DeathScreen();

		// If we exit from the death screen then we are saved as being dead. In the Java
		...
		{
			ui.NavigateToScene(player->GetPadIndex(),eUIScene_DeathMenu,NULL);
		}
	}
	...
	// if a screen has been set, go into menu mode
	// it's possible that player doesn't exist here yet
	/*if(screen!=NULL)
	{
	if(player && player->GetPadIndex()!=-1)
	{
	InputManager.SetMenuDisplayed(player->GetPadIndex(),true);
	}
	else
	{
	// set all
	...
	if(player && player->GetPadIndex()!=-1)
	{
	InputManager.SetMenuDisplayed(player->GetPadIndex(),false);
	}
	else
	{
...
void Minecraft::destroy()
{
	...
	// all try/catch/finally things in here removed
	//    try {
	...
	//} finally {
	Display::destroy();
	//    if (!hasCrashed) System.exit(0);	//removed
	//}
	//System.gc();	// removed
}
...
void Minecraft::run()
{
	...
	{
		//        try {	// removed try/catch
		//            if (minecraftApplet != null && !minecraftApplet.isActive()) break;	// removed
		AABB::resetPool();
		Vec3::resetPool();

		//            if (parent == NULL && Display.isCloseRequested()) {		// removed
		//                stop();
		//            }
		...
		//        if (!Keyboard::isKeyDown(Keyboard.KEY_F7)) Display.update();		// removed
		...
		*/
		...
		Sleep(0);	// was Thread.yield()

		//        if (Keyboard::isKeyDown(Keyboard::KEY_F7)) Display.update();	// removed condition
		Display::update();

		//        checkScreenshot();	// removed
...
// Selects which local player is currently active for processing by the existing minecraft code
...
void Minecraft::updatePlayerViewportAssignments()
{
	...
	if( viewportsRequired == 1 )
	{
		...
		for( int i = 0; i < XUSER_MAX_COUNT; i++ )
		{
			if( localplayers[i] != NULL ) localplayers[i]->m_iScreenSection = CRender::VIEWPORT_TYPE_FULLSCREEN;
		}
	}
	else if( viewportsRequired == 2 )
	{
		...
		for( int i = 0; i < XUSER_MAX_COUNT; i++ )
		{
			if( localplayers[i] != NULL )
			{
				...
				{
					localplayers[i]->m_iScreenSection = CRender::VIEWPORT_TYPE_SPLIT_LEFT + found;
				}
				else
				{
					localplayers[i]->m_iScreenSection = CRender::VIEWPORT_TYPE_SPLIT_TOP + found;
				}
				found++;
			}
		}
	...
	else if( viewportsRequired >= 3 )
	{
		...
		for( int i = 0; i < XUSER_MAX_COUNT; i++ )
		{
			if( localplayers[i] != NULL )
			{
				...
				// If the game hasn't started, ignore current allocations (as the players won't have seen them)
				// This fixes an issue with the primary player being the 4th controller quadrant, but ending up in the 3rd viewport.
				if(app.GetGameStarted())
				{
					if( ( localplayers[i]->m_iScreenSection >= CRender::VIEWPORT_TYPE_QUADRANT_TOP_LEFT ) &&
						( localplayers[i]->m_iScreenSection <= CRender::VIEWPORT_TYPE_QUADRANT_BOTTOM_RIGHT ) )
					{
						quadrantsAllocated[localplayers[i]->m_iScreenSection - CRender::VIEWPORT_TYPE_QUADRANT_TOP_LEFT] = true;
					}
				}
				else
				{
					// Reset the viewport so that it can be assigned in the next loop
					localplayers[i]->m_iScreenSection = CRender::VIEWPORT_TYPE_FULLSCREEN;
				}
			}
		}
			...
			if( localplayers[i] != NULL )
			{
					...
					( localplayers[i]->m_iScreenSection > CRender::VIEWPORT_TYPE_QUADRANT_BOTTOM_RIGHT ) )
				{
					for( int j = 0; j < 4; j++ )
					{
						...
						{
							localplayers[i]->m_iScreenSection = CRender::VIEWPORT_TYPE_QUADRANT_TOP_LEFT + j;
	...
	// If the game is not running we do not want to do this yet, and should wait until the task
...
bool Minecraft::addLocalPlayer(int idx)
{
	...
	{
		// Should we ever be in a state where this happens?
		assert(false);
...
shared_ptr<MultiplayerLocalPlayer> Minecraft::createExtraLocalPlayer(int idx, const wstring& name, int iPad, int iDimension, ClientConnection *clientConnection /*= NULL*/,MultiPlayerLevel *levelpassedin)
{
	...
	{
		int tempScreenSection = CRender::VIEWPORT_TYPE_FULLSCREEN;
		...
		// can't use the dimension from localplayers[idx], since there may be no localplayers at this point
		...
		// can't do this here because they use a render context, but this is running from a thread.
		...
		PlayerUID playerUIDOffline = INVALID_UID;
		PlayerUID playerUIDOnline = INVALID_UID;
		ProfileManager.GetUID(idx,&playerUIDOffline,false);
		ProfileManager.GetUID(idx,&playerUIDOnline,true);
		localplayers[idx]->setUid(playerUIDOffline);
		localplayers[idx]->setOnlineUid(playerUIDOnline);
		...
		localplayers[idx]->SetPadIndex(iPad);
...
//		// can't call this when this function is called from the qnet thread (GetGameStarted will be false)
//		if(app.GetGameStarted())
//		{
...
void Minecraft::storeExtraLocalPlayer(int idx)
{
	...
	{
		localplayers[idx]->name = convStringToWstring( ProfileManager.GetUsername(idx) );
	}
}
...
#ifdef _LINUX
		// A signout early in the game creation before this player has connected to the game server
		updateXui = false;
#endif
		// Adding this back in for exactly the reason my comment above suggests it was added in the first place
#ifdef _LINUX
		g_NetworkManager.RemoveLocalPlayerByUserIndex(idx);
#endif
	}
...
void Minecraft::createPrimaryLocalPlayer(int iPad)
{
	...
	{
		user->name = convStringToWstring( ProfileManager.GetUsername(ProfileManager.GetPrimaryPad()) );
	}
}
...
void Minecraft::run_middle()
{
	...
	if(running)
	{
		...
		//while (running)
		{
			...
			// AUTOSAVE TIMER - only in the full game and if the player is the host
			...
			// Once we're in the level, check if the players have the level in their banned list and ask if they want to play it
			for( int i = 0; i < XUSER_MAX_COUNT; i++ )
			{
				if( localplayers[i] && (app.GetBanListCheck(i)==false) && !Minecraft::GetInstance()->isTutorial() && ProfileManager.IsSignedInLive(i) && !ProfileManager.IsGuest(i) )
				{
					...
					if(!ProfileManager.IsSystemUIDisplayed())
					{
						...
						// check if the level is in the banned level list
#if defined _LINUX || defined _LINUX
						INetworkPlayer *pHostPlayer = g_NetworkManager.GetHostPlayer();

#ifdef _LINUX
						PlayerUID uid=((NetworkPlayerLinux *)pHostPlayer)->GetUID();
#else
						PlayerUID uid=pHostPlayer->GetUID();
#endif

						if(app.IsInBannedLevelList(i,uid,app.GetUniqueMapName()))
						{
							// put up a message box asking if the player would like to unban this level
							app.DebugPrintf("This level is banned/n");
			...
			// store any button toggles for the players, since the minecraft::tick may not be called if we're running fast, and a button press and release will be missed
			...
			for( int i = 0; i < XUSER_MAX_COUNT; i++ )
			{
				...
				if ( m_pPsPlusUpsell != NULL && m_pPsPlusUpsell->hasResponse() && m_pPsPlusUpsell->m_userIndex == i )
				{
					...
					if ( ProfileManager.HasOnlineSubscription(i) )
					{
						app.DebugPrintf("<Minecraft.cpp> Player_%i is now authorised for PsPlus./n", i);
						if (!ui.PressStartPlaying(i)) ui.ShowPressStart(i);
					}
					else
					{
						UINT uiIDA[1] = { IDS_OK };
						ui.RequestMessageBox( IDS_CANTJOIN_TITLE, IDS_NO_ONLINE_SUBSCRIPTION, uiIDA, 1, i, NULL, NULL, app.GetStringTable() );
					}
				}
				else
#endif
					if(localplayers[i])
				{
					// add these to check for coming out of idle
					...
					// If we're flying, the sneak needs to be held on to go down
				...
				else
				{
					...
					// Check that content restriction information has been received
					...
					if(tryJoin)
					{
						if(!ui.PressStartPlaying(i))
						{
							...
							if (g_NetworkManager.IsLocalGame() || !ProfileManager.RequestingOnlineSubscription(i))
#endif
							{
								ui.ShowPressStart(i);
							}
...
#endif
							{
								...
								if(ProfileManager.IsSignedIn(i))
								{
									...
									if( g_NetworkManager.IsLocalGame() || (ProfileManager.IsSignedInLive(i) && ProfileManager.AllowedToPlayMultiplayer(i) ) )
									{
										...
										else if(!g_NetworkManager.IsLocalGame() && !ProfileManager.HasOnlineSubscription(i))
										{
										...
										else
										{
											...
											if( player == NULL)
											{
												player = createExtraLocalPlayer(i, (convStringToWstring( ProfileManager.GetUsername(i) )).c_str(), i, level->dimension->id);
											}
										}
									}
									else
									{
										if( ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad()) && !ProfileManager.AllowedToPlayMultiplayer(i) )
										{
											...
											// Don't allow converting to guests as we don't allow any guest sign-in while in the game
											//ProfileManager.RequestConvertOfflineToGuestUI( &Minecraft::InGame_SignInReturned, this,i);
				...
				for(unsigned int iPad = XUSER_MAX_COUNT; iPad < (XUSER_MAX_COUNT + InputManager.MAX_GAMEPADS); ++iPad)
				{
					...
					else
					{
						...
						if(InputManager.ButtonPressed(iPad, MINECRAFT_ACTION_PAUSEMENU))
						{
							...
							// If we are joining a pad here, then we don't want to try and join any others
							break;
						}
					}
				}
			...
			for (int i = 0; i < timer->ticks; i++)
			{
				...
				// the tick here can run more than once, and this is a problem for our input, which would see the a key press twice with the same time - let's tick the inputmanager again
				if(i!=0)
				{
					InputManager.Tick();
				...
				{
					// If we are waiting for this connection to do something, then tick it here.
				...
				// added - now do the equivalent of level::animateTick, but taking into account the positions of all our players
...
// 				SparseDataStorage::tick();		// 
			...
			// changing this to be per player
			//if (player != NULL && player->isInWall()) options->thirdPersonView = false;
			...
			if (!noRender)
			{
				...
				for( int i = 0; i < XUSER_MAX_COUNT; i++ )
				{
					if( setLocalPlayerIdx(i) )
					{
						...
						RenderManager.StateSetViewport((CRender::eViewportType)player->m_iScreenSection);
				...
				{
					// render a logo
					RenderManager.StateSetViewport((CRender::eViewportType)(CRender::VIEWPORT_TYPE_QUADRANT_TOP_LEFT + unoccupiedQuadrant));
					glClearColor(0, 0, 0, 0);
					glClear(GL_COLOR_BUFFER_BIT);

					ui.SetEmptyQuadrantLogo(CRender::VIEWPORT_TYPE_QUADRANT_TOP_LEFT + unoccupiedQuadrant);
				}
				setLocalPlayerIdx(iPrimaryPad);
				RenderManager.StateSetViewport(CRender::VIEWPORT_TYPE_FULLSCREEN);
...
{
	// lots of try/catches removed here, and garbage collector things
	levelRenderer->clear();
	AABB::clearPool();
	Vec3::clearPool();
	setLevel(NULL);
...
void Minecraft::pauseGame()
{
	...
	//    setScreen(new PauseScreen());	// TODO put back in
}
...
void Minecraft::resize(int width, int height)
{
	...
	if (screen != NULL)
	{
		...
		int screenWidth = ssc.getWidth();
		int screenHeight = ssc.getHeight();
		//        screen->init(this, screenWidth, screenHeight);	// TODO - put back in
	}
}
...
// added bFirst parameter, which is true for the first active viewport in splitscreen
// added bUpdateTextures, which is true if the actual renderer textures are to be updated - this will be true for the last time this tick runs with bFirst true
void Minecraft::tick(bool bFirst, bool bUpdateTextures)
{
	int iPad=player->GetPadIndex();
	//OutputDebugString("Minecraft::tick/n");

	//only tick this player's stats
	...
	gameRenderer->pick(1);
#if 0
	// removed - we don't use ChunkCache anymore
	if (player != NULL)
	{
		...
		if (dynamic_cast<ChunkCache *>(cs) != NULL)
		{
			...
			// there was also Mth::floors on these ints but that seems superfluous
	...
	*/
	if (screen == NULL && player != NULL )
	{
		...
		{
			//            setScreen(new InBedChatScreen());		// TODO put back in
		}
	}
	...
	if (screen == NULL && !ui.GetMenuDisplayed(iPad) )
	{
		...
		*piAction=-1;
		...
		//  another special case for when the player is sleeping in a bed
		...
		else
		{
			...
			// Moved all this here to avoid having it in 3 different places.
			if (itemInstance)
			{
				// very special case for boat and empty bucket and glass bottle and more
			...
			if (hitResult!=NULL)
			{
				...
				case HitResult::TILE:
					{
						...
						{
							// For the tutorial we want to be able to record what items we look at so that we can give hints
							gameMode->getTutorial()->onLookAt(iTileID,iData);
						}

						// Call the useItemOn with the TestOnly flag set
						...
						case Tile::cake_Id:
							...
							{
								if (player->getFoodData()->needsFood() ) // Changed from healthto hunger.
								{
				...
				case HitResult::ENTITY:
					...
					case eTYPE_MUSHROOMCOW:
						...
						{
							// Fix for #13081 - No tooltip is displayed for hitting a cow when you have nothing in your hand
					...
					case eTYPE_OZELOT:
						{
							...
							if(ocelot->isTame())
							{
								// if you have a raw fish in your hand, you will feed the ocelot rather than have it sit/follow
		...
		if (wheel != 0)
		{
			...
			if( gameMode != NULL && gameMode->getTutorial() != NULL )
			{
				// For the tutorial we want to be able to record what items we are using so that we can give hints
		...
		// This is how we used to handle the USE action. It has now been replaced with the block below which is more like the way the Java game does it,
		...
		*/
		...
		else if( gameMode->isInputAllowed(MINECRAFT_ACTION_USE) )
		{
			if( player->abilities.instabuild )
			{
				// attempt to handle click in special creative mode fashion if possible (used for placing blocks at regular intervals)
		...
		{
#ifndef _LINUX
			// debugoverlay for primary player only
		...
		if((player->ullButtonsPressed&(1LL<<MINECRAFT_ACTION_CRAFTING)) && gameMode->isInputAllowed(MINECRAFT_ACTION_CRAFTING))
		{
			...
			// reordered the if statement so creative mode doesn't bring up the crafting table
			...
			// Microsoft request that we use the 3x3 crafting if someone presses X while at the workbench
		...
		{
			app.DebugPrintf("PAUSE PRESS PROCESSING - ipad = %d, NavigateToScene/n",player->GetPadIndex());
			ui.PlayUISFX(eSFX_Press);
		...
		bool selected = false;
#ifdef _LINUX
		// use the touchscreen for quickselect
		...
		if(pTouchData->reportNum==1)
		{
				...
				(pTouchData->report[0].y>QuickSelectRect[iHudSize].top)&&(pTouchData->report[0].y<QuickSelectRect[iHudSize].bottom))
			{
				...
				selected = true;
				app.DebugPrintf("Touch %d/n",player->inventory->selected);
			}
		}
#endif
...
#if 0
	// TODO - some replacement for input handling...
	...
	if (level != NULL)
	{
		...
		// We are setting the difficulty the same as the server so that leaderboard updates work correctly
		...
		// we want to tick each level once only per frame, and do it when a player that is actually in that level happens to be active.
		...
		for(unsigned int i = 0; i < levels.length; ++i)
		{
			...
			// this doesn't fully tick the animateTick here, but does register this player's position. The actual
			...
			if( levels[i] != NULL )
			{
...
#if defined _LINUX && !defined DISABLE_SPU_CODE
				...
				// We are always online, but still could be paused
		...
		// Keep ticking the connections if paused so that they don't time out
...
{
	//    System.out.println("FORCING RELOAD!");		// removed
	soundEngine = new SoundEngine();
...
// This code was within setLevel, but I moved it out so that I can call it at a better
...
void Minecraft::forceStatsSave(int idx)
{
	...
	//If the player is signed in, save the leaderboards
...
// Removed as redundant with default values in params.
...
void Minecraft::setLevel(MultiPlayerLevel *level, int message /*=-1*/, shared_ptr<Player> forceInsertPlayer /*=NULL*/, bool doForceStatsSave /*=true*/, bool bPrimaryPlayerSignedOut /*=false*/)
{
	...
	// since we now play music in the menu, just let it keep playing
	//soundEngine->playStreaming(L"", 0, 0, 0, 0, 0);

	// stop update thread from processing this level, which blocks until it is safe to move on - will be re-enabled if we set the level to be non-NULL
	gameRenderer->DisableUpdateThread();

	for(unsigned int i = 0; i < levels.length; ++i)
	{
		//  only need to save out in multiplayer is we are setting the level to NULL
		// If we ever go back to making single player only then this will not work properly!
		if (levels[i] != NULL && level == NULL)
		{
			// This is really only relevant for single player (ie not what we do at the moment)
			if((doForceStatsSave==true) && player!=NULL)
				forceStatsSave(player->GetPadIndex() );

			// Added these for the case when we exit a level so we are setting the level to NULL
	...
	//  we are setting the level to NULL then we are exiting, so delete the levels
	if( level == NULL )
	{
		if(levels[0]!=NULL)
		{
	...
	if (level != NULL)
	{
		...
		if (player == NULL)
		{
			...
			PlayerUID playerUIDOffline = INVALID_UID;
			PlayerUID playerUIDOnline = INVALID_UID;
			ProfileManager.GetUID(iPrimaryPlayer,&playerUIDOffline,false);
			ProfileManager.GetUID(iPrimaryPlayer,&playerUIDOnline,true);
#ifdef _LINUX
			if(CGameNetworkManager::usingAdhocMode() && playerUIDOnline.getOnlineID()[0] == 0)
			{
				// player doesn't have an online UID, set it from the player name
				playerUIDOnline.setForAdhoc();
			}
#endif
			player->setUid(playerUIDOffline);
			player->setOnlineUid(playerUIDOnline);
			...
			player->SetPadIndex(iPrimaryPlayer);
		...
		if (levelRenderer != NULL) levelRenderer->setLevel(player->GetPadIndex(), level);
		...
		if (dynamic_cast<ChunkCache *>(cs) != NULL)
		{
			...
			// these had a Mth::floor which seems unrequired
			int xt = ((int) player->x) >> 4;
			int zt = ((int) player->z) >> 4;
		...
		// allow update thread to start processing the level now both it & the player should be ok
		gameRenderer->EnableUpdateThread();
	}
	else
	{
...
{
	gameRenderer->DisableUpdateThread(); // don't do updating whilst we are adjusting the player & localplayer array
	...
	int iTempPad=localPlayer->GetPadIndex();
	...
	PlayerUID playerUIDOffline = INVALID_UID;
	PlayerUID playerUIDOnline = INVALID_UID;
	ProfileManager.GetUID(iTempPad,&playerUIDOffline,false);
	ProfileManager.GetUID(iTempPad,&playerUIDOnline,true);
	player->setUid(playerUIDOffline);
	player->setOnlineUid(playerUIDOnline);
	...
	// are we the primary player or a local player?
	if(iPad==ProfileManager.GetPrimaryPad())
	{
	...
	player->setShowOnMaps(app.GetGameHostOption(eGameHostOption_Usernames)!=0?true:false);
	...
	// added isClientSide check here
	if (!level->isClientSide)
	{
		prepareLevel(IDS_PROGRESS_RESPAWNING);
	}

	//  for multiplayer. At this point we know everything is ready to run again
...
void Minecraft::startAndConnectTo(const wstring& name, const wstring& sid, const wstring& url)
{
	...
	/* removed window handling things here
	...
	*/
	...
	Minecraft *minecraft;
	// was new Minecraft(frame, canvas, NULL, 854, 480, fullScreen);
	...
	*/
	...
	{
		if (userName != L"" && sid != L"")	// username & side were compared with NULL rather than empty strings
		{
	...
	*/
	// TODO - consider whether we need to actually create a thread here
	minecraft->run();
}
...
void Minecraft::main()
{
	...
	// This block generates XML for the game rules schema
	...
	// Can't call this for the first 5 seconds of a game - MS rule
	//if (ProfileManager.IsFullVersion())
	{
		name = L"Player" + _toString<__int64>(System::currentTimeMillis() % 1000);
		sessionId = L"-";
		/* TODO - get a session ID from somewhere?
...
if (button == 0)
{
app.DebugPrintf("handleMouseClick - Player %d is swinging/n",player->GetPadIndex());
player->swing();
}
...
// Adding a special case in here for sleeping in a bed in a multiplayer game - we need to wake up, and we don't have the inbedchatscreen with a button
...
{
mayUse = false;
app.DebugPrintf("Player %d is swinging/n",player->GetPadIndex());
player->swing();
}
if (item == NULL)
{
...
{
	// Fix for bug that was flooding Sentient with LevelStart events
		...
		else if( ProfileManager.IsSignedInLive(iPad) && ProfileManager.AllowedToPlayMultiplayer(iPad) )
		{
			...
			if( player == NULL)
			{
				...
				else
				{
					...
					if( player == NULL)
					{
						player = pClass->createExtraLocalPlayer(iPad, (convStringToWstring( ProfileManager.GetUsername(iPad) )).c_str(), iPad, pClass->level->dimension->id);
					}
				}
			}
		}
...
int Minecraft::InGame_SignInReturned(void *pParam,bool bContinue, int iPad)
{
	...
	{
		// There seems to be a bug in the signin ui call that enables guest sign in. We never allow this within game, so make sure that it's disabled
	...
	if(bContinue==true && g_NetworkManager.IsInSession() && pMinecraftClass->localplayers[iPad] == NULL)
	{
		...
		if(ProfileManager.IsSignedIn(iPad))
		{
			...
			else if( g_NetworkManager.IsLocalGame() || (ProfileManager.IsSignedInLive(iPad) && ProfileManager.AllowedToPlayMultiplayer(iPad)) )
			{
				...
				else if(!g_NetworkManager.IsLocalGame() && !ProfileManager.HasOnlineSubscription(iPad))
				{
				...
				else
				{
					...
					if( player == NULL)
					{
						player = pMinecraftClass->createExtraLocalPlayer(iPad, (convStringToWstring( ProfileManager.GetUsername(iPad) )).c_str(), iPad, pMinecraftClass->level->dimension->id);
					}
				}
			}
...
#if defined _LINUX
int Minecraft::MustSignInReturnedPSN(void *pParam, int iPad, CStorage::EMessageResult result)
{
    Minecraft* pMinecraft = (Minecraft *)pParam;

    if(result == CStorage::EMessage_ResultAccept) 
    {        
		SQRNetworkManager::AttemptSignIn(&Minecraft::InGame_SignInReturned, pMinecraft, false, iPad);
    }
