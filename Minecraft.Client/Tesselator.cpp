...
{
	// this block of things moved to constructor from general initialisations round Java class
	vertices = 0;
	hasColor = false;
	hasTexture = false;
	hasTexture2 = false;
	...
	// adding these things to constructor just to be sure that they are initialised with something
	u = v = 0;
	col = 0;
	mode = 0;
	xo = yo = zo = 0;
	...
	vboMode = USE_VBO;	// && GLContext.getCapabilities().GL_ARB_vertex_buffer_object;
	if (vboMode)
	{
...
void Tesselator::end()
{
//    if (!tesselating) throw new IllegalStateException("Not tesselating!");	// removed
    tesselating = false;
#ifdef _LINUX
...
#endif
	{
		// a lot of stuff taken out here for fiddling round with enable client states etc.
		// that don't matter for our renderer
        if (!hasColor)
		{
			// TEMP put in fixed vertex colors if we don't have any, until we have a shader that can cope without them
        ...
        if (mode == GL_QUADS && TRIANGLE_MODE)
		{
			...
			RenderManager.DrawVertices(D3DPT_TRIANGLELIST,vertices,_array->data,
									   useCompactFormat360?CRender::VERTEX_TYPE_PS3_TS2_CS1:CRender::VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1,
									   useProjectedTexturePixelShader?CRender::PIXEL_SHADER_TYPE_PROJECTION:CRender::PIXEL_SHADER_TYPE_STANDARD);
#else
			RenderManager.DrawVertices(CRender::PRIMITIVE_TYPE_TRIANGLE_LIST,vertices,_array->data,
									   useCompactFormat360?CRender::VERTEX_TYPE_COMPRESSED:CRender::VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1,
									   useProjectedTexturePixelShader?CRender::PIXEL_SHADER_TYPE_PROJECTION:CRender::PIXEL_SHADER_TYPE_STANDARD);
#endif
        }
		else
		{
			...
			{
				vertexCount = p / 2;
				RenderManager.DrawVertices((D3DPRIMITIVETYPE)mode,vertexCount,_array->data,CRender::VERTEX_TYPE_PS3_TS2_CS1, CRender::PIXEL_SHADER_TYPE_STANDARD);
			}
			else
			{
				if( useProjectedTexturePixelShader )
				{
					RenderManager.DrawVertices((D3DPRIMITIVETYPE)mode,vertexCount,_array->data,CRender::VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1_TEXGEN, CRender::PIXEL_SHADER_TYPE_PROJECTION);
				}
				else
				{
					RenderManager.DrawVertices((D3DPRIMITIVETYPE)mode,vertexCount,_array->data,CRender::VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1, CRender::PIXEL_SHADER_TYPE_STANDARD);
				}
			}
#else
			int vertexCount = vertices;
			if( useCompactFormat360 )
			{
				...
				if( vertexCount )
				{
				RenderManager.DrawVertices((CRender::ePrimitiveType)mode,vertexCount,_array->data,CRender::VERTEX_TYPE_COMPRESSED, CRender::PIXEL_SHADER_TYPE_STANDARD);
				}
				if( vertices2 )
				{
					RenderManager.DrawVerticesCutOut((CRender::ePrimitiveType)mode,vertices2,_array2->data,CRender::VERTEX_TYPE_COMPRESSED, CRender::PIXEL_SHADER_TYPE_STANDARD);
				}
#else
			...
			else
			{
				if( useProjectedTexturePixelShader )
				{
					RenderManager.DrawVertices((CRender::ePrimitiveType)mode,vertexCount,_array->data,CRender::VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1_TEXGEN, CRender::PIXEL_SHADER_TYPE_PROJECTION);
				}
				else
				{
					RenderManager.DrawVertices((CRender::ePrimitiveType)mode,vertexCount,_array->data,CRender::VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1, CRender::PIXEL_SHADER_TYPE_STANDARD);
				}
			}
#endif
        }
						  ...
						  )
{
	...
	else
	{
		...
		bounds.addVert(x1+xo, y1+yo, z1+zo);	// added
		...
		bounds.addVert(x2+xo, y2+yo, z2+zo);	// added
		...
		bounds.addVert(x3+xo, y3+yo, z3+zo);	// added
		...
		bounds.addVert(x4+xo, y4+yo, z4+zo);	// added
...
{
	bounds.addVert(x+xo, y+yo, z+zo);	// added
    count++;
	...
	// this format added for 360 to keep memory size of tesselated tiles down -
...
#define X11Y11Z10N_Z_SHIFT 22
	...
	if (x<-1.0f || x>1.0f)	{ printf("Value (%5.3f) should be in range [-1..1].  Conversion will clamp to X11Y11Z10N./n", x); }
	if (y<-1.0f || y>1.0f)	{ printf("Value (%5.3f) should be in range [-1..1].  Conversion will clamp to X11Y11Z10N./n", y); }
	if (z<-1.0f || z>1.0f)	{ printf("Value (%5.3f) should be in range [-1..1].  Conversion will clamp to X11Y11Z10N./n", z); }
#endif
