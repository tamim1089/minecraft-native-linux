...
protected:
	...
	//  overrides to capture events for tutorial
...
public:
	...
	// send the custom skin texture data if there is one
	//void CustomSkin(PBYTE pbData, DWORD dwBytes);

	//  this so we can flag an event for the tutorial
	virtual void ride(shared_ptr<Entity> e);

	// added for the Stop Sleeping
