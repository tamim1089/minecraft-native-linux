...
{	
	...
	// Changed to a quad so that we can use within a command buffer
#if 1
	t->begin();
...
int Font::width(const wstring& str)
{
	...
	if (cleanStr == L"") return 0;	// was NULL comparison
	int len = 0;
...
void Font::drawWordWrapInternal(const wstring& string, int x, int y, int w, int col, bool darken, int h)
{
    ...
    if (lines.size() > 1)
	{
		...
		{
			// Don't draw text that will be partially cutoff/overlap something it shouldn't
...
// Not in use
/*// this code is lifted from #if 0 section above, so that we can directly create what would have gone in each of our 256 + 32 command buffers
