...
{
	//  as we aren't using a templated class
	...
	for (int i = 0; i < 16; i++)
	{
		...
		if (i == 0)
		{
			this->bindTexture(TN_MISC_TUNNEL); //  "/misc/tunnel.png"
			br = 0.1f;
			dist = 65;
			sscale = 1 / 8.0f;
			glEnable(GL_BLEND);
		...
		if (i == 1)
		{
			this->bindTexture(TN_MISC_PARTICLEFIELD); //  "/misc/particlefield.png"
			glEnable(GL_BLEND);
		...
		// note that the glTexGeni/glEnable calls don't actually do anything in our opengl wrapper version, everything is currently just inferred from the glTexGen calls.
		...
		t->useProjectedTexture(true);				// turns on both the generation of texture coordinates in the vertex shader & perspective divide of the texture coord in the pixel shader
		t->begin();
