...
// added to handle incoming connections, to replace thread that original used to have
...
bool ServerConnection::addPendingTextureRequest(const wstring &textureName)
{
	...
	// We want to request this texture from everyone, if we have a duplicate it's most likely because the first person we asked for it didn't have it
...
// 		{
// 			// hide usernames
// 			pMinecraft->options->SetUsernameSetting(true);
// 		}
// 		else
// 		{
// 			pMinecraft->options->SetUsernameSetting(false);
// 		}
// 		for (unsigned int i = 0; i < players.size(); i++)
// 		{
// 			shared_ptr<PlayerConnection> playerconnection = players[i];
// 			playerconnection->setShowOnMaps(pMinecraft->options->GetUsernameSetting());				
// 		}
// 	}
}