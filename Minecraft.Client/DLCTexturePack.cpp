...
DLCTexturePack::DLCTexturePack(DWORD id, DLCPack *pack, TexturePack *fallback) : AbstractTexturePack(id, NULL, pack->getName(), fallback)
{
	...
	// These calls need to be in the most derived version of the class
	loadIcon();
	loadName();
	loadDescription();
...
void DLCTexturePack::loadColourTable()
{
	...
	{
		// We can delete the default colour table, but not the one from the DLCColourTableFile
...
int DLCTexturePack::packMounted(LPVOID pParam,int iPad,DWORD dwErr,DWORD dwLicenceMask)
{
	...
	else
	{
			...
			if(texturePack->m_dlcDataPack != NULL)
			{
				...
				if(xzpPath.exists())
				{
					...
					HANDLE fileHandle = CreateFile(
						...
						0, // share mode // TODO Will we need to share file? Probably not but...
						NULL, // Unused
						OPEN_EXISTING , // how to create // TODO Assuming that the file already exists if we are opening to read from it
						...
						if (!dlcFile->getGrfPath().empty())
						{
							...
							if (grf.exists())
							{
								...
								if( fileHandle != INVALID_HANDLE_VALUE )
								{
									...
									// is it possible that we can get here after a read fail and it's not an error?
		...
		// we need to leave the texture pack mounted if it contained streaming audio
