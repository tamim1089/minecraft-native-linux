...
{
	// dynamic cast required because we aren't using templates/generics in our version
	...
	if (item->getIconType() == Icon::TYPE_TERRAIN && tile != NULL && TileRenderer::canRender(tile->getRenderShape()))
	{
        ...
        bindTexture(TN_TERRAIN);	//  L"/terrain.png"
        float s = 1 / 4.0f;
	...
	else if (item->getItem()->hasMultipleSpriteLayers())
	{
		...
		bindTexture(TN_GUI_ITEMS); //  "/gui/items.png"
	...
	else
	{
		...
		// For rendering the static compass, we give it a non-zero aux value
...
// this used to take x and y as ints, and no scale and alpha - but this interface is now implemented as a wrapper round this more fully featured one
void ItemRenderer::renderGuiItem(Font *font, Textures *textures, shared_ptr<ItemInstance> item, float x, float y, float fScaleX,float fScaleY, float fAlpha, bool useCompiled)
{
	...
	{
		PIXBeginNamedEvent(0,"3D gui item render %d/n",itemId);
		MemSect(31);
...
#endif
		// pass the alpha value in - the grass block render has the top surface coloured differently to the rest of the block
	...
	{
		PIXBeginNamedEvent(0,"Potion gui item render %d/n",itemIcon);
	...
	{
		PIXBeginNamedEvent(0,"2D gui item render %d/n",itemIcon);
        glDisable(GL_LIGHTING);
		MemSect(31);
		if (item->getIconType() == Icon::TYPE_TERRAIN)
		{
...
#ifdef _LINUX
			// make sure we've got linear sampling on minification here as non-mipmapped things like this currently
...
// original interface, now just a wrapper for preceding overload
...
// this used to take x and y as ints, and no scale, alpha or foil - but this interface is now implemented as a wrapper round this more fully featured one
...
// added isConstantBlended and blendFactor parameters. This is true if the gui item is being rendered from a context where it already has blending enabled to do general interface fading
...
void ItemRenderer::renderAndDecorateItem(Font *font, Textures *textures, const shared_ptr<ItemInstance> item, float x, float y,float fScaleX, float fScaleY,float fAlpha, bool isFoil, bool isConstantBlended, bool useCompiled)	
{
	...
	if (isFoil || item->isFoil())
	{
		...
		glDisable(GL_LIGHTING);
		glDepthMask(false);
		textures->bindTexture(TN__BLUR__MISC_GLINT); //  "%blur%/misc/glint.png"
		blitOffset -= 50;
		if( !isConstantBlended )	glEnable(GL_BLEND);
		
		glBlendFunc(GL_DST_COLOR, GL_ONE);		// changed blend equation from GL_DST_COLOR, GL_DST_COLOR so we can fade this out

		float blendFactor = isConstantBlended ? Gui::currentGuiBlendFactor : 1.0f;

		glColor4f(0.5f * blendFactor, 0.25f * blendFactor, 0.8f * blendFactor, 1);		// scale back colourisation with blendFactor
		// scale the x and y by the scale factor
		if((fScaleX!=1.0f) ||(fScaleY!=1.0f))
		{
			// Scales were multiples of 20, making 16 to not overlap in xui scenes
...
// a few changes here to get x, y, w, h in as floats (for xui rendering accuracy), and to align
...
void ItemRenderer::blitGlint(int id, float x, float y, float w, float h)
{
	...
	// calculate what the pixel coordinates will be in final screen coordinates
...
void ItemRenderer::blit(float x, float y, int sx, int sy, float w, float h)
{
	...
	// subtracting 0.5f (actual screen pixels, so need to compensate for physical & game width) from each x & y coordinate to compensate for centre of pixels in directx vs openGL
