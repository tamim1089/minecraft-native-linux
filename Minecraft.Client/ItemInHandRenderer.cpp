...
ItemInHandRenderer::ItemInHandRenderer(Minecraft *mc, bool optimisedMinimap)
{
	...
	// replaced mesh that is used to render held items with individual cubes, so we can make it all join up properly without seams. This
...
{
	// code borrowed from render method below, although not factoring in brightness as that should already be being taken into account
    ...
    if (item->getIconType() == Icon::TYPE_TERRAIN && tile != NULL && TileRenderer::canRender(tile->getRenderShape()))
	{
		...
		MemSect(0);
        tileRenderer->renderTile(tile, item->getAuxValue(), SharedConstants::TEXTURE_LIGHTING ? 1.0f : mob->getBrightness(1));		// change brought forward from 1.8.2
    }
	else
	{
		MemSect(31);
		...
		{
            mc->textures->bindTexture(TN_GUI_ITEMS);	//  L"/gui/items.png"
        }
		MemSect(0);
        Tesselator *t = Tesselator::getInstance();
		...
		// No, I don't know why.
		// Make them the right way round...u coords were swapped
		float u0 = icon->getU0();
		float u1 = icon->getU1();
		float v0 = icon->getV0();
		float v1 = icon->getV1();
        ...
        if (item != NULL && item->isFoil() && layer == 0)
		{
            ...
            mc->textures->bind(mc->textures->loadTexture(TN__BLUR__MISC_GLINT)); //  L"%blur%/misc/glint.png"
            glEnable(GL_BLEND);
...
void ItemInHandRenderer::renderItem3D(Tesselator *t, float u0, float v0, float u1, float v1, int width, int height, float depth, bool isGlint)
{
	...
	//  since we are setting the colour to other values at the start of the function now
...
void ItemInHandRenderer::render(float a)
{
	...
	// added so we can adjust the position of the hands for horizontal & vertical split screens
	float fudgeX = 0.0f;
	float fudgeY = 0.0f;
	float fudgeZ = 0.0f;
	...
	if( localPlayer )
	{
		if( localPlayer->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_BOTTOM ||
			localPlayer->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_TOP )
		{
			fudgeY = 0.08f;
			splitHoriz = true;
		}
		else if( localPlayer->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_LEFT ||
				 localPlayer->m_iScreenSection == CRender::VIEWPORT_TYPE_SPLIT_RIGHT )
		{
			fudgeX = -0.18f;
		}
	}
    ...
    if (localPlayer)
	{
		...
		// was using player->xRot and yRot directly here rather than interpolating between old & current with a
    ...
    if (item != NULL && item->id == Item::map->id)
	{
		...
		// move the map away a bit if we're in horizontal split screen, so it doesn't clip out of the save zone
		if( splitHoriz )
		{
        ...
        {
			// if we've got a player texture, use that
		   ...
		   mc->textures->clearLastBoundId();
            for (int i = 0; i < 2; i++)
			{
				...
				if ((itemInstance && (itemInstance->getItem()->id==Item::map_Id)) || app.GetGameSettings(localPlayer->GetPadIndex(),eGameSetting_DisplayHand)!=0 )
                {
		...
		MemSect(31);
        mc->textures->bindTexture(TN_MISC_MAPBG);	//  L"/misc/mapbg.png"
		MemSect(0);
        Tesselator *t = Tesselator::getInstance();

//        glNormal3f(0, 0, -1);	// changed to use tesselator
        t->begin();
		int vo = 7;
		t->normal(0,0,-1);
...
#else
		static const float swingPowFactor = 4.0f;		// to slow the swing down when nearest the player for avoiding luminance flash issues
#endif
...
void ItemInHandRenderer::renderScreenEffect(float a)
{
	...
	{
		MemSect(31);
        mc->textures->bindTexture(TN_MISC_WATER);	//  L"/misc/water.png"
		MemSect(0);
        renderWater(a);
    }
