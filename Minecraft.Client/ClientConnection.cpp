...
#endif
#ifdef _LINUX
#include "Linux/Network/SonyVoiceChat.h"
#endif
#include "DLCTexturePack.h"

#ifdef _LINUX
#include "../Minecraft.World/PlatformStats.h"
#include "../Minecraft.World/GenericStats.h"
#endif

ClientConnection::ClientConnection(Minecraft *minecraft, const wstring& ip, int port)
{
	// No longer used as we use the socket version below.
	assert(FALSE);
#if 0
	// added initiliasers
	random = new Random();
	...
	{
	    socket = new Socket();	// Local connection
	}
	else
	{
		socket = new Socket(ip);	// Connection over xrnm - hardcoded IP at present
	}
...
ClientConnection::ClientConnection(Minecraft *minecraft, Socket *socket, int iUserIndex /*= -1*/)
{
	...
	{
		connection = NULL;
		// TODO This will cause issues since the session player owns the socket
		//delete socket;
	}
}
...
void ClientConnection::handleLogin(shared_ptr<LoginPacket> packet)
{
	...
	{	
		pPlayerData=app.GetPlayerDataForUid(OnlineUid);
	}
	...
	// load the local player skin (from the global title user storage area) if there is one
	...
	if(iUserID!=-1)
	{
		...
		{
			// a skin?
			if(pPlayerData->wchSkin[0]!=0L)
			{
				wstring wstr=pPlayerData->wchSkin;
				...
				{			
					...
					eTMSStatus=StorageManager.ReadTMSFile(iUserID,CStorage::eGlobalStorage_Title,CStorage::eTMS_FileType_Graphic,pPlayerData->wchSkin,&pBuffer, &dwSize);

					bRes=(eTMSStatus==CStorage::ETMSStatus_Idle);
#endif
				}
			...
			// a cloak?
			if(pPlayerData->wchCape[0]!=0L)
			{		
				wstring wstr=pPlayerData->wchCape;
				// check the file is not already in
				bRes=app.IsFileInMemoryTextures(wstr);
				if(!bRes)
				{		
#ifdef _LINUX
					CStorage::ETMSStatus eTMSStatus;
					eTMSStatus=StorageManager.ReadTMSFile(iUserID,CStorage::eGlobalStorage_Title,CStorage::eTMS_FileType_Graphic,pPlayerData->wchCape,&pBuffer, &dwSize);
					bRes=(eTMSStatus==CStorage::ETMSStatus_Idle);
#endif
				}
	...
	if( m_userIndex == ProfileManager.GetPrimaryPad() )
	{
		...
		if( dimensionLevel == NULL )
		{
			...
			// We want to share the SavedDataStorage between levels
			...
			app.DebugPrintf("ClientConnection - DIFFICULTY --- %d/n",packet->difficulty);
	...
	else
	{
		...
		if(level==NULL)
		{
			...
			// At time of writing ProfileManager.GetUsername() does not always return the correct name,
...
void ClientConnection::handleAddEntity(shared_ptr<AddEntityPacket> packet)
{
	...
	// replacing this massive if nest with switch
	switch(packet->type)
	{
	...
	case AddEntityPacket::FISH_HOOK:
		{
			// Brought forward from 1.4 to be able to drop XP from fishing
			shared_ptr<Entity> owner = getEntity(packet->data);

			// check all local players to find match
			if( owner == NULL )
			{
				for( int i = 0; i < XUSER_MAX_COUNT; i++ )
				{
			...
			if (player != NULL)
			{
				...
				e = hook;
				// Move the player->fishing out of the ctor as we cannot reference 'this'
				player->fishing = hook;
			}
			packet->data = 0;
		}
 	...
 	case AddEntityPacket::ITEM_FRAME:
		{
			...
			int iy=(int) y;
			int iz = (int) z;
		app.DebugPrintf("ClientConnection ITEM_FRAME xyz %d,%d,%d/n",ix,iy,iz);
		}
 ...
 /*   if (packet->type == AddEntityPacket::MINECART_RIDEABLE) e = shared_ptr<Entity>( new Minecart(level, x, y, z, Minecart::RIDEABLE) );
	...
	*/
    ...
    if (e != NULL)
	{
        ...
        if (packet->data > -1) // changed "no data" value to be -1, we can have a valid entity id of 0
		{
...
void ClientConnection::handleAddPlayer(shared_ptr<AddPlayerPacket> packet)
{
	...
	{
 		PlayerUID playerUIDOnline = INVALID_UID, playerUIDOffline = INVALID_UID;
 		ProfileManager.GetUID(idx,&playerUIDOnline,true);
 		ProfileManager.GetUID(idx,&playerUIDOffline,false);
 		if( (playerUIDOnline != INVALID_UID && ProfileManager.AreUIDSEqual(playerUIDOnline,packet->uid) ) ||
			(playerUIDOffline != INVALID_UID && ProfileManager.AreUIDSEqual(playerUIDOffline,packet->uid) ) )
		{
			app.DebugPrintf("AddPlayerPacket received with UID of local player/n");
			return;
		}
	}
...
#ifdef _LINUX
	INetworkPlayer *networkPlayer = g_NetworkManager.GetPlayerByUid(player->getUid());
	if (networkPlayer != NULL) player->displayName = networkPlayer->GetDisplayName();
#else
	player->displayName = player->name;
#endif

	//	printf("/t/t/t/t%d: Add player/n",packet->id,packet->yRot);
	...
	if(!player->customTextureUrl.empty() && player->customTextureUrl.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(player->customTextureUrl))
	{
		...
		{
			app.DebugPrintf("Client sending TextureAndGeometryPacket to get custom skin %ls for player %ls/n",player->customTextureUrl.c_str(), player->name.c_str());
	...
	app.DebugPrintf("Custom skin for player %ls is %ls/n",player->name.c_str(),player->customTextureUrl.c_str());
	...
	if(!player->customTextureUrl2.empty() && player->customTextureUrl2.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(player->customTextureUrl2))
	{
		...
		{
			app.DebugPrintf("Client sending texture packet to get custom cape %ls for player %ls/n",player->customTextureUrl2.c_str(), player->name.c_str());
	...
	app.DebugPrintf("Custom cape for player %ls is %ls/n",player->name.c_str(),player->customTextureUrl2.c_str());
...
void ClientConnection::handleTeleportEntity(shared_ptr<TeleportEntityPacket> packet)
{
    ...
    double z = e->zp / 32.0;
	// make sure xRot stays within -90 -> 90 range
...
//	printf("/t/t/t/t%d: Teleport to %d (lerp to %f)/n",packet->id,packet->yRot,yRot);
...
void ClientConnection::handleMoveEntity(shared_ptr<MoveEntityPacket> packet)
{
    ...
    e->zp += packet->za;
    double x = e->xp / 32.0;
	// The original code did not add the 1/64.0f like the teleport above did, which caused minecarts to fall through the ground
    double y = e->yp / 32.0 + 1 / 64.0f;
    double z = e->zp / 32.0;
	// have changed rotation to be relative here too
	e->yRotp += packet->yRot;
	e->xRotp += packet->xRot;
...
void ClientConnection::handleMovePlayer(shared_ptr<MovePlayerPacket> packet)
{
	...
	{		
        ...
        player->yo = player->y;
        player->zo = player->z;
		// added setting xOld/yOld/zOld here too, as otherwise at the start of the game we interpolate the player position from the origin to wherever its first position really is
...
{
	// changed to encode level in packet
	...
	if( dimensionLevel )
	{
		...
		int xo = packet->xc * 16;
		int zo = packet->zc * 16;
		// Unshare before we make any changes incase the server is already another step ahead of us
		...
		for (int i = 0; i < packet->count; i++)
		{
			...
			{
				PIXBeginNamedEvent(0,"Chunk data unsharing/n");
				dimensionLevel->unshareChunkAt(xo,zo);
				PIXEndNamedEvent();
				forcedUnshare = true;
			}

			// Changes now that lighting is done at the client side of things...
			...
			// remove any tite entities in this region which are associated with a tile that is now no longer a tile entity. Without doing this we end up with stray
		...
		dimensionLevel->shareChunkAt(xo,zo);	// added - only shares if chunks are same on server & client
		PIXEndNamedEvent();
...
{
	// using a block of 255 to signify that this is a packet for destroying a tile, where we need to inform the level renderer that we are about to do so.
	...
	if( dimensionLevel )
	{
		...
		// In creative mode, we don't update the tile locally then get it confirmed by the server - the first point that we know we are about to destroy a tile is here. Let
		...
		PIXBeginNamedEvent(0,"Setting data/n");
		...
		PIXBeginNamedEvent(0,"Sharing data/n");
		dimensionLevel->shareChunkAt(packet->x,packet->z); // added - only shares if chunks are same on server & client
		PIXEndNamedEvent();
...
void ClientConnection::handleTakeItemEntity(shared_ptr<TakeItemEntityPacket> packet)
{
	...
	// the original game could assume that if getEntity didn't find the player, it must be the local player. We
    ...
    if (from != NULL)
	{
		...
		if( isLocalPlayer )
		{
			...
			// Fix for #10213 - UI: Local clients cannot progress through the tutorial normally.
...
void ClientConnection::handleChat(shared_ptr<ChatPacket> packet)
{
	...
	if (packet->m_stringArgs.size() >= 1) playerDisplayName = GetDisplayNameByUsername(packet->m_stringArgs[0]);
	if (packet->m_stringArgs.size() >= 2) sourceDisplayName = GetDisplayNameByUsername(packet->m_stringArgs[1]);
...
{
//	printf("Client: handlePreLogin/n");
#if 1
	// Check that we can play with all the players already in the game who have Friends-Only UGC set
	BOOL canPlay = TRUE;
	...
	if(!g_NetworkManager.IsHost())
	{
		...
		// if we go straight in from the menus via an invite, we won't have the DLC info
	...
	if(!g_NetworkManager.IsHost() && !app.GetGameHostOption(eGameHostOption_FriendsOfFriends))
	{
		if(m_userIndex == ProfileManager.GetPrimaryPad() )
		{
			for(DWORD idx = 0; idx < XUSER_MAX_COUNT; ++idx)
			{
				...
				{
					PlayerUID playerUid = INVALID_UID;
					if( ProfileManager.IsSignedInLive(idx) )
					{
 						ProfileManager.GetUID(idx,&playerUid,true);
					}
					if( playerUid != INVALID_UID )
					{
						// Is this user friends with the host player?			
						BOOL result;
						DWORD error;
						error = XUserAreUsersFriends(idx,&packet->m_playerUids[packet->m_hostIndex],1,&result,NULL);
		...
		else
		{
			...
			else
			{
				...
				{
 					ProfileManager.GetUID(m_userIndex,&playerUid,true);
				}
				if( playerUid != INVALID_UID )
				{
					// Is this user friends with the host player?			
					BOOL result;
					DWORD error;
					error = XUserAreUsersFriends(m_userIndex,&packet->m_playerUids[packet->m_hostIndex],1,&result,NULL);
					if(error == ERROR_SUCCESS && result != TRUE)
					{
						canPlay = FALSE;
						isFriendsWithHost = FALSE;
		...
		for(DWORD i = 0; i < packet->m_dwPlayerCount; ++i)
		{
			...
			for(DWORD idx = 0; idx < XUSER_MAX_COUNT; ++idx)
			{
				if( ProfileManager.IsSignedInLive(idx) )
				{
					...
					if( ProfileManager.AreUIDSEqual(playerUID,packet->m_playerUids[i]) ) localPlayer = true;
				}
			...
			if(!localPlayer)
			{
				...
				if(m_userIndex == ProfileManager.GetPrimaryPad() )
				{
					canPlayLocal = ProfileManager.CanViewPlayerCreatedContent(m_userIndex,false,&packet->m_playerUids[i],1);

					// Everyone joining needs to have at least one friend in the game
					...
					if( isAtLeastOneFriend != TRUE )
					{
						...
						for(DWORD idx = 0; idx < XUSER_MAX_COUNT; ++idx)
						{
							...
							{
								error = XUserAreUsersFriends(idx,&packet->m_playerUids[i],1,&result,NULL);
				...
				else
				{
					...
					canPlayLocal = ProfileManager.CanViewPlayerCreatedContent(m_userIndex,true,&packet->m_playerUids[i],1);
				}
		...
		if(m_userIndex == ProfileManager.GetPrimaryPad() )
		{
			...
			{			
				...
				for( unsigned int i = 0; i < friendCount; i++ )
				{
					...
					if( ret == 0 )
					{
						if(strcmp(npid.handle.data, packet->m_playerUids[packet->m_hostIndex].getOnlineID()) == 0)
						{
							isFriend = true;
							break;
						}
	...
	if(!g_NetworkManager.IsHost() && m_userIndex == ProfileManager.GetPrimaryPad())
	{
		...
		auto friendsUids = DQRNetworkManager::GetFriends();
		...
		if (app.GetGameHostOption(eGameHostOption_FriendsOfFriends))
		{
			...
			{
				auto friendsUid = friendsUids->GetAt(i);
				...
				{
					Platform::String^ userId = ref new Platform::String(g_NetworkManager.GetPlayerByIndex(j)->GetUID().toString().data());
					if (friendsUid == userId)
					{
						isAtLeastOneFriend = true;
						break;
					}
			...
			app.DebugPrintf("ClientConnection::handlePreLogin: User has at least one friend? %s/n", isAtLeastOneFriend ? "Yes" : "No");
		}
		else
		{
			...
			Platform::String^ hostUserId = ref new Platform::String(g_NetworkManager.GetHostPlayer()->GetUID().toString().data());

			for (int i = 0; i < friendsUids->Size; i++)
			{
				if (friendsUids->GetAt(i) == hostUserId)
				{
					isFriend = true;
					break;
				}
			...
			app.DebugPrintf("ClientConnection::handlePreLogin: User is friends with the host? %s/n", isFriendsWithHost ? "Yes" : "No");
		}

		app.DebugPrintf("ClientConnection::handlePreLogin: Friendship checks took %i ms/n", System::currentTimeMillis() - startTime);
	}
#endif
	...
	if(!canPlay || !canPlayLocal || !isAtLeastOneFriend || cantPlayContentRestricted)
	{
		...
		if(m_userIndex == ProfileManager.GetPrimaryPad())			
		{
			...
			app.DebugPrintf("Exiting world on handling Pre-Login packet due UGC privileges: %d/n", reason);
		...
		{						
			...
			app.DebugPrintf("Exiting player %d on handling Pre-Login packet due UGC privileges: %d/n", m_userIndex, reason);
			UINT uiIDA[1];
			uiIDA[0]=IDS_CONFIRM_OK;
			...
			// this locks up on the read and write threads not closing down, because they are trying to lock the incoming critsec when it's already locked by this thread			
...
//			app.SetAction(m_userIndex,eAppAction_ExitPlayer);
	...
	else
	{
		...
		if(m_userIndex == ProfileManager.GetPrimaryPad())
		{
			...
			{
				app.DebugPrintf("Selected texture pack %d from Pre-Login packet/n", packet->m_texturePackId);
			}
			else
			{
				app.DebugPrintf("Could not select texture pack %d from Pre-Login packet, requesting from host/n", packet->m_texturePackId);

				// we need to upsell the texture pack to the player
		...
		if(!g_NetworkManager.IsHost() )
		{
 		...
 		PlayerUID offlineUID = INVALID_UID;
		PlayerUID onlineUID = INVALID_UID;
		if( ProfileManager.IsSignedInLive(m_userIndex) )
		{
 			ProfileManager.GetUID(m_userIndex,&onlineUID,true);
		}
#ifdef _LINUX
		if(CGameNetworkManager::usingAdhocMode() && onlineUID.getOnlineID()[0] == 0)
		{
			// player doesn't have an online UID, set it from the player name
			onlineUID.setForAdhoc();
		}
#endif
...
#endif
		{
 			ProfileManager.GetUID(m_userIndex,&offlineUID,false);
		}
		BOOL allAllowed, friendsAllowed;
		ProfileManager.AllowedPlayerCreatedContent(m_userIndex,true,&allAllowed,&friendsAllowed);
		send( shared_ptr<LoginPacket>( new LoginPacket(minecraft->user->name, SharedConstants::NETWORK_PROTOCOL_VERSION, offlineUID, onlineUID, (allAllowed!=TRUE && friendsAllowed==TRUE), 
...
//	printf("/t/t/t/t%d: Add mob rot %d/n",packet->id,packet->yRot);
	...
	// Slimes have a different BB depending on their size which is set in the entity data, so update the BB
...
void ClientConnection::handleTexture(shared_ptr<TexturePacket> packet)
{
	...
	if(packet->dwBytes==0)
	{
		// Request for texture
			...
			wprintf(L"Client received request for custom texture %ls/n",packet->textureName.c_str());
#endif
		PBYTE pbData=NULL;
		DWORD dwBytes=0;		
...
#ifndef _CONTENT_PACKAGE
		wprintf(L"Client received custom texture %ls/n",packet->textureName.c_str());
#endif
...
void ClientConnection::handleTextureAndGeometry(shared_ptr<TextureAndGeometryPacket> packet)
{
	...
	if(packet->dwTextureBytes==0)
	{
		// Request for texture
#ifndef _CONTENT_PACKAGE
		wprintf(L"Client received request for custom texture and geometry %ls/n",packet->textureName.c_str());
#endif
		PBYTE pbData=NULL;
		DWORD dwBytes=0;		
		app.GetMemFileDetails(packet->textureName,&pbData,&dwBytes);
	...
	else
	{
		// Response with texture data
#ifndef _CONTENT_PACKAGE
		wprintf(L"Client received custom TextureAndGeometry %ls/n",packet->textureName.c_str());
#endif
...
void ClientConnection::handleTextureChange(shared_ptr<TextureChangePacket> packet)
{
...
#ifndef _CONTENT_PACKAGE
	wprintf(L"Skin for remote player %ls has changed to %ls (%d)/n", player->name.c_str(), player->customTextureUrl.c_str(), player->getPlayerDefaultSkin() );
#endif
		break;
...
#ifndef _CONTENT_PACKAGE
	wprintf(L"Cape for remote player %ls has changed to %ls/n", player->name.c_str(), player->customTextureUrl2.c_str() );
#endif
		break;
	}
	...
	if(!packet->path.empty() && packet->path.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(packet->path))
	{
		...
		{
#ifndef _CONTENT_PACKAGE
			wprintf(L"handleTextureChange - Client sending texture packet to get custom skin %ls for player %ls/n",packet->path.c_str(), player->name.c_str());
#endif
...
void ClientConnection::handleTextureAndGeometryChange(shared_ptr<TextureAndGeometryChangePacket> packet)
{
	...
	if(!packet->path.empty() && packet->path.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(packet->path))
	{
		...
		{
#ifndef _CONTENT_PACKAGE
			wprintf(L"handleTextureAndGeometryChange - Client sending TextureAndGeometryPacket to get custom skin %ls for player %ls/n",packet->path.c_str(), player->name.c_str());
#endif
...
void ClientConnection::handleRespawn(shared_ptr<RespawnPacket> packet)
{
	...
	if( packet->dimension != minecraft->localplayers[m_userIndex]->dimension || packet->mapSeed != minecraft->localplayers[m_userIndex]->level->getSeed() )
	{
		...
		if( dimensionLevel == NULL )
		{
			...
			// We want to shared the savedDataStorage between both levels
			...
			dimensionLevel->difficulty = packet->difficulty; // 
			app.DebugPrintf("dimensionLevel->difficulty - Difficulty = %d/n",packet->difficulty);
...
//		minecraft->addPendingLocalConnection(m_userIndex, this);
		...
		// Fix for #13543 - Crash: Game crashes if entering a portal with the inventory menu open
		ui.CloseUIScenes( m_userIndex );
    ...
    //minecraft->respawnPlayer(minecraft->player->GetPadIndex(),true, packet->dimension);
    ...
    minecraft->respawnPlayer(minecraft->localplayers[m_userIndex]->GetPadIndex(),packet->dimension,packet->m_newEntityId);
...
void ClientConnection::handleExplosion(shared_ptr<ExplodePacket> packet)
{
	...
	{
		//app.DebugPrintf("Received ExplodePacket with explosion data/n");
    ...
    Explosion *e = new Explosion(minecraft->level, nullptr, packet->x, packet->y, packet->z, packet->r);
		...
		// now directly pass a pointer to the toBlow array in the packet rather than copying around
	...
	{
		//app.DebugPrintf("Received ExplodePacket with knockback only data/n");
	}

	//app.DebugPrintf("Adding knockback (%f,%f,%f) for player %d/n", packet->getKnockbackX(), packet->getKnockbackY(), packet->getKnockbackZ(), m_userIndex);
...
void ClientConnection::handleContainerSetSlot(shared_ptr<ContainerSetSlotPacket> packet)
{
	...
	else
	{
		...
		{
			// Reworked a bit to fix a bug where things being collected while the creative menu was up replaced items in the creative menu
...
void ClientConnection::handleSignUpdate(shared_ptr<SignUpdatePacket> packet)
{
    ...
    if (minecraft->level->hasChunkAt(packet->x, packet->y, packet->z))
	{
		...
		// on a client connecting, the line below fails
        if (dynamic_pointer_cast<SignTileEntity>(te) != NULL)
		{
			...
			app.DebugPrintf("verified = %d/tCensored = %d/n",packet->m_bVerified,packet->m_bCensored);
		...
		{
			app.DebugPrintf("dynamic_pointer_cast<SignTileEntity>(te) == NULL/n");
		}
    }
	else
	{
		app.DebugPrintf("hasChunkAt failed/n");
	}
}
...
void ClientConnection::handleSetEquippedItem(shared_ptr<SetEquippedItemPacket> packet)
{
    ...
    if (entity != NULL)
	{
		// Brought forward change from 1.3 to fix #64688 - Customer Encountered: TU7: Content: Art: Aura of enchanted item is not displayed for other players in online game
...
{
	PIXBeginNamedEvent(0,"Handle tile event/n");
...
void ClientConnection::handleGameEvent(shared_ptr<GameEventPacket> gameEventPacket)
{
	...
	{
        if (GameEventPacket::EVENT_LANGUAGE_ID[event] > 0)	// was NULL check
		{
...
#else
		app.DebugPrintf("handleGameEvent packet for WIN_GAME - %d/n", m_userIndex);
		// This just allows it to be shown
		if(minecraft->localgameModes[ProfileManager.GetPrimaryPad()] != NULL) minecraft->localgameModes[ProfileManager.GetPrimaryPad()]->getTutorial()->showTutorialPopup(false);
...
void ClientConnection::handlePlayerInfo(shared_ptr<PlayerInfoPacket> packet)
{
	...
	// Repurposed this packet for player info that we want
	...
	// I don't think we care about this, so not converting it (came from 1.8.2)
#if 0
...
{
	int userIndex = player->GetPadIndex();
...
void ClientConnection::handleServerSettingsChanged(shared_ptr<ServerSettingsChangedPacket> packet)
{
	...
	{
		//options
		//minecraft->options->SetUsernameSetting((packet->data==0)?false:true);
		app.SetGameHostOption(eGameHostOption_Usernames, packet->data);
	}
...
void ClientConnection::handleUpdateGameRuleProgressPacket(shared_ptr<UpdateGameRuleProgressPacket> packet)
{
	...
	{
		app.DebugPrintf("handleUpdateGameRuleProgressPacket: Data tag is in range, so updating profile data/n");
...
// TU-1 hotfix
// Fix for #13191 - The host of a game can get a message informing them that the connection to the server has been lost
int ClientConnection::HostDisconnectReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
	// if they have a trial texture pack, they don't get to save the world
...
int ClientConnection::ExitGameAndSaveReturned(void *pParam,int iPad,CStorage::EMessageResult result)
{
...
wstring ClientConnection::GetDisplayNameByUsername(wstring username)
{
#ifdef _LINUX
	wstring displayName = g_NetworkManager.GetDisplayNameByUsername(username);
	return displayName;
#else
	return username;
#endif
