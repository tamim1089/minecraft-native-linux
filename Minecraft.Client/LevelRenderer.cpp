...
#ifdef _LINUX
#include "Linux/SPU_Tasks/LevelRenderer_cull/LevelRenderer_cull.h"
#include "Linux/SPU_Tasks/LevelRenderer_FindNearestChunk/LevelRenderer_FindNearestChunk.h"
#include "CSpursJob.h"
...
CThread *LevelRenderer::rebuildThreads[MAX_CHUNK_REBUILD_THREADS];
CThread::EventArray *LevelRenderer::s_rebuildCompleteEvents;
CThread::Event *LevelRenderer::s_activationEventA[MAX_CHUNK_REBUILD_THREADS];
...
const int netherSize = HELL_LEVEL_MAX_WIDTH + 2; // The plus 2 is really just to make our total chunk count a multiple of 8 for the flags, we will never see these in the nether
...
LevelRenderer::LevelRenderer(Minecraft *mc, Textures *textures)
{
	...
	for( int i = 0; i < 4; i++ )
	{
		//		sortedChunks[i] = NULL;	// removed - not sorting our chunks anymore
	...
	// create geometry for rendering clouds
	createCloudMesh();
	...
	glDepthMask(false);				// added to get depth mask disabled within the command buffer
	float yy;
	int s = 64;
	int d = (256 / s) + 2;
	yy = (float) (16);
...
#ifdef _LINUX
	m_jobPort_CullSPU = new CSpursJobQueue::Port("CSpursJob_LevelRenderer_cull");
	m_jobPort_FindNearestChunk = new CSpursJobQueue::Port("CSpursJob_LevelRenderer_FindNearestChunk");
#endif // _LINUX
}
...
void LevelRenderer::renderStars()
{
	...
	for (int i = 0; i < 1500; i++)
	{
		...
		if (d < 1 && d > 0.01)
		{
			...
			y *= d;
			z *= d;
			double xp = x * 160;	// moved further away (were 100) as they were cutting through far chunks
			double yp = y * 160;
			double zp = z * 160;
...
void LevelRenderer::setLevel(int playerIndex, MultiPlayerLevel *level)
{
	...
	{
		//		printf("NULLing player %d, chunks @ 0x%x/n",playerIndex,chunks[playerIndex]);
		if( chunks[playerIndex].data != NULL )
		{
			...
			//			delete sortedChunks[playerIndex];	// removed - not sorting our chunks anymore
			//			sortedChunks[playerIndex] = NULL;	// removed - not sorting our chunks anymore
		}

		// If we do this for splitscreen players leaving, then all the tile entities in the world dissappear
...
{
	int playerIndex = mc->player->GetPadIndex();	// 
	allChanged(playerIndex);
}
...
{
	// This was required by the threaded Minecraft::tick(). If we need to add it back then:
	...
	//	sortedChunks[playerIndex] = new vector<Chunk *>(xChunks * yChunks * zChunks);		// removed - not sorting our chunks anymore
	int id = 0;
	int count = 0;
	...
	// we now only fully clear this on exiting the game (setting level to NULL). Apart from that, the chunk rebuilding is responsible for maintaining this
	...
	for (int x = 0; x < xChunks; x++)
	{
		for (int y = 0; y < yChunks; y++)
		{
			for (int z = 0; z < zChunks; z++)
			{
				...
				//				sortedChunks[playerIndex]->at((z * yChunks + y) * xChunks + x) = chunks[playerIndex]->at((z * yChunks + y) * xChunks + x);	// removed - not sorting our chunks anymore
	...
	if (level != NULL)
	{
		...
		if (player != NULL)
		{
			...
			//			sort(sortedChunks[playerIndex]->begin(),sortedChunks[playerIndex]->end(), DistanceChunkSorter(player));	// removed - not sorting our chunks anymore
		}
	}
...
void LevelRenderer::renderEntities(Vec3 *cam, Culler *culler, float a)
{
	...
	// Set these up every time, even when not rendering as other things (like particle render) may depend on it for those frames.
	...
	mc->gameRenderer->turnOnLightLayer(a);		// brought forward from 1.8.2
	...
	Lighting::turnOn();
	// have restructed this so that the tile entities are stored within a hashmap by chunk/dimension index. The index
	...
	mc->gameRenderer->turnOffLightLayer(a);		// brought forward from 1.8.2
}
...
int LevelRenderer::render(shared_ptr<Mob> player, int layer, double alpha, bool updateChunks)
{
	...
	// added - if the number of players has changed, we need to rebuild things for the new draw distance this will require
...
int LevelRenderer::renderChunks(int from, int to, int layer, double alpha)
{
...
#if 1
	// cut down version, we're not using offsetted render lists, or a sorted chunk list, anymore
	mc->gameRenderer->turnOnLightLayer(alpha);		// brought forward from 1.8.2
	...
	glPopMatrix();
	mc->gameRenderer->turnOffLightLayer(alpha);		// brought forward from 1.8.2
...
void LevelRenderer::renderSky(float alpha)
{
	if (mc->level->dimension->id == 1)
	{
		...
		glDepthMask(false);
		textures->bind(textures->loadTexture(TN_MISC_TUNNEL)); //  L"/1_2_2/misc/tunnel.png"
		Tesselator *t = Tesselator::getInstance();
	...
	glPushMatrix();
	{
		...
		MemSect(31);
		textures->bindTexture(TN_TERRAIN_SUN);	//  L"/terrain/sun.png"
		MemSect(0);
		t->begin();
		...
		ss = 20;
		textures->bindTexture(TN_TERRAIN_MOON_PHASES); //  L"/1_2_2/terrain/moon_phases.png"
	...
	double yy = mc->player->getPos(alpha)->y - level[playerIndex]->getHorizonHeight();	// getHorizonHeight moved forward from 1.2.3
	if (yy < 0)
	{
		glPushMatrix();
		...
		// can't work out what this big black box is for. Taking it out until someone misses it... it causes a big black box to visible appear in 3rd person mode whilst under the ground.
#if 0        
		float ss = 1;
...
void LevelRenderer::renderHaloRing(float alpha)
{
	...
	//app.DebugPrintf("Luminance = %f, brightness = %f/n", Y, br);
	glColor3f(br,br,br);
	...
	glDepthMask(false);
	textures->bindTexture(L"misc/haloRing.png"); //  L"/1_2_2/misc/tunnel.png"
	Tesselator *t = Tesselator::getInstance();
...
void LevelRenderer::renderClouds(float alpha)
{
	...
	textures->bindTexture(TN_ENVIRONMENT_CLOUDS);	//  L"/environment/clouds.png"
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
// new geometry for clouds. This is a full array of cubes, one per texel - the original is an array of intersecting fins which aren't ever going to render perfectly.
...
void LevelRenderer::renderAdvancedClouds(float alpha)
{
	...
	// most of our viewports are now rendered with no clip planes but using stencilling to limit the area drawn to. Clouds have a relatively large fill area compared to
	...
	// we are now conditionally rendering the clouds in two ways
	...
	int radius = 3;
	if( activePlayers() > 2 ) radius = 2;	// reduce the cloud render distance a bit for 3 & 4 player split screen
	float e = 1 / 1024.0f;
	glScalef(ss, 1, ss);
	...
	for (int pass = 0; pass < 2; pass++)
	{
		if (pass == 0)
		{
			// changed to use blend rather than color mask to avoid writing to frame buffer, to work with our command buffers
		...
		for (int xPos = -radius + 1; xPos <= radius; xPos++)
		{
			...
			{
				// reimplemented the clouds with full cube-per-texel geometry to get rid of seams. This is a huge amount more quads to render, so
...
bool LevelRenderer::updateDirtyChunks()
{
	...
	if( ( throttle % 100 ) == 0 )
	{
		app.DebugPrintf("CBuffSize: %d/n",memAlloc/(1024*1024));
	}
	throttle++;
	*/
	...
	if( dirtyChunkPresent )
	{
		...
		PIXBeginNamedEvent(0,"Finding nearest chunk/n");
#if defined _LINUX && !defined DISABLE_SPU_CODE
		...
		CSpursJob_LevelRenderer_FindNearestChunk findJob(&g_findNearestChunkDataIn);
...
//			app.DebugPrintf("[%d,%d,%d]/n",nearestClipChunks.empty(),considered,wouldBeNearButEmpty);
		}
#endif // _LINUX
		PIXEndNamedEvent();
	}
	...
	if(!nearestClipChunks.empty())
	{
		...
		for(; index >=0; --index)
		{
			...
			if( bAtomic || (index == 0) ) 
			{
				...
				//app.DebugPrintf("Rebuilding permaChunk %d/n", index);
				...
				//		countTime++;
				//		printf("%d : %f/n", countTime, (float)totalTime / (float)countTime);
				//PIXEndNamedEvent();
			}
			else
			{
...
void LevelRenderer::renderDestroyAnimation(Tesselator *t, shared_ptr<Player> player, float a)
{
	...
	if (!destroyingBlocks.empty())
	{
		...
		while (it != destroyingBlocks.end())
		{
			...
			if (xd * xd + yd * yd + zd * zd < 32 * 32) // now only culling instead of removing, as the list is shared in split screen
			{
				int iPad = mc->player->GetPadIndex();	// 
				...
				tileRenderer[iPad]->tesselateInWorldFixedTexture(tile, block->getX(), block->getY(), block->getZ(), breakingTextures[block->getProgress()]);	//  to differentiate from tesselateInWorld
			}
			++it;
		}
...
void LevelRenderer::renderHitOutline(shared_ptr<Player> player, HitResult *h, int mode, shared_ptr<ItemInstance> inventoryItem, float a)
{
	...
	if (mode == 0 && h->type == HitResult::TILE)
	{
		...
		// If Display HUD is false, don't render the hit outline
...
void LevelRenderer::setDirty(int x0, int y0, int z0, int x1, int y1, int z1, Level *level)	// added level param
{
	// level is passed if this is coming from setTilesDirty, which could come from when connection is being ticked outside of normal level tick, and player won't
	// be set up
	if( level == NULL ) level = this->level[mc->player->GetPadIndex()];		
	//	EnterCriticalSection(&m_csDirtyChunks);
	...
	for (int x = _x0; x <= _x1; x++)
	{
		for (int y = _y0; y <= _y1; y++)
		{
			...
			{
				//				printf("Setting %d %d %d dirty/n",x,y,z);
...
void LevelRenderer::setTilesDirty(int x0, int y0, int z0, int x1, int y1, int z1, Level *level)	// added level param
{
...
void LevelRenderer::cull_SPU(int playerIndex, Culler *culler, float a)
{
	...
	CSpursJob_LevelRenderer_cull cullJob(&g_cullDataIn[playerIndex]);
	CSpursJob_LevelRenderer_zSort sortJob(&g_cullDataIn[playerIndex]);
...
void LevelRenderer::playSound(int iSound, double x, double y, double z, float volume, float pitch, float fSoundClipDist)
{
	...
	// find min distance to any players rather than just the current one
...
// original function. I've changed to an enum instead of string compares
// /*
...
shared_ptr<Particle> LevelRenderer::addParticleInternal(ePARTICLE_TYPE eParticleType, double x, double y, double z, double xa, double ya, double za)
{
	...
	// the java code doesn't distance cull these two particle types, we need to implement this behaviour differently as our distance check is
	...
	// this is a bit of hack to get communication through from the level itself, but if Minecraft::animateTickLevel is NULL then
	...
	if( mc->animateTickLevel == NULL )
	{
		...
		// Changed this as we need to check all local players in case one of them is in range of this particle
	...
	case eParticleType_smoke:
		...
		break;
	case eParticleType_endportal: // Added.
		{
...
void LevelRenderer::entityAdded(shared_ptr<Entity> entity)
{
	...
	// these empty string comparisons used to check for NULL references, but we don't have string pointers (currently) in entities,
	...
	// adding these from global title storage
...
void LevelRenderer::levelEvent(shared_ptr<Player> source, int type, int x, int y, int z, int data)
{
	...
	case LevelEvent::SOUND_PLAY_RECORDING:
		{
			...
			{
				// only play streaming music if there isn't already some playing - the CD playing may have finished, and game music started playing already
				if(!mc->soundEngine->GetIsPlayingStreamingGameMusic())
				{
					level[playerIndex]->playStreamingMusic(L"", x, y, z);	// used to pass NULL, but using empty string here now instead
				}
			}
			mc->localplayers[playerIndex]->updateRichPresence();
		}
		break;
		// new level event sounds brought forward from 1.2.3
	...
	case LevelEvent::SOUND_ZOMBIE_CONVERTED:
		...
		break;
		//  TU9 to fix #77475 - TU9: Content: Art: Dragon egg teleport particle effect isn't present.
	case LevelEvent::END_EGG_TELEPORT:
		//  to show the paricles when the End egg teleports after being attacked
...
// added for new render list handling. Render lists used to be allocated per chunk, but these are now allocated per fixed chunk position
...
{
	s_rebuildCompleteEvents = new CThread::EventArray(MAX_CHUNK_REBUILD_THREADS);
	char threadName[256];
	for(unsigned int i = 0; i < MAX_CHUNK_REBUILD_THREADS; ++i)
	{
		sprintf(threadName,"Rebuild Chunk Thread %d/n",i);
		rebuildThreads[i] = new CThread(rebuildChunkThreadProc,(void *)i,threadName);

		s_activationEventA[i] = new CThread::Event();
...
int LevelRenderer::rebuildChunkThreadProc(LPVOID lpParam)
{
	...
	while(true)
	{
		...
		//app.DebugPrintf("Rebuilding permaChunk %d/n", index + 1);
