...
ModelPart * HumanoidModel::AddOrRetrievePart(SKIN_BOX *pBox)
{
	...
	if(pNewBox)
	{
		...
		{
			app.DebugPrintf("HumanoidModel::AddOrRetrievePart - Box geometry was found, but with different uvs/n");
			pNewBox=NULL;
		}
	}
	if(pNewBox==NULL)
	{
		//app.DebugPrintf("HumanoidModel::AddOrRetrievePart - Adding box to model part/n");
		...
		// don't compile here, since the lighting isn't set up. It'll be compiled on first use.
...
void HumanoidModel::setupAnim(float time, float r, float bob, float yRot, float xRot, float scale, unsigned int uiBitmaskOverrideAnim)
{
	...
	
	{
		...
		// Weeping Angel - does't look good holding something in the arm that's up
		...
		if (attackTime > -9990.0f)
		{
			...
			arm0->xRot -= aa * 1.2f + bb;	// changed 1.2 -> 1.2f
