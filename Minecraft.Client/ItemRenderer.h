...
{
private:
//	TileRenderer *tileRenderer;	// removed - this is shadowing the tilerenderer from entityrenderer
    Random *random;
	bool m_bItemFrame;
public:
	bool setColor;
...
public:
	...
	void renderGuiItem(Font *font, Textures *textures, shared_ptr<ItemInstance> item, float x, float y, float fScaleX,float fScaleY, float fAlpha, bool useCompiled); //  useCompiled
	...
	void renderAndDecorateItem(Font *font, Textures *textures, const shared_ptr<ItemInstance> item, float x, float y, float fScaleX, float fScaleY, float fAlpha, bool isFoil, bool isConstantBlended, bool useCompiled = true);	// added isConstantBlended and useCompiled
...
private:
	void blitGlint(int id, float x, float y, float w, float h);		// changed x,y,w,h to floats
...
public:
	void blit(float x, float y, int sx, int sy, float w, float h);		// changed x,y,w,h to floats
