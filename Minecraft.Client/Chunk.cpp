...
#ifdef _LINUX
#include "Linux/SPU_Tasks/ChunkUpdate/ChunkRebuildData.h"
#include "Linux/SPU_Tasks/ChunkUpdate/TileRenderer_SPU.h"
#include "Linux/SPU_Tasks/CompressedTile/CompressedTileStorage_SPU.h"

#include "CThread_SPU.h"
#include "CSpursJob.h"
#endif
...
#else
// Don't want this when multi-threaded
Tesselator *Chunk::t = Tesselator::getInstance();
#endif
LevelRenderer *Chunk::levelRenderer;

// TODO -  how input entity vector is set up and decide what way is best to pass this to the function
...
void Chunk::setPos(int x, int y, int z)
{
...
#if 1
	// we're not using offsetted renderlists anymore, so just set the full position of this chunk into x/y/zRenderOffs where
	...
	float g = 6.0f;
	// changed to just set the value rather than make a new one, if we've already created storage
	if( bb == NULL )
	{
 	...
 	{
		// bounds are relative to the position now, so the AABB will be setup already, either above, or from the tesselator bounds.
...
//	printf("/t/t [inc] refcount %d at %d, %d, %d/n",refCount,x,y,z);
...
//	int idx = levelRenderer->getGlobalIndexForChunk(x, y, z, level);
	...
	if( refCount == 1 )
	{
//		printf("Setting %d %d %d dirty [%d]/n",x,y,z, idx);
...
#else
	Chunk::t = Tesselator::getInstance(); // added - static initialiser being set at the wrong time
#endif
...
//	unordered_set<shared_ptr<TileEntity> > oldTileEntities(renderableTileEntities.begin(),renderableTileEntities.end());		//  this & next line
//	renderableTileEntities.clear();

	vector<shared_ptr<TileEntity> > renderableTileEntities;	// added
	...
	level->getChunkAt(x,z)->getBlockData(tileArray);		// TODO - now our data has been re-arranged, we could just extra the vertical slice of this chunk rather than the whole thing
	...
	for( int yy = y0; yy < y1; yy++ )
	{
		for( int zz = 0; zz < 16; zz++ )
		{
			...
			{
				// tile data is ordered in 128 blocks of full width, lower 128 then upper 128
				int indexY = yy;
				int offset = 0;
	...
	if( empty )
	{
		// added - clear any renderer data associated with this
	...
	Tesselator::Bounds bounds;	// added
	{
	...
	for (int currentLayer = 0; currentLayer < 2; currentLayer++)
	{
		...
		// changed loop order here to leave y as the innermost loop for better cache performance
		for (int z = z0; z < z1; z++)
		{
			for (int x = x0; x < x1; x++)
			{
				for (int y = y0; y < y1; y++)
				{
					...
					// get tile from those copied into our local array in earlier optimisation
...
//					int tileId = region->getTile(x,y,z);
					if (tileId > 0)
					{
						if (!started)
						{
							...
							translateToPos();
							float ss = 1.000001f;
							// have removed this scale as I don't think we should need it, and have now optimised the vertex
...
#endif
			t->end();
			bounds.addBounds(t->bounds);		// added
			glPopMatrix();
			glEndList();
		...
		{
			// added - clear any renderer data associated with this unused list
			levelRenderer->setGlobalChunkFlag(this->x, this->y, this->z, level, LevelRenderer::CHUNK_FLAG_EMPTY0, currentLayer);
			RenderManager.CBuffClear(lists + currentLayer);
		}
	...
	// added this to take the bound from the value calc'd in the tesselator
	if( bb )
	{
	...
	// have rewritten the way that tile entities are stored globally to make it work more easily with split screen. Chunks are now
	...
	// newTileEntities is now renderableTileEntities with any old ones from oldTileEntitesRemoved (so just new things added)
	...
	// All these new things added to globalRenderableTileEntities
	...
	// oldTileEntities is now the removed items
	...
	// These removed items are now also removed from globalRenderableTileEntities
...
void RunSPURebuild()
{
	...
	static CSpursJobQueue::Port p("CSpursJob_ChunkUpdate");
	CSpursJob_CompressedTile tileJob(&g_tileCompressDataIn,g_tileCompressDataOut);
	CSpursJob_ChunkUpdate chunkJob(&g_rebuildDataIn, &g_rebuildDataOut);
...
void Chunk::reset()
{
	if( assigned )
	{
		...
		assigned = false;
//		printf("/t/t [dec] refcount %d at %d, %d, %d/n",refCount,x,y,z);
		if( refCount == 0 )
		{
...
void Chunk::renderBB()
{
//	glCallList(lists + 2);	// removed - TODO put back in
}
...
void Chunk::setDirty()
{
	// not used, but if this starts being used again then we'll need to investigate how best to handle it.
	__debugbreak();
	levelRenderer->setGlobalChunkFlag(x, y, z, level, LevelRenderer::CHUNK_FLAG_DIRTY);
}
