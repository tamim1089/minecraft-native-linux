...
public:
	...
	void tesselateInWorldFixedTexture( Tile* tile, int x, int y, int z, Icon *fixedTexture );	//  to differentiate from tesselateInWorld
	void tesselateInWorldNoCulling( Tile* tile, int x, int y, int z, int forceData = -1,
									shared_ptr< TileEntity > forceEntity = shared_ptr< TileEntity >() );	//  forceData, forceEntity param
	bool tesselateInWorld( Tile* tt, int x, int y, int z, int forceData = -1, shared_ptr< TileEntity > forceEntity =
						   shared_ptr< TileEntity >() );	//  forceData, forceEntity param
...
private:
	...
	int downFlip;
	public:
	void tesselatePistonBaseForceExtended( Tile* tile, int x, int y, int z, int forceData = -1 ); //  data param
	private:
	bool tesselatePistonBaseInWorld( Tile* tt, int x, int y, int z, bool forceExtended, int forceData = -1 );	//  data param
	...
	public:
	void tesselatePistonArmNoCulling( Tile* tile, int x, int y, int z, bool fullArm, int forceData = -1 ); //  data param
	private:
	bool tesselatePistonExtensionInWorld( Tile* tt, int x, int y, int z, bool fullArm, int forceData = -1 ); //  data param
	public:
	...
	// brought forward changes from 1.8.2
	...
	int blsmooth;
	int tc1, tc2, tc3, tc4; // brought forward changes from 1.8.2
	...
	void renderTile( Tile* tile, int data, float brightness, float fAlpha = 1.0f, bool useCompiled = true ); //  useCompiled
