...
{
	// I have copied this code for XUI_BaseScene. If/when it gets changed it should be broken out
	// altered to force full screen mode to 3X scaling, and any split screen modes to 2X scaling. This is so that the further scaling by 0.5 that
	...
	int guiScale;// = ( minecraft->player->m_iScreenSection == CRender::VIEWPORT_TYPE_FULLSCREEN ? 3 : 2 );
	int iPad=minecraft->player->GetPadIndex();
	...
	if(minecraft->player->m_iScreenSection == CRender::VIEWPORT_TYPE_FULLSCREEN)
	{
	...
	// turn off the slot display if a xui menu is up, or if we're autosaving
	...
	// Turn off interface if eGameSetting_DisplayHUD is off - for screen shots/videos.
    ...
    glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);	// added - this did actually get set in renderVignette but that code is currently commented out
	...
	if (!minecraft->gameMode->isCutScene())
	{
		...
		// this is where to set the blend factor for gui things
	...
	//	RenderManager.StateSetBlendFactor(0x40ffffff);
		...
		{
			MemSect(31);
			minecraft->textures->bindTexture(TN_GUI_GUI);	//  L"/gui/gui.png"
			MemSect(0);
			...
			if(bTwoPlayerSplitscreen)
			{
				...
				// Moved this push and scale further up as we still need to do it for the few HUD components not replaced by xui
				//glPushMatrix();		
				//glScalef(fScaleFactorWidth, fScaleFactorHeight, fScaleFactorWidth);
			
				// move into the safe zone, and account for 2 player splitscreen
			...
			// We don't want to adjust the cursor by the safezone, we want it centred
			if(bTwoPlayerSplitscreen)
			{
...
// 		}
		...
		if(bDisplayGui)
		{
			...
			{
				int xLeft, xRight;
				// TODO Work out proper positioning for splitscreen
				if(bTwoPlayerSplitscreen)
				{
...
// 					}
		...
		// turn off the slot display if a xui menu is up
		...
		if(bDisplayGui)
		{
	//		glDisable(GL_BLEND);		removed - we want to be able to fade our gui
		...
		// do render of crouched player. This code is largely taken from the inventory render of the player, with some special hard-coded positions
		...
		{
			//int playerIdx = minecraft->player->GetPadIndex();
			...
			if( bDisplayGui && (displayCrouch || displaySprint || displayFlying) )
			{
				...
				// This is all based on the inventory player renderer, with changes to ensure that capes render correctly
	...
	// added to disable blends, which we have enabled previously to allow gui fading
	glDisable(GL_BLEND);
	...
	// Request  to have a red death screen
    ...
    if (minecraft->options->renderDebug)
	{
		...
		//font->drawShadow(minecraft->gatherStats5(), iSafezoneXHalf+2, 32 + 10, 0xffffff);
       {
		...
		// Moved these so that they don't overlap
	...
	lastTickA = a;
	// This is now displayed in a xui scene
#if 0
	// Jukebox CD message
    if (overlayMessageTime > 0)
	{
        ...
        if (alpha > 0) 
		{
			...
			// this is the string displayed when cds are placed in a jukebox
...
// We have moved the chat text to a xui
#if 0
    glPushMatrix();
	//  we need to move this up a bit because we've moved the quick select
	...
	// we need gui messages for each of the possible 4 splitscreen players
	if(bDisplayGui)
	{
		int iPad=minecraft->player->GetPadIndex();
		for (unsigned int i = 0; i < guiMessages[iPad].size() && i < max; i++)
		{
			if (guiMessages[iPad][i].ticks < 20 * 10 || isChatting)
			{
				...
				if (alpha > 0)
				{
					...
					// fill the black bar across the whole screen, otherwise it looks odd due to the safe area
...
#if 0  // removed - TODO put back when we have blend functions implemented
    glDisable(GL_DEPTH_TEST);
    glDepthMask(false);
...
void Gui::tick()
{
	...
	{	
		// Fix for #10929 - MP LAB: Network Disconnects: Host does not receive an error message stating the client left the game when viewing the Pause Menu.
...
{
	wstring string = _string;	// Take copy of input as it is const
	//int iScale=1;

	//if((minecraft->player->m_iScreenSection==CRender::VIEWPORT_TYPE_SPLIT_TOP) ||
	//	(minecraft->player->m_iScreenSection==CRender::VIEWPORT_TYPE_SPLIT_BOTTOM))
	//{
	//	iScale=2;
	//}
