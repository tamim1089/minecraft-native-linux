...
public:
	...
	// more info to this packet so that we can check if anyone has a UGC privilege that won't let us
	...
	PreLoginPacket(wstring userName, PlayerUID *playerUids, DWORD playerCount, BYTE friendsOnlyBits, DWORD ugcPlayersVersion,char *pszUniqueSaveName, DWORD serverSettings, BYTE hostIndex, DWORD texturePackId);
	~PreLoginPacket();
