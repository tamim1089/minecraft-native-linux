...
void HellRandomLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks)
{
    ...
    doubleArray sandBuffer(16*16);	// used to be declared with class level scope but moved here for thread safety
    ...
    for (int x = 0; x < 16; x++)
	{
        for (int z = 0; z < 16; z++)
		{
            ...
            for (int y = Level::genDepthMinusOne; y >= 0; y--)
			{
				...
				// End  to build walls around the level
					...
					else if (old == Tile::hellRock_Id)
					{
                        if (run == -1)
						{
							...
							else if (y >= waterHeight - 4 && y <= waterHeight + 1)
							{
								...
								if (sand)
								{
									// Make some nether wart spawn outside of the nether fortresses
                            ...
                            run = runDepth;
							// If sand, then allow adding nether wart at heights below the water level
...
//    byteArray blocks = byteArray(16 * level->depth * 16);
	...
	// this now creates compressed block data from the blocks array passed in, so needs to be after data is finalised.
...
void HellRandomLevelSource::postProcess(ChunkSource *parent, int xt, int zt)
{
	...
	// added. The original java didn't do any setting of the random seed here. We'll be running our postProcess in parallel with getChunk etc. so
