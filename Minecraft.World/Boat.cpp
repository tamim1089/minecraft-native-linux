...
void Boat::tick()
{
	...
	if (level->isClientSide && doLerp)
	{
		...
		else
		{
			...
			// Fix for various boat bugs, ensure that we check collision on client-side movement
			this->move(xd,yd,zd);
			...
			zd *= 0.99f;
#else
			// Fix for #8280 - Gameplay : Boats behave erratically when exited next to land.
...
bool Boat::interact(shared_ptr<Player> player)
{
	...
	if (!level->isClientSide)
	{
		// Fixed issue with player not being able to dismount boat (issue #4446)
