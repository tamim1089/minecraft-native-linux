...
#include "../Minecraft.Client/Linux/LinuxExtras/libdivide.h"
...
{
	// Some changes moved here from 1.2.3. Temperature & downfall layers are no longer created & returned, and a debug layer is isn't.
...
//	islandLayer = shared_ptr<Layer>(new AddMushroomIslandLayer(5, islandLayer));		// old position of mushroom island layer
	...
	for (int i = 0; i < zoomLevel; i++)
	{
		...
		if (i == 0)
		{
			// moved mushroom islands to here. This skips 3 zooms that the old location of the add was, making them about 1/8 of the original size. Adding
		...
		if (i == 1 )
		{	
			// now expand mushroom islands up again. This does a simple region grow to add a new mushroom island element when any of the neighbours are also mushroom islands.
