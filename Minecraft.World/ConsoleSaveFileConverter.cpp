...
#ifdef _LINUX
			// As the UIDs on X360 and X1 are different, we don't want to transfer these over. However as the first player
			if(fileIdx > 0) break;
			PlayerUID uid;
			ProfileManager.GetUID(ProfileManager.GetPrimaryPad(), &uid, false);
			ConsoleSavePath targetPlayerDatPath( L"players/" + uid.toString() + L".dat" );
#else
