...
void Animal::breedWith(shared_ptr<Animal> target)
{
	...
	// we have offspring of NULL returned when we have hit our limits of spawning any particular type of animal. In these cases try and do everything we can apart from actually
	// spawning the entity.
	if (offspring != NULL)
	{
...
{
	return !isDespawnProtected();	// was false
}
...
bool Animal::interact(shared_ptr<Player> player)
{
	...
	if (item != NULL && isFood(item) && getAge() == 0)
	{
		...
		// If we can't produce another animal through breeding because of the spawn limits, display a message here
