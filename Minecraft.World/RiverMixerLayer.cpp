	...
	for (int i = 0; i < w * h; i++)
	{
		...
		else
		{
			if (r[i] >= 0)
			{
				...
				else if (b[i] == Biome::mushroomIsland->id || b[i] == Biome::mushroomIslandShore->id) result[i] = Biome::mushroomIsland->id;		// don't make mushroom island shores as we don't have any island left once we do this as our islands are small (this used to change to mushroomIslandShore)
				else result[i] = r[i];
			}
			else
			{
