...
Random::Random()
{
	// jave now uses the system nanosecond counter added to a "seedUniquifier" to get an initial seed. Our nanosecond timer is actually only millisecond accuate, so
...
int Random::nextInt(int n)
{
    ...
    if ((n & -n) == n)  // i.e., n is a power of 2
        return (int)(((__int64)next(31) * n) >> 31); // Made __int64 instead of long
