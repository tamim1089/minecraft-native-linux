...
void OldChunkStorage::save(LevelChunk *lc, Level *level, CompoundTag *tag)
{
	...
	// changes here for new storage. Now have static storage for getting lighting data for block, data, and sky & block lighting. This
	...
	// As we now save on multiple threads, the static data has been moved to TLS
	ThreadStorage *tls = (ThreadStorage *)TlsGetValue(tlsIdx);
	...
	tag->putShort(L"TerrainPopulatedFlags", lc->terrainPopulated);		// changed from "TerrainPopulated" to "TerrainPopulatedFlags" as now stores a bitfield, java stores a bool
...
LevelChunk *OldChunkStorage::load(Level *level, CompoundTag *tag)
{
	...
	LevelChunk *levelChunk = new LevelChunk(level, x, z);
	// the original code uses the data in the tag directly, but this is now just used as a source when creating the compressed data, so
	...
	// changed to use our new methods for accessing lighting
	...
	// TerrainPopulated was a bool (java), then changed to be a byte bitfield, then replaced with TerrainPopulatedShort to store a wider bitfield
...
#if 0
	// removed - we shouldn't need this any more
	if (!levelChunk->data->isValid())
	{
		levelChunk->data = new DataLayer(LevelChunk::BLOCKS_LENGTH, level->depthBits);		// BLOCKS_LENGTH was levelChunk->blocks.length
	}
#endif
