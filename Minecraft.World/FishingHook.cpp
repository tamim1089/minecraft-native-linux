...
void FishingHook::tick()
{
	...
	if (res != NULL)
	{
		if (res->entity != NULL) 
		{
			//  Move fix for : fix for #48587 - CRASH: Code: Gameplay: Hitting another player with the fishing bobber crashes the game. [Fishing pole, line]
...
int FishingHook::retrieve()
{
	...
	else if (nibble > 0)
	{
		...
		level->addEntity(ie);
		owner->level->addEntity( shared_ptr<ExperienceOrb>( new ExperienceOrb(owner->level, owner->x, owner->y + 0.5f, owner->z + 0.5f, random->nextInt(3) + 1) ) ); //  brought forward from 1.4
		dmg = 1;
	}
	if (inGround) dmg = 2;
