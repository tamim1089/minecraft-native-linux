...
//  Added this mainly to allow is to record telemetry for player deaths
...
{
friend class Gui; // Added to be able to access the shared flag functions and constants, without making them publicly available to everything
public:
	// added to replace (e instanceof Type), avoiding dynamic casts
...
private:
	...
	static const int FLAG_EFFECT_WEAKENED = 7; //needed for cure villager tooltip.
...
protected:
	//  so that client side simulations on the host are not affected by zero-lag
	bool m_ignoreVerticalCollisions;

public:
	Entity(Level *level, bool useSmallId = true);	// added useSmallId parameter
	virtual ~Entity();
...
public:
	...
	virtual void move(double xa, double ya, double za, bool noEntityCubes=false);	// added noEntityCubes parameter
...
public:
	...
	virtual int getLightColor(float a);		// change brought forward from 1.8.2
...
public:
	//  damageSource param to enable telemetry on player deaths
	...
	virtual int getTexture();			// changed from wstring to int
...
public:
	...
	virtual void findStandUpPosition(shared_ptr<Entity> vehicle); // Brought forward from 12w36 to fix #46282 - TU5: Gameplay: Exiting the minecart in a tight corridor damages the player
	...
	virtual void setEquippedSlot(int slot, shared_ptr<ItemInstance> item); // Brought forward change from 1.3 to fix #64688 - Customer Encountered: TU7: Content: Art: Aura of enchanted item is not displayed for other players in online game
	...
	// we need to see if this is weakened
...
public:
	...
	// added to manage allocation of small ids
private:
