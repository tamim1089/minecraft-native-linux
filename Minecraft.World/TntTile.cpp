...
{
	// added - don't every create on the client, I think this must be the cause of a bug reported in the java
	// version where white tnts are created in the network game
	if (level->isClientSide) return;	

	// added condition to have finite limit of these
	// Fix for #90934 - Customer Encountered: TU11: Content: Gameplay: TNT blocks are triggered by explosions even though "TNT explodes" option is unchecked.
...
bool TntTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
