...
void PathfinderMob::serverAiStep()
{
	...
	if (attackTarget == NULL)
	{
		...
		if (attackTarget != NULL)
		{
			setPath(level->findPath(shared_from_this(), attackTarget, maxDist, true, false, false, true)); // changed to setPath from path =
		}
	}
	else
	{
	...
	*/
	...
	// a few changes here so that we can call findRandomStrollLocation for a sub-set of things that it normally wouldn't be in the java game.
	...
	if (path == NULL || random->nextInt(100) == 0)
	{
		...
		setPath(NULL);// changed to setPath from path =
		return;
	}
...
void PathfinderMob::findRandomStrollLocation(int quadrant/*=-1*/)	// added quadrant
{
	bool hasBest = false;
	int xBest = -1;
	int yBest = -1;
	...
	for (int i = 0; i < 10; i++)
	{
		// added quadrant parameter to this method so that the caller can request that only stroll locations in one quadrant be found. If -1 is passed then
	...
	if (hasBest)
	{
		setPath(level->findPath(shared_from_this(), xBest, yBest, zBest, 10, true, false, false, true)); // changed to setPath from path =
	}
}
