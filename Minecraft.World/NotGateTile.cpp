...
// added, to tie in with other changes brought forward from 1.3.2 to associate toggles with a level. In addition to what the java
...
{
	// brought forward changes to associate toggles with a level from 1.3.2
	if( recentToggles.find(level) == recentToggles.end() )
	{
...
void NotGateTile::tick(Level *level, int x, int y, int z, Random *random)
{
	...
	// brought forward changes from 1.3.2 to associate toggles with level
	if( recentToggles.find(level) != recentToggles.end() )
	{
