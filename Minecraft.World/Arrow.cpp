...
// WESTY : Added for other award, kill creeper with arrow.
	...
	if (res != NULL)
	{
		if (res->entity != NULL)
		{
			...
			if(res->entity->hurt(damageSource, dmg))
			{
				...
				// We should not set the entity on fire unless we can cause some damage (this doesn't necessarily mean that the arrow hit lowered their health)
				...
				// WESTY : For award, need to track if creeper was killed by arrow from the player.
				...
				// sound change brought forward from 1.2.3
		...
		else
		{
			...
			//  check - zero dd here was creating NaNs
			if( dd > 0.0001f )
			{
				x -= (xd / dd) * 0.05f;
				y -= (yd / dd) * 0.05f;
