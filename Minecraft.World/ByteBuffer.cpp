...
//  so we can write this to a file
...
{
	// TODO Should this be the capcity and not the limit?
	return m_limit;
}
// End //Absolute get method. Reads the byte at the given index.
//Parameters:
...
{
	// TODO Should this function be writing from the start of the buffer, or from position?
...
ByteBuffer *ByteBuffer::put(byteArray inputArray)
{
	if( inputArray.length > remaining() )
		assert( false ); //TODO Some kind of exception?
...
{
	// TODO Is it safe to just cast our byte array pointer to another type?
