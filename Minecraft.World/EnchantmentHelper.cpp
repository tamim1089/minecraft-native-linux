...
int EnchantmentHelper::getEnchantmentCost(Random *random, int slot, int bookcases, shared_ptr<ItemInstance> itemInstance)
{
	...
	// Updated function to 1.3 version for TU7
	if (bookcases > 15)
	{
		bookcases = 15;
	}
...
vector<EnchantmentInstance *> *EnchantmentHelper::selectEnchantment(Random *random, shared_ptr<ItemInstance> itemInstance, int enchantmentCost)
{
	...
	// Update function to 1.3 version for TU7
	itemBonus /= 2;
	...
	if (availableEnchantments != NULL && !availableEnchantments->empty())
	{
		...
		if (instance != NULL)
		{
			...
			results->push_back( instance->copy() ); // Inserting a copy so we can clear memory from the availableEnchantments collection
			...
			while (random->nextInt(50) <= bonusChance)
			{
				...
				{					
					...
					values.clear();
					results->push_back( nextInstance->copy() ); // Inserting a copy so we can clear memory from the availableEnchantments collection
				}
