...
#ifdef _LINUX
#include "../Minecraft.Client/Linux/SPU_Tasks/PerlinNoise/PerlinNoiseJob.h"
#include "CSpursJob.h"
...
RandomLevelSource::RandomLevelSource(Level *level, __int64 seed, bool generateStructures) : generateStructures( generateStructures )
{
	...
	pprandom = new Random(seed);	// added, so that we can have a separate random for doing post-processing in parallel with creation
...
void RandomLevelSource::prepareHeights(int xOffs, int zOffs, byteArray blocks)
{
	...
	BiomeArray biomes;	//  locally here for thread safety, java has this as a class member

	level->getBiomeSource()->getRawBiomeBlock(biomes, xOffs * CHUNK_WIDTH - 2, zOffs * CHUNK_WIDTH - 2, xSize + 5, zSize + 5);

	doubleArray buffer;	// used to be declared with class level scope but tidying up for thread safety reasons
                ...
                for (int y = 0; y < CHUNK_HEIGHT; y++)
				{
                    ...
                    for (int x = 0; x < CHUNK_WIDTH; x++)
					{
                        ...
                        for (int z = 0; z < CHUNK_WIDTH; z++)
						{
							...
							// slightly rearranged this code (as of java 1.0.1 merge) to better fit with
							// changes we've made edge-of-world things - original sets blocks[offs += step] directly
							// here rather than setting a tileId
							int tileId = 0;
							// this comparison used to just be with 0.0f but is now varied by block above
                            if ((val += vala) > comp)
							{
							...
							// more extra code to make sure that the column at the edge of the world is just water & rock, to match the infinite sea that
...
void RandomLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
	...
	doubleArray depthBuffer(16*16); // used to be declared with class level scope but moved here for thread safety
    ...
    for (int x = 0; x < 16; x++)
	{
        for (int z = 0; z < 16; z++)
		{
            ...
            for (int y = Level::genDepthMinusOne; y >= 0; y--)
			{
				...
				if (y <= 1 + random->nextInt(2))	// changed to make the bedrock not have bits you can get stuck in
...
LevelChunk *RandomLevelSource::getChunk(int xOffs, int zOffs)
{
	...
	// now allocating this with a physical alloc & bypassing general memory management so that it will get cleanly freed
...
//    byteArray blocks = byteArray(16 * level->depth * 16);
   ...
   // LevelChunk *levelChunk = new LevelChunk(level, blocks, xOffs, zOffs);		// moved to below

	prepareHeights(xOffs, zOffs, blocks);

	// Some changes made here to how biomes, temperatures and downfalls are passed around for thread safety
	BiomeArray biomes;
	...
	//  Design Change - 1.8 gen goes stronghold, mineshaft, village, canyon
...
//    levelChunk->recalcHeightmap();		// removed & moved into its own method	

	// this now creates compressed block data from the blocks array passed in, so moved it until after the blocks are actually finalised. We also
...
// removed & moved into its own method from getChunk, so we can call recalcHeightmap after the chunk is added into the cache. Without
...
doubleArray RandomLevelSource::getHeights(doubleArray buffer, int x, int y, int z, int xSize, int ySize, int zSize, BiomeArray& biomes)
{
	...
	doubleArray pnr, ar, br, sr, dr, fi, fis;	// used to be declared with class level scope but moved here for thread safety
	...
	CSpursJobQueue::Port port("CSpursJob_PerlinNoise");
	CSpursJob_PerlinNoise perlinJob1(&g_scaleNoise_SPU);
	CSpursJob_PerlinNoise perlinJob2(&g_depthNoise_SPU);
	CSpursJob_PerlinNoise perlinJob3(&g_perlinNoise1_SPU);
	CSpursJob_PerlinNoise perlinJob4(&g_lperlinNoise1_SPU);
	CSpursJob_PerlinNoise perlinJob5(&g_lperlinNoise2_SPU);
...
// changed this to used pprandom rather than random, so that we can run it concurrently with getChunk
void RandomLevelSource::postProcess(ChunkSource *parent, int xt, int zt)
{
	...
	// brought forward from 1.2.3 to get snow back in taiga biomes
    xo += 8;
    zo += 8;
    for (int x = 0; x < 16; x++)
	{
        for (int z = 0; z < 16; z++)
		{
			...
			{
                level->setTileNoUpdate(x + xo, y - 1, z + zo, Tile::ice_Id);		// changed from setTile, otherwise we end up creating a *lot* of dynamic water tiles as these ice tiles are set
            }
