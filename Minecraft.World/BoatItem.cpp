...
{
	// added for tooltips to test use
	//  really we should have the crosshair hitresult telling us if it hit water, and at what distance, so we don't need to do this again
...
shared_ptr<ItemInstance> BoatItem::use(shared_ptr<ItemInstance> itemInstance, Level *level, shared_ptr<Player> player)
{
	...
	if (hr->type == HitResult::TILE)
	{
		...
		if (!level->isClientSide) 
		{
			...
			if( level->countInstanceOf(eTYPE_BOAT, true) < Level::MAX_LINUX_BOATS )		// added limit
			{
