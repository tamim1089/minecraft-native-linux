...
bool CauldronTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	...
	else if (item->id == Item::glassBottle_Id)
	{
        if (currentData > 0)
		{
			...
			// Brought forward change to update inventory when filling bottles with water
			...
			// don't lose the water in creative mode
