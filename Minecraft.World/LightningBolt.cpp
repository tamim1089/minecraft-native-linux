	...
	GlobalEntity( level )
{
	...
	// Microsoft request due to photosensitivity issue with multiple flashes of lightning
	//flashes = random->nextInt(3) + 1;
	flashes = 1;

	// added clientside check
		...
		{
			{
				...
				int yt = Mth::floor(y);
				int zt = Mth::floor(z);
				// don't go setting tiles if we aren't tracking them for network synchronisation
...
void LightningBolt::tick()
{
	...
	if (life == START_LIFE) {
		// this volume seems off the scale! But the volume is used to check the distance from the camera player - (volume*32) squared
