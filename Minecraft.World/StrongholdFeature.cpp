...
bool StrongholdFeature::isFeatureChunk(int x, int z,bool bIsSuperflat)
{
    if (!isSpotSelected)
	{
		...
		// Changed so that we keep trying more until we have found somewhere in the world to place a stronghold
		...
		do
		{
			for (int i = 0; i < strongholdPos_length; i++)
			{
...
#else
				// Design change: Original spawns at *32 chunks rather than *10 chunks from (0,0) but that is outside our world
				...
				// The original (pre-TU9) calculation for selecting a start point could put the stronghold very close to the edge
				...
				{
					app.DebugPrintf("Placed stronghold in INVALID biome at (%d, %d)/n", selectedX, selectedZ);
				}
			...
			// We want to make sure that we have at least one stronghold in this world
			++findAttempts;

			// Randomise the angles for retries as well
#ifdef _LARGE_WORLDS
