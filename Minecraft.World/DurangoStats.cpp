...
#include "../Minecraft.Client/Linux/ServiceConfig/Events-XBLA.8-149E11AEEvents.h"
...
// for tiles/items we want to record stats together.
	...
	if (paramBlob.length == sizeof(Param))
	{
			...
			|| (param->methodId == eAcquisitionMethod_Mined &&	onLeaderboard(eLeaderboardId_FARMING, eAcquisitionMethod_Mined, param)) )
		{
			EventWriteLeaderboardTotals(
				PlatformStats::getUserId(player), //UserId
				PlatformStats::getPlayerSession(), // PlayerSessionId
				player->level->difficulty, // Difficulty,
				eLeaderboardId_FARMING, // ScoreboardId
				param->itemCount);

			app.DebugPrintf("<%ls>/tscoreboardFarming(%i:%i:%i)/n", PlatformStats::getUserId(player),
		...
		if ( param->methodId == eAcquisitionMethod_Mined && onLeaderboard(eLeaderboardId_MINING, eAcquisitionMethod_Mined, param) )
		{
			EventWriteLeaderboardTotals(
				...
				param->itemCount);
			app.DebugPrintf("<%ls>/tscoreboardMining(%i:%i:%i)/n", PlatformStats::getUserId(player),
				player->level->difficulty, eLeaderboardId_MINING, param->itemCount);
		}

		// Debug printout.
		string method = nameMethods[(int)param->methodId];
		app.DebugPrintf("<%ls>/t%s(%i:%i:%i)/n", PlatformStats::getUserId(player),
		...
		if (param->methodId == eAcquisitionMethod_Placed)
		{
			EventWriteBlockPlaced(
				PlatformStats::getUserId(player),
				PlatformStats::getPlayerSession(),
...
/* note that mined events will only fire with 'Instant_Mine' on if you are carrying an appropriate
...
void DsMobKilled::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	if (paramBlob.length == sizeof(Param))
	{
		...
		if (EventWriteMobKilled(PlatformStats::getUserId(player),
							0,
							PlatformStats::getPlayerSession(),
							PlatformStats::getMultiplayerCorrelationId(),
							0,
							player->level->difficulty,
							PlatformStats::getPlayerSession(), // ROUND ID
							0,
							param->weaponId,
							param->mobType, 
							param->isRanged?1:0,
			...
			== 0)
		{
			app.DebugPrintf("<%ls>/t%s(%i:%i:%i:%i)/n", PlatformStats::getUserId(player),
		...
		case eTYPE_SLIME:
			...
			if (EventWriteLeaderboardTotals(
				...
				== 0)
			{
				app.DebugPrintf("<%ls>/tscoreboardKills(%i:%i:1)/n", PlatformStats::getUserId(player),
...
void DsMobInteract::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	if (paramBlob.length == sizeof(Param))
	{
		...
		app.DebugPrintf("<%ls>/t%s(%i)/n", PlatformStats::getUserId(player),
...
void DsTravel::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	if (paramBlob.length == sizeof(Param))
	{
		...
		int newDistance = cache(player->GetPadIndex(), *param);
...
void DsTravel::write(shared_ptr<LocalPlayer> player, eMethod method, int distance)
{
	...
	app.DebugPrintf("<%ls>/t%s(%i)/n", PlatformStats::getUserId(player), nameMethods[method].c_str(), distance);
...
void DsItemUsed::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	...
	{
		Param *param = (Param*) paramBlob.data;
		app.DebugPrintf("<%ls>/titemUsed(%i,%i,%i)/n", PlatformStats::getUserId(player),
			param->itemId,
			param->aux,
			param->count
			);
...
void DsAchievement::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	if (paramBlob.length == sizeof(SmallParam))
	{
		...
		assert( PlatformStats::binaryAchievement(paramS->award) );
		app.DebugPrintf("<%ls>/tAchievement(%i)/n", PlatformStats::getUserId(player), paramS->award);
	...
	else if (paramBlob.length == sizeof(LargeParam))
	{
		...
		assert( PlatformStats::enhancedAchievement(paramL->award) );

		switch(paramL->award)
		{
		case eAward_musicToMyEars:
			app.DebugPrintf("<%ls>/tmusicToMyEars(%i)/n", PlatformStats::getUserId(player), paramL->count);
		...
		case eAward_chestfulOfCobblestone:
			app.DebugPrintf("<%ls>/tchestfulOfCobblestone(%i)/n", PlatformStats::getUserId(player), paramL->count);
		...
		case eAward_overkill:
			app.DebugPrintf("<%ls>/toverkill(%i)/n", PlatformStats::getUserId(player), paramL->count);
			EventWriteOverkill(
				PlatformStats::getUserId(player),
				PlatformStats::getPlayerSession(),
				paramL->count
		...
		case eAward_OnARail:
			app.DebugPrintf("<%ls>/nonARail(%i)/n", PlatformStats::getUserId(player), paramL->count);
			EventWriteOnARail(
				PlatformStats::getUserId(player),
				PlatformStats::getPlayerSession(),
				paramL->count
...
void DsChangedDimension::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	...
	{
		Param *param = (Param*) paramBlob.data;
		app.DebugPrintf("<%ls>/tchangedDimension(%i:%i)/n", PlatformStats::getUserId(player), 
...
void DsEnteredBiome::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
	...
	{
		Param *param = (Param*) paramBlob.data;
		app.DebugPrintf("<%ls>/tenteredBiome(%i)/n", PlatformStats::getUserId(player), param->biomeId);
...
Stat *PlatformStats::get_stat(int i)
{
	switch (i) 
	{
...
Stat* PlatformStats::get_walkOneM()
{
	return travel;
}

Stat* PlatformStats::get_swimOneM()
{
	return travel;
}

Stat* PlatformStats::get_fallOneM()
{
	return travel;
}

Stat* PlatformStats::get_climbOneM()
{
	return travel;
}

Stat* PlatformStats::get_minecartOneM()
{
	return travel;
}

Stat* PlatformStats::get_boatOneM()
{
	return travel;
}

Stat* PlatformStats::get_pigOneM()
{
	return travel;
}

Stat *PlatformStats::get_cowsMilked()
{
	return get_itemsCrafted(Item::milk_Id);
}

Stat* PlatformStats::get_killMob()
{
	return mobKilled;
}

Stat* PlatformStats::get_breedEntity(eINSTANCEOF entityId)
{
	return mobInteract;
}

Stat* PlatformStats::get_tamedEntity(eINSTANCEOF entityId)
{
	return mobInteract;
}

Stat* PlatformStats::get_curedEntity(eINSTANCEOF entityId)
{
	return mobInteract;
}

Stat* PlatformStats::get_craftedEntity(eINSTANCEOF entityId)
{
	return mobInteract;
}

Stat* PlatformStats::get_shearedEntity(eINSTANCEOF entityId)
{
	return mobInteract;
}

Stat* PlatformStats::get_timePlayed()
{
	return travel;
}

Stat* PlatformStats::get_blocksPlaced(int blockId)
{
	return (Stat*) itemsAcquired;
}

Stat* PlatformStats::get_blocksMined(int blockId)
{
	return (Stat*) itemsAcquired;
}

Stat* PlatformStats::get_itemsCollected(int itemId, int itemAux)
{
	return (Stat*) itemsAcquired;
}

Stat* PlatformStats::get_itemsCrafted(int itemId)
{
	switch (itemId)
	{
	// These items can be crafted trivially to and from their block equivalents, 
...
Stat* PlatformStats::get_itemsSmelted(int itemId)
{
	// Context needed for itemCrafted for iron/gold being smelted.
	return (Stat*) itemsAcquired;
}

Stat* PlatformStats::get_itemsUsed(int itemId)
{
	return (Stat*) itemUsed;
}

Stat *PlatformStats::get_itemsBought(int itemId)
{
	return (Stat*) itemsAcquired;
}

Stat* PlatformStats::get_changedDimension(int from, int to)
{
	return (Stat*) changedDimension;
}

Stat* PlatformStats::get_enteredBiome(int biomeId)
{
	return (Stat*) enteredBiome;
}

Stat* PlatformStats::get_achievement(eAward achievementId)
{
...
byteArray PlatformStats::getParam_itemsBought(int id, int aux, int count)
{
	return DsItemEvent::createParamBlob(DsItemEvent::eAcquisitionMethod_Bought, id, aux, count);
}

byteArray PlatformStats::getParam_mobKill(shared_ptr<Player> player, shared_ptr<Mob> mob, DamageSource *dmgSrc)
{
	return DsMobKilled::createParamBlob(player,mob,dmgSrc);
}

byteArray PlatformStats::getParam_breedEntity(eINSTANCEOF entityId)
{
	return DsMobInteract::createParamBlob(DsMobInteract::eInteract_Breed, entityId);
}

byteArray PlatformStats::getParam_tamedEntity(eINSTANCEOF entityId)
{
	return DsMobInteract::createParamBlob(DsMobInteract::eInteract_Tamed, entityId);
}

byteArray PlatformStats::getParam_curedEntity(eINSTANCEOF entityId)
{
	return DsMobInteract::createParamBlob(DsMobInteract::eInteract_Cured, entityId);
}

byteArray PlatformStats::getParam_craftedEntity(eINSTANCEOF entityId)
{
	return DsMobInteract::createParamBlob(DsMobInteract::eInteract_Crafted, entityId);
}

byteArray PlatformStats::getParam_shearedEntity(eINSTANCEOF entityId)
{
	return DsMobInteract::createParamBlob(DsMobInteract::eInteract_Sheared, entityId);	
}

byteArray PlatformStats::getParam_time(int timediff)
{
	return DsTravel::createParamBlob(DsTravel::eMethod_time, timediff);
}

byteArray PlatformStats::getParam_changedDimension(int from, int to)
{
	return DsChangedDimension::createParamBlob(from,to);
}

byteArray PlatformStats::getParam_enteredBiome(int biomeId)
{
	return DsEnteredBiome::createParamBlob(biomeId);
}

byteArray PlatformStats::getParam_achievement(eAward id)
{
...
byteArray PlatformStats::getParam_onARail(int dist)
{
	return DsAchievement::createLargeParamBlob(eAward_OnARail, dist);
}

byteArray PlatformStats::getParam_chestfulOfCobblestone(int count)
{
	return DsAchievement::createLargeParamBlob(eAward_chestfulOfCobblestone, count);
}

byteArray PlatformStats::getParam_overkill(int dmg)
{
	return DsAchievement::createLargeParamBlob(eAward_overkill, dmg);
}

byteArray PlatformStats::getParam_musicToMyEars(int recordId)
{
	return DsAchievement::createLargeParamBlob(eAward_musicToMyEars, recordId);
}

bool PlatformStats::binaryAchievement(eAward achievementId)
{
	switch (achievementId)
	{
...
*/
bool PlatformStats::enhancedAchievement(eAward achievementId)
{
	switch (achievementId)
	{
	//case eAward_TakingInventory:
...
void PlatformStats::generatePlayerSession()
{
	PlatformStats *dsInstance = (PlatformStats *) GenericStats::getInstance();
	CoCreateGuid( &dsInstance->playerSessionId );
}

LPCGUID PlatformStats::getPlayerSession()
{
	PlatformStats *dsInstance = (PlatformStats *) GenericStats::getInstance();
...
void PlatformStats::setMultiplayerCorrelationId(Platform::String^ mcpId)
{
	((PlatformStats*)GenericStats::getInstance())->multiplayerCorrelationId = mcpId;
}

LPCWSTR PlatformStats::getMultiplayerCorrelationId()
{
	return ((PlatformStats*)GenericStats::getInstance())->multiplayerCorrelationId->Data();
}

LPCWSTR PlatformStats::getUserId(shared_ptr<LocalPlayer> player)
{
	return getUserId(player->GetPadIndex());
}

LPCWSTR PlatformStats::getUserId(int iPad)
{
	static wstring cache = L"";
	PlayerUID uid = INVALID_UID;
	ProfileManager.GetUID(iPad, &uid, true);
	cache = uid.toString();
	return cache.c_str();
}

void PlatformStats::playerSessionStart(PlayerUID uid, shared_ptr<Player> plr)
{
...
void PlatformStats::playerSessionStart(int iPad)
{
	PlayerUID puid; shared_ptr<Player> plr;
	ProfileManager.GetUID(iPad, &puid, true);
	plr = Minecraft::GetInstance()->localplayers[iPad];
	playerSessionStart(puid,plr);
}

void PlatformStats::playerSessionPause(int iPad)
{
...
void PlatformStats::playerSessionResume(int iPad)
{
	shared_ptr<MultiplayerLocalPlayer> plr = Minecraft::GetInstance()->localplayers[iPad];
	if (plr != NULL && plr->level != NULL && plr->level->getLevelData() != NULL)
	{
...
void PlatformStats::playerSessionEnd(int iPad)
{
	shared_ptr<MultiplayerLocalPlayer> plr = Minecraft::GetInstance()->localplayers[iPad];
	if (plr != NULL)
	{
		PlatformStats::getInstance()->travel->flush(plr);
	}
}