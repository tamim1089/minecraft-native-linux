...
// The Pllayer is managed by shared_ptrs elsewhere, but it owns us so we don't want to also
...
#endif
	// Changed this a little from Java to be less funn
	if( slot >= items.length )
	{
...
{
	// Changed this a little from the Java so it's less funny
	if( slot >= items.length )
	{
