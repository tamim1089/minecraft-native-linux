...
//    LevelChunk *levelChunk = new LevelChunk(level, blocks, xOffs, zOffs);			//  below
...
void TheEndLevelRandomLevelSource::postProcess(ChunkSource *parent, int xt, int zt)
{
	...
	// added. The original java didn't do any setting of the random seed here, and passes the level random to the biome decorator.
    ...
    biome->decorate(level, pprandom, xo, zo);		// passing pprandom rather than level->random here to make this consistent with our parallel world generation
