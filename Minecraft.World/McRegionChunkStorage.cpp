...
CThread *McRegionChunkStorage::s_saveThreads[3];
...
LevelChunk *McRegionChunkStorage::load(Level *level, int x, int z)
{
	...
	else
	{
		...
		if (!levelChunk->isAt(x, z))
		{
			...
			// We delete the data within OldChunkStorage::load, so we can never reload from it
...
//    try {
...
#ifdef _LINUX
		// Headless port: flush the chunk stream SYNCHRONOUSLY on the calling thread instead of
		// handing it to the save-worker pool. The async handoff acquires the ConsoleSaveFile lock
		// on this thread and releases it on the worker; combined with WaitForAll (this thread waits
		// for the worker) that lock-ordering deadlocks. Synchronous save keeps all save-file locking
		// on one thread. TODO(port): restore the parallel save pool with correct cross-thread locking.
		output->close();
	...
	else
	{
		...
		// getChunkDataOutputStream makes a new DataOutputStream that points to a new ChunkBuffer( ByteArrayOutputStream )
	...
	// Override this with our save file size to stop all the RegionFileCache lookups
...
void McRegionChunkStorage::staticCtor()
{
	...
	for(unsigned int i = 0; i < 3; ++i)
	{
		...
		s_saveThreads[i] = new CThread(runSaveThreadProc,NULL,threadName);
