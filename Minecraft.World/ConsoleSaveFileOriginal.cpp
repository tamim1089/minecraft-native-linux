...
ConsoleSaveFileOriginal::ConsoleSaveFileOriginal(const wstring &fileName, LPVOID pvSaveData /*= NULL*/, DWORD dFileSize /*= 0*/, bool forceCleanSave /*= false*/, ESavePlatform plat /*= SAVE_FILE_PLATFORM_LOCAL*/)
{
	...
	if( pvHeap == NULL )
	{
		...
		// AP - The Vita doesn't have virtual memory so a pretend system has been implemented in LinuxStubs.cpp. 
...
#ifdef _LINUX
		if(plat == SAVE_FILE_PLATFORM_LINUX)
		{
...
void ConsoleSaveFileOriginal::Flush(bool autosave, bool updateThumbnail )
{
	...
	ReleaseSaveAccess();
#elif (defined _LINUX || defined _LINUX || defined _LINUX || defined _LINUX || defined _LINUX)
