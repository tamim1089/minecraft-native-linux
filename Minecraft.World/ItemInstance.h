...
// While this is not really an abstract class, we don't want to make new instances of it,
...
public:
	...
	// Brought forward for enchanting/game rules
	CompoundTag *tag;
...
public:
	...
	ItemInstance *copy_not_shared() const; // Added for use in recipes
	static bool tagMatches(shared_ptr<ItemInstance> a, shared_ptr<ItemInstance> b); //  forward from 1.2
...
public:
	...
	bool sameItemWithTags(shared_ptr<ItemInstance> b); //
	bool sameItem_not_shared(ItemInstance *b); // Added this for the one time I need it
	...
	vector<wstring> *getHoverTextOnly(shared_ptr<Player> player, bool advanced, vector<wstring> &unformattedStrings); // 
	bool isFoil();
