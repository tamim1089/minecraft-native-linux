...
void LevelChunk::init(Level *level, int x, int z)
{
	...
	// lighting change brought forward from 1.8.2, introduced an array of bools called gapsToRecheck, which are now a single bit in array of nybble flags in this version
	for(int i = 0; i < 8 * 16; i++ )
	{
		columnFlags[i] = 0;		
	}

	// to flag if any emissive tile has been added to this chunk (will be cleared when lighting has been successfully completed for this chunk). Defaulting to true
...
// note that since we now compress the block storage, the parameter blocks is used as a source of data, but doesn't get used As the source data so needs
...
LevelChunk::LevelChunk(Level *level, byteArray blocks, int x, int z) : ENTITY_BLOCKS_LENGTH( Level::maxBuildHeight/16 )
{
	...
	else
	{
		...
		// changed to new SpareLightStorage class for these
...
// this ctor added to be able to make a levelchunk that shares its underlying block data between the server chunk cache & the multiplayer chunk cache.
...
//  so we can track unsaved chunks better
...
void LevelChunk::recalcHeightmapOnly()
{
	...
	for (int x = 0; x < 16; x++)
		for (int z = 0; z < 16; z++)
		{
			rainHeights[x + (z << 4)] = 255; // changed from int to unsigned char & this special value changed from -999 to 255

			int y = Level::maxBuildHeight - 1;
			//            int p = x << level->depthBitsPlusFour | z << level->depthBits;		// removed
#ifdef _LINUX
			int Index = ( x << 11 ) + ( z << 7 );
			int offset = Level::COMPRESSED_CHUNK_SECTION_TILES;
            y = 127;
			while (y > 0 && Tile::lightBlock[blockData[Index + offset + (y - 1)]] == 0)		// was blocks->get() was blocks[p + y - 1]
			{
				y--;
			}
			if( y == 0 )
			...
			while (y > 0 && Tile::lightBlock[blocks->get(x,(y - 1) % Level::COMPRESSED_CHUNK_SECTION_HEIGHT,z) & 0xff] == 0)		// was blocks->get() was blocks[p + y - 1]
			{
				y--;
...
//            int p = x << level->depthBitsPlusFour | z << level->depthBits;			// removed
            ...
            if (!level->dimension->hasCeiling)
			{
                ...
                do
				{
                    br -= Tile::lightBlock[blockData[Index + offset + yy]];					// blocks->get() was blocks[p + yy]
                    if (br > 0)
					{
                ...
                do
				{
                    br -= Tile::lightBlock[blocks->get(x,(yy % Level::COMPRESSED_CHUNK_SECTION_HEIGHT),z) & 0xff];					// blocks->get() was blocks[p + yy]
                    if (br > 0)
					{
...
// this code is fully commented out in the java version, but we have reimplemented something here to try and light
...
{
	// lighting change brought forward from 1.8.2, introduced an array of bools called gapsToRecheck, which are now a single bit in array of nybbles in this version
	int slot = ( x >> 1 ) | (z * 8);
	int shift = ( x & 1 ) * 4;
...
{
	// otherwise we can end up doing a very broken kind of lighting since for an empty chunk, the heightmap is all zero, but it
	...
	// note - this test will currently return true for chunks at the edge of our world. Making further checks inside the loop now to address this issue.
        ...
        for (int x = 0; x < 16; x++)
            for (int z = 0; z < 16; z++)
			{
                ...
                if (bForce || ( columnFlags[slot] & ( eColumnFlag_recheck << shift ) ) )
				{
					...
					// rewritten this to make sure that the minimum neighbour height which is calculated doesn't involve getting any heights from beyond the edge of the world,
					...
					if( !bForce )	// if doing a full forced thing over every single column, we don't need to do these offset checks too
					{
...
//    int p = x << level->depthBitsPlusFour | z << level->depthBits;		// removed
	...
	CompressedTileStorage *blocks = (y-1) >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT?upperBlocks : lowerBlocks;
	while (y > 0 && Tile::lightBlock[blocks->get(x,(y-1) % Level::COMPRESSED_CHUNK_SECTION_HEIGHT,z) & 0xff] == 0)		// blocks->get() was blocks[p + y - 1]
	{
        y--;
		blocks = (y-1) >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT?upperBlocks : lowerBlocks;
	}
...
//    level->lightColumnChanged(x, z, y, yOld);		// this call moved below & corrected - see comment further down
    heightmap[z << 4 | x] = (byte) y;
	...
	if (!level->dimension->hasCeiling)
	{
		...
		{
			// lighting change brought forward from 1.8.2
	...
	// changed to use xOffs and zOffs rather than the (incorrect) x and z it used to, and also moved so that it happens after all the lighting should be
	...
	// lighting changes brought forward from 1.8.2
...
bool LevelChunk::setTileAndData(int x, int y, int z, int _tile, int _data)
{
	...
	// flag if something emissive is being added. This is used during level creation to determine what chunks need extra lighting processing
	...
	// There isn't any point in recalculating heights or updating sky lighting if this tile has
    ...
    if (_tile != 0)
	{
		...
		{
			// in general we don't want to run the onPlace method on the client, but do a specific bit of the fireTile onPlace code here,
...
{
	// Now using setTileAndData (like in 1.5 Java) so there is only one place we have to fix things
...
void LevelChunk::removeEntity(shared_ptr<Entity> e, int yc)
{
	...
	// was entityBlocks[yc]->remove(e);
	AUTO_VAR(it, find(entityBlocks[yc]->begin(),entityBlocks[yc]->end(),e));
	if( it != entityBlocks[yc]->end() )
	{
		entityBlocks[yc]->erase(it);
		// we don't want storage creeping up here as thinkgs move round the world accumulating up spare space
		MemSect(31);
#ifdef _LINUX
		...
		if(bShowMsg)
		{ 
			app.DebugPrintf("Need to add C++11 shrink_to_fit for Linux/n");
			bShowMsg = false;
		}
#else
...
shared_ptr<TileEntity> LevelChunk::getTileEntity(int x, int y, int z)
{
	...
	// Changed as we should not be using the [] accessor (causes an insert when we don't want one)
	...
	if (it == tileEntities.end())
	{
		...
		// Chests try to get their neighbours when being destroyed,
        ...
        //tileEntity = tileEntities[pos];		// TODO - this doesn't seem right - assignment wrong way? Check

		// It should have been inserted by now, but check to be sure
		EnterCriticalSection(&m_csTileEntities);
...
void LevelChunk::setTileEntity(int x, int y, int z, shared_ptr<TileEntity> tileEntity)
{
	...
	if (getTile(x, y, z) == 0 || !Tile::tiles[getTile(x, y, z)]->isEntityTile())	// was !(Tile.tiles[getTile(x, y, z)] instanceof EntityTile))
	{
...
void LevelChunk::unload(bool unloadTileEntities)	// added parameter
{
    loaded = false;
	if( unloadTileEntities )
	{
		...
		for( AUTO_VAR(it, tileEntities.begin()); it != tileEntities.end(); it++ )
		{
		// m 1.7.3 was it->second->setRemoved();
...
#ifdef _LARGE_WORLDS
	if ( !m_bUnloaded ) // If we unload a chunk twice, we delete all the entities/tile-entities its saved in the entitiesTag.
	{
		m_bUnloaded = true;
		if(!level->isClientSide)
		{
			delete m_unloadedEntitiesTag;
			// Save out entities to a cached format that won't interfere with other systems
...
void LevelChunk::getEntitiesOfClass(const type_info& ec, AABB *bb, vector<shared_ptr<Entity> > &es)
{
    ...
    for (int yc = yc0; yc <= yc1; yc++)
	{
		...
		for (AUTO_VAR(it, entities->begin()); it != itEnd; it++)
		{
			...
			// note needs to be equivalent to baseClass.isAssignableFrom(e.getClass())
        }
    }
#ifndef _LINUX
	LeaveCriticalSection(&m_csEntities);
...
int LevelChunk::getBlocksAndData(byteArray *data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting/* = true*/)
{
	...
	// Added this because some "min" functions don't let us use our constants :(
	int compressedHeight = Level::COMPRESSED_CHUNK_SECTION_HEIGHT;

	// replaced block storage as now using CompressedTileStorage
	...
	// replaced data storage as now using SparseDataStorage
	...
	if( includeLighting )
	{
		// replaced block and skylight storage as these now use our SparseLightStorage
...
// return true if setBlocksAndData would change any blocks
...
int LevelChunk::setBlocksAndData(byteArray data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting/* = true*/)
{
	...
	// replaced block storage as now uses CompressedTileStorage
	...
	// replaced data storage as now uses SparseDataStorage
...
// check for any pre-1.8.2 chests in the chunk at (x,z), and calculate their facing direction & relight to bring up to date with the post 1.8.2 build
...
// optimisation brought forward from 1.8.2
...
//  as optimisation, these biome checks are expensive so caching through flags in levelchunk
