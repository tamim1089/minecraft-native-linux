...
public:
	...
	virtual void setTilesDirty(int x0, int y0, int z0, int x1, int y1, int z1, Level *level) = 0;		// added level param
	...
	// virtual void addParticle(const wstring& name, double x, double y, double z, double xa, double ya, double za) = 0;

	virtual void addParticle(ePARTICLE_TYPE name, double x, double y, double z, double xa, double ya, double za) = 0; // 

	virtual void entityAdded(shared_ptr<Entity> entity) = 0;

	virtual void entityRemoved(shared_ptr<Entity> entity) = 0;

	virtual void playerRemoved(shared_ptr<Entity> entity) = 0;		// for when a player is removed from the level's player array, not just the entity storage
