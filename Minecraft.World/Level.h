...
// This value should be big enough that we don't get any crashes causes by memory overwrites,
...
public:
	static const int MAX_LINUX_BOATS = 40;										// Max number of boats
    ...
    static const int minBuildHeight = 0;		// brought forward from 1.2.3
	static const int maxBuildHeight = 256;		// brought forward from 1.2.3
	...
	static const int COMPRESSED_CHUNK_SECTION_TILES = COMPRESSED_CHUNK_SECTION_HEIGHT * 16 * 16; // Fixed size

	int seaLevel;

	// added, making instaTick flag use TLS so we can set it in the chunk rebuilding thread without upsetting the main game thread
	static DWORD tlsIdx;
...
public:
	...
	CRITICAL_SECTION				m_tileEntityListCS;		// 
...
public:
	ChunkSource *chunkSource;		// changed to public
protected:
...
public:
	Biome *getBiome(int x, int z);				// brought forward from 1.2.3
	virtual BiomeSource *getBiomeSource();

private:

	// Added these ctors to handle init of member variables
	void _init();
...
public:
	...
	bool reallyHasChunksAt(int x0, int y0, int z0, int x1, int y1, int z1);		// 
...
public:
	...
	bool setTileNoUpdateNoLightCheck(int x, int y, int z, int tile);		// 
	Material *getMaterial(int x, int y, int z);
	virtual int getData(int x, int y, int z);
	void setData(int x, int y, int z, int data, bool forceUpdate=false);		//  forceUpdate
...
public:
	...
	int getBrightnessPropagate(LightLayer::variety layer, int x, int y, int z, int tileId);	//  tileId
	void getNeighbourBrightnesses(int *brightnesses, LightLayer::variety layer, int x, int y, int z);	// 
	int getBrightness(LightLayer::variety layer, int x, int y, int z);
	void setBrightness(LightLayer::variety layer, int x, int y, int z, int brightness, bool noUpdateOnClient=false);		//  noUpdateOnClient
	void setBrightnessNoUpdateOnClient(LightLayer::variety layer, int x, int y, int z, int brightness);	// 
	...
	virtual int getLightColor(int x, int y, int z, int emitt, int tileId = -1);		// brought forward from 1.8.2
	...
	// void addParticle(const wstring& id, double x, double y, double z, double xd, double yd, double zd);
	void addParticle(ePARTICLE_TYPE id, double x, double y, double z, double xd, double yd, double zd); // 
...
protected:
	...
	virtual void playerRemoved(shared_ptr<Entity> e);	// 
...
public:
	AABBList *getCubes(shared_ptr<Entity> source, AABB *box, bool noEntities=false, bool blockAtEdge=false); // added noEntities & blockAtEdge parameters
	AABBList *getTileCubes(AABB *box, bool blockAtEdge); // Brought forward from 12w36 to fix #46282 - TU5: Gameplay: Exiting the minecart in a tight corridor damages the player
	int getOldSkyDarken(float a);		// change brought forward from 1.8.2
	float getSkyDarken(float a);		// change brought forward from 1.8.2
	...
	// destroyBlocks param brought forward as part of fix for tnt cannons
	...
	virtual bool isTopSolidBlocking(int x, int y, int z);	// brought forward from 1.3.2
...
protected:
	...
	// snow & ice checks brought forward from 1.2.3
public:
	...
	void checkLight(int x, int y, int z, bool force = false, bool rootOnlyEmissive = false);		//  force, rootOnlySource parameters
private:
	int *toCheckLevel;		
	int getExpectedSkyColor(lightCache_t *cache, int oc, int x, int y , int z, int ct, int block);
	int getExpectedBlockColor(lightCache_t *cache, int oc, int x, int y, int z, int ct, int block, bool propagatedOnly);	//  parameter
public:
	void checkLight(LightLayer::variety layer, int xc, int yc, int zc, bool force = false, bool rootOnlyEmissive = false);		//  force, rootOnlySource parameters
...
//	unsigned int countInstanceOf(BaseObject::Class *clas);
	unsigned int countInstanceOf(eINSTANCEOF clas, bool singleType, unsigned int *protectedCount = NULL, unsigned int *couldWanderCount = NULL);			// 
	unsigned int countInstanceOfInRange(eINSTANCEOF clas, bool singleType, int range, int x, int y, int z);													// 
	...
	shared_ptr<Player> getPlayerByUUID(const wstring& name); // 
	...
	void setOverrideTimeOfDay(__int64 time); //  so we can override timeOfDay without changing tick time
	__int64 getSeed();
	__int64 getTime();
	...
	int getAuxValueForMap(PlayerUID uid, int dimension, int centreXC, int centreZC, int scale);
	...
	// added for implementation of finite limit to number of item entities, tnt and falling block entities
public:
	virtual bool newPrimedTntAllowed() { return true; }
	virtual bool newFallingTileAllowed() { return true; }

	// added for new lighting from 1.8.2
	CRITICAL_SECTION			m_checkLightCS;

private:
	int m_iHighestY;		// for the end portal in The End
public:
