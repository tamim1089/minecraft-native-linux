...
// allowAnywhere is a static in java, implementing as TLS here to make thread safe
...
void TheEndPortal::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
    float r = 1 / 16.0f;
...
void TheEndPortal::entityInside(Level *level, int x, int y, int z, shared_ptr<Entity> entity)
{
    if (entity->riding == NULL && entity->rider.lock() == NULL)
	{
        if (dynamic_pointer_cast<Player>(entity) != NULL)
		{
			...
			{
				// Update the level data position so that the stronghold portal can be shown on the maps
				int x,z;
				x = z = 0;
