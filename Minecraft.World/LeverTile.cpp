...
void LeverTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
    int dir = level->getData(x, y, z) & 7;
    float r = 3 / 16.0f;
    if (dir == 1)
...
bool LeverTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	if( soundOnly )
	{
	...
	{
		// added stuff to play sound in this case too
		int data = level->getData(x, y, z);
		int dir = data & 7;
		int open = 8 - (data & 8);
		level->playSound(x + 0.5, y + 0.5, z + 0.5, eSoundType_RANDOM_CLICK, 0.3f, open > 0 ? 0.6f : 0.5f);
