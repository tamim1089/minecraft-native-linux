    ...
    if (PotionBrewing::SIMPLIFIED_BREWING)
	{
        ...
        for (int dest = 0; dest < 3; dest++)
		{
            if (items[dest] != NULL && items[dest]->id == Item::potion_Id)
			{
				...
				// this code replaces an expression "currentEffects.equals(newEffects)" in the java.
...
int BrewingStandTileEntity::applyIngredient(int currentBrew, shared_ptr<ItemInstance> ingredient)
{
	...
	if (!PotionBrewing::SIMPLIFIED_BREWING)
	{
		...
		// SIMPLIFIED_BREWING is on, so we never use this
...
{
	// Changed the implementation of this function to be the same as ChestTileEntity to enable the "Pickup Half"
