...
PreLoginPacket::PreLoginPacket(wstring userName, PlayerUID *playerUids, DWORD playerCount, BYTE friendsOnlyBits, DWORD ugcPlayersVersion,char *pszUniqueSaveName, DWORD serverSettings, BYTE hostIndex, DWORD texturePackId) 
{
	this->loginKey = userName;
	m_playerUids = playerUids;
...
PreLoginPacket::~PreLoginPacket()
{
	if( m_playerUids != NULL ) delete [] m_playerUids;
}
...
void PreLoginPacket::read(DataInputStream *dis) //throws IOException 
{
	...
	{
		m_playerUids = new PlayerUID[m_dwPlayerCount];
		for(DWORD i = 0; i < m_dwPlayerCount; ++i)
		{
			m_playerUids[i] = dis->readPlayerUID();
		}
	}
...
void PreLoginPacket::write(DataOutputStream *dos) //throws IOException 
{
	...
	for(DWORD i = 0; i < m_dwPlayerCount; ++i)
	{
		dos->writePlayerUID( m_playerUids[i] );
	}
