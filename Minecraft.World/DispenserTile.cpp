...
bool DispenserTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	...
	case Item::arrow_Id:
		{
			...
			if(currentProjectiles < Level::MAX_DISPENSABLE_PROJECTILES)	// added limit
			{
	...
	case Item::fireball_Id: // TU9
		{
			...
			if(currentFireballs < Level::MAX_DISPENSABLE_FIREBALLS)	// added limit
			{
	...
	case Item::monsterPlacer_Id:
		{
			...
			if (mob != NULL)
			{		
				// Changed the line below slightly since mobs were sticking to the dispenser rather than dropping down when fired
	...
	case Item::minecart_furnace_Id:
		{
			...
			if( level->countInstanceOf(eTYPE_MINECART, true) < Level::MAX_CONSOLE_MINECARTS )		// added limit
			{
	...
	case Item::boat_Id:
		{
			...
			if( bLaunchBoat && level->countInstanceOf(eTYPE_BOAT, true) < Level::MAX_LINUX_BOATS )		// added limit
			{
