...
void ThrownEgg::onHit(HitResult *res)
{
	...
	{
		if(level->canCreateMore( eTYPE_CHICKEN, Level::eSpawnType_Breed) )	// added limit for number of chickens in world
		{
			int count = 1;
			...
			for (int i = 0; i < count; i++)
			{
				...
				chicken->setDespawnProtected();		// default to being protected against despawning
