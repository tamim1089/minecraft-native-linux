...
// These are special map slots that are used on local machines. They will never be an actual map,
...
// Added this which we will write out as a file. Map id's are stored in itemInstances
...
{
	PlayerUID uids[MAXIMUM_MAP_SAVE_DATA];
	byte dimensions[MAXIMUM_MAP_SAVE_DATA/4];

	_MapDataMappings();
	int getDimension(int id);
	void setMapping(int id, PlayerUID uid, int dimension);
} MapDataMappings;
...
{
private:
	/* Probably no need for this as theres no exceptions being thrown.
...
public:
    ...
    virtual bool load(shared_ptr<Player> player);  //  return val to bool to check if new player or loaded player
    virtual CompoundTag *loadPlayerDataTag(PlayerUID uid);
	...
	virtual int getAuxValueForMap(PlayerUID uid, int dimension, int centreXC, int centreZC, int scale);
...
private:
	void dontSaveMapMappingForPlayer(PlayerUID uid);
	void deleteMapFilesForPlayer(PlayerUID uid);
};
