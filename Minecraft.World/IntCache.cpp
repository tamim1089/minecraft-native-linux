...
//		app.DebugPrintf("IntCache: New max size: %d/n" , size);
		...
		// added - all the vectors in cache & allocated are smaller than maxSize so should be discarded. However, we
...
void IntCache::releaseAll()
{
	...
	// added - we can now remove the vectors that were deemed as too small (see comment in IntCache::allocate)
...
//  so that we can fully reset between levels
void IntCache::Reset()
{
	ThreadStorage *tls = (ThreadStorage *)TlsGetValue(tlsIdx);
	tls->maxSize = TINY_CUTOFF;
