...
public:
	...
	// added for a red death fade in the gui
...
public:
	...
	// not needed, since cutomtextureurl2 is the same thing wstring cloakTexture;
...
private:
	...
	//These are in cms, every time they go > 1m they are entered into the stats
...
public:
	...
	// Made protected so that we can access it from MultiPlayerLocalPlayer
protected:
...
public:
	//bool addResource(int resource); // Removed 1.0.1
	int getScore();
...
public:
	...
	virtual bool openContainer(shared_ptr<Container> container);	// added bool return
	virtual bool startEnchanting(int x, int y, int z);				// added bool return
	virtual bool startRepairing(int x, int y, int z);				// added bool return
	virtual bool startCrafting(int x, int y, int z);				// added boo return
	virtual void take(shared_ptr<Entity> e, int orgCount);
	virtual float getHeadHeight();

	// added to keep the code happy with the change to make the third person view per player
...
public:
	...
	virtual bool openFurnace(shared_ptr<FurnaceTileEntity> container);		// added bool return
	virtual bool openTrap(shared_ptr<DispenserTileEntity> container);		// added bool return
	virtual void openTextEdit(shared_ptr<SignTileEntity> sign);
	virtual bool openBrewingStand(shared_ptr<BrewingStandTileEntity> brewingStand); // added bool return
	virtual bool openTrading(shared_ptr<Merchant> traderTarget); // added bool return
...
public:
	...
	// Added to allow callback to tutorial to stay within Minecraft.Client
	// Overidden in LocalPlayer
	virtual void onCrafted(shared_ptr<ItemInstance> item) {}

	//  this so that we can have some different default skins
	virtual int getTexture();		//  from wstring to int
	...
	PlayerUID getUid()																			{ return m_uid; }
	void setOnlineUid(PlayerUID uid)															{ m_OnlineUid = uid; }
	PlayerUID getOnlineUid()																	{ return m_OnlineUid; }
...
private:
	...
	// Used to show which colour the player is on the map/behind their name
	DWORD m_playerIndex;

	// to track debug options from the server player
...
private:
	// Used to track what actions players have been allowed to perform by the host
...
public:
	// Added hooks for the game rules
...
#if defined(_LINUX) || defined(_LINUX)
	ePlayerNameValidState m_ePlayerNameValidState; // to ensure we have the characters for this name in our font, or display a player number instead
#endif
};
