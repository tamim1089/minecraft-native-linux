...
CraftingMenu::CraftingMenu(shared_ptr<Inventory> inventory, Level *level, int xt, int yt, int zt) : AbstractContainerMenu()
{
	...
	slotsChanged();	// removed craftSlots parameter, see comment below
}

void CraftingMenu::slotsChanged()  //  to take a shared_ptr<Container> but wasn't using it, so removed to simplify things
{
