...
int Entity::entityCounter = 2048;		// changed initialiser to 2048, as we are using range 0 - 2047 as special unique smaller ids for things that need network tracked
DWORD Entity::tlsIdx = TlsAlloc();

// added getSmallId & freeSmallId methods
...
// added for common ctor code
// Do all the default initialisations done in the java class
void Entity::_init(bool useSmallId)
{
	// changed to assign two different types of ids. A range from 0-2047 is used for things that we'll be wanting to identify over the network,
	...
	yRot = xRot = 0.0f;
	yRotO = xRotO = 0.0f;
	bb = AABB::newPermanent(0, 0, 0, 0, 0, 0); //  final
	onGround = false;
...
Entity::Entity(Level *level, bool useSmallId)	// added useSmallId parameter
{
	MemSect(16);
	_init(useSmallId);
	MemSect(0);
	...
	entityData->define(DATA_AIR_SUPPLY_ID, TOTAL_AIR_SUPPLY); // Brought forward from 1.2.3 to fix 38654 - Gameplay: Player will take damage when air bubbles are present if resuming game from load/autosave underwater.

	// We cannot call virtual functions in ctors, as at this point the object
...
void Entity::move(double xa, double ya, double za, bool noEntityCubes)   // added noEntityCubes parameter
{
	if (noPhysics)
	{
		bb->move(xa, ya, za);
	...
	// Particles (and possibly other entities) don't have xChunk and zChunk set, so calculate the chunk instead
    int xc = Mth::floor(x / 16);
    int zc = Mth::floor(z / 16);
	if(!level->isClientSide || level->reallyHasChunk(xc, zc))
	{
		// It's horrible that the client is doing any movement at all! But if we don't have the chunk
	...
	if (footSize > 0 && og && (isPlayerSneaking || ySlideOffset < 0.05f) && ((xaOrg != xa) || (zaOrg != za)))
	{
		...
		AABB *normal = bb->copy();
		bb->set(bbOrg);
		// added extra expand, as if we don't move up by footSize by hitting a block above us, then overall we could be trying to move as much as footSize downwards,
...
bool Entity::isUnderLiquid(Material *material)
{
	...
	int xt = Mth::floor(x);
	int yt = Mth::floor(yp);	// this used to be a nested pair of floors for some reason
	int zt = Mth::floor(z);
	int t = level->getTile(xt, yt, zt);
...
// used to be wstring return type, returning L""
...
ListTag<FloatTag> *Entity::newFloatList(unsigned int number, float firstValue, float secondValue)
{
	...
	// TODO -  For some reason the va_list wasn't working correctly here
...
void Entity::ride(shared_ptr<Entity> e)
{
	...
	if (e == NULL)
	{
		if (riding != NULL)
		{
			// Position should already be updated before the SetRidingPacket comes in
...
void Entity::findStandUpPosition(shared_ptr<Entity> vehicle)
{
	...
	for (double xDiff = -1.5; xDiff < 2; xDiff += 1.5)
	{
		for (double zDiff = -1.5; zDiff < 2; zDiff += 1.5)
		{
			...
			// Added loop over y to restaring the bb into 2 block high spaces if required (eg the track block plus 1 air block above it for minecarts)
...
void Entity::lerpTo(double x, double y, double z, float yRot, float xRot, int steps)
{
	...
	// don't know what this special y collision is specifically for, but its definitely bad news
...
// Brought forward from 1.2.3 to fix 38654 - Gameplay: Player will take damage when air bubbles are present if resuming game from load/autosave underwater.
...
{	
	if(app.GetGameSettings(eGameSetting_CustomSkinAnim)==0 )
	{
		...
		// treat all the eAnim_Disable flags as a force anim
