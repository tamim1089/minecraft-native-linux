...
LevelData::LevelData(CompoundTag *tag)
{
	...
	newSeaLevel = tag->getBoolean(L"newSeaLevel"); // only use new sea level for newly created maps. This read defaults to false. (sea level changes in 1.8.2)
	hasBeenInCreative = tag->getBoolean(L"hasBeenInCreative"); //  so we can not award achievements to levels modified in creative

	// for stronghold position
	...
	// for stronghold end portal position
...
LevelData::LevelData(LevelSettings *levelSettings, const wstring& levelName) 
{
    ...
    this->ySpawn = 0;
    this->zSpawn = 0;
    this->time = -1;		// Edited: To know when this is uninitialized.
    this->lastPlayed = 0;
    this->sizeOnDisk = 0;
//    this->loadedPlayerTag = NULL;	// we don't store this anymore
    this->dimension = 0;
    this->version = 0;
    this->rainTime = 0;
    this->raining = false;
	...
	this->newSeaLevel = levelSettings->useNewSeaLevel();	// only use new sea level for newly created maps (sea level changes in 1.8.2)
	this->hasBeenInCreative = levelSettings->getGameType() == GameType::CREATIVE; // 

	//  for the stronghold position
...
//    this->loadedPlayerTag = copy->loadedPlayerTag;		// we don't store this anymore
...
{
	// removed all code for storing tags for players
	return createTag();
}
...
{
    return NULL;	// we don't store this anymore
}

//  TU9 as it's never accurate due to the dimension never being set
