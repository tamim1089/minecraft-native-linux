...
FlatLevelSource::FlatLevelSource(Level *level, __int64 seed, bool generateStructures) 
{
	...
	this->pprandom = new Random(seed);		// added, so that we can have a separate random for doing post-processing in parallel with creation
...
//	LevelChunk *levelChunk = new LevelChunk(level, blocks, xOffs, zOffs);		// moved below
...
{
	// changed from random to pprandom so we can run in parallel with getChunk etc.
