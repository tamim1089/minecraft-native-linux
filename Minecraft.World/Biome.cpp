...
void Biome::staticCtor()
{
	...
	// brought forward Taiga temperature change from 0.3f to 0.05f, from 1.2.3
...
Biome::Biome(int id) : id(id)
{
	//  Default inits
	color = 0;
//  snowCovered = false;	// this isn't set by the java game any more so removing to save confusion
	...
	*/
	...
	friendlies_chicken.push_back(new MobSpawnerData(eTYPE_CHICKEN, 10, 4, 4));		// moved chickens to their own category
...
Feature *Biome::getTreeFeature(Random *random)
{
	...
	{
        return new BasicTree(false); //  to return member fancyTree, now returning newly created object so that caller can be consistently resposible for cleanup
    }
    return new TreeFeature(false); //  to return member normalTree, now returning newly created object so that caller can be consistently resposible for cleanup
}
...
bool Biome::hasSnow()
{
	// snowCovered flag removed as it wasn't being set by the game anymore - snow is now temperature dependent to match code in rain rendering, shouldFreeze functions etc.
...
bool Biome::hasRain()
{
	// snowCovered flag removed as it wasn't being set by the game anymore, replaced by call to hasSnow()
