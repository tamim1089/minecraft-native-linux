...
unsigned char Tile::_sendTileData[TILE_NUM_COUNT];		// was bool, now bitfield to indicate which bits are important to be sent
...
void Tile::staticCtor()
{
	...
	// for leaves, have specified that only the data bits that encode the type of leaf are important to be sent
...
// 	Tile::stoneSlabHalf = (new StoneSlabTile(44, false))	->setBaseItemTypeAndMaterial(Item::eBaseItemType_halfslab,	Item::eMaterial_stone)->setDestroyTime(2.0f)->setExplodeable(10)->setSoundType(Tile::SOUND_STONE)->setDescriptionId(IDS_TILE_STONESLAB)->setUseDescriptionId(IDS_DESC_HALFSLAB);
	...
	// change of destroy time from 10.0f -> 50.0f for obsidian brought forward from 1.2.3
	...
	Item::items[Tile::monsterStoneEgg_Id]	= ( new StoneMonsterTileItem(Tile::monsterStoneEgg_Id - 256))->setTextureName(L"monsterStoneEgg")->setDescriptionId(IDS_TILE_STONE_SILVERFISH)->setUseDescriptionId(IDS_DESC_STONE_SILVERFISH); // Brought forward from post-1.2 to fix stacking problem
...
void Tile::_init(int id, Material *material, bool isSolidRender)
{
	...
	// Removed these in favour of TLS versions
	...
	*/
	...
	this->id = id;
	updateDefaultShape();
	// note these used to call isSolidRender(), but that always calls Tile::isSolidRender in C++ so have added as a parameter that can be varied from
	// derived ctors
...
{
	Tile::_sendTileData[id] = importantMask;		// changed was bool, now bitfield to indicate which bits are important to be sent. Default behaviour with this method is all 4 bits
	return this;
}
...
// adding so we can class different items together for the new crafting menu
...
{
	ThreadStorage *tls = (ThreadStorage *)TlsGetValue(Tile::tlsIdxShape);
	// Added this so that the TLS shape is correct for this tile
...
// Adding a TestUse for tooltip display
bool Tile::TestUse()
{
	return false;
}
...
bool Tile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	return false;
}
...
void Tile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
	ThreadStorage *tls = (ThreadStorage *)TlsGetValue(Tile::tlsIdxShape);
	// Added this so that the TLS shape is correct for this tile
	if(tls->tileId != this->id) updateDefaultShape();
...
{
	// Special case - only record a crop destroy if is fully grown
	if( id==Tile::crops_Id )
	{
	...
	player->awardStat(GenericStats::totalBlocksMined(), GenericStats::param_noArgs());	// WESTY : Added for other award.
...
/*
	These are necessary on the Linux.
		(and 4 and Vita).
*/
