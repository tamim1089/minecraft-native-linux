...
// for the debug option of not removing items
...
bool TileItem::useOn(shared_ptr<ItemInstance> instance, shared_ptr<Player> player, Level *level, int x, int y, int z, int face, float clickX, float clickY, float clickZ, bool bTestUseOnOnly) 
{
    ...
    if (level->mayPlace(tileId, x, y, z, false, face, player)) 
	{
		if(!bTestUseOnOnly)
		{
			...
			{
				// Snow/Iron Golems do not have owners apparently.
				...
				// Java 1.4 change - getStepSound replaced with getPlaceSound
...
#ifndef _FINAL_BUILD
				// If we have the debug option on, don't reduce the number of this item
...
//  to colourise some tile types in the hint popups
