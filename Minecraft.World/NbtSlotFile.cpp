...
NbtSlotFile::NbtSlotFile(File file)
{
	...
	{
		raf = CreateFileA(wstringtofilename(file.getPath()), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        writeHeader();
    }
	else
	{
...
//    if (magic != MAGIC_NUMBER) throw new IOException("Bad magic number: " + magic);		// TODO
    short version;
	ReadFile(raf,&version,2,&numberOfBytesRead,NULL);
//    if (version != 0) throw new IOException("Bad version number: " + version);		// TODO
...
vector<CompoundTag *> *NbtSlotFile::readAll(int slot)
{
	...
	for (AUTO_VAR(it, fileSlots->begin()); it != itEnd; it++)
	{
        ...
        do
		{
			...
			{
                skipped++;
                goto fileSlotLoop;	// used to be continue fileSlotLoop, with for loop labelled as fileSlotLoop
            }

//            if (oldSlot != expectedSlot) throw new IOException("Wrong slot! Got " + oldSlot + ", expected " + expectedSlot);	// TODO
...
// removed - don't see how toReplace can ever have anything in here, and might not be initialised
//    if (toReplace->size() > 0)
//	{
