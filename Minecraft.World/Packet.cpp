...
void Packet::staticCtor()
{
	...
	// Note that item IDs are now defined in virtual method for each packet type

	// The values for canSendToAnyClient may not necessarily be the correct choices
	...
	// we need to send to any client for the sleep in bed
	...
	// we need to send to any client for the wake up from sleeping
	...
	// needs to go to any player, due to the knockback effect when a played is hit
	...
	// needs to go to any player, to create sound effect when a player is hit
	...
	// don't see the need for this, we can use 61
...
#ifndef _CONTENT_PACKAGE
	// We have some debug code that uses this packet to send data back to the server from the client
	...
	map(201, true, true, false, false, typeid(PlayerInfoPacket), PlayerInfoPacket::create); // TODO New for 1.8.2 - Repurposed by map(202, true, true, true, false, typeid(PlayerAbilitiesPacket), PlayerAbilitiesPacket::create);
	// These added 1.3.2, but don't think we need them
...
//  to record data for outgoing packets
...
shared_ptr<Packet> Packet::getPacket(int id) 
{
	...
	//	catch (exception e)
	//	{
	//		// TODO  print stack trace, newInstance doesnt throw an exception in c++ yet.
...
shared_ptr<Packet> Packet::readPacket(DataInputStream *dis, bool isServer) // throws IOException TODO should this declare a throws?
{
	int id = 0;
	...
	// Don't bother tracking stats in a content package
	// This changes a bit in 1.0.1, but we don't really use it so stick with what we have
#ifndef _CONTENT_PACKAGE
#if PACKET_ENABLE_STAT_TRACKING
...
void Packet::writePacket(shared_ptr<Packet> packet, DataOutputStream *dos) // throws IOException TODO should this declare a throws?
{
...
void Packet::writeUtf(const wstring& value, DataOutputStream *dos) // throws IOException TODO should this declare a throws?
{
#if 0
...
wstring Packet::readUtf(DataInputStream *dis, int maxLength) // throws IOException TODO should this declare a throws?
{
...
// Brought these functions forward for enchanting/game rules
