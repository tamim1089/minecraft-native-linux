...
{
#ifndef _LINUX
	ZeroMemory(uids,sizeof(PlayerUID)*MAXIMUM_MAP_SAVE_DATA);
#endif
...
void _MapDataMappings::setMapping(int id, PlayerUID uid, int dimension)
{
	uids[id] = uid;
...
void _MapDataMappings_old::setMapping(int id, PlayerUID uid, int dimension)
{
	uids[id] = uid;
	if( dimension == 0 )
	{
	...
	if (!m_bHasLoadedMapDataMappings && !mapFile.getName().empty() && getSaveFile()->doesFileExist( mapFile ))
	{
...
#ifdef _LINUX
		// This version changed happened before initial release
...
#elif defined(_LINUX)
		// This version changed happened before initial release
		if(getSaveFile()->getSaveVersion() < SAVE_FILE_VERSION_LINUX_CHANGE_MAP_DATA_MAPPING_SIZE)
		{
			// Delete the old file
			if(fileEntry) getSaveFile()->deleteFile( fileEntry );
...
#endif
		{
			...
			{			
				...
				{
					m_saveableMapDataMappings.setMapping(i,oldMapDataMappings.uids[i],oldMapDataMappings.getDimension(i));
				}
			}
			else
			{
...
{
	// removed try/catch.
	PlayerUID playerUid = player->getUid();
#if defined(_LINUX) || defined(_LINUX)
	if( playerUid != INVALID_UID )
#else
	if( playerUid != INVALID_UID && !player->isGuest() )
#endif
	{
		CompoundTag *tag = new CompoundTag();
		player->saveWithoutId(tag);
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
		ConsoleSavePath realFile = ConsoleSavePath( m_saveFile->getPlayerDataFilenameForSave(playerUid).c_str() );
#elif defined(_LINUX)
		ConsoleSavePath realFile = ConsoleSavePath( playerDir.getName() + player->getUid().toString() + L".dat" );
#else
		ConsoleSavePath realFile = ConsoleSavePath( playerDir.getName() + _toString( player->getUid() ) + L".dat" );
#endif
	...
	else if( playerUid != INVALID_UID )
	{
		app.DebugPrintf("Not saving player as their UID is a guest/n");
		dontSaveMapMappingForPlayer(playerUid);
	}
}
...
{
	bool newPlayer = true;
	CompoundTag *tag = loadPlayerDataTag( player->getUid() );
	if (tag != NULL)
	{
		newPlayer = false;
		player->load(tag);
...
CompoundTag *DirectoryLevelStorage::loadPlayerDataTag(PlayerUID uid)
{
	// removed try/catch.
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
	ConsoleSavePath realFile = ConsoleSavePath( m_saveFile->getPlayerDataFilenameForLoad(uid).c_str() );
#elif defined(_LINUX)
	ConsoleSavePath realFile = ConsoleSavePath( playerDir.getName() + uid.toString() + L".dat" );
#else
	ConsoleSavePath realFile = ConsoleSavePath( playerDir.getName() + _toString( uid ) + L".dat" );
#endif
	AUTO_VAR(it, m_cachedSaveData.find(realFile.getName()));
	if(it != m_cachedSaveData.end() )
	{
	...
	if( playerFiles != NULL )
	{
		...
		if(app.DebugSettingsOn() && app.GetGameSettingsDebugMask(ProfileManager.GetPrimaryPad())&(1L<<eDebugSetting_DistributableSave))
		{
			for(unsigned int i = 0; i < playerFiles->size(); ++i )
			{
				...
				wstring uidStr = replaceAll( replaceAll(file->data.filename,playerDir.getName(),L""),L".dat",L"");
#if defined(_LINUX) || defined(_LINUX) || defined(_LINUX)
				PlayerUID uid(uidStr);
#else
				PlayerUID uid = _fromString<PlayerUID>(uidStr);
#endif
				deleteMapFilesForPlayer(uid);
...
int DirectoryLevelStorage::getAuxValueForMap(PlayerUID uid, int dimension, int centreXC, int centreZC, int scale)
{
	int mapId = -1;
	bool foundMapping = false;

#ifdef _LARGE_WORLDS
	AUTO_VAR(it, m_playerMappings.find(uid) );
	...
	if(!foundMapping)
	{
		for(unsigned int i = 0; i < m_usedMappings.length; ++i)
		{
			if(m_usedMappings[i] < 0xFF)
			{
				...
				mapId = (i*8) + offset;
				m_playerMappings[uid].addMapping(mapId, centreXC, centreZC, dimension, scale);
	...
	for(unsigned int i = 0; i < MAXIMUM_MAP_SAVE_DATA; ++i)
	{
		if(m_mapDataMappings.uids[i] == uid && m_mapDataMappings.getDimension(i) == dimension)
		{
			foundMapping = true;
			mapId = i;
			break;
		}
		if( mapId < 0 && m_mapDataMappings.uids[i] == INVALID_UID )
		{
			mapId = i;
		}
	}
	...
	{
		m_mapDataMappings.setMapping(mapId, uid, dimension);
		m_saveableMapDataMappings.setMapping(mapId, uid, dimension);
...
void DirectoryLevelStorage::dontSaveMapMappingForPlayer(PlayerUID uid)
{
#ifdef _LARGE_WORLDS
	AUTO_VAR(it, m_playerMappings.find(uid) );
	if(it != m_playerMappings.end())
	...
	for(unsigned int i = 0; i < MAXIMUM_MAP_SAVE_DATA; ++i)
	{
		if(m_saveableMapDataMappings.uids[i] == uid)
		{
			m_saveableMapDataMappings.setMapping(i,INVALID_UID,0);
		}
	}
#endif
}
...
{
	PlayerUID playerUid = player->getUid();
	if(playerUid != INVALID_UID) deleteMapFilesForPlayer(playerUid);
}

void DirectoryLevelStorage::deleteMapFilesForPlayer(PlayerUID uid)
{
#ifdef _LARGE_WORLDS
	AUTO_VAR(it, m_playerMappings.find(uid) );
	if(it != m_playerMappings.end())
	...
	for(unsigned int i = 0; i < MAXIMUM_MAP_SAVE_DATA; ++i)
	{
		if(m_mapDataMappings.uids[i] == uid)
		{
			changed = true;
			...
			m_mapDataMappings.setMapping(i,INVALID_UID,0);
			m_saveableMapDataMappings.setMapping(i,INVALID_UID,0);
			break;
		}
	}
