...
void SavedDataStorage::set(const wstring& id, shared_ptr<SavedData> data) 
{
	if (data == NULL)
	{
		// TODO throw new RuntimeException("Can't set null data");
		assert( false );
	}
	AUTO_VAR(it, cache.find(id));
	if ( it != cache.end() )
...
int SavedDataStorage::getFreeAuxValueFor(const wstring& id)
{
    ...
    if (!file.getName().empty())
	{
		...
		// TODO This was iterating over the keySet in Java, so potentially we are looking at more items?
...
int SavedDataStorage::getAuxValueForMap(PlayerUID uid, int dimension, int centreXC, int centreZC, int scale)
{
	if( levelStorage == NULL )
	{
		switch(dimension)
	...
	{
		return levelStorage->getAuxValueForMap(uid, dimension, centreXC, centreZC, scale);
	}
}
