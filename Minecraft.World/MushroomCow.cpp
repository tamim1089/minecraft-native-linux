...
MushroomCow::MushroomCow(Level *level) : Cow(level)
{
	...
	this->textureIdx = TN_MOB_RED_COW;//  "/mob/redcow.png";
...
bool MushroomCow::interact(shared_ptr<Player> player)
{
	...
	if (item != NULL && item->id == Item::shears_Id && getAge() >= 0)
	{
		...
		{
			// We don't need to check spawn limits when adding the new cow, as we are removing the MushroomCow
			remove();
...
// added so that mushroom cows have more of a chance of spawning, they can now spawn on mycelium as well as grass - seems a bit odd that they don't already really
...
{
	// added limit to number of animals that can be bred
