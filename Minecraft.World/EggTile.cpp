...
bool EggTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	...
	for (int i = 0; i < 1000; i++)
	{
		...
		if (level->getTile(xt, yt, zt) == 0)
		{
			...
			if(!level->isClientSide)
			{
				...
				// The PC version is wrong as the particles calculated on the client side will point towards a different
			...
			// This code will not work correctly on the client as it will show the particles going in the wrong direction
...
//  for Fix for #77475 - TU9: Content: Art: Dragon egg teleport particle effect isn't present.
