...
Recipes::Recipes() 
{
	...
	// These just don't work with our crafting menu
	...
	// changing the order to the way we want to have things in the crafting menu
	// bed
	...
	// moving into decorations to make the structures list smaller
	...
	// TODO - put these new 1.7.3 items in required place within recipes
	...
	*/
	...
	//  removed System.out.println(recipies->size() + L" recipes");

	// build the array of ingredients required per recipe
	buildRecipeIngredientsArray();
}

// this function has been substantially changed due to the differences with a va_list of classes in C++ and Java
ShapedRecipy *Recipes::addShapedRecipy(ItemInstance *result, ...)
{
	...
	va_start(vl,result);
	// second argument is a list of the types
	// s - string
	// w - string array
	// a - char *
	// c - char
...
void Recipes::addShapelessRecipy(ItemInstance *result,... ) 
{
	...
	for(int i = 0; szTypes[i] != L'\0'; ++i ) 
	{
		...
		case L'z':
			pItemInstance=va_arg(vl,ItemInstance *);
			// original code copies the item instance, copy the pointer isnt the same...
			// TODO
