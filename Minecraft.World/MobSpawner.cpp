...
{
	// Added 1.2.3 but we don't need it as it was only used to access sections
...
const int MobSpawner::tick(ServerLevel *level, bool spawnEnemies, bool spawnFriendlies)
{
	...
	for (unsigned int i = 0; i < MobCategory::values.length; i++)
	{
		...
		// this is now quite different to the java version. We just have global max counts for the level whereas the original has a max per chunk that
		...
		for (AUTO_VAR(it, chunksToPoll.begin()); it != itEndCTP; it++)
		   {
			   ...
			   // don't let this actually create/load a chunk that isn't here already - we'll let the normal updateDirtyChunks etc. processes do that, so it can happen on another thread
...
//						   catch (exception e)
//						   {
//							   // TODO  can't print a stack trace, and the newInstance function doesn't throw an exception just now anyway
//							   //e.printStackTrace();
//							   return count;
//						   }

							// If it is an animal or a monster, don't let any one type of mob represent more than 50% of the total amount of these things. This
						   ...
						   if (mob->canSpawn())
						   {
							   // check if we are going to despawn straight away too, and don't add if we will - otherwise we'll be sending
							   ...
							   if( !mob->removed )
							   {
								   clusterSize++;
								   categoryCount++;
								   mob->setDespawnProtected();	// default to protected against despawning
								   level->addEntity(mob);
								   finalizeMobSettings(mob, level, xx, yy, zz);
								   mob->finalizeMobSpawn();
								   // change here so that we can't ever make more than the desired amount of entities in each priority. In the original java version
...
bool MobSpawner::isSpawnPositionOk(MobCategory *category, Level *level, int x, int y, int z)
{
	...
	{
		// changed to spawn water things only in deep water
		int yo = 0;
		int liquidCount = 0;
		...
		// Sometimes deep water could be just a waterfall, so check that it's wide as well
...
//  TODO This was an array of Class type. I haven't made a base Class type yet, but don't need to
...
//					{
						...
						// there was a classes array here which duplicated the bedEnemies array but have removed it
						mob = dynamic_pointer_cast<Mob>(EntityIO::newByEnumType(bedEnemies[type], level ));
//					}
//					catch (exception e)
//					{
//						// TODO We can't print a stack trace, and newInstance doesn't currently throw an exception anyway
//						//e.printStackTrace();
...
{
	// not for our version. Creates a few too many mobs.
#if 0
