...
bool BedTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
...
void BedTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
	setShape();
}
...
void BedTile::neighborChanged(Level *level, int x, int y, int z, int type)
{
    ...
    } else
	{
        if (level->getTile(x + HEAD_DIRECTION_OFFSETS[direction][0], y, z + HEAD_DIRECTION_OFFSETS[direction][1]) != id)
		{
			...
			{
                Tile::spawnResources(level, x, y, z, data, 0);		// had to add Tile:: here for C++ since this class doesn't have this overloaded method itself
            }
        }
    }
}
...
Pos *BedTile::findStandUpPosition(Level *level, int x, int y, int z, int skipCount)
{
    ...
    for (int step = 0; step <= 1; step++)
	{
        ...
        for (int standX = startX; standX <= endX; standX++)
		{
			...
			{
				// Changed to check isSolidBlockingTile rather than isEmpty for the blocks that we wish to place the player
