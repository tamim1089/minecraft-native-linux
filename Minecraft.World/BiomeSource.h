...
public:
#ifdef _LINUX
	static __int64 findSeed(LevelType *generator, bool* pServerRunning);	// MGH - added pRunning, so we can early out of this on Vita as it can take up to 60 secs							// 
#else
	static __int64 findSeed(LevelType *generator);	// 
#endif
	~BiomeSource();
...
public:
	...
	// changed the interface for these methods, mainly for thread safety
	...
	float scaleTemp(float temp, int y ) const;	// brought forward from 1.2.3
