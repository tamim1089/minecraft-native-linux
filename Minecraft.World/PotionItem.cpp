...
// Based loosely on a function that gets added in java much later on (1.3)
	...
	if (s_uniquePotionValues.empty())
	{
		for (int brew = 0; brew <= PotionBrewing::BREW_MASK; ++brew)
		{
			...
			if (effects != NULL)
			{
				if(!effects->empty())
				{
					// Based on implementation of Java List.hashCode() at http://docs.oracle.com/javase/6/docs/api/java/util/List.html#hashCode()
