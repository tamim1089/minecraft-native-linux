...
public:
	...
	private:
		const int type;
		const int id;
		// there used to be one "value" type here of general type Object, just storing the different (used) varieties
...
public:
	...
	// this function used to be a template, but there's only 3 varieties of use I've found so just hard-coding now, as
	...
	void checkId(int id);	// added to contain common code from overloaded define functions above
	byte getByte(int id);
	...
	Pos *getPos(int id);
	// using overloads rather than template here
