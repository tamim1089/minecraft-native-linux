...
{
	// Just to be sure, make sure the read and write threads terminate themselves before the connection object is destroyed
	running = false;
...
Connection::Connection(Socket *socket, const wstring& id, PacketListener *packetListener) // throws IOException
{
	...
	m_hWakeReadThread = new CThread::Event;
	m_hWakeWriteThread = new CThread::Event;
	...
	readThread = new CThread(runRead, (void*)this, readThreadName, READ_STACK_SIZE);
	writeThread = new CThread(runWrite, this, writeThreadName, WRITE_STACK_SIZE);
...
void Connection::send(shared_ptr<Packet> packet)
{
	...
	{
		//  have delayed it enough by putting it in the slow queue, so don't delay when we actually send it
...
bool Connection::writeTick()
{
	...
	// If the connection is closed and the output stream has been deleted
	...
	if (!outgoing.empty() && (fakeLag == 0 || System::currentTimeMillis() - outgoing.front()->createTime >= fakeLag))
	{
		...
		// Changed this so that rather than writing to the network stream through a buffered stream we want to:
...
{
	// TODO How to interrupt threads? Or do we need to change the multithreaded functions a bit more
...
bool Connection::readTick()
{
	...
	// If the connection has closed and the input stream has been deleted
	if(dis==NULL)
		return didSomething;
...
void Connection::tick()
{
	...
	// Moved this a bit later in the function to stop the race condition of Disconnect packets not being processed when local client leaves
	...
	// Send a KeepAlivePacket every now and then to ensure that our read and write threads don't timeout
	tickCount++;
    if (tickCount % 20 == 0)
	{
	...
	// 1.8.2 changed from 100 to 1000
	int max = 1000;
	...
	EnterCriticalSection(&incoming_cs);
	// If disconnected, then we shouldn't process incoming packets
	...
	// split the following condition (used to be disconnect && iscoming.empty()) so we can wrap the access in a critical section
	if (disconnected)
	{
		EnterCriticalSection(&incoming_cs);
		bool empty = incoming.empty();
		...
		if( empty )
		{
			...
			disconnected = false; // don't keep sending this every tick
		}
	}
}
...
#if 1
	// this used to be in a thread but not sure why, and is causing trouble for us if we kill the connection
...
int Connection::runRead(void* lpParam)
{
	...
	while (con->running && !con->quitting && ShutdownManager::ShouldRun(ShutdownManager::eConnectionReadThreads))
	{
		...
		// try {
		//Sleep(100L);
		// TODO - 1.8.2 changes these sleeps to 2L, but not sure whether we should do that as well
...
int Connection::runWrite(void* lpParam)
{
	...
	// Adding this to force us to run through the writeTick at least once after the event is fired
