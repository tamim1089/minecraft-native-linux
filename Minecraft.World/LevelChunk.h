...
public:
	byteArray biomes; // Made public

	// No longer static in 1.8.2
...
//    byteArray blocks;
	// actual storage for blocks is now private with public methods to access it
private:
...
public:
    ...
    bool loaded;
	unsigned char rainHeights[16*16];		// optimisation brought forward from 1.8.2 (was int arrayb in java though)
	unsigned char columnFlags[16*8];		// lighting update brought forward from 1.8.2, was a bool array but now mixed with other flags in our version, and stored in nybbles
    Level *level;

	// actual storage for data is now private with public methods to access it
private:
...
//    DataLayer *data;
private:
	// actual storage for sky & block lights is now private with new methods to be able to access it.
...
public:
    ...
    short terrainPopulated;			// changed from bool to bitfield within short
	short *serverTerrainPopulated;	// 

	void setUnsaved(bool unsaved); // 
protected:
	// Stopped this being private so we can add some more logic to it
    bool m_unsaved;
...
public:
	void recheckGaps(bool bForce = false);	// added parameter, made public
private:
...
public:
    ...
    virtual bool setData(int x, int y, int z, int val, int mask, bool *maskedBitsChanged);		//  mask
    virtual int getBrightness(LightLayer::variety layer, int x, int y, int z);
	virtual void getNeighbourBrightnesses(int *brightnesses, LightLayer::variety layer, int x, int y, int z);	// 
    ...
    virtual void load();
    virtual void unload(bool unloadTileEntities) ;	// added parameter
#ifdef _LARGE_WORLDS
    ...
    virtual int getBlocksAndData(byteArray *data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting = true);	// added includeLighting parameter
	static void tileUpdatedCallback(int x, int y, int z, void *param, int yparam);		// 
    virtual int setBlocksAndData(byteArray data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting = true);	// added includeLighting parameter
	virtual bool testSetBlocksAndData(byteArray data, int x0, int y0, int z0, int x1, int y1, int z1, int p);	// 
...
#ifdef SHARING_ENABLED
	static CRITICAL_SECTION m_csSharing;	// 
#endif
	// added
...
#endif
	static CRITICAL_SECTION m_csTileEntities;	// added
	...
	int getTopRainBlock(int x, int z); // optimisation brought forward from 1.8.2
	void tick();	// lighting change brought forward from 1.8.2
	ChunkPos *getPos();
