...
class Tile
{
	// Stair tile accesses the protected members of a Tile object passed in
	friend class StairTile;
	friend class ChunkRebuildData;
	friend class WallTile;

protected:
	//  so we can have separate shapes for different threads
	class ThreadStorage
	{
	public:
...
private:
	// Was const but had to change it so that we can initialise it in TileStaticInit
...
// 		wstring stepSound;
    ...
    static  unsigned char _sendTileData[TILE_NUM_COUNT];		// was bool, changed to bitfield so we can indicate which bits are important to be sent
	static  bool	propagate[TILE_NUM_COUNT];

	// this array of simple constants made so the compiler can optimise references to Ids that were previous of the form Tile::<whatever>->id, and are now simply Tile::whatever_Id
...
protected:
	...
	// Removed this in favour of a TLS version
    //double xx0, yy0, zz0, xx1, yy1, zz1;
...
protected:
	...
	virtual ~Tile() {}
protected:
    virtual Tile *sendTileData(unsigned char importantMask=15);		// added importantMask to indicate which bits in the data are important
protected:
    virtual void init();
...
public:
	...
	virtual int getLightColor(LevelSource *level, int x, int y, int z, int tileId=-1);		// brought forward from 1.8.2
    ...
    virtual bool isSolidRender(bool isServerLevel = false);							// Added isServerLevel param
...
public:
	...
	virtual bool use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly = false); //  soundOnly param
    ...
    virtual void updateShape(LevelSource *level, int x, int y, int z, int forceData = -1, shared_ptr<TileEntity> forceEntity = shared_ptr<TileEntity>());	//  forceData, forceEntity param
...
public:
	...
	//  so we can check before we try to add a tile to the tick list if it's actually going to do seomthing
...
public:
	...
	virtual float getShadeBrightness(LevelSource *level, int x, int y, int z);	// brought forward from 1.8.2
