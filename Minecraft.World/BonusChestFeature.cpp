...
// original virtual method
bool BonusChestFeature::place(Level *level, Random *random, int x, int y, int z)
{
	return place(level, random, x, y, z, false );
}

// added force parameter - trying to keep this as similar as possible to the original algorithm, but would also like it to definitely place a
...
bool BonusChestFeature::place(Level *level, Random *random, int x, int y, int z, bool force)
{
    ...
    for (int i = 0; i < 4; i++)
	{
		...
		{
			x2 = x;
			y2 = y - 1;	// the position passed in is actually two above the top solid block, as the calling function adds 1 to getTopSolidBlock, and that actually returns the block above anyway.
