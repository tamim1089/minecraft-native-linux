...
// NOTE - this ignoreUpdate stuff has been removed from the java version, but I'm not currently sure how the java version does without it... there must be
...
// ignoreUpdate is a static in java, implementing as TLS here to make thread safe
...
bool PistonBaseTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	return false;
}
...
void PistonBaseTile::triggerEvent(Level *level, int x, int y, int z, int param1, int facing)
{
    ...
    if (param1 == TRIGGER_EXTEND)
	{
		...
		{
			// it is (currently) critical that this setData sends data to the client, so have added a bool to the method so that it sends data even if the data was already set to the same value
	...
	else if (param1 == TRIGGER_CONTRACT)
	{
        ...
        if (isSticky)
		{
			...
			{
				stopSharingIfServer(level, x + Facing::STEP_X[facing], y + Facing::STEP_Y[facing], z + Facing::STEP_Z[facing]);	// 
				ignoreUpdate(false);
...
void PistonBaseTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
...
bool PistonBaseTile::isPushable(int block, Level *level, int cx, int cy, int cz, bool allowDestroyable)
{
	...
	if( Tile::tiles[block]->isEntityTile() )	// java uses instanceof EntityTile here
	{
...
bool PistonBaseTile::canPush(Level *level, int sx, int sy, int sz, int facing)
{
    ...
    for (int i = 0; i < MAX_PUSH_DEPTH + 1; i++)
	{
		...
		// added to also check for out of bounds in x/z for our finite world
