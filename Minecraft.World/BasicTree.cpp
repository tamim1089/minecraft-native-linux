...
void BasicTree::prepare()
{
	...
	// Rather than copying the array, we are storing the number of valid elements in the array
	...
	// original code for above is the following, it isn't obvious to me why it is doing a copy of the array, so let's not for now
...
void BasicTree::foliageCluster(int x, int y, int z)
{
    ...
    int cury = topy - 1;
    float radius;
	// Generate foliage from the top down so that we don't keep recalculating heightmaps
    while (cury >= y)
	{
...
int BasicTree::checkLine(int *start, int *end)
{
    ...
    // primsign is digit 1 or -1 depending on whether the limb is headed
    // along the positive or negative primidx axis.
    char primsign; // Was byte, but we use in a sum below and byte=unsigned char so we were setting endoffset incorrectly
    if (delta[primidx] > 0) primsign = 1;
    else primsign = -1;
    // Initilize the per-step movement for the non-primary axies.
    double secfac1 = ((double) delta[secidx1]) / ((double) delta[primidx]);
...
bool BasicTree::checkLocation()
{
	...
	//  Added to stop tree features generating areas previously place by game rule generation
