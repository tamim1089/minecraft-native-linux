...
{
	// note that this code seems to basically be a hack to fix a problem where post-processed things like lakes aren't getting lit properly
    ...
    if (material == Material::lava)
	{
        if (level->getMaterial(x, y + 1, z) == Material::air && !level->isSolidRenderTile(x, y + 1, z))
		{
            if (random->nextInt(100) == 0)
			{
				...
				// new sound brought forward from 1.2.3
