...
public:
	...
	// Added for the in-game tooltips
	bool IsHeldItem();
...
public:
	...
	//  added to get the right resource from the inventory for removal
	bool removeResource(int type,int iAuxVal);
	void removeResources(shared_ptr<ItemInstance> item); //  for trading
	
	//  added to the get the item that would be affected by the removeResource functions
...
public:
	...
	int countMatches(shared_ptr<ItemInstance> itemInstance); // 
};