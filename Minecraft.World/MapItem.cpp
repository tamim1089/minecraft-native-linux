...
{	
	...
	{
		// This call comes from ClientConnection, but i don't see why we should be trying to work out
...
shared_ptr<MapItemSavedData> MapItem::getSavedData(shared_ptr<ItemInstance> itemInstance, Level *level)
{
	...
	if (mapItemSavedData == NULL)
	{
		// I don't see why we should be trying to work out the id again when it's passed as a param.
...
void MapItem::inventoryTick(shared_ptr<ItemInstance> itemInstance, Level *level, shared_ptr<Entity> owner, int slot, bool selected)
{
	...
	if (dynamic_pointer_cast<Player>(owner) != NULL) 
	{
		...
		// If the player has a map that belongs to another player, then merge the data over and change this map id to the owners id
		int ownersAuxValue = level->getAuxValueForMap(player->getUid(), data->dimension, data->x, data->z, data->scale);
...
void MapItem::onCraftedBy(shared_ptr<ItemInstance> itemInstance, Level *level, shared_ptr<Player> player) 
{
	...
	itemInstance->setAuxValue(level->getAuxValueForMap(player->getUid(), player->dimension, centreXC, centreZC, mapScale));
