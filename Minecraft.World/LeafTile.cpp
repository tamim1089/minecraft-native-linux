...
// changed interface to have data passed in, and put existing interface as wrapper above
...
void LeafTile::tick(Level *level, int x, int y, int z, Random *random)
{
    ...
    if ((currentData & UPDATE_LEAF_BIT) != 0 && (currentData & PERSISTENT_LEAF_BIT) == 0)
	{
        ...
        if (level->hasChunksAt(x - r2, y - r2, z - r2, x + r2, y + r2, z + r2))
		{
			// Assuming we remain in the same chunk, getTile accesses an array that varies least by y
...
{
	// The server level shouldn't care how the tile is rendered!
