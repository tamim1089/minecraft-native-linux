...
RegionFile::RegionFile(ConsoleSaveFile *saveFile, File *path)
{
	...
	/*  using files instead of strings: 
...
//    try {
	...
	/* Removed as _lastModifed not used and this is always failing as checking wrong thing
	if(  path->exists() )
	{
	...
	*/
	...
	{
		// the original code used to write out 2 empty sectors here, which we don't want to do as we might be at a point where we shouldn't be touching the save file.
	...
	int nSectors;
	if( m_bIsEmpty )		// added this case for our empty files that we now don't create
	{
		nSectors = 2;
	}
	else
	...
	for (int i = 0; i < SECTOR_INTS; ++i)
	{
		...
		if( !m_bIsEmpty )		//  condition, don't read back if we've just created an empty file as we don't immediately write this anymore
		{
...
//        debugln("READ", x, z, "invalid sector");
	...
	// this differs a bit from the java file format. Java has length stored as an int, then a type as a byte, then length-1 bytes of data
...
//        debugln("READ", x, z, "invalid length: " + length + " > 4096 * " + numSectors);
	...
	// was InflaterInputStream in here too, but we've already decompressed
...
{
	// was DeflatorOutputStream in here too, but we've already compressed
...
{
	// Do the compression here so that we know how much space we need to store the compressed data
...
//	app.DebugPrintf(">>>>>>>>>>>>>> writing compressed data for 0x%.8x, %d %d/n",fileEntry->data.regionIndex,x,z);
	...
	//        debug("SAVE", x, z, length, "rewrite");
		...
		else
		{
			...
			// zero this now unused region of the file, so it can be better compressed until it is reused
			zero(sectorNumber, SECTOR_BYTES * sectorsAllocated);

			PIXBeginNamedEvent(0,"Scanning for free space/n");
			/* scan for a free space large enough to store this chunk */
			int runStart = (int)(find(sectorFree->begin(),sectorFree->end(),true) - sectorFree->begin());  // was sectorFree.indexOf(true)
			int runLength = 0;
			if (runStart != -1)
			{
...
void RegionFile::write(int sectorNumber, byte *data, int length, unsigned int compLength)
{
	...
	// We need to do the compression at a level above this, where it is checking for free space

	compLength |= 0x80000000;	// signify that this has been encoded with RLE method ( see code in getChunkDataInputStream() for matching detection of this)
...
// write the initial two sectors that used to be written in the ctor when the file was empty
