...
void FallingTile::_init()
{
	...
	//  so that client-side falling tiles can fall through blocks
...
FallingTile::FallingTile(Level *level, double x, double y, double z, int tile, int data) : Entity( level )
{
	...
	// without this newly created falling tiles weren't interpolating their render positions correctly
	xOld = x;
	yOld = y;
	zOld = z;
}
