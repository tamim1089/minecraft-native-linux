...
void StemTile::tick(Level *level, int x, int y, int z, Random *random)
{
    ...
    if (level->getRawBrightness(x, y + 1, z) >= Level::MAX_BRIGHTNESS - 6)
	{
		...
		// Brought forward change from 1.2.3 to make fruit more likely to grow
        if (random->nextInt((int) (25 / growthSpeed) + 1) == 0)
		{
			...
			else
			{
                ...
                if (dir == 2) zz--;
                if (dir == 3) zz++;
				// Brought forward change from 1.2.3 to not require farmland to grow fruits
...
void StemTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
