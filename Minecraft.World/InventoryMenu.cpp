...
void InventoryMenu::_init(shared_ptr<Inventory> inventory, bool active)
{
	...
	for (int i = 0; i < 4; i++)
	{
		//  I removed an anonymous class that was here whose only purpose seemed to be a way of using the
	...
	slotsChanged();  //  craftSlots parameter, see comment below
}

void InventoryMenu::slotsChanged()  //  to take a shared_ptr<Container> but wasn't using it, so removed to simplify things
{
	MemSect(23);
...
shared_ptr<ItemInstance> InventoryMenu::quickMoveStack(shared_ptr<Player> player, int slotIndex)
{
	...
	if (slot != NULL && slot->hasItem())
	{
		...
		{
			// Brought forward change from 1.2
...
// Added for achievement 'Iron Man'.
