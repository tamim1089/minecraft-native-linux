...
AddPlayerPacket::AddPlayerPacket(shared_ptr<Player> player, PlayerUID uid, PlayerUID OnlineUid,int xp, int yp, int zp, int yRotp, int xRotp, int yHeadRotp) 
{
	id = player->entityId;
	name = player->name;

	// Send "previously sent" value of position as well so that we stay in sync
	x = xp;//Mth::floor(player->x * 32);
	y = yp;//Mth::floor(player->y * 32);
	z = zp;//Mth::floor(player->z * 32);
	// changed - send current "previously sent" value of rotations to put this in sync with other clients
	yRot = yRotp;
	xRot = xRotp;
	yHeadRot = yHeadRotp; // 
...
void AddPlayerPacket::read(DataInputStream *dis) //throws IOException
{
	...
	uid = dis->readPlayerUID();
	OnlineUid = dis->readPlayerUID();
...
int AddPlayerPacket::getEstimatedSize()
{
	...
	{
		// This is an incoming value which we aren't currently analysing
		//iSize += unpack->get
	}
