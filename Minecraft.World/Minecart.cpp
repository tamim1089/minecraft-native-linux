...
void Minecart::_init()
{
	//  This gets replaced again later so should maybe be inited as NULL?
...
bool Minecart::hurt(DamageSource *source, int hurtDamage)
{
	...
	// If someone is riding in this, then it can tick multiple times which causes the damage to
	...
	// Brought froward from 12w36 to fix #46611 - TU5: Gameplay: Minecarts and boat requires more hits than one to be destroyed in creative mode
	...
	{
		// Fixed issue with player falling through the ground on destroying a minecart while riding (issue #160607)
...
void Minecart::tick()
{
	// make minecarts (server-side) tick twice, to put things back to how they were when we were accidently ticking them twice
	for( int i = 0; i < 2; i++ )
	{
		...
		// Fix for #8284 - Gameplay: Collision: Minecart clips into/ through blocks at the end of the track, prevents player from riding
		if (level->isClientSide) // && lSteps > 0)
		{
			...
			return;	// return here stops the client-side version of this from ticking twice
		}
		xo = x;
		yo = y;
		zo = z;
		...
		if (RailTile::isRail(tile))
		{
			...
			//  Is this a good way to copy the bit of the array that we need?
			int exits[2][3];
			...
			if (entities != NULL && !entities->empty())
			{
				...
				for (AUTO_VAR(it, entities->begin()); it != itEnd; it++)
				{
					...
					if (e != rider.lock() && e->isPushable() && e->GetType() == eTYPE_MINECART)
					{
						...
						// We should only be pushed by one minecart per tick, the closest one
...
void Minecart::push(shared_ptr<Entity> e)
{
	...
	if (dd >= 0.0001f)
	{
		...
		if (e->GetType() == eTYPE_MINECART)
		{
			...
			//Brought forward changes to fix minecarts pushing each other
			...
			else
			{
				...
				// Decelerate the cart that is pushing this one if they are too close
				e->xd /= modifier;
				e->zd /= modifier;
...
bool Minecart::interact(shared_ptr<Player> player)
{
	if (type == Minecart::RIDEABLE)
	{
		...
		if (!level->isClientSide)
		{
			// Fixed issue with player not being able to dismount minecart (issue #4455)
