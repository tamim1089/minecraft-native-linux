...
// The java does not have ctor here (being an abstract) but we need one to initialise the member variables
	...
	if ((clickType == CLICK_PICKUP || clickType == CLICK_QUICK_MOVE) && (buttonNum == 0 || buttonNum == 1))
	{
		...
		else if (clickType == CLICK_QUICK_MOVE)
		{
			...
			if(slot != NULL && slot->mayPickup(player))
			{
				...
				if (piiClicked != NULL)
				{
					//int oldSize = piiClicked->count; // Commented 1.8.2 and replaced with below
					...
					if (slot != NULL)
					{
						...
						{
							// Brought forward loopClick from 1.2 to fix infinite recursion bug in creative
		...
		else
		{
			...
			if (slot != NULL)
			{
				...
				//  for dyable armour and combinining damaged items
...
// Brought forward from 1.2 to fix infinite recursion bug in creative
...
void AbstractContainerMenu::slotsChanged()//  to take a shared_ptr<Container> but wasn't using it, so removed to simplify things
{
	broadcastChanges();
}
...
// Brought a few changes in this function forward from 1.2 to make it return a bool
