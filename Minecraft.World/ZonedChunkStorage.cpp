...
// There are changes to this class for 1.8.2, but since we never use it anyway lets not worry about it
...
ZoneFile *ZonedChunkStorage::getZoneFile(int x, int z, bool create)
{
    ...
    if (zoneFiles.find(key) == zoneFiles.end())
	{
		...
		if ( !file.exists() )
		{
			...
			HANDLE ch = CreateFileA(wstringtofilename(file.getPath()), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			CloseHandle(ch);
        }
