...
private:
	...
	DataInputStream *dis;
	DataOutputStream *bufferedDos; //  is the same type of dos the java game has

	DataOutputStream *byteArrayDos; //  dos allows us to write individual packets to the socket
	ByteArrayOutputStream *baos;
	Socket::SocketOutputStream *sos;

	bool running;

	queue<shared_ptr<Packet> > incoming;			// was using synchronizedList...
	CRITICAL_SECTION incoming_cs;		// ... now has this critical section
	queue<shared_ptr<Packet> > outgoing;			// was using synchronizedList - but don't think it is required as usage is wrapped in writeLock critical section
	queue<shared_ptr<Packet> > outgoing_slow;		// was using synchronizedList - but don't think it is required as usage is wrapped in writeLock critical section
	...
	CThread::Event* m_hWakeReadThread;
	CThread::Event* m_hWakeWriteThread;

	DWORD saqThreadID, closeThreadID;

	bool disconnected;
	DisconnectPacket::eDisconnectReason disconnectReason;
	void **disconnectReasonObjects; //  pointer to an array.
...
private:
	...
	// these might be better of as private
