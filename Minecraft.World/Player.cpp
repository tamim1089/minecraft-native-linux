...
{
	// This function call had to be moved here from the Entity ctor to ensure that
	// the derived version of the function is called
	this->defineSynchedData();

	// This function call had to be moved here from the Entity ctor to ensure that the derived version of the function is called
	health = getMaxHealth();
	...
	textureIdx = TN_MOB_CHAR; // was L"/mob/char.png";
...
Player::~Player()
{
	// TODO //printf("A player has been destroyed./n");
...
void Player::releaseUsingItem()
{
	if (useItem != NULL)
	{
		...
		// Fix for various bugs where an incorrect bow was displayed when it broke (#70859,#93972,#93974)
		if (useItem->count == 0)
		{
			removeSelectedItem();
		}
...
void Player::tick()
{
	if (useItem != NULL)
	{
		...
		// Fix for #45508 - TU5: Gameplay: Eating one piece of food will result in a second piece being eaten as well
	...
	if (!level->isClientSide)
	{
		...
		if( count++ == 100 )
		{
...
#endif
#endif
			// Throw items out at the start of the level
			...
			// WESTY : Spawn some wolves to befriend!
			/*
...
#if 0
		// This makes a tunnel with a powered track just over length to get the On A Rail achievement
...
void Player::spawnEatParticles(shared_ptr<ItemInstance> useItem, int count)
{
	...
	if (useItem->getUseAnimation() == UseAnim_eat)
	{
		...
		// Was L"mob.eat" which doesnt exist
...
void Player::setUid(PlayerUID uid)
{
	m_uid = uid;
#ifdef _LINUX

	setUUID( uid.toString() );
#endif
}
...
void Player::setCustomCape(DWORD capeId)
{
	...
	else
	{
		...
		{
			// Cape
			if(pPlayerData->wchCape)
			{
				this->customTextureUrl2= pPlayerData->wchCape;
			}
			else
			{
...
void Player::prepareCustomTextures()
{
	...
	{
		// Skin
		if(pPlayerData->wchSkin)
		{
			this->customTextureUrl= pPlayerData->wchSkin;
		}

		// Don't update the cape here, it gets set elsewhere
		// Cape
		//if(pPlayerData->wchCape)
		//{
		//	this->customTextureUrl2= pPlayerData->wchCape;
...
void Player::rideTick()
{
	...
	{
		// I don't know why we would want to do this, but it means that the players head is locked in position and can't move around
		//xRot = preXRot;
		//yRot = preYRot;
...
void Player::aiStep()
{
	...
	// we were getting a NaN with zero xd & zd
...
float Player::getDestroySpeed(Tile *tile)
{
	...
	// onGround is set to true on the client when we are flying, which means
...
void Player::attack(shared_ptr<Entity> entity)
{
	...
	if (dmg > 0 || magicBoost > 0)
	{
		...
		if (dynamic_pointer_cast<Mob>( entity ) != NULL)
		{
			...
			// Brought forward wasHurt check to Fix 66140 - Bug: Fire Aspect bypasses "Player v Player" being Disabled
...
Player::BedSleepingResult Player::startSleepInBed(int x, int y, int z, bool bTestUse)
{
	if (!level->isClientSide || bTestUse)
	{
		...
		// I'm going to move the position of these tests below
		...
		if (!bTestUse)
		{
			// We still want the tooltip for Sleep
	...
	if(bTestUse)
	{
		// we're just testing use, and we get here, then the bed can be used
		return OK;
	}

	// You can use a bed from within a minecart, and this causes all sorts of problems.
	ride(nullptr);
...
void Player::jumpFromGround()
{
	...
	// This seems to have been missed from 1.7.3, but do we care?
	...
	if (riding != NULL)
	{
		...
		if (distance > 0)
		{
			if ( dynamic_pointer_cast<Minecart>( riding ) )
			{
				...
				// changed this because our world isn't big enough to go 1000m
				else 
				{
					int dist = minecartAchievementPos->dist(Mth::floor(x), Mth::floor(y), Mth::floor(z));
#ifdef _LINUX
					// send the event to cause the progress bar to increase on XB1
...
{
	// added the lavaslime enemy - fix for #64007 - TU7: Code: Achievements: TCR#073: Killing Magma Cubes doesn't unlock "Monster Hunter" Achievement.
...
void Player::causeFoodExhaustion(float amount)
{
	...
	// Added 1.8.2 bug fix (TU6) - If players cannot eat, then their food bar should not decrease due to exhaustion
...
void Player::startUsingItem(shared_ptr<ItemInstance> instance, int duration)
{
	...
	// hook for ItemUsed event, and ironbelly achievement.
...
#if (!defined _LINUX) && (defined _EXTENDED_ACHIEVEMENTS)
...
int Player::getTexture()
{
	...
	case eDefaultSkins_Skin0:
		return TN_MOB_CHAR; // was L"/mob/char.png";
	case eDefaultSkins_Skin1:
		return TN_MOB_CHAR1; // was L"/mob/char1.png";
	case eDefaultSkins_Skin2:
		return TN_MOB_CHAR2; // was L"/mob/char2.png";
	case eDefaultSkins_Skin3:
		return TN_MOB_CHAR3; // was L"/mob/char3.png";
	case eDefaultSkins_Skin4:
		return TN_MOB_CHAR4; // was L"/mob/char4.png";
	case eDefaultSkins_Skin5:
		return TN_MOB_CHAR5; // was L"/mob/char5.png";
	case eDefaultSkins_Skin6:
		return TN_MOB_CHAR6; // was L"/mob/char6.png";
	case eDefaultSkins_Skin7:
		return TN_MOB_CHAR7; // was L"/mob/char7.png";
...
{
	// TODO Should we just be using the pointers and hashing them?
#ifdef _LINUX
	return (int)boost::hash_value( k->name ); // Names are completely unique?
#elif defined(_LINUX)
	return (int)std::hash<std::wstring>()( k->name ); // Linux port: MSVC std::hash_value -> std::hash
#else
	return (int)std::hash_value( k->name ); // Names are completely unique?
#endif //_LINUX
}

bool Player::eq_test(const shared_ptr<Player> x, const shared_ptr<Player> y)
{
	// TODO Should we just be using the pointers and comparing them for equality?
	return x->name.compare( y->name ) == 0; // Names are completely unique?
}
...
bool Player::isAllowedToUse(shared_ptr<ItemInstance> item)
{
	...
	if(item != NULL && app.GetGameHostOption(eGameHostOption_TrustPlayers) == 0)
	{
		...
		// TU8 Players should always be able to eat food items, even if the build option is turned of
		switch(item->id)
		{
			// food
