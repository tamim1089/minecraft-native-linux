...
void MobSpawnerTileEntity::tick()
{
	...
	else
	{
		...
		for (int c = 0; c < spawnCount; c++) 
		{
			...
			int nearBy = (int)vecNearby->size(); //IB, TODO, Mob contains no getClass
			delete vecNearby;
			...
			// our mobspawner tiles should only be spawning monsters. Also respect the global limits we have for those so we don't go
