...
void Fireball::tick()
{
	...
	if (!level->isClientSide)
	{
		...
		{
			// TU9 bug fix - fireballs can hit the edge of the world, and stay there
	...
	for (AUTO_VAR(it, objects->begin()); it != itEnd; it++)
	{
        ...
        if (!e->isPickable() || (e->is(owner) )) continue; //Never collide with the owner (Enderdragon) // && flightTime < 25)) continue;
	...
	// bug fix for the fireballs in a saved game - they are saved with no/very small velocity, so end up hanging around in the air
	if (!level->isClientSide)
	{
