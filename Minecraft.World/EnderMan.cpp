...
EnderMan::EnderMan(Level *level) : Monster( level )
{
	...
	this->textureIdx = TN_MOB_ENDERMAN; //  "/mob/enderman.png";
	runSpeed = 0.2f;
	attackDamage = 7;
	setSize(0.6f, 2.9f);
	footSize = 1;
...
void EnderMan::aiStep()
{
	...
	if (!level->isClientSide)
	{
		if (getCarryingTile() == 0)
		{
			if (random->nextInt(20) == 0)
			{
				...
				if(EnderMan::MAY_TAKE[t]) // Brought forward from 1.2.3
				{
	...
	// Brought forward particles from 1.2.3
	for (int i = 0; i < 2; i++)
	{
	...
	if (level->isDay() && !level->isClientSide)
	{
		...
		if (br > 0.5f)
		{
			...
			{
				// Brought forward behaviour change from 1.2.3
				//onFire = 20 * 15;
	...
	//  forward behaviour change from 1.2.3
...
{
	// brought sound change forward from 1.2.3
...
//  forward from 1.2.3 to help fix Enderman behaviour
