...
Slime::Slime(Level *level) : Mob( level )
{
	...
	this->textureIdx = TN_MOB_SLIME; //  L"/mob/slime.png";
...
void Slime::tick()
{
	...
	// Brought forward from 1.3 in TU7 to fix lava slime render
...
void Slime::serverAiStep() 
{
	...
	if (onGround && jumpDelay-- <= 0) 
	{
		...
		//  TU7 to bring forward change to fix lava slime render in MP
		//targetSquish = 1;
...
bool Slime::canSpawn()
{
	...
	Random *lcr = lc->getRandom(987234911l);	// separated out so we can delete
