...
LoginPacket::LoginPacket()
{
	...
	this->m_offlineUid = INVALID_UID;
	this->m_onlineUid = INVALID_UID;
...
// Client -> Server
LoginPacket::LoginPacket(const wstring& userName, int clientVersion, PlayerUID offlineUid, PlayerUID onlineUid,  bool friendsOnlyUGC, DWORD ugcPlayersVersion, DWORD skinId, DWORD capeId, bool isGuest)
{
	...
	this->m_offlineUid = offlineUid;
	this->m_onlineUid = onlineUid;
...
void LoginPacket::read(DataInputStream *dis) //throws IOException 
{
	...
	m_offlineUid = dis->readPlayerUID();
	m_onlineUid = dis->readPlayerUID();
...
void LoginPacket::write(DataOutputStream *dos) //throws IOException 
{
	...
	dos->writePlayerUID(m_offlineUid);
	dos->writePlayerUID(m_onlineUid);
