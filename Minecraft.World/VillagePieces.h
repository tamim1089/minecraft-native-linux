...
public:
	...
	// added to replace use of Class<? extends VillagePiece> within this class
	enum EPieceClass
	{
    ...
    class PieceWeight {
	public:
		EPieceClass pieceClass;		// EPieceClass was Class<? extends VillagePiece>
        const int weight;
        int placeCount;
        int maxPlaceCount;

        PieceWeight(EPieceClass pieceClass, int weight, int maxPlaceCount);	// EPieceClass was Class<? extends VillagePiece>
        bool doPlace(int depth);
        bool isValid();
    };

    static list<PieceWeight *> *createPieceSet(Random *random, int villageSize);		// was ArrayList

	class StartPiece;
private:
	class VillagePiece;
	static int updatePieceWeight(list<PieceWeight *> *currentPieces);		// = was array list
...
private:
	...
	protected:
		...
		static bool isOkBox(BoundingBox *box, StartPiece *startRoom); //  startRoom param
...
public:
	...
	public:
		...
		list<PieceWeight *> *pieceSet;		// was ArrayList
		Level *m_level;

        // these queues are used so that the addChildren calls are
        // called in a random order
		vector<StructurePiece *> pendingHouses;		// was ArrayList
		vector<StructurePiece *> pendingRoads;		// was ArrayList
		
		StartPiece(BiomeSource *biomeSource, int genDepth, Random *random, int west, int north, list<PieceWeight *> *pieceSet, int villageSize, Level *level); //  level param
		virtual ~StartPiece();
