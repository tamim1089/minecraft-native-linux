...
{
	id = e->entityId;
	// We should add entities at their "last sent" position so that the relative update packets
	...
	this->type = type;
	this->data = data;
	if (data > -1)	// changed "no data" value to be -1, we can have a valid entity id of 0
	{
		double xd = e->xd;
		double yd = e->yd;
		double zd = e->zd;
...
{
	_init(e, type, -1, xp, yp, zp, yRotp, xRotp); // changed "no data" value to be -1, we can have a valid entity id of 0
}
...
void AddEntityPacket::read(DataInputStream *dis) // throws IOException  TODO  add throws statement
{
	id = dis->readShort();
	type = dis->readByte();
#ifdef _LARGE_WORLDS
...
void AddEntityPacket::write(DataOutputStream *dos) // throws IOException TODO  add throws statement
{
	dos->writeShort(id);
	dos->writeByte(type);
#ifdef _LARGE_WORLDS
