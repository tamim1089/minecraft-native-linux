...
//    if (bb.length != size) throw new IllegalArgumentException("Expected " + size + " bytes, got " + bb.length);	// TODO
...
ByteBuffer *ZoneIo::read(int size)
{
	...
	SetFilePointer(channel,(int)pos,NULL,NULL);
    ByteBuffer *buff = ByteBuffer::wrap(bb);
	// to investigate - why is this buffer flipped before anything goes in it?
    buff->order(ZonedChunkStorage::BYTEORDER);
    buff->position(size);
    buff->flip();
