...
void Mob::_init()
{
	...
	MemSect(31);
	hasHair = true;
	textureIdx = TN_MOB_CHAR;	//  L"/mob/char.png";
	allowAlpha = true;
	rotOffs = 0;
	modelName = L"";
	bobStrength = 1;
...
Mob::Mob( Level* level) : Entity(level)
{
	...
	// This will not call the correct derived function, so moving to each derived class
	//health = getMaxHealth();
	health = 0;
...
{	
	...
	if (deathTime == 20) 
	{
		// Added level->isClientSide check from 1.2 to fix XP orbs being created client side
...
{
	// Reworked this function a bit to show hurt damage on the client before the server responds.
	// Fix for #8823 - Gameplay: Confirmation that a monster or animal has taken damage from an attack is highly delayed
	// Change to the fix to only show damage when attacked, rather than collision damage
	// Fix for #10299 - When in corners, passive mobs may show that they are taking damage.
	// Change to the fix for TU6, as source is never NULL due to changes in 1.8.2 to what source actually is
	...
	{
		// for new achievement Stayin'Frosty, TODO merge with Java version.
	...
	if (sound)
	{
		...
		{
			hurtDir = (float) (int) ((Math::random() * 2) * 180); //  cast is the same as Java
		}
	}
...
void Mob::causeFallDamage(float distance) 
{
	...
	if (dmg > 0) 
	{
		// new sounds here brought forward from 1.2.3
		if (dmg > 4)
		{
...
// added for more accurate lighting of mobs. Takes a weighted average of all tiles touched by the bounding volume of the entity - the method in the Entity class (which used to be used for
...
void Mob::aiStep() 
{
	...
	if (lSteps > 0) 
	{
		...
		// this collision is carried out to try and stop the lerping push the mob through the floor,
...
bool Mob::canSpawn() 
{
	// altered to use special containsAnyLiquid variant
...
void Mob::handleEntityEvent(byte id) 
{
	if (id == EntityEvent::HURT) 
	{
		...
		MemSect(31);
		// added because villagers have no sounds
...
//  so we can not render mobs before their chunks are loaded - to resolve bug 10327 :Gameplay: NPCs can spawn over chunks that have not yet been streamed and display jitter.
...
{
	// I have no idea what was going on here (it gets changed in a later java version).
