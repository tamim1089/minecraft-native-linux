...
SignTileEntity::~SignTileEntity()
{
	// TODO ORBIS_STUBBED;
#ifndef _LINUX
	// we don't need to verify strings anymore - InputManager.CancelQueuedVerifyStrings(&SignTileEntity::StringVerifyCallback,(LPVOID)this);
#endif
}
...
void SignTileEntity::load(CompoundTag *tag)
{
	...
	// Fix for #13531 - Bug: Signs do not Censor after loading a save
...
void SignTileEntity::setChanged()
{
	...
	// For TU14 we are allowed to not verify strings anymore !
	m_bVerified=true;
	/*
...
int SignTileEntity::StringVerifyCallback(LPVOID lpParam,STRING_VERIFY_RESPONSE *pResults)
{
	...
	if(!pClass->level->isClientSide)
	{
		...
		// This callback gets called on the main thread, but tried to access things on the server thread. Change to go through the protected method.
