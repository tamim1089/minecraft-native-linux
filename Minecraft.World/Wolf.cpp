...
Wolf::Wolf(Level *level) : TamableAnimal( level )
{
	...
	this->textureIdx = TN_MOB_WOLF;	// was L"/mob/wolf.png";
...
int Wolf::getTexture() 
{
	if (isTame()) 
	{
		return TN_MOB_WOLF_TAME;	//  L"/mob/wolf_tame.png";
	}
	if (isAngry()) 
	{
		return TN_MOB_WOLF_ANGRY;	//  L"/mob/wolf_angry.png";
	}
			...
			if(dynamic_cast<FoodItem *>(Item::items[item->id]) != NULL)
			{
				...
				if (food->isMeat())
				{
					if(entityData->getInteger(DATA_HEALTH_ID) < MAX_HEALTH)
					{
						...
						// don't lose the bone in creative mode
	...
	else
	{
		if (item != NULL && item->id == Item::bone->id && !isAngry()) 
		{
			...
			if (!level->isClientSide) 
			{
				if (random->nextInt(3) == 0) 
				{
					// WESTY: Added for new acheivements.
		...
		// stop wild wolves going in to Love Mode (even though they do on Java, but don't breed)
...
{
	// changed - was 8 but we have a limit of only 8 wolves in the world so doesn't seem right potentially spawning them all in once cluster
	return 4;
}
...
{
	// added limit to wolves that can be bred
