...
shared_ptr<ItemInstance> ItemInstance::remove(int count) 
{
	...
	//  Fix for duplication glitch, make sure that item count is in range
	if(this->count <= 0)
	{
		this->count = 0;
	}
...
void ItemInstance::hurt(int i, shared_ptr<Mob> owner)
{
 	...
 	if (i > 0 && player != NULL)
	{
		...
		// If it's the clientside level, then always assume that no damage is done. This stops the case where the client random
 	...
 	// Changed in TU6 to not damage items in creative mode
...
// Added this as we need it in the recipe code
...
// Added this for the one time when we compare with a non-shared pointer
...
{
	// I don't think this function is ever used. It if is, it should probably return shared_from_this()
	assert(false);
    return this;
}
...
wstring ItemInstance::toString() 
{
	...
	// TODO - temp fix until ore recipe issue is fixed
...
// Brought forward these functions for enchanting/game rules
...
// to show strength on potions
bool ItemInstance::hasPotionStrengthBar() 
{
	// exclude a bottle of water from this
	if((id==Item::potion_Id) && (auxValue !=0))// && (!MACRO_POTION_IS_AKWARD(auxValue)))  leaving the bar on an awkward potion so we can differentiate it from a water bottle
	{
		return true;
	}
