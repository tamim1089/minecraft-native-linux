...
bool OreFeature::place(Level *level, Random *random, int x, int y, int z)
{
	...
	if( app.getLevelGenerationOptions() != NULL )
	{
		...
		// Optimise schematic intersection checks by first checking the max possible bounding box of this place call
		int minX = x0 - 1;
		int minY = y0 - 1;
		int minZ = z0 - 1;
    ...
    for (int d = 0; d <= count; d++)
	{
		...
		//  Added to stop ore features generating areas previously place by game rule generation
                    ...
                    if (xd * xd + yd * yd < 1)
					{
                        for (int z2 = zt0; z2 <= zt1; z2++)
						{
                            ...
                            if (xd * xd + yd * yd + zd * zd < 1)
							{
								...
								{
									level->setTileNoUpdateNoLightCheck(x2, y2, z2, tile);		//  from setTileNoUpdate
								}
                            }
                        }
                    }
