...
#else
	BOOL result = DeleteFileA( wstringtofilename(getPath()) );
#endif
	if( result == 0 )
	{
...
#else
	return CreateDirectoryA( wstringtofilename(getPath()),  NULL) != 0;
#endif
}
...
#else
		if( GetFileAttributesA(  wstringtofilename(pathToHere) ) == -1 )
		{
			DWORD result = CreateDirectoryA( wstringtofilename(pathToHere),  NULL);
			if( result == 0 )
			{
				// Failed to create
				return false;
...
{
	// TODO Possible we could get an error result from something other than the file not existing?
#ifdef _UNICODE
	return GetFileAttributes(  getPath().c_str() ) != -1;

#else
	return GetFileAttributesA(  wstringtofilename(getPath()) ) != -1;
#endif
}
...
{
	// The wstringtofilename function returns a pointer to the same location in memory every time it is
...
vector<File *> *File::listFiles() const
{
	...
	// TODO Also need to check for I/O errors?
	if( !isDirectory() )
		return vOutput;
...
#else
	return exists() && ( GetFileAttributesA( wstringtofilename(getPath()) ) & FILE_ATTRIBUTE_DIRECTORY) == FILE_ATTRIBUTE_DIRECTORY;
#endif
}
...
//  JEV, a better hash function may be nessesary.
