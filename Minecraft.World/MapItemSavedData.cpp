...
void MapItemSavedData::tickCarriedBy(shared_ptr<Player> player, shared_ptr<ItemInstance> item)
{
	...
	// Put this block back in if you want to display entity positions on a map (see below)
#if 0
	...
	for (AUTO_VAR(it, carriedBy.begin()); it != carriedBy.end(); )
	{
		...
		// Players in the same dimension as an item frame with a map need to be sent this data, so don't remove them
		...
		else
		{
			...
			// Put this block back in if you want to display entity positions on a map (see above as well)
#if 0
			...
			// display all the players in the map
				...
				for(AUTO_VAR(it3, players->players.begin()); it3 != players->players.end(); ++it3)
				{
					...
					if(decorationPlayer!=NULL && decorationPlayer->dimension == this->dimension)
					{
...
#endif
						{
							...
							// As we have added new icons for players on a new row below
