...
#if (defined _EXTENDED_ACHIEVEMENTS) && (!defined _LINUX)
...
void Stats::staticCtor()
{
	...
	// don't want the time played going to the server
...
// These stats are directly followed by the achievemnts in the profile data, so cannot be changed without migrating the profile data
	...
	// Added this function to allow us to add news stats from TU9 onwards
...
void Stats::buildCraftableStats()
{
	...
	// The following stats were added as it was too easy to cheat the leaderboards by dropping and picking up these items
...
void Stats::buildAdditionalStats()
{
...
#if (defined _EXTENDED_ACHIEVEMENTS) && (!defined _LINUX)
	{
		...
		// We don't need itemsCollected(emerald) so I'm using it to
		...
		// WHY ON EARTH DO THESE ARRAYS HAVE TO BE SO PAINFULLY LARGE WHEN THEY ARE GOING TO BE MOSTLY EMPTY!!!
