...
// WESTY : Added for time played stats.
#include "net.minecraft.stats.h"

// Caching of lighting data added. This is implemented as a 16x16x16 cache of ints (ie 16K storage in total). The index of the element to be used in the array is determined by the lower
...
// added following 2 functions to move instaBuild flag from being a class member, to TLS
...
void Level::_init()
{
	...
	toCheckLevel = new int[ 32 * 32 * 32];	// brought forward from 1.8.2
	InitializeCriticalSectionAndSpinCount(&m_checkLightCS, 5120);	// added for 1.8.2 lighting
	...
	: seaLevel(constSeaLevel)
{
	...
	if( !this->levelData->useNewSeaLevel() ) seaLevel = Level::genDepth / 2;		// sea level is one unit lower since 1.8.2, maintain older height for old levels
	...
	dimension->init(this);
	chunkSource = NULL;	// added flag so chunk source can be called from derived class instead
...
void Level::_init(shared_ptr<LevelStorage>levelStorage, const wstring& levelName, LevelSettings *levelSettings, Dimension *fixedDimension, bool doCreateChunkSource)
{
	...
	//  TU9 as getDimensions was never accurate. This path was never used anyway as we always set fixedDimension
	...
	chunkSource = doCreateChunkSource ? createChunkSource() : NULL;	// added flag so chunk source can be called from derived class instead
...
Level::~Level()
{
	...
	// savedDataStorage is shared between overworld and nether levels in the server, so it will already have been deleted on the first level delete
	...
	// At least one of the listeners is something we cannot delete, the LevelRenderer
	/*
...
{
	// was breaking spawning as not finding ground in superflat worlds
...
bool Level::setTileAndDataNoUpdate(int x, int y, int z, int tile, int data, bool informClients)
{
	...
	// changes for lighting brought forward from 1.8.2
	bool result;
...
void Level::setData(int x, int y, int z, int data, bool forceUpdate/*=false*/)	//  forceUpdate
{
...
bool Level::setDataNoUpdate(int x, int y, int z, int data)
{
	...
	int cx = x & 15;
	int cz = z & 15;
	// have changed _sendTileData to encode a bitfield of which bits are important to be sent. This will be zero where the original flag was false, and non-zero where the original
...
// update brought forward from 1.8.2
int Level::getBrightnessPropagate(LightLayer::variety layer, int x, int y, int z, int tileId)
{
	...
	{
		// The java LightLayer was an enum class type with a member "surrounding" which is what we
...
{
	// optimised. Not doing checks on x/z that are no longer necessary, and directly checking the cache within
...
//  as optimisation - if all the neighbouring brightesses are going to be in the one chunk, just get
...
void Level::setBrightness(LightLayer::variety layer, int x, int y, int z, int brightness, bool noUpdateOnClient/*=false*/)		//  noUpdateOnClient
{
	if (x < -MAX_LEVEL_SIZE || z < -MAX_LEVEL_SIZE || x >= MAX_LEVEL_SIZE || z >= MAX_LEVEL_SIZE)
	{
		return;
...
void Level::playSound(shared_ptr<Entity> entity, int iSound, float volume, float pitch)
{
	...
	for (AUTO_VAR(it, listeners.begin()); it != itEnd; it++)
	{
		// if the entity is a local player, don't play the sound
...
bool Level::addEntity(shared_ptr<Entity> e)
{
	...
	if (forced || hasChunk(xc, zc))
	{
		if (dynamic_pointer_cast<Player>( e ) != NULL)
		{
			...
			// Added so we don't continually add the player to the players list while they are dead
...
void Level::removeEntity(shared_ptr<Entity> e)
{
	...
	if (dynamic_pointer_cast<Player>( e ) != NULL)
	{
		...
		updateSleepingPlayerList();
		playerRemoved(e);	// this will let the entity tracker know that we have actually removed the player from the level's player list
	}
}
...
// added noEntities and blockAtEdge parameter
AABBList *Level::getCubes(shared_ptr<Entity> source, AABB *box, bool noEntities, bool blockAtEdge)
{
	...
	for (int x = x0; x < x1; x++)
		for (int z = z0; z < z1; z++)
		{
			// If we are outside the map, return solid AABBs (rock is a bit of an arbitrary choice here, just need a correct AABB)
	...
	// also stop player falling out of the bottom of the map if blockAtEdge is true. Again, rock is an arbitrary choice here
	// Don't stop entities falling into the void while in The End (it has no bedrock)
	...
	// final bounds check - limit vertical movement so we can't move above maxMovementHeight
	...
	// now add in collision for any blocks which have actually been removed, but haven't had their render data updated to reflect this yet. This is to stop the player
...
#ifndef _LINUX
	// Headless server: destroyedTileManager is a client-render visual-consistency concern
	// (collision vs removed-but-still-rendered tiles); no renderer exists headless.
...
// Brought forward from 12w36 to fix #46282 - TU5: Gameplay: Exiting the minecart in a tight corridor damages the player
...
//change brought forward from 1.8.2
...
{
	/*
	*  removed line below - notch committed 1.6.6 with the incorrect
	...
	*/
	...
	//  if so we can override timeOfDay without changing the time that affects ticking of things
...
{
	// optimisation brought forward from 1.8.2 - used to do full calculation here but result is now cached in LevelChunk
...
void Level::tickEntities()
{
	...
	/* using an iterator causes problems here as
	...
	for (unsigned int i = 0; i < entities.size(); )
	{
		...
		if (e->removed)
		{
			...
			//  the entity again before deleting, as things might have moved in the entity array eg
...
// Stuart  - check this is correct here
...
// added this to be used during mob spawning, and it returns true if there's any liquid in the bounding box, or might be because
...
float Level::getSeenPercent(Vec3 *center, AABB *bb)
{
	...
	int hits = 0;
	int count = 0;
	for (double xx = 0; xx <= 1; xx += xs) // xx, yy and zz were floats, made them doubles to remove warnings
...
bool Level::isSolidRenderTile(int x, int y, int z)
{
	...
	// addition here to make rendering big blocks of leaves more efficient. Normally leaves never consider themselves as solid, so
...
void Level::buildAndPrepareChunksToPoll()
{
...
#else
	// rewritten to add chunks interleaved by player, and to add them from the centre outwards. We're going to be
	...
	// Added 1.2.3, but not sure if we want to do it
...
{
	//lc->tick();	// brought this lighting update forward from 1.8.2
	...
	if (delayUntilNextMoodSound == 0)
	{
		...
		if (id == 0 && this->getDaytimeRawBrightness(x, y, z) <= random->nextInt(8) && getBrightness(LightLayer::Sky, x, y, z) <= 0)
		{
			...
			{
				// Fixed issue with cave audio event having 2 sounds at 192k
#ifdef _LINUX
...
void Level::checkLight(int x, int y, int z, bool force, bool rootOnlyEmissive)		//  force, rootOnlyEmissive parameters
{
...
int Level::getExpectedSkyColor(lightCache_t *cache, int oc, int x, int y , int z, int ct, int block)
{
	...
	if( block == 255 ) return 0;		//  as optimisation
	...
	else
	{
		...
		// changed this to attempt to get all 6 brightnesses of neighbours in a single call, as an optimisation
		int b[6];
...
int Level::getExpectedBlockColor(lightCache_t *cache, int oc, int x, int y, int z, int ct, int block, bool propagatedOnly)
{
	...
	if( block >= 15 ) return expected;	//  as optimisation
...
// Made changes here so that lighting goes through a cache, if enabled for this thread
void Level::checkLight(LightLayer::variety layer, int xc, int yc, int zc, bool force, bool rootOnlyEmissive)
{
	...
	if( force )
	{
		// special mode added so we can do lava lighting updates without having all neighbouring chunks loaded in
		if (!hasChunksAt(xc, yc, zc, 0)) return;
	}
	else
	{
		// this is normal java behaviour
...
//	if( cache ) XLockL2(XLOCKL2_INDEX_TITLE, cache, 128 * 1024, XLOCKL2_LOCK_SIZE_1_WAY, 0 );
	
    {
		...
		{
			// added tcn. This is the code that is run when checkLight has been called for a light source that has got darker / turned off.
            ...
            if (layer == LightLayer::Block || true)
			{
                ...
                while (tcp < tcc)
				{
                    ...
                    if (o == cexp)
					{
                        ...
                        // cexp--;		// removed, change from 1.2.3
                        if (cexp > 0)
						{
                            int xd = x - xc;
                            int yd = y - yc;
                            ...
                            if (xd + yd + zd < 17)
							{
                                ...
                                for (int j = 0; j < 6; j++)
								{
									...
									// added - don't let this lighting creep out of the normal fixed world and into the infinite water chunks beyond
									...
									// some changes here brought forward from 1.2.3
									int block = getBlockingCached(cache, layer, NULL, xx, yy, zz);
                                    if (block == 0) block = 1;
                                    if ((o == cexp - block) && (tcc < (32 * 32 * 32))) // 32 * 32 * 32 was toCheck.length
									{
                                        toCheck[tcc++] = (((xx - xc) + 32)) + (((yy - yc) + 32) << 6) + (((zz - zc) + 32) << 12) + ((cexp - block) << 18);
									}
									else
									{
										// added - keep track of which tiles form the edge of the region we are zeroing
										if( o > ( cexp - block ) )
										{
											edge = true;
										}
								...
								// added - keep track of which tiles form the edge of the region we are zeroing - can store over the original elements in the array because tcn must be <= tcp
								if( edge == true )
								{
									toCheck[tcn++] = p;
								}
			...
			tcp = 0;
//			darktcc = tcc;	///////////////////////////////////////////////////
			tcc = tcn;	// we've moved all the edge tiles to the start of the array, so only need to process these now. The original processes all tcc tiles again in the next section
        }
    }
    ...
    while (tcp < tcc)
	{
        ...
        if (expected != c)
		{
            ...
            if (expected > c)
			{
                ...
                if (yd < 0) yd = -yd;
                if (zd < 0) zd = -zd;
                if (xd + yd + zd < 17 && tcc < (32 * 32 * 32) - 6)		// 32 * 32 * 32 was toCheck.length
				{
					// added extra checks here to stop lighting updates moving out of the actual fixed world and into the infinite water chunks
...
// added - more limited (but faster) version of above, used to count water animals, animals, monsters for the mob spawner
...
void Level::addEntities(vector<shared_ptr<Entity> > *list)
{
	...
	for (AUTO_VAR(it, list->begin()); it != itEnd; it++)
	{
		...
		// Special change to remove duplicate enderdragons that a previous bug might have produced
...
shared_ptr<Player> Level::getNearestPlayer(double x, double y, double z, double maxDist, double maxYDist /*= -1*/)
{
	...
	for (AUTO_VAR(it, players.begin()); it != itEnd; it++)
	{
		...
		// Added check that this player is still alive
...
shared_ptr<Player> Level::getNearestAttackablePlayer(double x, double y, double z, double maxDist)
{
	...
	for (AUTO_VAR(it, players.begin()); it != itEnd; it++)
	{
		...
		// Added check that this player is still alive and privilege check
...
{
	// WESTY : Added to track game time played by players for other awards.
...
// changed to use new method of getting biomedata that caches results of rain & snow
...
int Level::getAuxValueForMap(PlayerUID uid, int dimension, int centreXC, int centreZC, int scale)
{
	return savedDataStorage->getAuxValueForMap(uid, dimension, centreXC, centreZC, scale);
}
...
// determine if a chunk has been done the post-post-processing stage. This happens when *its* neighbours have each been post-processed, and does some final lighting that can
...
// returns true if a chunk is fully, fully finalised - in that it can be sent to another machine. This is the case when all 8 neighbours of this chunk
...
bool Level::canCreateMore(eINSTANCEOF type, ESPAWN_TYPE spawnType)
{
	...
	if(spawnType == eSpawnType_Egg)
	{
		...
		case eTYPE_VILLAGER:
			...
			max = MobCategory::MAX_LINUX_VILLAGERS_WITH_SPAWN_EGG;
			break;
		case eTYPE_CHICKEN:
			count = countInstanceOf( eTYPE_CHICKEN, true);
			max = MobCategory::MAX_LINUX_CHICKENS_WITH_SPAWN_EGG;
			break;
		case eTYPE_WOLF:
			count = countInstanceOf( eTYPE_WOLF, true);
			max = MobCategory::MAX_LINUX_WOLVES_WITH_SPAWN_EGG;
			break;
		case eTYPE_MUSHROOMCOW:
			count = countInstanceOf( eTYPE_MUSHROOMCOW, true);
			max = MobCategory::MAX_LINUX_MUSHROOMCOWS_WITH_SPAWN_EGG;
			break;
		case eTYPE_SQUID:
			count = countInstanceOf( eTYPE_SQUID, true);
			max = MobCategory::MAX_LINUX_SQUIDS_WITH_SPAWN_EGG;
			break;
		case eTYPE_SNOWMAN:
			count = countInstanceOf( eTYPE_SNOWMAN, true);
			max = MobCategory::MAX_SNOWMEN;
			break;
		case eTYPE_VILLAGERGOLEM:
			count = countInstanceOf( eTYPE_VILLAGERGOLEM, true);
			max = MobCategory::MAX_IRONGOLEM;
			break;
		default:
			if((type & eTYPE_ANIMALS_SPAWN_LIMIT_CHECK) == eTYPE_ANIMALS_SPAWN_LIMIT_CHECK)
			{
				...
				max = MobCategory::MAX_LINUX_ANIMALS_WITH_SPAWN_EGG;
			}
			else if( (type & eTYPE_MONSTER) == eTYPE_MONSTER)
			{
				count = countInstanceOf( eTYPE_MONSTER, false);
				max = MobCategory::MAX_LINUX_MONSTERS_WITH_SPAWN_EGG;
			}
		};
	}
	else if(spawnType == eSpawnType_Breed)
	{
		...
		case eTYPE_CHICKEN:
			count = countInstanceOf( eTYPE_CHICKEN, true);
			max = MobCategory::MAX_LINUX_CHICKENS_WITH_BREEDING;
			break;
		case eTYPE_WOLF:
			count = countInstanceOf( eTYPE_WOLF, true);
			max = MobCategory::MAX_LINUX_WOLVES_WITH_BREEDING;
			break;
		case eTYPE_MUSHROOMCOW:
			count = countInstanceOf( eTYPE_MUSHROOMCOW, true);
			max = MobCategory::MAX_LINUX_MUSHROOMCOWS_WITH_BREEDING;
			break;
		default:
			if((type & eTYPE_ANIMALS_SPAWN_LIMIT_CHECK) == eTYPE_ANIMALS_SPAWN_LIMIT_CHECK)
			{
				count = countInstanceOf( eTYPE_ANIMALS_SPAWN_LIMIT_CHECK, false);
				max = MobCategory::MAX_LINUX_ANIMALS_WITH_BREEDING;
			}
			else if( (type & eTYPE_MONSTER) == eTYPE_MONSTER)
			{
