...
void EnderDragon::_init()
{
	...
	// Added for new dragon behaviour
...
EnderDragon::EnderDragon(Level *level) : BossMob(level)
{
	...
	neck = shared_ptr<BossMobPart>( new BossMobPart(this, L"neck", 6, 6) ); // 
	...
	this->textureIdx = TN_MOB_ENDERDRAGON; //  "/mob/enderdragon/ender.png";
	setSize(16, 8);
...
void EnderDragon::aiStep()
{
	...
	{
		// If saved when dead we need to make sure that the actual health is updated correctly on the client
	...
	if (level->isClientSide)
	{
		if (lSteps > 0)
		{
			...
			// The movement is so small that this head animation doesn't look good
			//if( getSynchedAction() == e_EnderdragonAction_Sitting_Flaming )
			//{
...
// 			double zTargetO = zTarget;
	...
	head->bbWidth = head->bbHeight = 1; // Replaced what was "head" with "neck" //3;
	...
	// ssTilt was negative sin(tilt), but this causes the bounding boxes of the parts to head in the wrong y direction
...

	{
		...
		// Changed the head entity to only be the head, and not include the neck parts
...
// Fireball attack taken from Ghast
	if (!level->isClientSide)
	{
	...
	else
	{
		...
		if(m_currentPath == NULL || m_currentPath->isDone() )
		{
			...
			if(m_remainingCrystalsCount <= 0)
			{
				...
				targetNodeIndex -= 12;
				targetNodeIndex = targetNodeIndex&7;	// was %8, but that could create a result of -1 here when targetNodeIndex was 11
				targetNodeIndex += 12;
			}
			else
			{
...
void EnderDragon::tickDeath()
{
	...
	if (dragonDeathTime == 200 && !level->isClientSide)
	{
		...
		//  changed to center this between the pillars
...
void EnderDragon::spawnExitPortal(int x, int z)
{
	...
	// The podium can be floating with nothing under it, so put some whiteStone under it if this is the case
...
// A* taken from PathFinder and modified
