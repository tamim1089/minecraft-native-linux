...
AABB *FenceGateTile::getAABB(Level *level, int x, int y, int z)
{
	...
	//  forward change from 1.2.3 to fix hit box rotation
...
// Brought forward from 1.2.3 to fix hit box rotation
void FenceGateTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
...
bool FenceGateTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) //  soundOnly param
{
	if( soundOnly )
	{
		// added - just do enough to play the sound
		level->levelEvent(player, LevelEvent::SOUND_OPEN_DOOR, x, y, z, 0);	// changed event to pass player rather than NULL as the source of the event so we can filter the broadcast properly
		return false;
	}
