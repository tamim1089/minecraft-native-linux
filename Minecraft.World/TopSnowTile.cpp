...
void TopSnowTile::updateShape(LevelSource *level, int x, int y, int z, int forceData, shared_ptr<TileEntity> forceEntity) //  forceData, forceEntity param
{
...
bool TopSnowTile::mayPlace(Level *level, int x, int y, int z)
{
	...
	// Assume when placing that this is the server level and we don't care how it's going to be rendered
...
bool TopSnowTile::shouldRenderFace(LevelSource *level, int x, int y, int z, int face)
{
	...
	// don't render faces if neighbouring tiles are also TopSnowTile with at least the same height as this one
