...
void EnchantmentMenu::broadcastChanges()
{
	...
	//  m_costsChanged to stop continually sending update packets even when no changes have been made
	if(m_costsChanged)
	{
...
void EnchantmentMenu::slotsChanged() //  to take a shared_ptr<Container> container but wasn't using it, so removed to simplify things
{
