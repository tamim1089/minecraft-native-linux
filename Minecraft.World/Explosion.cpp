...
void Explosion::explode()
{
	...
	for (AUTO_VAR(it, entities.begin()); it != itEnd; it++)
	{
		...
		// If the entity is not in a block that would be blown up, then they should not be damaged
		...
		if (dist <= 1)
		{
			...
			// Added this check to remove divide by zero errors (or rather the issues caused by
...
void Explosion::finalizeExplosion(bool generateParticles, vector<TilePos> *toBlowDirect/*=NULL*/)		// added toBlowDirect parameter
{
	...
	// use pointer to vector directly passed in if this is available - used to speed up calling this from an incoming packet
	...
	// try and make at most MAX_EXPLODE_PARTICLES pairs of particles
